...
class ConfigRouter:
    ...
    def merge_configs(
        self, base_cfg: Dict[str, Any], enhanced_cfg: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    def validate_config(self, cfg: Dict[str, Any]) -> bool:
        ...
        if not self.enhanced_mode and not cfg.get("parameters", {}).get("filepath"):
    ...
    def get_system_info(self, cfg: Dict[str, Any]) -> Dict[str, Any]:
        ...
        info = {
            ...
            "features": {
                ...
                "well_data_support": cfg.get("data", {}).get("well", False)
                or cfg.get("data", {}).get("apm", False),
                "production_data_support": cfg.get("data", {}).get("production", False),
                "platform_data_support": cfg.get("data", {}).get("platform", False),
                "pipeline_permit_support": cfg.get("data", {}).get(
                    "pipeline_permit", False
                ),
                "deepwater_structure_support": cfg.get("data", {}).get(
                    "deepwater_structure", False
                ),
                "pipeline_location_support": cfg.get("data", {}).get(
            ...
            "data_sources": {
                ...
                "platform": "fresh_download" if self.enhanced_mode else "not_supported",
                "pipeline_permit": (
                    "fresh_download" if self.enhanced_mode else "not_supported"
                ),
                "deepwater_structure": (
                    "fresh_download" if self.enhanced_mode else "not_supported"
                ),
