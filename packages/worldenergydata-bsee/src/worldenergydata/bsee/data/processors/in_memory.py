...
class MemoryProcessor:
    ...
    def __init__(self, use_optimized=True):
        ...
        if self.use_optimized:
            ...
            logger.info(
                "Memory processor initialized with optimized processing enabled"
            )
    ...
    def process_zip_in_memory(self, zip_data: ByteString) -> Dict[str, pd.DataFrame]:
        ...
        if not self.check_memory_availability(estimates["total_estimate_mb"]):
            logger.warning(
                "Proceeding despite memory warning - will use chunked processing if needed"
            )
        ...
        try:
                        ...
                        for encoding in encodings:
                            try:
                                ...
                                with zip_file.open(filename) as file:
                                    df = pd.read_csv(
                                        file, low_memory=False, encoding=encoding
                                    )
                                    logger.info(
                                        f"Successfully read {filename} with {encoding} encoding"
                                    )
                                    break
            ...
            logger.info(
                f"Ending memory usage: {mem_end['rss_mb']:.1f} MB "
                f"(delta: {mem_end['rss_mb'] - mem_start['rss_mb']:.1f} MB)"
            )
    ...
    def process_well_data_optimized(
        self, zip_data: ByteString, cfg: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    def process_production_data_optimized(
        self, zip_data: ByteString, cfg: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        # Use optimized processor
        data_dict = self.optimized_processor.process_zip_optimized(
            zip_data, "production"
        )
    ...
    def process_war_data_optimized(
        self, zip_data: ByteString, cfg: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    def process_war_data(
        ...
        logger.warning(
            "Processing large WAR data without optimization - this may be slow!"
        )
    ...
    def process_platform_data(
        ...
        """Process platform structure data from zip."""
        logger.info("Processing platform structure data in memory")
        data_dict = self.process_zip_in_memory(zip_data)
        if not data_dict:
            logger.error("No data extracted from platform zip file")
            return {}
        processed = {}
        for filename, df in data_dict.items():
            # Platform-specific date columns
            date_cols = ["INSTALL_DATE", "REMOVAL_DATE", "STE_CLRNCE_DATE"]
    ...
    def process_pipeline_permit_data(
        self, zip_data: ByteString, cfg: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Process pipeline permit data from zip."""
        logger.info("Processing pipeline permit data in memory")
        data_dict = self.process_zip_in_memory(zip_data)
        if not data_dict:
            logger.error("No data extracted from pipeline permit zip file")
            return {}
        processed = {}
        for filename, df in data_dict.items():
            date_cols = [
    ...
    def process_deepwater_structure_data(
        self, zip_data: ByteString, cfg: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Process deepwater (permitted) structure data from zip."""
        logger.info("Processing deepwater structure data in memory")
        data_dict = self.process_zip_in_memory(zip_data)
        if not data_dict:
            logger.error("No data extracted from deepwater structure zip file")
            return {}
        processed = {}
        for filename, df in data_dict.items():
            date_cols = ["INSTALL_DATE", "REMOVAL_DATE", "STE_CLRNCE_DATE"]
            for col in date_cols:
                if col in df.columns:
                    df[col] = pd.to_datetime(df[col], errors="coerce")
            numeric_cols = ["WATER_DEPTH", "LATITUDE", "LONGITUDE"]
            for col in numeric_cols:
                if col in df.columns:
                    df[col] = pd.to_numeric(df[col], errors="coerce")
            processed[filename] = {
    ...
    def process_pipeline_location_data(
        self, zip_data: ByteString, cfg: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Process pipeline location data from zip."""
        logger.info("Processing pipeline location data in memory")
        data_dict = self.process_zip_in_memory(zip_data)
        if not data_dict:
            logger.error("No data extracted from pipeline location zip file")
            return {}
        processed = {}
        for filename, df in data_dict.items():
            numeric_cols = ["POINT_NUM", "LATITUDE", "LONGITUDE", "WATER_DEPTH"]
            for col in numeric_cols:
                if col in df.columns:
                    df[col] = pd.to_numeric(df[col], errors="coerce")
            processed[filename] = {
    ...
    def save_dataframe_to_binary(
        self, df: pd.DataFrame, output_dir: str, filename: str
    ) -> str:
        """
        ...
        try:
            ...
            # e.g., mv_apd_data_all.txt -> mv_apd_data_all.bin
    ...
    def save_to_binary(
        self, data: Dict[str, Any], output_dir: str, prefix: str
    ) -> None:
        """
        ...
        try:
            ...
            for original_filename, file_data in data.items():
                # Extract base name from original filename
                # e.g., mv_apd_data_all.txt -> mv_apd_data_all
                ...
                logger.info(
                    f"Saved binary file: {output_file} (from {original_filename})"
                )
    ...
    def check_memory_availability(self, estimated_mb: float) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Memory check - Available: {available_mb:.1f} MB, Required: {estimated_mb:.1f} MB"
            )
    ...
    def cleanup_memory(self):
        ...
        logger.info(
            f"Memory usage after cleanup: "
            f"{mem_after['rss_mb']:.1f} MB ({mem_after['percent']:.1f}%)"
        )
    ...
    def validate_data_integrity(self, data: Dict[str, Any]) -> bool:
        ...
        for filename, file_data in data.items():
            if isinstance(file_data, dict) and "data" in file_data:
                ...
                if null_cols:
                    logger.warning(
                        f"{filename}: Columns with all null values: {null_cols}"
                    )
