...
class PaleowellsDataProcessor:
    ...
    def filter_epochs(
        self, input_file: Path, output_file: Path, epochs: Optional[List[str]] = None
    ) -> int:
        """
        ...
        try:
            ...
            logger.info(
                f"Filtered {records_written} records containing epochs {epochs}"
            )
    ...
    def parse_fixed_width_file(
        self, input_file: Path, output_csv: Optional[Path] = None
    ) -> pd.DataFrame:
        """
        ...
        """
        try:
            df = pd.read_fwf(input_file, widths=self.FWIDTHS, names=self.COLUMN_NAMES)
    ...
    def analyze_well_epochs(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        # Well count by epoch
        analysis["wells_by_epoch"] = (
            df.groupby("Epoch")["API Well Number"].nunique().to_dict()
        )
        ...
        depth_stats = df.groupby("Epoch")[
            ["Measured Depth", "True Vertical Depth"]
        ].describe()
        # Convert MultiIndex tuple keys to strings for JSON serialization
        analysis["depth_statistics"] = {
            str(k): v for k, v in depth_stats.to_dict().items()
        }

        # Definite vs Possible classifications
        analysis["classification_counts"] = (
            df["Definite/Possible"].value_counts().to_dict()
        )

        return analysis

    def process_paleowells_data(
        self,
        ...
        if raw_data_file:
            ...
            if not filtered_file.exists():
                raise FileNotFoundError(
                    f"No filtered data file found at {filtered_file}"
                )
