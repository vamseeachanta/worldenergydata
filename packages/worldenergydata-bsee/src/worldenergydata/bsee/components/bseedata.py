...
AppBlueprint = Blueprint(
    AppName, __name__, template_folder="templates", static_folder="static"
)
...
def assignFieldList():
    ...
    folder_with_file_type = os.path.join(
        os.getcwd(), "services", "bseedata", "static", "bseedata", "data", "*.json"
    )
    FieldFiles = read_data.get_file_list_from_folder(
        folder_with_file_type=folder_with_file_type,
        with_path=False,
...
class dropDownSelectionForm(FlaskForm):
    dropDownSelection = SelectField(
        context["dropDownSelectionLabel"],
