...
class ProductionPeriod(Enum):
    ...
    @classmethod
    def get_date_range(
        cls, period: "ProductionPeriod", reference_date: date
    ) -> Tuple[date, date]:
...
class ProductionMetrics:
    ...
    def oil_per_well_per_day(self) -> float:
        ...
        if self.active_well_count > 0 and self.days_in_period > 0:
            return self.oil_production_bbls / (
                self.active_well_count * self.days_in_period
            )
        return 0.0
...
class EconomicMetrics:
    ...
    def calculate_net_income(self) -> float:
        ...
        self.net_income = (
            self.revenue - self.operating_costs - self.capital_costs - self.royalties
        )
    ...
    def calculate_npv(self) -> float:
        ...
        if self.years_from_start > 0:
            return npf.npv(
                self.discount_rate, [-0] + [self.net_income] * self.years_from_start
            )[-1]
        return self.net_income
