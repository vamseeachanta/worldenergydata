...
class DatasetGenerator:
    ...
    def create_wellbore_dataset(
        self, wellbore_data: pd.DataFrame, field_data: pd.DataFrame = None
    ) -> pd.DataFrame:
        """
    ...
    def generate_wellbore_dataset(
        self,
        wellbore_data: Union[pd.DataFrame, List[Dict[str, Any]]],
    ...
    def generate_production_dataset(
        self,
        production_data: Union[pd.DataFrame, List[Dict[str, Any]]],
        field_data: Optional[pd.DataFrame] = None,
    ) -> pd.DataFrame:
        """
    ...
    def create_production_dataset(
        self, production_data: pd.DataFrame, field_data: pd.DataFrame
    ) -> pd.DataFrame:
        """
        ...
        try:
            ...
            if "production_start" in df.columns and "production_date" in df.columns:
                df["production_years"] = (
                    df["production_date"] - df["production_start"]
                ).dt.days / 365.25
            ...
            if "oil_production" in df.columns and "gas_production" in df.columns:
                ...
                df["boe_production"] = df["oil_production"] + (
                    df["gas_production"] / 6000
                )
    ...
    def create_cross_regional_dataset(
        self, sodir_data: pd.DataFrame, bsee_data: pd.DataFrame = None
    ) -> pd.DataFrame:
        """
        ...
        try:
            ...
            if bsee_data is not None:
                ...
                df = pd.concat(
                    [sodir_data[common_cols], bsee_data[common_cols]], ignore_index=True
                )
    ...
    def prepare_time_series(
        self, df: pd.DataFrame, date_col: str, value_cols: List[str], freq: str = "M"
    ) -> pd.DataFrame:
        """
    ...
    def create_summary_statistics(
        self, df: pd.DataFrame, group_cols: List[str], metric_cols: List[str]
    ) -> pd.DataFrame:
        """
