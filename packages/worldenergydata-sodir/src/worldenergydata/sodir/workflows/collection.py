...
class CollectionWorkflow:
    ...
    def collect_parallel(
        self, datasets: List[str], max_workers: int = 5
        ...
        if len(self.datasets) > 1 and self.config.get("parallel", False):
            ...
            for dataset in self.datasets:
                results[dataset] = self.sodir_data.collect_dataset(
                    dataset, validate=self.validation_enabled, filters=filters
                )
            return results
    ...
    def validate_data(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "valid": all(
                r.get("validation_rate", 0) > 0.9 for r in validation_results.values()
            ),
    ...
    def store_data(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for dataset_type, dataset_data in data.items():
            try:
                ...
                storage_results[dataset_type] = {"success": True, "path": str(path)}
            except Exception as e:
                storage_results[dataset_type] = {"success": False, "error": str(e)}
    ...
    def collect_dataset(self, dataset_type: str) -> Dict[str, Any]:
        ...
        return self.sodir_data.collect_dataset(
            dataset_type, validate=self.validation_enabled, filters=filters
        )
    ...
    def _collect_dataset_with_workflow(self, dataset_type: str) -> Dict[str, Any]:
        ...
        result = self.sodir_data.collect_dataset(
            dataset_type, validate=self.validation_enabled, since=since, filters=filters
        )
