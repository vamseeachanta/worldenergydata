...
class SodirVisualizer:
    ...
    def create_field_map(
        self, fields_df: pd.DataFrame, config: ChartConfig = None
    ) -> MapVisualization:
        """
        ...
        if "longitude" in fields_df.columns and "latitude" in fields_df.columns:
            ...
            if config.color_by and pd.api.types.is_numeric_dtype(
                fields_df[config.color_by]
            ):
            ...
            if (
                "field_name" in fields_df.columns
                and "recoverable_oil_mmbbl" in fields_df.columns
            ):
    ...
    def create_production_chart(
        self, production_data: pd.DataFrame, chart_type: str = "line"
    ) -> Any:
        """
        ...
        if chart_type == "line":
            ...
            ax.fill_between(
                x_data, 0, oil_data, label="Oil", color="darkgreen", alpha=0.7
            )
            ax.fill_between(
                x_data,
                oil_data,
    ...
    def create_comparison_dashboard(
        self,
        sodir_data: Dict[str, pd.DataFrame],
        ...
        dashboard = DashboardGenerator(
            charts=charts, layout=layout, title="SODIR vs BSEE Comparison Dashboard"
        )
    ...
    def _create_comparison_chart(
        ...
        if (
            ...
            and "fields" in sodir_data
            and "fields" in bsee_data
        ):
            ...
            metric == "water_depth" and "fields" in sodir_data and "fields" in bsee_data
        ):
            ...
            # Create histogram
            ax.hist(
                sodir_values.dropna(), bins=20, alpha=0.5, label="Norway", color="blue"
            )
            ax.hist(
                bsee_values.dropna(),
                bins=20,
            ...
            metric == "field_size" and "fields" in sodir_data and "fields" in bsee_data
        ):
            # Calculate BOE for field size
            sodir_boe = (
                sodir_data["fields"].get("recoverable_oil_mmbbl", 0)
                + sodir_data["fields"].get("recoverable_gas_bcf", 0) / 6
            )
            bsee_boe = (
                bsee_data["fields"].get("recoverable_oil_mmbbl", 0)
    ...
    def export_chart(
        self, chart: Any, filepath: str, format: str = "png", dpi: int = 300
    ) -> None:
        """
    ...
    def create_field_ranking_chart(
        self,
        ...
        chart = {"figure": fig, "data": top_fields, "ranking_metric": rank_by}
    ...
    def create_discovery_timeline(self, fields_df: pd.DataFrame) -> Any:
        ...
        if "discovery_year" in fields_df.columns:
            ...
            ax1.bar(
                discovery_counts.index, discovery_counts.values, color="navy", alpha=0.7
            )
            ...
            ax2.fill_between(
                cumulative.index, 0, cumulative.values, alpha=0.3, color="darkred"
            )
            ax2.set_xlabel("Year")
        ...
        chart = {
            "figure": fig,
            "discovery_counts": (
                discovery_counts if "discovery_year" in fields_df.columns else None
            ),
    ...
    def create_correlation_heatmap(
        self, data_df: pd.DataFrame, columns: List[str] = None
    ) -> Any:
        """
        ...
        chart = {"figure": fig, "correlation_matrix": correlation}
