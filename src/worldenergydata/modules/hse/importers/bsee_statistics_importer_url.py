...
class BSEEStatisticsImporterURL(BSEEStatisticsImporter):
    ...
    BSEE_PRODUCTION_DATA_URL = 'https://www.data.bsee.gov/Production/Files/ProductionRawData.zip'
    ...
    def fetch_data(self) -> List[Dict[str, Any]]:
        ...
        zip_data = self.scraper.download_zip_to_memory(
            self.BSEE_PRODUCTION_DATA_URL,
            data_type='production'
        )

        if zip_data is None:
            raise ValueError(f"Failed to download data from {self.BSEE_PRODUCTION_DATA_URL}")
        ...
        processed_data = self.processor.process_production_data(
            zip_data,
            cfg={
                'calculate_boe': True  # Calculate barrels of oil equivalent
            }
        )
