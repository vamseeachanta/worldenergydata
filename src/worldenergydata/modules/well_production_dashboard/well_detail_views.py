...
from worldenergydata.modules.bsee.analysis.well_data_verification.quality import DataQualityFramework
...
class ChartQualityIndicator:
    ...
    @staticmethod
    def add_quality_overlay(fig: Any, quality_scores: pd.Series, 
...
class ProductionChartBuilder:
    ...
    def create_time_series_chart(self, data: pd.DataFrame, 
        ...
        if not PLOTLY_AVAILABLE:
            return {'type': 'time_series', 'data': data.to_dict(), 'error': 'Plotly not available'}
            
        fig = make_subplots(
            rows=1, cols=1,
            subplot_titles=[f'{well_name} Production Over Time']
        )
        ...
        if 'water_production' in data.columns or 'water' in data.columns:
            water_col = 'water_production' if 'water_production' in data.columns else 'water'
            fig.add_trace(
                go.Scatter(
                    x=data['date'] if 'date' in data.columns else data.index,
                    y=data[water_col],
    ...
    def add_quality_indicators(self, chart: Dict[str, Any], 
                             quality_scores: pd.Series,
                             verification_status: pd.Series) -> Dict[str, Any]:
        ...
        if 'figure' in chart and PLOTLY_AVAILABLE:
            ...
            fig = self.quality_indicator.add_verification_markers(fig, verification_status)
            chart['figure'] = fig
    ...
    def create_decline_curve_chart(self, production: np.ndarray,
                                  dates: np.ndarray,
                                  well_name: str) -> Dict[str, Any]:
        ...
        if isinstance(dates[0], (pd.Timestamp, datetime)):
            last_date = dates[-1]
        elif hasattr(dates, 'dtype') and pd.api.types.is_datetime64_any_dtype(dates.dtype):
    ...
    def create_stacked_production_chart(self, data: pd.DataFrame,
                                       well_name: str) -> Dict[str, Any]:
    ...
    def add_annotations(self, chart: Dict[str, Any],
                       annotations: List[Dict[str, str]]) -> Dict[str, Any]:
...
class EconomicMetricsCalculator:
    ...
    def calculate_revenue(self, oil_production: np.ndarray,
                         oil_price: np.ndarray,
                         gas_production: np.ndarray,
                         gas_price: np.ndarray) -> np.ndarray:
    ...
    def create_waterfall_chart(self, revenue: float, opex: float,
                              capex: float, taxes: float,
                              well_name: str) -> Dict[str, Any]:
        ...
        fig.add_trace(
            go.Waterfall(
                ...
                text=[f"${revenue:,.0f}", f"-${opex:,.0f}", f"-${capex:,.0f}", 
                     f"-${taxes:,.0f}", f"${net_income:,.0f}"],
...
class DeclineCurveAnalyzer:
    ...
    def fit_exponential_decline(self, production: np.ndarray,
        ...
        if len(dates) > 0 and (isinstance(dates[0], (pd.Timestamp, datetime)) or 
                               (hasattr(dates, 'dtype') and pd.api.types.is_datetime64_any_dtype(dates.dtype))):
    ...
    def fit_hyperbolic_decline(self, production: np.ndarray,
                              dates: np.ndarray) -> Dict[str, float]:
    ...
    def calculate_fitted_values(self, params: Dict[str, float],
    ...
    def forecast_production(self, params: Dict[str, float],
                          periods: int = 12,
                          decline_type: str = 'exponential') -> np.ndarray:
    ...
    def calculate_eur(self, params: Dict[str, float],
                     economic_limit: float = 100,
                     decline_type: str = 'exponential') -> float:
    ...
    def create_type_curve(self, production: np.ndarray,
                         dates: np.ndarray,
                         well_name: str) -> Dict[str, Any]:
        ...
        initial_rate = production[0] if production[0] > 0 else production[production > 0][0]
...
class VerificationStatusBadge:
    ...
    def create(self, status: str, quality_score: float,
              timestamp: datetime, details: Optional[Dict] = None) -> Dict[str, Any]:
...
class AuditTrailLink:
    ...
    def create(self, well_id: str, verification_id: str,
    ...
    def create_batch(self, well_id: str,
                    verification_ids: List[str]) -> List[Dict[str, str]]:
        """Create multiple audit trail links."""
        return [
            self.create(well_id, ver_id, datetime.now())
            for ver_id in verification_ids
        ]
    
    def format_summary(self, total_verifications: int, passed: int,
                      failed: int, pending: int) -> Dict[str, Any]:
...
class WellDetailView:
    ...
    def render(self, well_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'production' in well_data:
            production_section = self.create_production_section(
                well_data['production'],
                well_data.get('verification', pd.DataFrame())
            )
        ...
        if 'economics' in well_data:
            economic_section = self.create_economic_section(
                well_data.get('economics'),
                well_data.get('production')
            )
    ...
    def create_production_section(self, production_data: pd.DataFrame,
                                 verification_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        if 'decline_curve' in self.config.chart_types:
            ...
            if oil_col in production_data.columns:
                dates = production_data['date'] if 'date' in production_data.columns else production_data.index
                chart = self.chart_builder.create_decline_curve_chart(
                    production_data[oil_col].values,
                    dates.values,
                    well_name
                )
        ...
        if not verification_data.empty and self.config.show_quality_indicators:
            section['quality_indicators'] = self._create_quality_summary(verification_data)
        
        return section
    
    def create_economic_section(self, economic_data: Optional[pd.DataFrame],
                               production_data: Optional[pd.DataFrame]) -> Dict[str, Any]:
        ...
        if 'revenue' in economic_data.columns and 'opex' in economic_data.columns:
            ...
            # Payback period
            payback = self.metrics_calculator.calculate_payback_period(cash_flows.values)
            section['payback_period'] = {'value': payback, 'formatted': f"{payback:.1f} months"}
        
        # Waterfall chart
        if 'economic_waterfall' in self.config.chart_types:
            total_revenue = economic_data['revenue'].sum() if 'revenue' in economic_data.columns else 0
            total_opex = economic_data['opex'].sum() if 'opex' in economic_data.columns else 0
            total_capex = economic_data['capex'].sum() if 'capex' in economic_data.columns else 0
    ...
    def create_verification_section(self, verification_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        # Current status
        latest_verification = verification_data.iloc[-1] if not verification_data.empty else {}
        ...
        if 'quality_score' in verification_data.columns:
            section['quality_timeline'] = {
                'dates': verification_data.index.tolist() if isinstance(verification_data.index, pd.DatetimeIndex) else verification_data['date'].tolist(),
        ...
        if 'verification_id' in verification_data.columns:
            verification_ids = verification_data['verification_id'].dropna().unique().tolist()
    ...
    def _create_quality_summary(self, verification_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        if 'quality_score' in verification_data.columns:
            ...
            summary['below_threshold'] = (verification_data['quality_score'] < self.config.quality_threshold).sum()
    ...
    def export_to_pdf(self, well_data: Dict[str, Any]) -> bytes:
        ...
        try:
            ...
            # Add basic content
            p.drawString(100, 750, f"Well Detail Report: {well_data.get('well_name', 'Unknown')}")
            p.drawString(100, 730, f"Field: {well_data.get('field', 'Unknown')}")
            p.drawString(100, 710, f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    ...
    def update_real_time(self, current_page: Dict[str, Any],
    ...
    def get_verification_details(self, well_id: str,
        ...
        return {
            ...
            'audit_trail': [
                ...
                {'timestamp': datetime.now().isoformat(), 'action': 'Quality checks performed'},
                {'timestamp': datetime.now().isoformat(), 'action': 'Verification completed'}
            ],
            'quality_checks': {
                'completeness': 0.95,
                'accuracy': 0.92,
