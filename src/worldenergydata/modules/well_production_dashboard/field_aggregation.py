...
from worldenergydata.modules.bsee.reports.comprehensive.aggregators.base import DataAggregator
...
class FieldAggregationConfig:
    ...
    comparison_metrics: List[str] = field(default_factory=lambda: ['production', 'economics', 'efficiency'])
...
class FieldAggregationDashboard:
    ...
    def aggregate_field_data(self, wells: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        logger.info(f"Aggregated data for {len(wells)} wells in {self.config.field_name}")
    ...
    def _calculate_economic_rollup(self, wells: List[Dict[str, Any]]) -> Dict[str, float]:
        ...
        return {
            ...
            'field_npv': self.economic_summary.calculate_field_npv(
                pd.Series([total_revenue] * 12),
                pd.Series([total_opex] * 12),
                total_capex
            ) if total_revenue > 0 else 0,
            'profit_margin': (total_revenue - total_opex) / total_revenue if total_revenue > 0 else 0
        }
    ...
    def _calculate_well_statistics(self, wells: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            'producing_wells': sum(1 for w in wells if w.get('status') in ['active', 'producing']),
    ...
    def _calculate_quality_metrics(self, wells: List[Dict[str, Any]]) -> Dict[str, float]:
        ...
        return {
            ...
            'wells_above_threshold': sum(1 for s in quality_scores if s >= self.config.quality_threshold)
        }
    ...
    def calculate_field_metrics(self, wells: List[Dict[str, Any]]) -> Dict[str, float]:
        ...
        total_production = sum(
            well.get('production_data', pd.DataFrame()).get('oil_bbls', pd.Series()).sum()
            for well in wells
        )
        metrics['average_production_per_well'] = total_production / len(wells) if wells else 0
        ...
        # Calculate water cut
        total_fluids = sum(
            well.get('production_data', pd.DataFrame()).get('water_bbls', pd.Series()).sum() +
            well.get('production_data', pd.DataFrame()).get('oil_bbls', pd.Series()).sum()
            for well in wells
        )
        total_water = sum(
        ...
        total_gas = sum(
            well.get('production_data', pd.DataFrame()).get('gas_mcf', pd.Series()).sum()
            for well in wells
        )
        metrics['gas_oil_ratio'] = total_gas / total_production if total_production > 0 else 0
    ...
    def apply_verification_overlay(self, aggregated_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        aggregated_data['quality_status'] = 'high' if aggregated_data['overall_quality'] >= 0.9 else 'medium'
    ...
    def create_field_dashboard(self, wells: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if wells:
            ...
            dashboard['charts']['production'] = self.chart_builder.create_production_chart(production_data)
...
class FieldComparator:
    ...
    def compare_production(self, fields_data: Dict[str, Dict]) -> Dict[str, Any]:
        ...
        comparison['rankings'] = {field: rank+1 for rank, (field, _) in enumerate(ranked)}
    ...
    def compare_economics(self, fields_data: Dict[str, Dict]) -> Dict[str, Any]:
        ...
        for field_name, data in fields_data.items():
            ...
            profit_margins[field_name] = (revenue - costs) / revenue if revenue > 0 else 0
            
            roi_values[field_name] = econ.get('roi', 0)
        
        # Rank by NPV
        npv_ranked = sorted(npv_values.items(), key=lambda x: x[1], reverse=True)
        comparison['npv_ranking'] = {field: rank+1 for rank, (field, _) in enumerate(npv_ranked)}
    ...
    def compare_efficiency(self, fields_data: Dict[str, Dict]) -> Dict[str, Any]:
        ...
        for field_name, data in fields_data.items():
            ...
            comparison['production_efficiency'][field_name] = efficiency.get('production_per_well', 0)
            
            # Operational uptime
            comparison['operational_uptime'][field_name] = efficiency.get('uptime', 0)
            
            # Resource utilization (wells per lease)
            comparison['resource_utilization'][field_name] = efficiency.get('wells_per_lease', 0)
            
            # Overall efficiency score
            score = (
                efficiency.get('production_per_well', 0) / 5000 * 0.4 +  # Normalized
                efficiency.get('uptime', 0) * 0.4 +
                min(efficiency.get('wells_per_lease', 0) / 10, 1) * 0.2  # Capped at 10
            )
    ...
    def generate_comparison_matrix(self, fields_data: Dict[str, Dict]) -> pd.DataFrame:
        ...
        # Add rankings
        for col in ['oil_production', 'gas_production', 'revenue', 'npv', 'efficiency_score']:
...
class FieldEconomicSummary:
    ...
    def calculate_field_npv(self, revenue: pd.Series, opex: pd.Series, 
                           capex: float, discount_rate: float = 0.1) -> float:
        """
    ...
    def calculate_irr(self, cash_flows: pd.Series) -> float:
        ...
        try:
            ...
            for _ in range(100):  # Max iterations
                ...
                npv_prime = sum(-i * cf / (1 + irr) ** (i + 1) for i, cf in enumerate(cf_array))
    ...
    def calculate_payback_period(self, revenue: pd.Series, opex: pd.Series, 
                                 capex: float) -> float:
        """
    ...
    def generate_summary(self, economic_data: Dict[str, Any]) -> Dict[str, float]:
        ...
        summary['profit_margin'] = (total_revenue - total_costs) / total_revenue if total_revenue > 0 else 0
        
        # Calculate break-even price
        production = economic_data.get('production', pd.Series([1]))
        total_production = production.sum()
        summary['break_even_price'] = total_costs / total_production if total_production > 0 else 0
        
        # Add quality scores if available
        if 'quality_scores' in economic_data:
            summary = self.apply_quality_scores(summary, economic_data['quality_scores'])
        
        return summary
    
    def apply_quality_scores(self, summary: Dict[str, float], 
...
class FieldProductionChart:
    ...
    def create_production_chart(self, production_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        if not PLOTLY_AVAILABLE or production_data.empty:
            return {
                ...
                'data': {},
                'layout': {},
                'error': 'Plotly not available or no data' if not PLOTLY_AVAILABLE else 'No data'
            }
        
        chart = {
            'type': 'production_time_series',
            'data': {
                'dates': production_data.index.tolist() if hasattr(production_data.index, 'tolist') else [],
    ...
    def create_stacked_production(self, production_data: pd.DataFrame) -> Dict[str, Any]:
        """
    ...
    def add_verification_overlay(self, base_chart: Dict[str, Any], 
                                 quality_scores: pd.Series) -> Dict[str, Any]:
        """
        ...
        chart_with_overlay['verification_layer'] = {
            'scores': quality_scores.tolist() if hasattr(quality_scores, 'tolist') else [],
            'threshold': 0.8,
            'color_scale': {
                'high': 'green',
                'medium': 'yellow', 
    ...
    def create_decline_curve(self, production_series: pd.Series) -> Dict[str, Any]:
        ...
        if len(y) > 1 and y[0] > 0:
            ...
            if len(x_valid) > 1:
                ...
                future_x = np.arange(len(production_series), len(production_series) + 12)
    ...
    def create_comparison_chart(self, fields_data: Dict[str, pd.Series]) -> Dict[str, Any]:
        """
        ...
        for field_name, production in fields_data.items():
            chart['data'].append({
                'name': field_name,
                'values': production.tolist() if hasattr(production, 'tolist') else [],
                'type': 'line'
            })
