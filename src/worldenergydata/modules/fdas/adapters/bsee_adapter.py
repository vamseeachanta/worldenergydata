...
class WellDataAdapter:
    ...
    def create_lease_mapping(self) -> List[LeaseMapping]:
        ...
        grouped = self.well_data.groupby('LEASE_NUMBER').agg({
    ...
    def save_enhanced_well_data(self, output_path: str) -> None:
        ...
        if self.well_data is None or 'DEV_SYSTEM' not in self.well_data.columns:
            raise AdapterError("Well data not enhanced. Call add_dev_system_classification() first.")
...
class ProductionAdapter:
    ...
    def __init__(self, production_path: str, lease_mappings: Optional[List[LeaseMapping]] = None):
        """
    ...
    def aggregate_monthly(self, by: str = 'DEV_NAME') -> pd.DataFrame:
        ...
        missing = [col for col in required_cols if col not in self.production_data.columns]
        if missing:
            raise AdapterError(f"Missing required columns: {missing}")
        ...
        # Aggregate
        aggregated = self.production_data.groupby([by, 'YEAR_MONTH']).agg({
            'OIL_VOLUME': 'sum',
            'WATER_VOLUME': 'sum',
            'GAS_VOLUME': 'sum' if 'GAS_VOLUME' in self.production_data.columns else lambda x: 0,
        }).reset_index()
    ...
    def create_chronological_analysis(self, dev_name: str) -> pd.DataFrame:
        ...
        """
        if self.production_data is None or 'DEV_NAME' not in self.production_data.columns:
            raise AdapterError("Production data not enhanced. Call enhance_with_lease_info() first.")

        # Filter to development
        dev_data = self.production_data[self.production_data['DEV_NAME'] == dev_name].copy()
...
class BseeAdapter:
    ...
    def get_development_production(self, dev_name: str) -> pd.DataFrame:
        ...
        if self.production_adapter is None:
            raise AdapterError("Production adapter not initialized. Call load_all() first.")
