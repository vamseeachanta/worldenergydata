...
from typing import Dict, List, Optional, Any, ByteString, Tuple
...
class ChunkMetadata:
    ...
    def __init__(self, chunk_id: str, checksum: str, timestamp: datetime, 
                 size_bytes: int, row_range: Optional[Tuple[int, int]] = None):
        """
...
class ChunkManager:
    ...
    def check_remote_changes(self, url: str, data_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Check Last-Modified
            if remote_metadata['last_modified'] and cached_metadata.get('last_modified'):
                if remote_metadata['last_modified'] != cached_metadata['last_modified']:
                    has_changed = True
                    change_reasons.append('Last-Modified changed')
            
            # Check file size
            if remote_metadata['content_length'] and cached_metadata.get('content_length'):
                if remote_metadata['content_length'] != cached_metadata['content_length']:
                    has_changed = True
            ...
            logger.info(f"{data_type}: {'Changed' if has_changed else 'Unchanged'} - {change_reasons}")
    ...
    def download_with_chunks(self, url: str, data_type: str, 
                           force_refresh: bool = False) -> Optional[ByteString]:
        """
        ...
        try:
            ...
            if accept_ranges == 'none':
                logger.info("Server doesn't support range requests, downloading full file")
                return self._download_full(url, data_type)
            ...
            logger.info(f"File size: {file_size / (1024*1024):.2f} MB, downloading in {num_chunks} chunks")
            ...
            for i in range(num_chunks):
                ...
                if cached_chunk and not force_refresh:
                    ...
                    # Download chunk
                    logger.info(f"Downloading chunk {i+1}/{num_chunks} ({start_byte}-{end_byte})")
            ...
            logger.info(f"Successfully downloaded {len(complete_data) / (1024*1024):.2f} MB in chunks")
    ...
    def process_incremental_changes(self, zip_data: ByteString, data_type: str,
                                   previous_data: Optional[pd.DataFrame] = None) -> pd.DataFrame:
        """
        ...
        try:
            ...
            if chunk_changes['type'] == 'append':
                ...
                updated_df = pd.concat([previous_data, new_df.iloc[chunk_changes['start_index']:]], 
                                      ignore_index=True)
    ...
    def _identify_dataframe_changes(self, old_df: pd.DataFrame, 
    ...
    def _update_metadata(self, data_type: str, chunks: List[ChunkMetadata], 
                        headers: Dict):
        ...
        # Update chunks
        self.metadata[data_type]['chunks'] = {
