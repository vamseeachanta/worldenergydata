...
class DataRefreshChunked:
    ...
    def router(self, cfg: Dict[str, Any]) -> tuple:
        ...
        if not enhanced_mode:
            logger.warning("Enhanced mode not enabled, chunked refresh requires enhanced mode")
            return cfg, None
    ...
    def _refresh_war_data_chunked(self, cfg: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if zip_data:
                # Track download size
                self.stats['bytes_downloaded'] += len(zip_data)
                logger.info(f"Downloaded {len(zip_data) / (1024*1024):.2f} MB of WAR data")
    ...
    def _get_binary_path(self, cfg: Dict[str, Any], data_type: str) -> Path:
        ...
        path_map = {
            'well': cfg.get('parameters', {}).get('filepath', {}).get('apm', {}).get('bin', 
                                                                                      'data/modules/bsee/bin/apd'),
            'production': cfg.get('parameters', {}).get('filepath', {}).get('production', {}).get('bin', 
                                                                                                  'data/modules/bsee/bin/production_raw'),
            'war': cfg.get('parameters', {}).get('filepath', {}).get('war', {}).get('bin', 
    ...
    def _load_cached_binary(self, data_type: str, cfg: Dict[str, Any]) -> Optional[Any]:
        ...
        try:
            ...
            for bin_file in bin_files:
                file_age = datetime.now() - datetime.fromtimestamp(bin_file.stat().st_mtime)
    ...
    def _load_previous_dataframe(self, data_type: str, cfg: Dict[str, Any]) -> Optional[Any]:
        """
    ...
    def _report_statistics(self):
        ...
        logger.info(f"Bytes downloaded: {self.stats['bytes_downloaded'] / (1024*1024):.2f} MB")
    ...
    def validate_remote_sources(self) -> Dict[str, Any]:
        ...
        for data_type, url in urls.items():
            ...
            cache_status = '(cached)' if results[data_type]['cache_current'] else '(outdated)' if results[data_type]['has_cached_version'] else '(no cache)'
            
            logger.info(f"{data_type}: {status} {results[data_type]['size_mb']:.1f} MB {cache_status}")
