...
class ExtractAPIDetails:
    ...
    def extract_api_details(self, cfg):
        ...
        if not df_filtered.empty:
            ...
            logger.info(f"✅ Final fix applied with custom parsing. {len(df_filtered)} rows saved to {output_file}")
        else:
    ...
    def format_and_sort_api_data(self, df_filtered):
        ...
        df_filtered[date_complete_col] = df_filtered[date_complete_col].apply(self.parse_date)

        logger.debug("\nParsed WAR_START_DT (non-null):", df_filtered[date_begin_col].notna().sum())
        logger.debug("Parsed WAR_END_DT (non-null):", df_filtered[date_complete_col].notna().sum())
