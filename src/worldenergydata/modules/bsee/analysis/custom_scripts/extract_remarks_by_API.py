...
class ExtractRemarksbyAPI:
    ...
    def extract_remarks_by_api(self, cfg):
        ...
        try:
            ...
            for _, row in remarks_df.iterrows():
                raw_snwar = str(row['SN_WAR']).strip() if 'SN_WAR' in remarks_df.columns else ''
                try:
                ...
                if snwar in snwar_to_api:
                    ...
                    text = str(row['TEXT_REMARK']).strip() if 'TEXT_REMARK' in remarks_df.columns else ''
                    if not text:
                        continue

                    for line in text.splitlines():
                        match = re.match(r'^[A-Za-z]{2} (\d{1,2}/\d{1,2}/\d{2,4}) - (.+)', line)
                        if match:
            ...
            for api, entries in remarks_by_api.items():
                ...
                sorted_entries = sorted(entries, key=lambda x: date_to_sortable_int(x[0]))
                ...
                if len(sorted_entries) >= 2:
                    try:
                        ...
                        days = (date_to_sortable_int(date2) - date_to_sortable_int(date1)) // 10000  # Approximate days
    ...
    def get_mapping_file(self,cfg):
        ...
        df["SN_WAR"] = df["SN_WAR"].apply(lambda x: str(abs(int(float(x)))) if pd.notna(x) else "")
