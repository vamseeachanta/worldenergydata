...
class BSEEDataAdapter:
    ...
    def load_production_data(
        self, data_source: Union[str, pd.DataFrame], **kwargs
    ) -> pd.DataFrame:
        """
        ...
        if isinstance(data_source, pd.DataFrame):
            logger.info(
                f"Loading production data from DataFrame with {len(data_source)} rows"
            )
            return data_source
    ...
    def validate_lease_numbers(
        self, df: pd.DataFrame, lease_col: str = "LEASE_NUM"
    ) -> pd.DataFrame:
        """
    ...
    def check_data_completeness(
        ...
        if report["field_completeness"]:
            completeness_values = [
                f["completeness_pct"] for f in report["field_completeness"].values()
            ]
            report["overall_completeness"] = sum(completeness_values) / len(
                completeness_values
            )
