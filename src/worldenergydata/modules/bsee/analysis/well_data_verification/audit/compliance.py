...
from .tracker import ActivityTracker, VerificationStatusManager, DataLineageTracker
...
class ComplianceReportGenerator:
    ...
    def _generate_comprehensive_report(self, report: ComplianceReport):
        ...
        period_sessions = [
            s for s in sessions
            if report.period_start <= datetime.fromisoformat(s["created_at"]) <= report.period_end
        ]
        
        # Calculate summary metrics
        total_verifications = len(period_sessions)
        completed_verifications = len([s for s in period_sessions if s["status"] == "completed"])
        ...
        report.summary = {
            ...
            "completion_rate": completed_verifications / total_verifications if total_verifications > 0 else 0,
    ...
    def _generate_monthly_compliance(self, report: ComplianceReport):
        ...
        if report.summary.get("total_events", 0) > 0:
            report.summary["daily_average_events"] = report.summary["total_events"] / days_in_period
            report.summary["daily_average_verifications"] = report.summary["total_wells_verified"] / days_in_period
        
        # Check monthly compliance requirements
        report.details["monthly_compliance_checks"] = {
            "minimum_verifications_met": report.summary.get("total_wells_verified", 0) >= 100,
            "audit_trail_complete": report.summary.get("total_events", 0) > 0,
            "error_rate_acceptable": report.summary.get("error_count", 0) / max(report.summary.get("total_events", 1), 1) < 0.05
        }
    ...
    def _generate_session_summary(self, report: ComplianceReport):
        ...
        report.summary = {
            "sessions_completed": len([s for s in period_sessions if s["status"] == "completed"]),
            "sessions_in_progress": len([s for s in period_sessions if s["status"] == "in_progress"]),
            "sessions_failed": len([s for s in period_sessions if s["status"] == "failed"]),
            "total_wells_verified": sum(s.get("verified_wells", 0) for s in period_sessions)
        }
    ...
    def _generate_data_governance_report(self, report: ComplianceReport):
        ...
        report.summary = {
            ...
            "unique_resources": len(set(e.details.get("resource", "") for e in data_access_events)),
            "data_modifications": len(self.audit_logger.query_events(
    ...
    def _check_data_retention(self) -> bool:
        ...
        old_events = self.audit_logger.query_events(
            end_date=oldest_required,
            limit=1
        )
        return len(old_events) > 0 or datetime.now() - oldest_required < timedelta(days=1)
    ...
    def _check_access_controls(self) -> bool:
        ...
        access_events = self.audit_logger.query_events(event_type="DATA_ACCESS", limit=1)
        return len(access_events) > 0
    
    def _check_data_integrity(self) -> bool:
        """Check if data integrity is maintained."""
        # Check for validation events
        validation_events = self.audit_logger.query_events(event_type="VALIDATION_FAILURE", limit=1)
    ...
    def _check_change_management(self) -> bool:
        ...
        change_events = self.audit_logger.query_events(event_type="CONFIG_CHANGE", limit=1)
        modification_events = self.audit_logger.query_events(event_type="MODIFICATION", limit=1)
    ...
    def _generate_recommendations(self, report: ComplianceReport) -> List[str]:
        ...
        if report.report_type == "data_governance" and not report.details.get("lineage_information"):
            recommendations.append(
    ...
    def export_pdf(self, report: ComplianceReport, output_path: Path):
        ...
        # Add title
        elements.append(Paragraph(f"Compliance Report: {report.report_type}", title_style))
