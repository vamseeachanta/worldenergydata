...
Examples:
  ...
  %(prog)s -d /data/bsee -o analysis.xlsx --oil-price high --discount-rate 0.08 --tax-rate 0.30
    ...
    format_group.add_argument(
        '--include-charts',
        action='store_true',
        help='Include charts in Excel output'
    )
    format_group.add_argument(
        '--summary-only',
        action='store_true',
        help='Generate summary sheets only'
    )
...
def build_config(args: argparse.Namespace) -> AnalysisConfig:
    ...
    if 'input_path' not in config_dict:
        raise ValueError("Input directory must be specified (--input-directory or in config file)")
...
def print_results_summary(result: AnalysisResult):
    ...
    if result.success:
        ...
        if result.metrics_summary:
            ...
            print(f"   Total Oil: {result.metrics_summary.get('total_oil_bbls', 0):,.0f} bbls")
            print(f"   Developments: {result.metrics_summary.get('development_count', 0)}")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    try:
        ...
        if args.dry_run:
            ...
            output_path = args.output_file or f"financial_analysis_{datetime.now():%Y%m%d_%H%M%S}.xlsx"
