...
class ExcelFormatter:
    ...
    def format_numbers(self, worksheet, percent_headers: List[str] = None, 
...
class ReportGenerator:
    ...
    def create_readme_sheet(self, version: str, generated_date: datetime, 
        ...
        inputs_text += "- drilling_and_completion data (D&C per-well totals/monthlies)\n"
        inputs_text += "- WTI prices (optional monthly WTI; otherwise flat WTI base is used)"
        ws["B7"] = inputs_text
    ...
    def create_executive_summary(self, development_data: Dict[str, Dict[str, Any]],
                                assumptions: Optional[Dict[str, Any]] = None):
    ...
    def _format_executive_summary_data(self, development_data: Dict[str, Dict[str, Any]]) -> List[Dict]:
        ...
        for dev_name, dev_data in development_data.items():
            ...
            row = {
                ...
                'DnC Total USD': metrics.get('total_capex_drill', 0) + metrics.get('total_capex_comp', 0),
    ...
    def _format_project_summary_data(self, development_data: Dict[str, Dict[str, Any]]) -> List[Dict]:
        ...
        for dev_name, dev_data in development_data.items():
            ...
            injector_count = producer_count // 7 if dev_system == 'subsea' and producer_count >= 7 else 0
            pump_count = producer_count // 8 if dev_system == 'subsea' and producer_count >= 8 else 0
            
            row = {
                'Project Name': dev_name,
                'TOTAL OIL BBL': metrics.get('total_oil_bbls', 0),
                'Facilities Host USD': metrics.get('host_usd', 0),
                'Facilities SURF USD': metrics.get('surf_usd', 0) if dev_system == 'subsea' else 0,
                'Subsea Pumps USD': metrics.get('pump_usd', 0) if dev_system == 'subsea' else 0,
                'Dry Well Systems USD': metrics.get('dry_sys_usd', 0) if dev_system != 'subsea' else 0,
                'Facilities Cost USD': metrics.get('total_capex_facilities', 0),
                'DnC Drill Total USD': metrics.get('total_capex_drill', 0),
                'DnC Comp Total USD': metrics.get('total_capex_comp', 0),
                'DnC Total USD': metrics.get('total_capex_drill', 0) + metrics.get('total_capex_comp', 0),
                ...
                'ECON PATH USED': 'dry: MODU->DRY at FO' if dev_system != 'subsea' else 'subsea: MODU',
    ...
    def _add_rate_columns(self, worksheet, assumptions: Dict[str, Any]):
        ...
        headers = ["Discount Rate (Annual)", "MIRR Reinvest Rate (Annual)", "MIRR Finance Rate (Annual)"]
        rates = {
            "Discount Rate (Annual)": assumptions.get('discount_rate_annual', 0.10),
            "MIRR Reinvest Rate (Annual)": assumptions.get('mirr_reinvest_rate_annual', 0.10),
            "MIRR Finance Rate (Annual)": assumptions.get('mirr_finance_rate_annual', 0.10)
        }
