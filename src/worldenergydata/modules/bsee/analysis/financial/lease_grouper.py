...
from .data_loader import normalize_lease_number, std_columns, pick_column
...
def aggregate_production_by_lease(production_df: pd.DataFrame,
                                 wells_df: pd.DataFrame,
                                 leases_df: pd.DataFrame) -> pd.DataFrame:
    """
...
def map_wells_to_leases(wells_df: pd.DataFrame,
                       leases_df: pd.DataFrame) -> Dict[str, Dict[str, Any]]:
    """
...
class LeaseGrouper:
    ...
    def group_by_development(self, leases_df: pd.DataFrame,
    ...
    def map_wells_to_leases(self, wells_df: pd.DataFrame,
                           leases_df: pd.DataFrame) -> Dict[str, Dict[str, Any]]:
        """
        Create well to lease mapping
    ...
    def aggregate_production_by_lease(self, production_df: pd.DataFrame,
                                     wells_df: pd.DataFrame,
                                     leases_df: pd.DataFrame) -> pd.DataFrame:
        """
        Aggregate production by lease
    ...
    def group_development_wells(self, dev_name: str,
                               wells_df: pd.DataFrame,
                               leases_df: pd.DataFrame) -> pd.DataFrame:
        """
    ...
    def create_lease_summary(self, leases_df: pd.DataFrame,
                           wells_df: pd.DataFrame,
        ...
        if not wells_df.empty and "LEASE_NUM" in wells_df.columns:
            well_counts = wells_df.groupby("LEASE_NUM").agg(
                WELL_COUNT=("LEASE_NUM", "count"),
                ACTIVE_WELLS=("STATUS", lambda x: (x == "ACTIVE").sum() if "STATUS" in wells_df.columns else len(x))
            ).reset_index()
        ...
        if not production_df.empty:
            lease_prod = self.aggregate_production_by_lease(production_df, wells_df, leases_df)
            
            if not lease_prod.empty:
                # Sum total production by lease
                if "LEASE_NUM" in lease_prod.columns:
                    total_prod = lease_prod.groupby("LEASE_NUM")["OIL_BBL"].sum().reset_index()
                    ...
                    total_prod = pd.DataFrame({
                        ...
                        "TOTAL_PRODUCTION": [lease_prod[c].sum() for c in lease_cols]
                    })
    ...
    def get_lease_time_range(self, lease_num: str,
                           production_df: pd.DataFrame,
                           wells_df: pd.DataFrame) -> Tuple[pd.Timestamp, pd.Timestamp]:
        """
    ...
    def get_development_summary(self, dev_name: str) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "lease_numbers": dev_group["LEASE_NUM"].tolist() if "LEASE_NUM" in dev_group.columns else [],
            "dev_type": dev_group["DEV_TYPE_EFF"].iloc[0] if "DEV_TYPE_EFF" in dev_group.columns else "",
            "blocks": dev_group["BLOCK"].unique().tolist() if "BLOCK" in dev_group.columns else []
        }
        
        return summary
    
    def create_development_hierarchy(self, leases_df: pd.DataFrame,
                                    wells_df: pd.DataFrame) -> Dict[str, Dict[str, List[str]]]:
        """
