...
class AnalysisConfig:
    ...
    development_types: Dict[str, str] = field(default_factory=lambda: {
        "Stones": "subsea",
        "Anchor": "dry_tree",
        "Stampede": "subsea",
        "Julia": "subsea"
...
class FinancialAnalyzer:
    ...
    def __init__(self, config: AnalysisConfig):
        ...
        self.data_loader = SMEDataLoader(
    ...
    def _init_economics(self):
        ...
        self.financial_params = FinancialParameters(
            ...
            royalty_rate=1.0 - self.config.net_revenue_interest,  # Convert NRI to royalty
    ...
    def run_analysis(self, output_path: Optional[str] = None) -> AnalysisResult:
        ...
        try:
            ...
            if drilling_data is not None and not self.validator.validate_drilling_data(drilling_data):
                raise ValueError("Drilling data validation failed")
            
            # Process developments
            development_results = self.process_developments(production_data, drilling_data)
            
            # Generate report
            if output_path is None:
                output_path = Path(self.config.output_path) / f"financial_analysis_{datetime.now():%Y%m%d_%H%M%S}.xlsx"
    ...
    def load_data(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        logger.info(f"Loaded {len(production_data)} production records and {len(drilling_data)} drilling records")
        
        return production_data, drilling_data
    
    def process_developments(self, production_data: pd.DataFrame, 
                           drilling_data: Optional[pd.DataFrame] = None) -> Dict:
        """
    ...
    def _process_single_development(self, name: str, production_data: pd.DataFrame,
                                  drilling_data: Optional[pd.DataFrame], 
                                  development_type: str) -> Dict:
        """
        ...
        first_oil_date = production_data['year_month'].min() if not production_data.empty else None
        producer_count = production_data['API'].nunique() if 'API' in production_data.columns else 0
    ...
    def calculate_financial_metrics(self, cash_flow_df: pd.DataFrame) -> Dict:
        ...
        npv = self.cash_flow_calculator.calculate_npv(
            cash_flow_df,
            self.financial_params.discount_rate_annual
        )
        ...
        total_revenue = cash_flow_df['Revenue_Net'].sum() if 'Revenue_Net' in cash_flow_df else 0
        ...
        total_oil = cash_flow_df['Gross_Oil_bbls'].sum() if 'Gross_Oil_bbls' in cash_flow_df else 0
        
        # Payback period
        cum_cash_flow = cash_flow_df['Net_Cash_Flow'].cumsum() if 'Net_Cash_Flow' in cash_flow_df else pd.Series()
        payback_months = (cum_cash_flow > 0).idxmax() if not cum_cash_flow.empty and (cum_cash_flow > 0).any() else None
        ...
        return {
            ...
            'roi': (total_revenue - total_opex - total_capex) / total_capex if total_capex > 0 else 0
        }
    ...
    def _calculate_summary_metrics(self, development_results: Dict) -> Dict:
        ...
        successful_count = sum(
            1 for res in development_results.values()
            if 'error' not in res
        )
