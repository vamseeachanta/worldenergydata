...
from .data_loader import std_columns, pick_column, normalize_lease_number

logger = logging.getLogger(__name__)


def calculate_drilling_costs(dc_data: pd.DataFrame,
                            cost_per_day: float,
                            fill_na: float = 0) -> Dict[str, float]:
    """
...
def calculate_completion_costs(dc_data: pd.DataFrame,
                              cost_per_day: float,
                              fill_na: float = 0) -> Dict[str, float]:
    """
...
def build_dc_day_maps(dc_totals: pd.DataFrame,
                     dc_monthly: pd.DataFrame) -> Tuple[Dict, Dict, Dict]:
    """
    ...
    if not dc_totals.empty:
        well_col = pick_column(dc_totals, ["WELL_NAME", "WELL", "API_WELL_NUMBER", "API"])
        ...
        if well_col:
            for _, row in dc_totals.iterrows():
                ...
                drill_days = float(pd.to_numeric(row.get("DRILL_DAYS", 0), errors="coerce") or 0)
                comp_days = float(pd.to_numeric(row.get("COMP_DAYS", 0), errors="coerce") or 0)
                spud_date = pd.to_datetime(row.get("WELL_SPUD_DATE", pd.NaT), errors="coerce")
                td_date = pd.to_datetime(row.get("TOTAL_DEPTH_DATE", pd.NaT), errors="coerce")
    ...
    if not dc_monthly.empty:
        well_col = pick_column(dc_monthly, ["WELL_NAME", "WELL", "API_WELL_NUMBER", "API"])
        
        if well_col:
            # Group by well to get totals
            grp = dc_monthly.groupby(well_col, dropna=True).agg({
                "DRILL_DAYS": "sum",
                "COMP_DAYS": "sum"
            }).reset_index()
...
def allocate_dc_costs_monthly(dc_monthly: pd.DataFrame,
                             cost_assumptions: Dict[str, float]) -> pd.DataFrame:
    """
    ...
    result["DRILL_COST"] = pd.to_numeric(result.get("DRILL_DAYS", 0), errors="coerce").fillna(0) * drill_cost_per_day
    result["COMP_COST"] = pd.to_numeric(result.get("COMP_DAYS", 0), errors="coerce").fillna(0) * comp_cost_per_day
...
class DrillingCompletionProcessor:
    ...
    def __init__(self, cost_assumptions: Optional[Dict[str, float]] = None):
        ...
        self.drill_cost_per_day = self.cost_assumptions.get("DRILL_COST_PER_DAY", 100000)
        self.comp_cost_per_day = self.cost_assumptions.get("COMP_COST_PER_DAY", 75000)
        self.subsea_drill_cost = self.cost_assumptions.get("SUBSEA_DRILL_COST_PER_DAY", 150000)
        self.subsea_comp_cost = self.cost_assumptions.get("SUBSEA_COMP_COST_PER_DAY", 100000)
        self._cache = {}
    
    def calculate_drilling_costs(self, dc_data: pd.DataFrame,
                                cost_per_day: float,
                                fill_na: float = 0) -> Dict[str, float]:
        """Calculate drilling costs for wells"""
        return calculate_drilling_costs(dc_data, cost_per_day, fill_na)
    
    def calculate_completion_costs(self, dc_data: pd.DataFrame,
                                  cost_per_day: float,
                                  fill_na: float = 0) -> Dict[str, float]:
        """Calculate completion costs for wells"""
        return calculate_completion_costs(dc_data, cost_per_day, fill_na)
    
    def build_day_maps_for_development(self, dev_name: str,
                                      dc_totals: pd.DataFrame,
                                      dc_monthly: pd.DataFrame,
                                      leases_df: Optional[pd.DataFrame] = None) -> Tuple[Dict, Dict, Set]:
        """
        ...
        # Build maps
        drill_map, comp_map, input_totals = build_dc_day_maps(totals_filtered, monthly_filtered)
    ...
    def _filter_by_development(self, df: pd.DataFrame, dev_name: str,
                              leases_df: Optional[pd.DataFrame] = None) -> pd.DataFrame:
        ...
        if leases_df is not None and not leases_df.empty:
            ...
            if "LEASE_NUM" in result.columns and "LEASE_NUM" in leases.columns:
                result = result.merge(leases[["LEASE_NUM", "DEV_NAME"]], 
                                    on="LEASE_NUM", how="left")
            elif "LEASE_NAME" in result.columns and "LEASE_NAME" in leases.columns:
                result = result.merge(leases[["LEASE_NAME", "DEV_NAME"]], 
                                    on="LEASE_NAME", how="left")
    ...
    def allocate_costs_monthly(self, dc_monthly: pd.DataFrame,
                              cost_assumptions: Optional[Dict[str, float]] = None) -> pd.DataFrame:
        """
    ...
    def process_dc_totals(self, dc_totals: pd.DataFrame,
                         dc_monthly: pd.DataFrame) -> Tuple[Dict, Dict]:
        """
    ...
    def calculate_system_specific_costs(self, dc_data: pd.DataFrame,
                                       cost_assumptions: Dict[str, float]) -> Dict[str, float]:
        """
    ...
    def validate_dates(self, dc_data: pd.DataFrame) -> None:
        ...
        if "WELL_SPUD_DATE" in dc_data.columns and "TOTAL_DEPTH_DATE" in dc_data.columns:
    ...
    def generate_capex_schedule(self, dc_monthly: pd.DataFrame,
        ...
        if not monthly_costs.empty:
            # Aggregate by month
            capex_by_month = monthly_costs.groupby("YearMonth").agg({
                "DRILL_COST": "sum",
                "COMP_COST": "sum",
                "TOTAL_DC_COST": "sum"
            }).reset_index()
            ...
            # Rename columns
            capex_by_month.columns = ["YearMonth", "DRILL_CAPEX", "COMP_CAPEX", "TOTAL_CAPEX"]
    ...
    def calculate_first_oil_dates(self, dc_totals: pd.DataFrame,
                                 production_df: pd.DataFrame) -> Dict[str, pd.Timestamp]:
        """
    ...
    def calculate_efficiency_metrics(self, dc_totals: pd.DataFrame) -> Dict[str, float]:
        ...
        if "DRILL_DAYS" in dc_totals.columns:
            drill_days = pd.to_numeric(dc_totals["DRILL_DAYS"], errors="coerce").dropna()
    ...
    def build_monthly_dc_map(self, daily_data: pd.DataFrame) -> Dict[Tuple, float]:
        ...
        for activity in ["DRILL_ACTIVITY", "COMP_ACTIVITY"]:
            ...
            # Group and sum
            grouped = daily_data[daily_data[activity] > 0].groupby(
                ["YearMonth", "WELL_NAME"]
            )[activity].sum()
    ...
    def aggregate_dc_by_lease(self, dc_by_well: pd.DataFrame) -> pd.DataFrame:
        ...
        # Aggregate by lease
        lease_agg = dc_by_well.groupby("LEASE_NUM").agg({
            "DRILL_DAYS": "sum",
            "COMP_DAYS": "sum",
            "DRILL_COST": "sum",
            "COMP_COST": "sum"
        ...
        # Add total cost
        lease_agg["TOTAL_DC_COST"] = lease_agg["TOTAL_DRILL_COST"] + lease_agg["TOTAL_COMP_COST"]
