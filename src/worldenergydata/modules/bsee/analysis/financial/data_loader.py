...
def pick_column(df: pd.DataFrame, keys: List[str], required: bool = False) -> Optional[str]:
    """
...
class SMEDataLoader:
    ...
    def load_drilling_completion_data(self, excel_path: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
        """
        ...
        for sheet in xls.sheet_names:
            ...
            if well_col and month_col and "DRILL_DAYS" in df.columns and "COMP_DAYS" in df.columns:
                df[month_col] = pd.to_datetime(df[month_col], errors="coerce")
                df = df.rename(columns={month_col: "YearMonth"})
                df["YearMonth"] = df["YearMonth"].dt.to_period("M").dt.to_timestamp()
                
                keep_cols = [c for c in ["YearMonth", well_col, "DRILL_DAYS", "COMP_DAYS",
                                        "LEASE_NUM", "LEASE_NAME", "DEV_NAME",
                                        "WELL_SPUD_DATE", "TOTAL_DEPTH_DATE"] if c in df.columns]
                monthly_frames.append(df[keep_cols].copy())
            
            # Process totals data
            if (well_col and "WELL_SPUD_DATE" in df.columns and 
                "TOTAL_DEPTH_DATE" in df.columns and "DRILL_DAYS" in df.columns):
                
                keep_cols = [c for c in [well_col, "WELL_SPUD_DATE", "TOTAL_DEPTH_DATE",
                                        "DRILL_DAYS", "COMP_DAYS", "LEASE_NUM",
                                        "LEASE_NAME", "DEV_NAME"] if c in df.columns]
    ...
    def filter_by_development(self, df: pd.DataFrame, dev_name: str,
        ...
        if "DEV_NAME" in result.columns:
            result = result[result["DEV_NAME"].astype(str).str.strip().str.upper() == 
    ...
    def _matrix_to_timeseries(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Melt to long format
        long = df.melt(id_vars=[well_key], value_vars=month_cols,
                      var_name="YearMonth", value_name="BBLS_PER_DAY")
    ...
    def build_development_day_maps(self, dev_name: str, leases_df: pd.DataFrame,
                                  production_df: pd.DataFrame,
                                  dc_totals: pd.DataFrame) -> Tuple[Dict, Dict, set]:
        """
        ...
        if not dc_dev.empty:
            well_col = pick_column(dc_dev, ["WELL_NAME", "WELL", "API_WELL_NUMBER", "API"])
            ...
            if well_col:
                for _, row in dc_dev.iterrows():
                    ...
                    if well:
                        ...
                        if "DRILL_DAYS" in row and pd.notna(row["DRILL_DAYS"]) and row["DRILL_DAYS"] > 0:
                            spud_date = pd.to_datetime(row.get("WELL_SPUD_DATE"), errors="coerce")
                        ...
                        if "COMP_DAYS" in row and pd.notna(row["COMP_DAYS"]) and row["COMP_DAYS"] > 0:
                            td_date = pd.to_datetime(row.get("TOTAL_DEPTH_DATE"), errors="coerce")
                            if pd.notna(td_date):
    ...
    def load_wti_price_deck(self, excel_path: Optional[str] = None,
                           flat_price: float = 80.0) -> PriceDeck:
        """
