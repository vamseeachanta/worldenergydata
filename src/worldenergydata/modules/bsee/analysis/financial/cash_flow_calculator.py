...
class CashFlowCalculator:
    ...
    def calculate_monthly_cash_flow(
        ...
        result['Revenue_Net'] = (result['Revenue_Gross'] - royalty_amt - 
                                 severance_amt + result['Revenue_Adjustments'])
        
        # Calculate OPEX
        var_opex_rate = (self.params.subsea_opex_per_bbl if development_type == DevelopmentType.SUBSEA 
        ...
        for month in result.index:
            ...
            result.loc[month, 'CAPEX_Drill'] = result.loc[month, 'DRILL_DAYS_TOT'] * dayrate
            result.loc[month, 'CAPEX_Comp'] = result.loc[month, 'COMP_DAYS_TOT'] * dayrate
        ...
        result['CAPEX'] = result['CAPEX_Drill'] + result['CAPEX_Comp'] + result['CAPEX_Facilities']
        
        # Calculate after-tax values
        result['CAPEX_Drill_Net'] = result['CAPEX_Drill'] * (1.0 - self.params.corporate_tax_rate)
        result['CAPEX_Comp_Net'] = result['CAPEX_Comp'] * (1.0 - self.params.corporate_tax_rate)
        result['CAPEX_Facilities_Net'] = result['CAPEX_Facilities'] * (1.0 - self.params.corporate_tax_rate)
        result['Tax_Savings'] = result['CAPEX'] * self.params.corporate_tax_rate
        
        # Calculate net cash flow
        result['Net_Cash_Flow'] = (result['Revenue_Net'] - result['OPEX'] - 
                                   result['CAPEX'] + result['Tax_Savings'])
    ...
    def _allocate_facilities_capex(
        ...
        if development_type == DevelopmentType.SUBSEA:
            ...
            if injector_count > 0:
                surf_total_usd += injector_count * self.params.surf_per_well_mm * 1_000_000
            
            if surf_total_usd > 0 and self.params.surf_prefo_months > 0:
                start_date = first_oil_date - pd.DateOffset(months=self.params.surf_prefo_months)
                end_date = first_oil_date - pd.DateOffset(months=1)
                window = pd.date_range(start_date, end_date, freq='MS')
        ...
        # Dry well systems (dry tree only)
            ...
            dry_sys_per_prod_usd = self.params.dry_well_system_per_producer_mm * 1_000_000
    ...
    def calculate_financial_metrics(self, cash_flow_df: pd.DataFrame) -> Dict[str, float]:
        """
        ...
        metrics['total_capex_facilities'] = float(cash_flow_df['CAPEX_Facilities'].sum())
