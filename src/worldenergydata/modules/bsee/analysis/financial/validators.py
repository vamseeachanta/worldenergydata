...
def validate_date_columns(df: pd.DataFrame, date_cols: List[str]) -> pd.DataFrame:
    ...
    for col in date_cols:
        if col in result.columns:
            ...
            if invalid_count > 0:
                logger.warning(f"Column '{col}' has {invalid_count} invalid date values")
    
    return result


def validate_numeric_columns(df: pd.DataFrame, numeric_cols: List[str],
                           fill_invalid: Optional[float] = None) -> pd.DataFrame:
    """
    ...
    for col in numeric_cols:
        if col in result.columns:
            ...
            invalid_count = result[col].isna().sum()
            if invalid_count > 0:
                logger.warning(f"Column '{col}' has {invalid_count} invalid numeric values")
    
    return result


def validate_lease_numbers(df: pd.DataFrame, lease_col: str = "LEASE_NUM") -> pd.DataFrame:
    """
...
def validate_drilling_data(df: pd.DataFrame) -> Tuple[bool, List[str]]:
    ...
    if "DRILL_DAYS" in df.columns and (pd.to_numeric(df["DRILL_DAYS"], errors='coerce') < 0).any():
        errors.append("DRILL_DAYS contains negative values")
    
    if "COMP_DAYS" in df.columns and (pd.to_numeric(df["COMP_DAYS"], errors='coerce') < 0).any():
    ...
    if "WELL_SPUD_DATE" in df.columns and "TOTAL_DEPTH_DATE" in df.columns:
        ...
        invalid_dates = (td_dates < spud_dates) & pd.notna(spud_dates) & pd.notna(td_dates)
...
def validate_financial_assumptions(assumptions: Dict[str, Any]) -> Tuple[bool, List[str]]:
    """
    ...
    # Validate costs
    cost_keys = ["Drilling_Cost_Per_Day_USD", "Completion_Cost_Per_Day_USD", "OPEX_Per_Barrel"]
...
class DataValidator:
    ...
    def validate_financial_assumptions(self, assumptions: Dict[str, Any]) -> Tuple[bool, List[str]]:
        """Validate financial assumptions"""
        self._validation_count += 1
    ...
    def check_data_consistency(self, production_df: pd.DataFrame,
                              drilling_df: pd.DataFrame) -> List[str]:
        """
        ...
        if prod_only:
            ...
            self._validation_report["warnings"].append(f"Production-only wells: {len(prod_only)}")
        
        if drill_only:
            warnings.append(f"Wells in drilling but not production: {drill_only}")
            self._validation_report["warnings"].append(f"Drilling-only wells: {len(drill_only)}")
        
        return warnings
    
    def validate_data_types(self, df: pd.DataFrame, 
                           type_map: Dict[str, str]) -> Tuple[bool, Dict[str, str]]:
        """
    ...
    def validate_ranges(self, df: pd.DataFrame,
                       range_rules: Dict[str, Tuple[Optional[float], Optional[float]]]) -> Dict[str, List[int]]:
        """
