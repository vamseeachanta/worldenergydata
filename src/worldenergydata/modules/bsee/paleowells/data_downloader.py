...
class BSEEDataDownloader:
    ...
    def __init__(self, data_directory: Optional[Path] = None,
        ...
        if data_directory is None:
            data_directory = Path(__file__).parent.parent.parent.parent.parent.parent / "data" / "modules" / "bsee" / "downloads"
                ...
                if not overwrite:
                    for member in zf.namelist():
                        if (extract_to / member).exists():
                            logger.info(f"Extraction skipped - files already exist in {extract_to}")
                            return True
        ...
        if self.download_file(url, download_path, overwrite):
            ...
            if extract and zipfile.is_zipfile(download_path):
                if self.extract_zip(download_path, dataset_dir, overwrite):
                    ...
                    extracted = [f for f in dataset_dir.iterdir() 
