...
Usage:
    python -m worldenergydata.modules.bsee.paleowells [command] [options]
"""
...
def process_command(args):
    ...
    if args.input_file:
        ...
        df = processor.parse_fixed_width_file(
            ...
            Path(args.output_directory) / "paleowells.csv" if args.output_directory else None
        )
...
def visualize_command(args):
    ...
    if args.csv_file:
        ...
        if not default_csv.exists():
            print(f"Error: No CSV file found. Please specify with --csv-file or run process command first.")
            sys.exit(1)
    ...
    output_dir = Path(args.output_directory) if args.output_directory else Path.cwd() / "paleowells_figures"
    ...
    if args.report:
        ...
        if args.epochs:
            ...
            print(f"Created epoch distribution plot")
            
        if args.depth:
            fig = visualizer.plot_depth_distribution(
                df, 
                depth_type=args.depth_type,
                save_path=output_dir / f"{args.depth_type.lower().replace(' ', '_')}_distribution.png"
            )
            print(f"Created depth distribution plot")
            
        if args.classification:
            fig = visualizer.plot_classification_pie(df, output_dir / "classification.png")
            print(f"Created classification pie chart")
...
def main():
    ...
    process_parser.add_argument('--input-file', '-i', 
                              help='Input file with raw paleo data')
    process_parser.add_argument('--data-directory', '-d',
                              default='data/modules/bsee/paleowells',
                              help='Data directory (default: data/modules/bsee/paleowells)')
    process_parser.add_argument('--output-directory', '-o',
                              help='Output directory for processed files')
    process_parser.add_argument('--analyze', '-a', action='store_true',
    ...
    download_parser.add_argument('--data-directory', '-d',
                                default='data/modules/bsee/downloads',
                                help='Directory to store downloads')
    download_parser.add_argument('--extract', '-e', action='store_true', default=True,
    ...
    viz_parser.add_argument('--csv-file', '-f',
                          help='CSV file with processed paleo data')
    viz_parser.add_argument('--data-directory', '-d',
                          default='data/modules/bsee/paleowells',
                          help='Data directory')
    viz_parser.add_argument('--output-directory', '-o',
                          help='Output directory for figures')
    viz_parser.add_argument('--report', '-r', action='store_true',
                          help='Generate comprehensive visual report')
    viz_parser.add_argument('--epochs', action='store_true',
                          help='Plot wells by epoch')
    viz_parser.add_argument('--depth', action='store_true',
                          help='Plot depth distribution')
    viz_parser.add_argument('--depth-type', 
                          choices=['True Vertical Depth', 'Measured Depth'],
                          default='True Vertical Depth',
                          help='Type of depth to plot')
    viz_parser.add_argument('--classification', action='store_true',
                          help='Plot classification pie chart')
