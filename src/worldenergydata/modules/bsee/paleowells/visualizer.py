...
class PaleowellsVisualizer:
    ...
    def plot_wells_by_epoch(
        ...
        df["Epoch"] = df["Paleo Age"].str.extract(
            r"(Paleocene|Eocene|Oligocene|Miocene|Pliocene)"
        )

        # Count unique wells by epoch
        epoch_counts = (
            df.groupby("Epoch")["API Well Number"]
            .nunique()
        ...
        ax.set_title(
            "Distribution of Wells by Geological Epoch", fontsize=16, fontweight="bold"
        )
    ...
    def plot_depth_distribution(
        ...
        plt.suptitle(
            "Depth Analysis of Paleowells", fontsize=16, fontweight="bold", y=1.02
        )
        plt.tight_layout()
    ...
    def plot_classification_pie(
        ...
        legend_labels = [
            f"{label}: {count:,} ({count/classification_counts.sum()*100:.1f}%)"
    ...
    def create_comprehensive_report(
        self, df: pd.DataFrame, output_directory: Path
    ) -> Dict[str, Path]:
        """
        ...
        try:
            ...
            epoch_fig = self.plot_wells_by_epoch(
                df, save_path=output_directory / "wells_by_epoch.png"
            )
            ...
            figures["tvd_distribution"] = (
                output_directory / "true_vertical_depth_distribution.png"
            )
            plt.close(tvd_fig)
            ...
            figures["md_distribution"] = (
                output_directory / "measured_depth_distribution.png"
            )
            plt.close(md_fig)

            # Classification pie chart
            class_fig = self.plot_classification_pie(
                df, save_path=output_directory / "classification_distribution.png"
            )
            figures["classification"] = (
                output_directory / "classification_distribution.png"
            )
            plt.close(class_fig)
