...
class HierarchicalDataLoader:
    ...
    def __init__(self, data_path: Optional[Path] = None, use_enhanced_refresh: bool = True):
        """
    ...
    def refresh_data_if_needed(self, cfg: Dict[str, Any], force_refresh: bool = False) -> None:
        """
    ...
    def load_hierarchy(self, block_number: Optional[str] = None,
        ...
        """
        logger.info(f"Loading hierarchy: block={block_number}, field={field_name}, lease={lease_number}")
    ...
    def _load_block_data(self, block_number: str, cfg: Optional[Dict[str, Any]] = None) -> pd.DataFrame:
        """
        ...
        if 'parameters' not in block_cfg:
            block_cfg['parameters'] = {'filepath': {'Well_APD_Default': str(self.data_path / "binary")}}
        ...
        try:
            ...
            output_path = Path(block_cfg.get('Analysis', {}).get('result_folder', 'results')) / 'Data'
    ...
    def _load_lease_data(self, lease_number: str, cfg: Optional[Dict[str, Any]] = None) -> pd.DataFrame:
        """
        ...
        if 'parameters' not in lease_cfg:
            lease_cfg['parameters'] = {'filepath': {'Well_APD_Default': str(self.data_path / "binary")}}
        ...
        try:
            ...
            # Load the generated CSV file
            output_path = Path(lease_cfg.get('Analysis', {}).get('result_folder', 'results')) / 'Data'
    ...
    def _load_comprehensive_data(self, block_number: Optional[str] = None,
                                field_name: Optional[str] = None,
                                lease_number: Optional[str] = None,
                                block_df: Optional[pd.DataFrame] = None,
                                lease_df: Optional[pd.DataFrame] = None) -> Dict[str, Any]:
        """
        ...
        if lease_df is not None and not lease_df.empty:
            ...
            if 'Lease Number' in lease_df.columns or 'LEASE_NUMBER' in lease_df.columns:
                lease_col = 'Lease Number' if 'Lease Number' in lease_df.columns else 'LEASE_NUMBER'
                leases = lease_df[lease_col].unique()
                for lease in leases:
                    raw_data['leases'].append({
                        'id': str(lease),
                        'number': str(lease),
                        'field_id': lease_df[lease_df[lease_col] == lease].iloc[0].get('Field Name', 'Unknown')
                    })
        ...
        if not raw_data['wells']:
            raw_data.update(self._load_from_binary(self.data_path / "binary", block_number, field_name))
    ...
    def _load_production_data(self, block_number: Optional[str] = None,
                             field_name: Optional[str] = None,
                             lease_number: Optional[str] = None) -> List[Dict[str, Any]]:
        """
            ...
            try:
                with open(prod_file, 'rb') as f:
                    ...
                    for prod_record in all_production:
                        # Apply filters
                        if lease_number and prod_record.get('lease_number') != lease_number:
                            continue
                        if field_name and prod_record.get('field_name') != field_name:
                            continue
                        if block_number and block_number not in prod_record.get('area_block', ''):
                            continue
    ...
    def _load_well_data(self, api_number: str, cfg: Optional[Dict[str, Any]] = None) -> pd.DataFrame:
        """
        ...
        if 'parameters' not in well_cfg:
            well_cfg['parameters'] = {'filepath': {'Well_APD_Default': str(self.data_path / "binary")}}
        ...
        try:
            ...
            results = self.well_data_fetcher.get_api_data_from_input_bin_files([api_number])
    ...
    def fetch_wells_by_api_list(self, api_numbers: List[str], cfg: Optional[Dict[str, Any]] = None) -> pd.DataFrame:
        """
        ...
        try:
            ...
            results = self.well_data_fetcher.get_api_data_from_input_bin_files(api_numbers)
            ...
            if results:
                ...
                if 'API Well Number' in combined_df.columns:
                    combined_df = combined_df.drop_duplicates(subset=['API Well Number'])
                
                logger.info(f"Successfully fetched data for {len(combined_df)} unique wells")
                return combined_df
            else:
