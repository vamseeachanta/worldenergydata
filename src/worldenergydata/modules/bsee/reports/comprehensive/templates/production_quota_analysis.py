...
from .compliance_template import ProductionQuota, ComplianceMetrics
...
class QuotaVarianceAnalysis:
    ...
    def get_variance_status(self) -> Dict[str, str]:
        ...
        oil_status = "Over" if self.is_oil_over_quota() else "Under" if self.oil_variance_bbls < 0 else "On Target"
        gas_status = "Over" if self.is_gas_over_quota() else "Under" if self.gas_variance_mcf < 0 else "On Target"
        
        return {
            "oil_status": oil_status,
            "gas_status": gas_status,
            "overall_status": "Over Quota" if self.is_any_quota_exceeded() else "Within Limits"
        }
    
    def calculate_penalty_exposure(self, oil_penalty_rate: float = 25.0, 
                                 gas_penalty_rate: float = 5.0) -> Dict[str, float]:
        """Calculate potential penalty exposure for over-production"""
        oil_penalty = max(0, self.oil_variance_bbls) * oil_penalty_rate if self.is_oil_over_quota() else 0
        gas_penalty = max(0, self.gas_variance_mcf) * gas_penalty_rate if self.is_gas_over_quota() else 0
    ...
    def calculate_opportunity_cost(self, oil_price_usd: float = 75.0, 
                                 gas_price_usd: float = 4.0) -> Dict[str, float]:
        """Calculate opportunity cost of under-production"""
        oil_opportunity = abs(min(0, self.oil_variance_bbls)) * oil_price_usd if self.oil_variance_bbls < 0 else 0
        gas_opportunity = abs(min(0, self.gas_variance_mcf)) * gas_price_usd if self.gas_variance_mcf < 0 else 0
...
class ProductionQuotaAnalyzer:
    ...
    def analyze_quota_compliance(self, production: ProductionMetrics, 
                               quota: ProductionQuota) -> QuotaVarianceAnalysis:
    ...
    def analyze_multiple_periods(self, production_list: List[ProductionMetrics], 
                               quota_list: List[ProductionQuota]) -> List[QuotaVarianceAnalysis]:
    ...
    def _find_matching_quota(self, production: ProductionMetrics, 
                           quota_list: List[ProductionQuota]) -> Optional[ProductionQuota]:
        """Find matching quota for production data"""
        for quota in quota_list:
            if (quota.entity_id == production.entity_id and
                self._periods_overlap(production, quota)):
                return quota
        return None
    
    def _periods_overlap(self, production: ProductionMetrics, 
                        quota: ProductionQuota) -> bool:
        """Check if production and quota periods overlap"""
        if not all([production.period_start, production.period_end, 
                   quota.period_start, quota.period_end]):
            return False
        
        return (production.period_start <= quota.period_end and
                production.period_end >= quota.period_start)
    
    def calculate_compliance_trend(self, analyses: List[QuotaVarianceAnalysis]) -> Dict[str, Any]:
        ...
        return {
            ...
            "overall_trend": "Improving" if (oil_trend + gas_trend) > 0.01 
                           else "Declining" if (oil_trend + gas_trend) < -0.01 
                           else "Stable",
    ...
    def generate_compliance_rating(self, analysis: QuotaVarianceAnalysis) -> Dict[str, str]:
    ...
    def create_quota_dashboard_data(self, analyses: List[QuotaVarianceAnalysis]) -> Dict[str, Any]:
        ...
        dashboard_data = {
            ...
            "historical_performance": {
                ...
                "oil_compliance_avg": np.mean(oil_compliances) if oil_compliances else 0,
                ...
                "gas_compliance_avg": np.mean(gas_compliances) if gas_compliances else 0,
            ...
            "risk_metrics": {
                "periods_over_quota": len([a for a in analyses if a.is_any_quota_exceeded()]),
                "max_oil_overage": max([a.oil_variance_bbls for a in analyses if a.oil_variance_bbls > 0], default=0),
                "max_gas_overage": max([a.gas_variance_mcf for a in analyses if a.gas_variance_mcf > 0], default=0)
            }
        }
        
        return dashboard_data
    
    def export_quota_analysis_report(self, analyses: List[QuotaVarianceAnalysis], 
    ...
    def _create_executive_summary(self, analyses: List[QuotaVarianceAnalysis]) -> Dict[str, Any]:
        ...
        summary = {
            "overall_compliance_rate": (compliant_periods / total_periods * 100) if total_periods > 0 else 0,
            ...
            "recommendation": self._generate_compliance_recommendation(oil_avg, gas_avg, compliant_periods, total_periods)
        }
        
        return summary
    
    def _generate_compliance_recommendation(self, oil_avg: float, gas_avg: float, 
                                         compliant_periods: int, total_periods: int) -> str:
        """Generate compliance recommendation"""
        compliance_rate = (compliant_periods / total_periods) if total_periods > 0 else 0
