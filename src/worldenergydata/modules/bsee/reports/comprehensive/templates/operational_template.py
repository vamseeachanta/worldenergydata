...
class EquipmentMetrics:
    ...
    def equipment_availability(self) -> float:
        ...
        total_time = self.total_runtime_hours + self.unplanned_downtime_hours + self.planned_downtime_hours
        if total_time > 0:
...
class OperationalTemplate(BaseReportTemplate):
    ...
    def __init__(self, template_name: str = "operational_report", 
    ...
    def build_operational_context(self,
        ...
        self._add_operational_summary(context, well_metrics, production_metrics, 
                                     equipment_metrics)
        ...
        kpis = self.calculate_operational_kpis(well_metrics, production_metrics, 
                                              equipment_metrics)
    ...
    def _add_operational_summary(self, context: Dict[str, Any],
                                well_metrics: List[WellOperationalMetrics],
        ...
        avg_availability = np.mean([w.availability_percentage() for w in well_metrics]) if well_metrics else 0
        ...
        if production_metrics:
            summary["production_efficiency"] = round(production_metrics.production_efficiency(), 2)
        
        if equipment_metrics:
            avg_reliability = np.mean([e.equipment_reliability() for e in equipment_metrics])
            summary["equipment_reliability"] = round(avg_reliability, 2)
        
        context["operational_summary"] = summary
    
    def _add_well_performance_analysis(self, context: Dict[str, Any],
                                      well_metrics: List[WellOperationalMetrics]):
        ...
        drilling_efficiencies = [w.drilling_efficiency() for w in well_metrics 
                                if w.actual_drilling_days > 0]
        if drilling_efficiencies:
            performance["average_drilling_efficiency"] = round(np.mean(drilling_efficiencies), 2)
        
        completion_efficiencies = [w.completion_efficiency() for w in well_metrics 
                                  if w.actual_completion_days > 0]
        if completion_efficiencies:
            performance["average_completion_efficiency"] = round(np.mean(completion_efficiencies), 2)
        ...
        performance["total_production_boe"] = sum(w.cumulative_production_boe for w in well_metrics)
    ...
    def _add_production_efficiency_analysis(self, context: Dict[str, Any],
    ...
    def _add_equipment_reliability_analysis(self, context: Dict[str, Any],
    ...
    def _add_maintenance_schedule(self, context: Dict[str, Any],
                                 maintenance_records: List[MaintenanceRecord]):
        """Add maintenance schedule to context"""
        schedule = {
            "total_maintenance": len(maintenance_records),
            "preventive": sum(1 for m in maintenance_records if m.maintenance_type == "preventive"),
            "corrective": sum(1 for m in maintenance_records if m.maintenance_type == "corrective"),
        ...
        upcoming = [m for m in maintenance_records if m.next_scheduled_date and not m.is_overdue()]
    ...
    def _add_failure_analysis(self, context: Dict[str, Any],
                             failure_records: List[FailureAnalysis]):
    ...
    def add_drilling_performance_analysis(self, context: Dict[str, Any],
                                        well_metrics: List[WellOperationalMetrics]):
        ...
        performance = {
            ...
            "average_drilling_days": np.mean([w.actual_drilling_days for w in drilling_data]),
            "average_drilling_depth": np.mean([w.drilling_depth_ft for w in drilling_data]),
            "average_drilling_rate": np.mean([w.drilling_rate_ft_per_day() for w in drilling_data]),
            "drilling_efficiency": np.mean([w.drilling_efficiency() for w in drilling_data]),
            "total_drilling_cost": sum(w.drilling_cost for w in drilling_data),
            "average_cost_per_foot": np.mean([w.cost_per_foot_drilled() for w in drilling_data]),
            "wells_by_status": {}
        }
        
        # Count wells by drilling performance
        performance["wells_on_time"] = sum(1 for w in drilling_data 
    ...
    def add_completion_efficiency_metrics(self, context: Dict[str, Any],
                                        well_metrics: List[WellOperationalMetrics]):
        ...
        metrics = {
            ...
            "average_completion_days": np.mean([w.actual_completion_days for w in completion_data]),
            "completion_efficiency": np.mean([w.completion_efficiency() for w in completion_data]),
            "total_completion_cost": sum(w.completion_cost for w in completion_data),
            "average_frac_stages": np.mean([w.frac_stages for w in completion_data if w.frac_stages > 0]),
            "total_proppant_lbs": sum(w.proppant_lbs for w in completion_data)
        }
        
        context["completion_metrics"] = metrics
    
    def add_production_optimization_tracking(self, context: Dict[str, Any],
                                           production_metrics: ProductionEfficiencyMetrics):
        ...
        optimization = {
            ...
            "capacity_utilization": (production_metrics.actual_production_boe / 
                                   (production_metrics.design_capacity_boe * production_metrics.production_days) * 100
                                   if production_metrics.design_capacity_boe > 0 else 0),
    ...
    def add_equipment_utilization_analysis(self, context: Dict[str, Any],
                                         equipment_metrics: List[EquipmentMetrics]):
        ...
        analysis = {
            "total_equipment": len(equipment_metrics),
            "average_availability": np.mean([e.equipment_availability() for e in equipment_metrics]),
            "average_utilization": np.mean([e.equipment_utilization() for e in equipment_metrics]),
            "average_reliability": np.mean([e.equipment_reliability() for e in equipment_metrics]),
            "average_mtbf": np.mean([e.mtbf_hours for e in equipment_metrics if e.mtbf_hours > 0]),
            "average_mttr": np.mean([e.mttr_hours for e in equipment_metrics if e.mttr_hours > 0]),
    ...
    def calculate_operational_kpis(self,
                                  well_metrics: List[WellOperationalMetrics],
                                  production_metrics: Optional[ProductionEfficiencyMetrics] = None,
                                  equipment_metrics: Optional[List[EquipmentMetrics]] = None) -> List[OperationalKPI]:
        """
        ...
        if well_metrics:
            ...
            kpis.append(OperationalKPI(
                ...
                status="good" if avg_availability >= 90 else "warning" if avg_availability >= 85 else "critical",
                trend="stable"
            ))
            ...
            if drilling_wells:
                avg_drilling_eff = np.mean([w.drilling_efficiency() for w in drilling_wells])
                kpis.append(OperationalKPI(
                    kpi_id="KPI-002",
                    ...
                    status="good" if avg_drilling_eff >= 95 else "warning" if avg_drilling_eff >= 90 else "critical",
        ...
        if production_metrics:
            ...
            kpis.append(OperationalKPI(
                ...
                status="good" if prod_eff >= 85 else "warning" if prod_eff >= 75 else "critical",
                trend="stable"
            ))
            ...
            kpis.append(OperationalKPI(
                ...
                status="good" if op_eff >= 95 else "warning" if op_eff >= 90 else "critical",
                trend="stable"
            ))
        ...
        if equipment_metrics:
            ...
            kpis.append(OperationalKPI(
                ...
                status="good" if avg_reliability >= 98 else "warning" if avg_reliability >= 95 else "critical",
                trend="stable"
            ))
            
            # Equipment utilization
            avg_utilization = np.mean([e.equipment_utilization() for e in equipment_metrics])
            kpis.append(OperationalKPI(
                kpi_id="KPI-006",
                ...
                status="good" if avg_utilization >= 85 else "warning" if avg_utilization >= 75 else "critical",
                trend="improving" if avg_utilization > 80 else "declining"
            ))
        
        return kpis
    
    def generate_operational_visualizations(self, context: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def _create_well_status_chart(self, summary: Dict[str, Any]) -> str:
        ...
        values = [
            ...
            summary.get("total_wells", 0) - summary.get("wells_producing", 0) - 
            summary.get("wells_drilling", 0) - summary.get("wells_offline", 0)
        ]
    ...
    def _create_reliability_chart(self, reliability: Dict[str, Any]) -> str:
        ...
        fig = go.Figure(data=[
            go.Bar(name='Availability', x=names, y=availabilities, marker_color='#3498db'),
            go.Bar(name='Reliability', x=names, y=reliabilities, marker_color='#2ecc71')
        ])
    ...
    def _create_kpi_dashboard(self, kpis: List[Dict[str, Any]]) -> str:
        ...
        for i, kpi in enumerate(kpis):
            ...
            color = "#2ecc71" if kpi["status"] == "good" else "#f39c12" if kpi["status"] == "warning" else "#e74c3c"
