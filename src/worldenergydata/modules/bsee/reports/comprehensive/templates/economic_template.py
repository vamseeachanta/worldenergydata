...
class EconomicTemplate(BaseReportTemplate):
    ...
    def _apply_goby_revenue_calculations(
        self, production: ProductionMetrics
    ) -> Dict[str, Any]:
        ...
        production_data = {
            ...
            "gas_mcf": production.gas_production_mcf,
            "ngl_bbls": getattr(
                production, "ngl_production_bbls", 0.0
            ),  # Add if available
            "water_bbls": production.water_production_bbls,
        }
        ...
        return {
            ...
            "goby_economic_summary": {
                ...
                "boe_production": production_data["oil_bbls"]
                + (production_data["gas_mcf"] / 6),
                "revenue_per_boe": (
                    revenue_calculations["gross_revenue"]
                    / (production_data["oil_bbls"] + (production_data["gas_mcf"] / 6))
                    if (production_data["oil_bbls"] + (production_data["gas_mcf"] / 6))
                    > 0
                    else 0
                ),
                "cost_per_boe": (
                    cost_calculations["total_costs"]
                    / (production_data["oil_bbls"] + (production_data["gas_mcf"] / 6))
                    if (production_data["oil_bbls"] + (production_data["gas_mcf"] / 6))
                    > 0
                    else 0
    ...
    def _calculate_goby_14_row_structure(
        self,
        production_data: Dict[str, float],
    ...
    def _prepare_tornado_chart_data(
        self,
    ...
    def add_economic_forecast(
        ...
        forecast_npv = npf.npv(
            0.10, [0] + forecasted_cash_flows
        )  # No initial investment assumed
    ...
    def calculate_enhanced_netback_analysis(self) -> Dict[str, Any]:
        ...
        revenue_per_boe_breakdown = {
            ...
            "total_revenue_per_boe": revenue_breakdown.get("total_revenue", 0)
            / total_boe,
        }

        # Cost components per BOE
        cost_per_boe_breakdown = {
            "operating_cost_per_boe": cost_analysis.get("operating_costs", 0)
            / total_boe,
            ...
            / total_boe,
            "processing_cost_per_boe": cost_analysis.get("processing_costs", 0)
            / total_boe,
    ...
    def add_enhanced_cost_structure_analysis(self) -> Dict[str, Any]:
        ...
        if total_costs > 0:
            cost_breakdown_pct = {
                ...
                "royalties_pct": (
                    cost_analysis.get("royalties", 0) / total_costs * 100
                ),
                "severance_tax_pct": (
    ...
    def add_revenue_optimization_analysis(self) -> Dict[str, Any]:
        ...
        revenue_quality = {
            ...
            "high_value_revenue_pct": oil_pct
            + ngl_pct,  # Oil and NGL typically higher value
    ...
    def _identify_revenue_optimization_opportunities(
        self, revenue_mix: Dict[str, float], revenue_quality: Dict[str, float]
    ) -> List[str]:
        ...
        if revenue_quality.get("revenue_diversification_index", 0) < 0.3:
            opportunities.append(
                "Improve revenue diversification across hydrocarbon types"
            )
    ...
    def get_economic_kpis(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "secondary_kpis": {
                ...
                "operating_netback_per_boe": netback_analysis[
                    "netback_calculation"
                ].get("operating_netback_per_boe", 0),
            },
            "financial_ratios": {
            ...
            "cost_efficiency_metrics": {
                "operating_cost_per_boe": netback_analysis[
                    "cost_per_boe_breakdown"
                ].get("operating_cost_per_boe", 0),
        ...
        well_economics = {
            ...
            "cost_efficiency": {
                ...
                "initial_cost_per_daily_boe": (
                    initial_well_cost
                    / (
                        well_context["production_metrics"]["oil_bbls"] / 365
                        + well_context["production_metrics"]["gas_mcf"] / (365 * 6)
                    )
                    if well_context["production_metrics"]["oil_bbls"] > 0
                    or well_context["production_metrics"]["gas_mcf"] > 0
                    else 0
                ),
            },
            "profitability_assessment": {
                ...
                "investment_grade": self._determine_well_investment_grade(
    ...
    def _determine_well_investment_grade(
        self, npv_12: float, irr: float, payback_years: float
    ) -> str:
    ...
    def compare_wells_economic_performance(
        ...
        paybacks = [
            w["payback_years"] for w in well_comparisons if w["payback_years"] < 20
        ]
        ...
        return {
            ...
            "investment_recommendations": self._generate_well_investment_recommendations(
                portfolio_stats, well_comparisons
            ),
        }

    def _generate_well_investment_recommendations(
        self, portfolio_stats: Dict[str, Any], well_comparisons: List[Dict[str, Any]]
    ) -> List[str]:
        ...
        profitable_rate = (
            portfolio_stats["profitable_wells_12_percent"]
            / portfolio_stats["total_wells"]
        )
        if profitable_rate < 0.6:
            recommendations.append(
                "Portfolio has low profitability rate - review drilling strategy and target selection"
            )
        ...
        if excellent_wells:
            recommendations.append(
                f"Focus on replicating success patterns from {len(excellent_wells)} excellent-grade wells"
            )

        poor_wells = [w for w in well_comparisons if w["investment_grade"] == "Poor"]
        if poor_wells:
            recommendations.append(
                f"Investigate and address issues with {len(poor_wells)} poor-performing wells"
            )
    ...
    def generate_economic_dashboard(
        ...
        values = [
            netback_analysis["revenue_per_boe_breakdown"].get(
                "total_revenue_per_boe", 0
            ),
        ...
        fig.update_yaxes(
            title_text="Value per BOE ($)", tickformat="$,.2f", row=3, col=1
        )
    ...
    def generate_sensitivity_analysis_tables(self) -> Dict[str, str]:
        ...
        for oil_price in oil_price_scenarios:
            ...
            annual_gas_revenue = (
                temp_context.get("annual_gas_mcf", 500000) * gas_price_base
            )
        ...
        for gas_price in gas_price_scenarios:
            ...
            # Simplified NPV calculation for sensitivity
            annual_oil_revenue = (
                temp_context.get("annual_oil_bbl", 100000) * oil_price_base
            )
        ...
        for oil_price in oil_scenarios:
            ...
            for gas_price in gas_scenarios:
                ...
                annual_oil_revenue = (
                    self.economic_context.get("annual_oil_bbl", 100000) * oil_price
                )
                annual_gas_revenue = (
                    self.economic_context.get("annual_gas_mcf", 500000) * gas_price
                )
                annual_revenue = annual_oil_revenue + annual_gas_revenue
                ...
                for year in range(10):
                    decline_factor = (
                        1 - self.economic_context.get("decline_rate", 0.08)
                    ) ** year
                    cash_flows.append(annual_net_cash * decline_factor)
