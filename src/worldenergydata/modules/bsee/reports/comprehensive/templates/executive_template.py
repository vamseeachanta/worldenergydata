...
class ExecutiveTemplate(BaseReportTemplate):
                ...
                kpis.append(
                    ExecutiveKPI(
                        ...
                        trend=self._calculate_trend(
                            fin_data.get("revenue_history", [])
                        ),
                        status=self._determine_financial_status(
            ...
            if "efficiency_rate" in ops_data:
                kpis.append(
                    ExecutiveKPI(
                        ...
                        trend=self._calculate_trend(
                            ops_data.get("efficiency_history", [])
                        ),
                        status=self.determine_kpi_status(
            ...
            if "total_boe" in prod_data:
                kpis.append(
                    ExecutiveKPI(
                        ...
                        trend=self._calculate_trend(
                            prod_data.get("production_history", [])
                        ),
                        status=self._determine_production_status(
            ...
            if "emissions_tons_co2" in env_data:
                kpis.append(
                    ExecutiveKPI(
                        ...
                        trend=self._calculate_trend(
                            env_data.get("emissions_history", [])
                        ),
                        status=self._determine_emissions_status(
    ...
    def _determine_production_status(
        self, value: float, target: Optional[float]
    ) -> str:
    ...
    def analyze_year_over_year(self, data: Dict) -> Dict:
        ...
        if (
            len(years) > 2
            and "revenue" in data[years[0]]
            and "revenue" in data[latest_year]
        ):
    ...
    def generate_strategic_forecast(
        ...
        if "production" in historical_data.columns:
            ...
            future_x = np.arange(
                len(production_values), len(production_values) + periods
            )
    ...
    def create_traffic_light_grid(self, metrics: List[Dict]) -> Any:
        ...
        fig.update_layout(
            height=100 * rows, title="Status Indicators", showlegend=False
        )
    ...
    def create_multi_panel_dashboard(self, panels: List[Dict], data: Dict) -> Any:
        ...
        fig = make_subplots(
            ...
            specs=[
                [{"type": p.get("type", "scatter")}] * max_col for _ in range(max_row)
            ],
        )
            ...
            if panel_type == "kpi_grid" and "kpis" in data:
                ...
                fig.add_trace(
                    go.Bar(
                        ...
                        marker_color=[
                            self._get_status_color(k.get("status", "gray"))
                            for k in kpis
                        ],
                    ),
                    row=row,
