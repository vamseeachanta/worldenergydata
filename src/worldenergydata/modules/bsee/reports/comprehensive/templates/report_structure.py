...
class GoByReportStructure:
    ...
    def to_template_context(self) -> Dict[str, Any]:
        ...
        context["header_rows"] = [r for r in context["rows"] if r["row_type"] == "header"]
        context["field_info_rows"] = [r for r in context["rows"] if r["row_type"] == "field_info"]
        context["production_rows"] = [r for r in context["rows"] if r["row_type"] == "production_summary"]
        context["well_rows"] = [r for r in context["rows"] if r["row_type"] == "well_details"]
        context["economic_rows"] = [r for r in context["rows"] if r["row_type"] == "economics"]
    ...
    def validate_structure(self) -> Dict[str, Any]:
        ...
        required_types = {ReportRowType.HEADER, ReportRowType.PRODUCTION_SUMMARY, ReportRowType.ECONOMICS}
        missing_types = required_types - row_types
        if missing_types:
            results["warnings"].append(f"Missing recommended row types: {[t.value for t in missing_types]}")
        
        # Check for rows without values
        empty_rows = [row.row_number for row in self.rows if row.value is None and row.row_type != ReportRowType.SPACER]
        if empty_rows:
...
class GoByReportMapper:
    ...
    @staticmethod
    def from_production_metrics(production_data: Any, entity_info: Dict[str, Any]) -> GoByReportStructure:
        ...
        if context["total_revenue"] and context["operating_costs"]:
            context["net_income"] = context["total_revenue"] - context["operating_costs"]
            if context["total_revenue"] > 0:
                context["operating_margin"] = (context["net_income"] / context["total_revenue"]) * 100
        
        report.populate_from_context(context)
        return report
    
    @staticmethod
    def from_economic_metrics(economic_data: Any, entity_info: Dict[str, Any]) -> GoByReportStructure:
    ...
    @staticmethod
    def create_custom_structure(row_definitions: List[Dict[str, Any]], entity_info: Dict[str, Any]) -> GoByReportStructure:
