...
if TYPE_CHECKING:
    from ..models import ProductionMetrics, EconomicMetrics, WellSummary
...
class BaseReportTemplate:
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.template_path = Path(template_path) if template_path else self._get_default_template_path()
    ...
    def _get_custom_filters(self) -> Dict[str, callable]:
        ...
        return {
            'currency': lambda x, symbol='$': f"{symbol}{x:,.2f}" if x is not None else "N/A",
            'percentage': lambda x, decimals=1: f"{x:.{decimals}f}%" if x is not None else "N/A",
            'number_format': lambda x, decimals=0: f"{x:,.{decimals}f}" if x is not None else "N/A",
    ...
    def _get_custom_functions(self) -> Dict[str, callable]:
        ...
        return {
            ...
            'sum_list': lambda lst, key=None: sum(item[key] if key else item for item in lst if item is not None),
            'avg_list': lambda lst, key=None: sum(item[key] if key else item for item in lst if item is not None) / len([x for x in lst if x is not None]) if lst else 0
        }
    ...
    def set_context(self, context: Dict[str, Any]):
        ...
        self.context.require_fields(list(self.context.required_fields))  # Preserve requirements
    ...
    def render(self, template_file: Optional[str] = None, context: Optional[Dict[str, Any]] = None) -> str:
        ...
        try:
            ...
            raise TemplateRuntimeError(f"Error rendering template {self.template_name}: {str(e)}")
        except Exception as e:
            raise TemplateError(f"Unexpected error rendering template {self.template_name}: {str(e)}")
