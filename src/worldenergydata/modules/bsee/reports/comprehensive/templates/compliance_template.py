...
class ComplianceTemplate(BaseReportTemplate):
    ...
    def add_production_quota_analysis(self, production: Any, quota: ProductionQuota):
        ...
        # Calculate variance
        quota_analysis["oil_variance"] = quota_analysis["oil_actual"] - quota.oil_quota_bbls
        quota_analysis["gas_variance"] = quota_analysis["gas_actual"] - quota.gas_quota_mcf
        quota_analysis["oil_variance_pct"] = (quota_analysis["oil_variance"] / quota.oil_quota_bbls * 100) if quota.oil_quota_bbls > 0 else 0
        quota_analysis["gas_variance_pct"] = (quota_analysis["gas_variance"] / quota.gas_quota_mcf * 100) if quota.gas_quota_mcf > 0 else 0
    ...
    def add_production_quota_analysis(self, production: ProductionMetrics, 
        ...
        # Calculate variance
        quota_analysis["oil_variance"] = production.oil_production_bbls - quota.oil_quota_bbls
        quota_analysis["gas_variance"] = production.gas_production_mcf - quota.gas_quota_mcf
        quota_analysis["oil_variance_pct"] = (quota_analysis["oil_variance"] / quota.oil_quota_bbls * 100) if quota.oil_quota_bbls > 0 else 0
        quota_analysis["gas_variance_pct"] = (quota_analysis["gas_variance"] / quota.gas_quota_mcf * 100) if quota.gas_quota_mcf > 0 else 0
        
        self.context["quota_analysis"] = quota_analysis
    
    def add_environmental_compliance_tracking(self, environmental_metrics: EnvironmentalMetrics):
        ...
        env_compliance["threshold_compliance"] = self._check_environmental_thresholds(environmental_metrics)
        
        # Add environmental performance benchmarks
        env_compliance["benchmarks"] = self._get_environmental_benchmarks()
        env_compliance["benchmark_comparison"] = self._compare_to_benchmarks(environmental_metrics)
        
        # Add environmental risk assessment
        env_compliance["risk_assessment"] = self._assess_environmental_risks(environmental_metrics)
        
        # Add compliance trends with historical analysis
        env_compliance["trends"] = self._analyze_environmental_trends(environmental_metrics)
        
        # Add corrective actions and recommendations
        env_compliance["corrective_actions"] = self._generate_environmental_actions(environmental_metrics)
        
        # Add environmental KPIs
        env_compliance["kpis"] = self._calculate_environmental_kpis(environmental_metrics)
    ...
    def _check_environmental_thresholds(self, metrics: EnvironmentalMetrics) -> Dict[str, Any]:
        """Check environmental metrics against regulatory thresholds"""
        thresholds = self._get_environmental_thresholds()
        
        compliance_checks = {
            "spill_incidents_compliant": metrics.spill_incidents <= thresholds["spill_incidents_monthly"],
            "spill_volume_compliant": metrics.total_spill_volume_bbls <= thresholds["spill_volume_monthly_bbls"],
            "air_emissions_compliant": metrics.air_emissions_tons <= thresholds["air_emissions_annual_tons"],
            "water_discharge_compliant": metrics.water_discharge_bbls <= thresholds["water_discharge_monthly_bbls"],
            "waste_compliant": metrics.waste_generated_tons <= thresholds["waste_monthly_tons"],
            "violations_compliant": metrics.environmental_violations <= thresholds["violation_tolerance"]
        }
        
        # Calculate overall threshold compliance
        compliant_count = sum(compliance_checks.values())
        total_checks = len(compliance_checks)
        compliance_checks["overall_threshold_compliance"] = (compliant_count / total_checks) * 100
    ...
    def _compare_to_benchmarks(self, metrics: EnvironmentalMetrics) -> Dict[str, Any]:
        ...
        comparison = {
            "vs_industry_average": "Above" if env_score > benchmarks["industry_average"]["environmental_score"] else "Below",
            "vs_best_in_class": "Above" if env_score > benchmarks["best_in_class"]["environmental_score"] else "Below",
            "vs_regulatory_minimum": "Above" if env_score > benchmarks["regulatory_minimum"]["environmental_score"] else "Below",
            "performance_percentile": self._calculate_performance_percentile(env_score),
            "improvement_potential": max(0, benchmarks["best_in_class"]["environmental_score"] - env_score)
        }
    ...
    def _assess_environmental_risks(self, metrics: EnvironmentalMetrics) -> Dict[str, Any]:
        ...
        return {
            ...
            "mitigation_priority": "Immediate" if risk_level == "High" else "Planned" if risk_level == "Medium" else "Routine"
        }
    ...
    def _analyze_environmental_trends(self, metrics: EnvironmentalMetrics) -> Dict[str, Any]:
        ...
        return {
            ...
            "trend_analysis": {
                ...
                "forecast": "Monitor closely" if env_score < 0.8 else "Continue current practices"
            },
            "leading_indicators": {
    ...
    def _generate_environmental_actions(self, metrics: EnvironmentalMetrics) -> List[Dict[str, Any]]:
    ...
    def _calculate_environmental_kpis(self, metrics: EnvironmentalMetrics) -> Dict[str, Any]:
        ...
        return {
            ...
            "calculated_kpis": {
                "spill_rate_per_incident": (
                    ...
                    if metrics.spill_incidents > 0 else 0
                ),
                "environmental_efficiency": metrics.calculate_environmental_score() * 100,
                "regulatory_compliance_rate": (
                    100 if metrics.environmental_violations == 0 else 
                    max(0, 100 - (metrics.environmental_violations * 25))
                )
            },
            "performance_targets": {
    ...
    def add_regulatory_milestone_tracking(self, milestones: List[RegulatoryMilestone]):
        ...
        milestone_tracking = {
            ...
            "pending": len([m for m in milestones if not m.is_completed() and not m.is_overdue()]),
            "overdue": len([m for m in milestones if m.is_overdue()]),
            "upcoming_30_days": len([m for m in milestones if 0 <= m.days_until_due() <= 30]),
            "milestones": milestones
        }
        
        # Calculate completion rate
        if milestone_tracking["total_milestones"] > 0:
            milestone_tracking["completion_rate"] = (milestone_tracking["completed"] / 
                                                   milestone_tracking["total_milestones"]) * 100
        else:
    ...
    def _create_production_quota_chart(self) -> str:
        ...
        quota_values = [quota_data["oil_quota"], quota_data["gas_quota"] / 1000]  # Convert gas to Mcf
    ...
    def _create_environmental_trends_chart(self) -> str:
        ...
        fig.add_trace(
            go.Scatter(x=months, y=spill_incidents, mode='lines+markers', 
                      name="Spill Incidents", line=dict(color='red')),
            secondary_y=False,
        )
        
        fig.add_trace(
            go.Scatter(x=months, y=environmental_scores, mode='lines+markers',
                      name="Environmental Score", line=dict(color='green')),
            secondary_y=True,
        )
        ...
        fig.update_layout(
            title="Environmental Compliance Trends",
            height=400
        )
    ...
    def _create_milestone_timeline(self) -> str:
        ...
        if not milestones:
            ...
            fig.update_layout(
                ...
                annotations=[dict(text="No milestones to display", 
                                showarrow=False, x=0.5, y=0.5)]
            )
            return pio.to_html(fig, include_plotlyjs='cdn')
        
        # Prepare data for timeline
        milestone_names = [m.description[:50] + "..." if len(m.description) > 50 else m.description 
                          for m in milestones]
        ...
        if completion_dates:
            completed_names = [milestone_names[i] for i, m in enumerate(milestones) 
    ...
    def add_regulatory_references(self):
        ...
        reference_metadata = {
            ...
            "compliance_areas": list(set([ref["compliance_area"] for ref in all_references if ref.get("compliance_area")])),
    ...
    def generate_compliance_report_sections(self) -> Dict[str, Any]:
        ...
        if self.compliance_sections["production_compliance"]:
            sections["production_compliance"] = self._generate_production_compliance_section()
        
        # Environmental Compliance Section
        if self.compliance_sections["environmental_compliance"]:
            sections["environmental_compliance"] = self._generate_environmental_section()
    ...
    def _generate_recommendations(self, summary: Dict[str, Any]) -> List[str]:
        ...
        if prod_compliance < 95:
            recommendations.append("Review production practices to ensure compliance with permitted levels")
        
        # Environmental recommendations
        env_score = summary.get("environmental_score", 1.0)
        if env_score < 0.9:
            recommendations.append("Implement enhanced environmental monitoring and spill prevention measures")
        
        # Safety recommendations  
        safety_score = summary.get("safety_score", 1.0)
        if safety_score < 0.9:
            recommendations.append("Increase safety training and incident prevention protocols")
        
        # Violations recommendations
        violations = summary.get("regulatory_violations", 0)
        if violations > 0:
            recommendations.append("Address outstanding regulatory violations and implement corrective actions")
        
        if not recommendations:
            recommendations.append("Continue current compliance practices and maintain monitoring protocols")
    ...
    def _generate_production_compliance_section(self) -> Dict[str, Any]:
        ...
        return {
            "permitted_production": compliance.permitted_production_bbls if compliance else 0,
            "actual_production": compliance.actual_production_bbls if compliance else 0,
            "compliance_percentage": compliance.production_compliance_percentage() if compliance else 0,
            "quota_analysis": quota_analysis,
            "status": compliance.overall_compliance_status() if compliance else "Unknown"
        }
