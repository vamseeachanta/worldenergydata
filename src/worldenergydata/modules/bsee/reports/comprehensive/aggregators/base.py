...
from ..models import (
    ...
    OrganizationalUnit, Well, Lease, Field, Block
)
...
class DataAggregator(ABC):
    ...
    def stream_data(self, data_source: Any, chunk_size: int = 1000) -> Iterator[Dict[str, Any]]:
        """
    ...
    def aggregate_revenue_costs(self, unit: OrganizationalUnit, 
                               price_deck: Dict[str, float]) -> Dict[str, float]:
        """
        ...
        total_operating_cost = oil_operating_cost + water_disposal_cost_total + gas_processing_cost_total
    ...
    def aggregate_hierarchical_economics(self, hierarchy: Dict[str, Any],
                                        price_deck: Dict[str, float]) -> Dict[str, Any]:
        """
        ...
        for lease_id, lease in hierarchy.get('leases', {}).items():
            ...
            economic_summary['totals']['gross_revenue'] += lease_economics['gross_revenue']
            economic_summary['totals']['operating_income'] += lease_economics['operating_income']
    ...
    def validate_data_quality(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        quality_metrics['completeness'] = present_fields / len(required_fields) if required_fields else 0
    ...
    def aggregate_with_streaming(self, data_source: Any, 
                                chunk_size: int = 1000,
                                memory_limit_mb: int = 500) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for chunk_num, chunk in enumerate(self.stream_data(data_source, chunk_size), 1):
                # Check memory usage
                current_memory = process.memory_info().rss / 1024 / 1024
                if current_memory - initial_memory > memory_limit_mb:
                    logger.warning(f"Memory limit exceeded: {current_memory:.1f}MB used")
                    ...
                    current_memory = process.memory_info().rss / 1024 / 1024
                    if current_memory - initial_memory > memory_limit_mb:
                        logger.error(f"Memory limit still exceeded after GC: {current_memory:.1f}MB")
                        break
                ...
                # Log progress
                if chunk_num % 100 == 0:
                    logger.info(f"Processed {chunk_num} chunks, {total_results['record_count']} records")
        ...
        logger.info(f"Streaming aggregation complete: {total_results['chunks_processed']} chunks, "
                   f"{total_results['record_count']} records in {total_results['processing_time_sec']:.1f}s")
        
        return total_results
    
    def stream_large_file(self, file_path: Path, 
                         file_type: str = 'csv',
                         chunk_size: int = 10000) -> Iterator[Dict[str, Any]]:
        """
