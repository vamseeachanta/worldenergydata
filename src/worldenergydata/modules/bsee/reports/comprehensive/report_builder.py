...
class GoByReportBuilder:
    ...
    def _build_metadata(self, report_type: str, entity_identifier: str) -> Dict[str, Any]:
        """
    ...
    def _create_well_row(self, field: Optional[Dict], 
                        lease: Dict, 
                        well: Dict) -> List[Any]:
        """
        ...
        return [
            ...
            well.get('spud_date', 'N/A') if well.get('spud_date') else 'N/A',  # Spud Date
    ...
    def export_to_excel(self, report: Dict[str, Any], 
                       output_path: Path,
    ...
    def generate_step_by_step_report(self, 
                                    report_type: str,
                                    entity_identifier: str,
                                    output_path: Path,
        ...
        if detail_rows:
            for row in detail_rows:
                if len(row) != len(self.GOBY_ROW_STRUCTURE):
                    logger.warning(f"Row length mismatch: expected {len(self.GOBY_ROW_STRUCTURE)}, got {len(row)}")
