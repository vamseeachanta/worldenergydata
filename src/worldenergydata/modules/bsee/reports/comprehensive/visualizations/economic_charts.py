...
class EconomicChart:
    ...
    def create_waterfall_chart(self, categories: List[str],
                              values: List[float],
                              title: str = None,
                              measure: Optional[List[str]] = None) -> go.Figure:
        """
        ...
        if measure is None:
            ...
            for i, cat in enumerate(categories):
                ...
                elif 'total' in cat.lower() or 'net' in cat.lower() and i == len(categories) - 1:
    ...
    def create_roi_analysis(self, metrics: EconomicMetrics,
        ...
        # ROI Gauge
        roi_color = 'green' if metrics.roi > 15 else 'orange' if metrics.roi > 10 else 'red'
        ...
        fig.add_trace(go.Indicator(
            ...
            delta={'reference': benchmarks.get('npv', 100) / 1_000_000 if benchmarks else 100},
            number={'valueformat': ',.1f'},
            domain={'x': [0.5, 1], 'y': [0.5, 1]}
        ), row=1, col=2)
        
        # IRR Gauge
        irr_color = 'green' if metrics.irr > 20 else 'orange' if metrics.irr > 15 else 'red'
        fig.add_trace(go.Indicator(
            mode="gauge+number+delta",
            value=metrics.irr,
            title={'text': "IRR (%)"},
        ...
        payback_color = 'green' if metrics.payback_period < 3 else 'orange' if metrics.payback_period < 5 else 'red'
        fig.add_trace(go.Indicator(
            mode="gauge+number+delta",
            value=metrics.payback_period,
            title={'text': "Payback (Years)"},
            delta={'reference': benchmarks.get('payback', 4) if benchmarks else 4,
    ...
    def create_cashflow_projection(self, cashflow_data: pd.DataFrame,
                                  title: str = None,
                                  show_cumulative: bool = True) -> go.Figure:
        """
    ...
    def create_cost_breakdown(self, costs: Dict[str, float],
                            title: str = None,
    ...
    def create_revenue_streams(self, revenue_data: pd.DataFrame,
    ...
    def create_sensitivity_analysis(self, base_npv: float,
    ...
    def create_investment_comparison(self, investments: Dict[str, Dict[str, float]],
                                   title: str = None) -> go.Figure:
        """
    ...
    def create_breakeven_analysis(self, production_data: pd.DataFrame,
        ...
        volumes = production_data['volume'] if 'volume' in production_data.columns else production_data.iloc[:, 0]
        ...
        for scenario, price in price_scenarios.items():
            if price > variable_costs:
                ...
                fig.add_trace(go.Scatter(
                    ...
                    hovertemplate=f'<b>Breakeven Point</b><br>' +
