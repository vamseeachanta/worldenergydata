...
class ProductionChart:
    ...
    def create_production_trend(self, data: pd.DataFrame, 
        ...
        for product in products:
            if product in data.columns:
                fig.add_trace(go.Scatter(
                    ...
                    mode='lines',
                    line=dict(
                        color=colors.get(product, '#757575'),
                        width=2
                    ),
    ...
    def create_cumulative_production(self, data: pd.DataFrame,
                                    products: List[str] = None,
                                    title: str = None) -> go.Figure:
        """
    ...
    def create_production_forecast(self, historical: pd.DataFrame,
        ...
        fig.add_trace(go.Scatter(
            x=historical.index,
            y=historical['oil'] if 'oil' in historical.columns else historical.iloc[:, 0],
            name='Historical',
            mode='lines',
    ...
    def create_well_comparison(self, well_data: Dict[str, pd.DataFrame],
                              metric: str = 'oil',
                              title: str = None) -> go.Figure:
        """
    ...
    def create_production_efficiency(self, data: pd.DataFrame,
                                    target: float,
                                    title: str = None) -> go.Figure:
        """
        ...
        # Actual efficiency
        efficiency_col = 'efficiency' if 'efficiency' in data.columns else data.columns[0]
        ...
        fig.add_trace(go.Scatter(
            ...
            y=data[efficiency_col],
            fill='tonexty',
            fillcolor='rgba(76, 175, 80, 0.2)' if data[efficiency_col].mean() >= target else 'rgba(244, 67, 54, 0.2)',
            line=dict(width=0),
            showlegend=False,
            hoverinfo='skip'
        ))
    ...
    def create_monthly_summary(self, data: pd.DataFrame,
                              products: List[str] = None,
                              title: str = None) -> go.Figure:
        """
    ...
    def export_chart(self, fig: go.Figure, 
                    filename: str,
                    format: str = 'png',
                    **kwargs) -> bool:
        """
    ...
    def create_decline_curve(self, data: pd.DataFrame,
        ...
        if decline_params:
            ...
            if b_factor == 0:
                # Exponential decline
                decline_production = initial_rate * np.exp(-decline_rate * time_days / 365)
            else:
                # Hyperbolic decline
                decline_production = initial_rate / (1 + b_factor * decline_rate * time_days / 365) ** (1/b_factor)
