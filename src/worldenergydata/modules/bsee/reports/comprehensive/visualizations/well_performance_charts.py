...
class WellPerformanceChart:
    ...
    def create_heat_map(self, data: pd.DataFrame,
                       wells: List[str] = None,
        ...
        if wells:
            data = data[data.index.isin(wells) | data.get('well_name', pd.Series()).isin(wells)]
        if metrics:
            data = data[metrics]
    ...
    def create_bubble_chart(self, data: pd.DataFrame,
                           x_metric: str,
                           y_metric: str,
                           size_metric: str,
        ...
        size_scaled = (size_values - size_values.min()) / (size_values.max() - size_values.min()) * 50 + 10
    ...
    def create_performance_radar(self, metrics: PerformanceMetrics,
        ...
        """
        categories = ['Efficiency', 'Uptime', 'Production Rate',
                      'Water Cut', 'GOR', 'Operating Cost']
    ...
    def create_well_ranking(self, data: pd.DataFrame,
                          metric: str,
                          top_n: int = 10,
                          title: str = None) -> go.Figure:
        """
        ...
        colors = ['#4CAF50' if val >= data[metric].quantile(0.75) else
                  '#FFC107' if val >= data[metric].quantile(0.5) else
                  '#FF9800' if val >= data[metric].quantile(0.25) else
                  '#F44336' for val in sorted_data[metric]]
        
        fig = go.Figure()
        
        fig.add_trace(go.Bar(
            x=sorted_data[metric],
            y=sorted_data.index if 'well_name' not in sorted_data.columns else sorted_data['well_name'],
            orientation='h',
    ...
    def create_time_series_comparison(self, data: Dict[str, pd.DataFrame],
                                     metric: str,
                                     title: str = None) -> go.Figure:
        """
    ...
    def create_efficiency_matrix(self, data: pd.DataFrame,
                                title: str = None) -> go.Figure:
        """
        ...
        fig = make_subplots(
            rows=2,
            cols=2,
            subplot_titles=('Production Efficiency', 'Operating Time',
