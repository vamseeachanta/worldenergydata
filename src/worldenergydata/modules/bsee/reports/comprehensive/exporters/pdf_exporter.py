...
class PDFExporter(ReportExporter):
    ...
    def export(self, data: Dict[str, Any], config: ExportConfig) -> ExportResult:
        ...
        try:
            ...
            # Write PDF
            html_doc.write_pdf(str(config.output_path), stylesheets=[css_doc])
            ...
            if result.file_size > 5 * 1024 * 1024:  # 5MB warning
                result.add_warning(
                    f"Large PDF size: {result.file_size / (1024*1024):.1f}MB"
                )
    ...
    def generate_html(self, data: Dict[str, Any]) -> str:
        ...
        <body>
        """.format(
                title=data.get("report_metadata", {}).get("title", "Report")
            )
        )
        ...
        if "executive_summary" in data.get("content", {}):
            html_parts.append(
                self._generate_section(
                    "Executive Summary", data["content"]["executive_summary"]
                )
            )
        ...
        if "key_findings" in data.get("content", {}):
            html_parts.append(
                self._generate_list_section(
                    "Key Findings", data["content"]["key_findings"]
                )
            )
        ...
        if "production_data" in data:
            html_parts.append(
                self._generate_table_section("Production Data", data["production_data"])
            )
    ...
    def embed_chart_as_image(self, chart_data: Any) -> str:
        ...
        if base64_data:
            return (
                f'<img src="data:image/png;base64,{base64_data}" class="chart-image">'
            )
