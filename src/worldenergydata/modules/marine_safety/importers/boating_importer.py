...
from ..constants import IncidentType, VesselType, VesselFlag
...
class BoatingImporter(BaseImporter):
    ...
    def _load_related_data(self):
        ...
        if self.vessels_file and self.vessels_file.exists():
            ...
            print(f"Loaded {len(self.vessels_by_bardid)} unique BARD incidents with vessel data")
        ...
        if self.deaths_file and self.deaths_file.exists():
            ...
            print(f"Loaded {sum(len(v) for v in self.deaths_by_bardid.values())} death records")
        ...
        if self.injuries_file and self.injuries_file.exists():
            ...
            with open(self.injuries_file, 'r', encoding='utf-8', errors='replace') as f:
                ...
                for row in reader:
                    bardid = row.get('BARDID', '').strip()
                    if bardid:
                        self.injuries_by_bardid[bardid] = self.injuries_by_bardid.get(bardid, 0) + 1
    ...
    def parse_record(self, raw_record: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if 'estimated_damage_usd' in parsed:
                try:
                    damage_str = parsed['estimated_damage_usd'].replace(',', '').replace('$', '')
            ...
            if state:
                parsed['location'] = {
                    ...
                    'body_of_water': raw_record.get('NameofBodyofWater', '').strip().strip('"'),
                }
            ...
            if bardid in self.vessels_by_bardid:
                ...
                if vessels:
                    ...
                    parsed['vessel'] = {
                        'vessel_name': vessel_data.get('VesselName', '').strip().strip('"'),
                        ...
                        'registration_number': vessel_data.get('RegistrationNumber', '').strip().strip('"'),
                    }
    ...
    def _get_or_create_location_from_state(self, location_data: Dict[str, Any]) -> Optional[int]:
