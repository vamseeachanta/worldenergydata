...
class TSBImporter(BaseImporter):
    ...
    def parse_record(self, raw_record: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            parsed["injuries"] = self._parse_int(
                raw_record.get("TotalMinorInjuries", "0")
            ) + self._parse_int(raw_record.get("TotalSeriousInjuries", "0"))
            parsed["missing"] = self._parse_int(
                raw_record.get("TotalMissingIndividuals", "0")
            )
            ...
            if province or lat or lon:
                parsed["location"] = {
                    ...
                    "latitude": lat,
                    "longitude": lon,
                    "region": raw_record.get(
                        "RegionOfOccurrenceDisplayEng", ""
                    ).strip(),
            ...
            if occno in self.vessels_by_occno:
                ...
                if vessels:
                    ...
                    parsed["vessel"] = {
                        ...
                        "flag_state": vessel_data.get(
                            "VesselFlagDisplayEng", ""
                        ).strip(),
                        "length_meters": self._parse_float(
                            vessel_data.get("Length_Meters", "")
                        ),
                        "gross_tonnage": self._parse_float(
                            vessel_data.get("GrossTonnage", "")
                        ),
                        "year_built": self._parse_int(vessel_data.get("YearBuilt", "")),
                        "crew_count": self._parse_int(
                            vessel_data.get("NumberOfCrew", "0")
                        ),
                        "passenger_count": self._parse_int(
                            vessel_data.get("NumberOfPass", "0")
                        ),
                        "total_people": self._parse_int(
                            vessel_data.get("TotalPeopleOnBoard", "0")
                        ),
                    }
            ...
            if raw_record.get("WeatherConditionDisplayEng"):
                metadata["weather"] = raw_record.get(
                    "WeatherConditionDisplayEng", ""
                ).strip()
            if raw_record.get("WindSpeed_Knots"):
                metadata["wind_speed_knots"] = self._parse_float(
                    raw_record.get("WindSpeed_Knots", "")
                )
            ...
            if raw_record.get("VisibilityDistance_Nm"):
                metadata["visibility_nm"] = self._parse_float(
                    raw_record.get("VisibilityDistance_Nm", "")
                )
            if raw_record.get("LightConditionDisplayEng"):
                metadata["light_condition"] = raw_record.get(
                    "LightConditionDisplayEng", ""
                ).strip()
