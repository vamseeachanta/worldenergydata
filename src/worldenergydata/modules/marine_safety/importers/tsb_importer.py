...
class TSBImporter(BaseImporter):
    ...
    def _load_related_data(self):
        ...
        if self.vessels_file and self.vessels_file.exists():
            ...
            count = 0
            with open(self.vessels_file, 'r', encoding='utf-8-sig', errors='replace') as f:
                reader = csv.DictReader(f)
                for row in reader:
                    # Strip BOM from keys (handles \ufeff)
                    cleaned_row = {k.lstrip('\ufeff').strip(): v for k, v in row.items()}
                    ...
                    if occno:
                        ...
                        count += 1
            print(f"Loaded {count} vessel records for {len(self.vessels_by_occno)} unique occurrences")

        # Load injuries
        if self.injuries_file and self.injuries_file.exists():
            print(f"Loading injuries data from {self.injuries_file}...")
            count = 0
            with open(self.injuries_file, 'r', encoding='utf-8-sig', errors='replace') as f:
                reader = csv.DictReader(f)
                for row in reader:
                    # Strip BOM from keys
                    cleaned_row = {k.lstrip('\ufeff').strip(): v for k, v in row.items()}
                    ...
                    if occno:
                        ...
                        count += 1
            print(f"Loaded {count} injury records for {len(self.injuries_by_occno)} unique occurrences")
        ...
        print(f"Equipment files will be processed on-demand (large datasets)")
    ...
    def read_source(self) -> Generator[Dict[str, Any], None, None]:
        ...
        with open(self.source_path, 'r', encoding='utf-8-sig', errors='replace') as f:
            ...
            for row in reader:
                ...
                normalized_row = {k.lstrip('\ufeff').strip() if k else k: v for k, v in row.items() if k is not None}
    ...
    def parse_record(self, raw_record: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Skip duplicates - TSB data has many duplicate OccNos (same incident, multiple vessels/events)
            ...
            # Casualties
            parsed['fatalities'] = self._parse_int(raw_record.get('TotalDeaths', '0'))
            parsed['injuries'] = (
                self._parse_int(raw_record.get('TotalMinorInjuries', '0')) +
                self._parse_int(raw_record.get('TotalSeriousInjuries', '0'))
            )
            parsed['missing'] = self._parse_int(raw_record.get('TotalMissingIndividuals', '0'))
            ...
            if province or lat or lon:
                parsed['location'] = {
                    ...
                    'position_description': raw_record.get('NearestLocationDescription', '').strip(),
                    'position_distance_nm': self._parse_float(raw_record.get('NearestLocationDistance_Nm', '')),
                }

            # Vessel information from vessel.csv
            if occno in self.vessels_by_occno:
                vessels = self.vessels_by_occno[occno]
                if vessels:
            ...
            metadata = {}
            if raw_record.get('WeatherConditionDisplayEng'):
                metadata['weather'] = raw_record.get('WeatherConditionDisplayEng', '').strip()
            if raw_record.get('WindSpeed_Knots'):
                metadata['wind_speed_knots'] = self._parse_float(raw_record.get('WindSpeed_Knots', ''))
            if raw_record.get('SeaStateDisplayEng'):
                metadata['sea_state'] = raw_record.get('SeaStateDisplayEng', '').strip()
            if raw_record.get('VisibilityDistance_Nm'):
                metadata['visibility_nm'] = self._parse_float(raw_record.get('VisibilityDistance_Nm', ''))
            if raw_record.get('LightConditionDisplayEng'):
                metadata['light_condition'] = raw_record.get('LightConditionDisplayEng', '').strip()
    ...
    def _get_or_create_vessel(self, vessel_data: Dict[str, Any]) -> Optional[int]:
        ...
        try:
            ...
            # Create new vessel
            metadata = {}
            for key in ['official_number', 'mmsi', 'call_sign', 'crew_count',
                       'passenger_count', 'total_people', 'gross_tonnage']:
