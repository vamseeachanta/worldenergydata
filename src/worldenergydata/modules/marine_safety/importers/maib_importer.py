    ...
    def parse_record(self, raw_record: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if occid in self.vessels_by_occid:
                ...
                if vessels:
                    ...
                    parsed["vessel"] = {
                        ...
                        "vessel_category_l1": vessel_data.get(
                            "Vessel_Category_L1", ""
                        ).strip(),
                        "vessel_category_l2": vessel_data.get(
                            "Vessel_Category_L2", ""
                        ).strip(),
                        "ship_type_l1": vessel_data.get(
                            "Ship_Craft_Type_L1", ""
                        ).strip(),
                        "ship_type_l2": vessel_data.get(
                            "Ship_Craft_Type_L2", ""
                        ).strip(),
                        "ship_type_l3": vessel_data.get(
                            "Ship_Craft_Type_L3", ""
                        ).strip(),
                        ...
                        "gross_tonnage": self._parse_float(
                            vessel_data.get("GT_Gross_Tonnage", "")
                        ),
                        "year_built": self._parse_int(
                            vessel_data.get("Year_Built", "")
                        ),
                        "hull_material": vessel_data.get("Hull_Material", "").strip(),
                        "is_commercial": vessel_data.get("Is_Commercial_Vessel", "")
                        .strip()
                        .lower()
                        == "true",
                        "crew_voyage": self._parse_int(
                            vessel_data.get("Crew_Voyage", "0")
                        ),
                        "deaths_crew": self._parse_int(
                            vessel_data.get("Deaths_Crew", "0")
                        ),
                        "deaths_passenger": self._parse_int(
                            vessel_data.get("Deaths_Passenger", "0")
                        ),
                        "deaths_other": self._parse_int(
                            vessel_data.get("Deaths_Other", "0")
                        ),
                        "injuries_crew": self._parse_int(
                            vessel_data.get("Injuries_Crew", "0")
                        ),
                        "injuries_passenger": self._parse_int(
                            vessel_data.get("Injuries_Passenger", "0")
                        ),
                        "injuries_other": self._parse_int(
                            vessel_data.get("Injuries_Other", "0")
                        ),
                    }
            ...
            if raw_record.get("SAR_Intervention"):
                metadata["sar_intervention"] = raw_record.get(
                    "SAR_Intervention", ""
                ).strip()
