...
class CauseVisualizer:
    ...
    def create_cause_frequency_chart(
        ...
        # Map colors
        colors = [self.color_scheme.get(CauseCategory(c), '#95A5A6')
        ...
        fig = go.Figure(data=[
            go.Bar(
                ...
                hovertemplate=(
                    '<b>%{x}</b><br>' +
                    'Count: %{y}<br>' +
                    ('Percentage: %{customdata:.1f}%<br>' if show_percentages else '') +
                    '<extra></extra>'
                ),
    ...
    def create_cause_trend_over_time(
        ...
        data_copy['period'] = data_copy[date_column].dt.to_period(freq).dt.to_timestamp()

        # Count incidents by period and cause
        trends = data_copy.groupby(['period', cause_column]).size().reset_index(name='count')
    ...
    def create_cause_proportion_pie_chart(
        ...
        fig = go.Figure(data=[
            go.Pie(
                ...
                pull=[0.05 if v == max(values) else 0 for v in values]  # Pull out largest slice
            )
        ])
    ...
    def create_cause_severity_heatmap(
        ...
        """
        if data.empty or cause_column not in data.columns or severity_column not in data.columns:
            fig = go.Figure()
            fig.update_layout(title=title)
            return fig
        ...
        x_labels = [f'Level {s}' if isinstance(s, int) else str(s)
                    for s in crosstab.columns]
    ...
    def create_hatch_maloperation_severity_breakdown(
        ...
        grouped = data.groupby([type_column, severity_column]).size().reset_index(name='count')
        ...
        for severity in sorted(data[severity_column].unique()):
            ...
            fig.add_trace(go.Bar(
                ...
                x=[str(t).replace('_', ' ').title() for t in severity_data[type_column]],
