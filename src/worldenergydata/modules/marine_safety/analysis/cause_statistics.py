...
from sqlalchemy import func, and_, or_, extract, Integer
from sqlalchemy.orm import Session
...
from worldenergydata.modules.marine_safety.constants import (
    CauseCategory, SeverityLevel, IncidentType
)
...
class StatisticalSummary:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'date_range': [str(d) for d in self.date_range],
            'severity_distribution': {str(k): v for k, v in self.severity_distribution.items()},
            'cause_distribution': {str(k): v for k, v in self.cause_distribution.items()},
            'metadata': self.metadata
        }
    ...
    def print_report(self) -> None:
        ...
        print(f"Least Common Cause: {self.least_common_cause} ({self.cause_distribution[self.least_common_cause]} occurrences)")

        print("\n--- Cause Category Distribution ---")
        for cause, count in sorted(self.cause_distribution.items(), key=lambda x: x[1], reverse=True):
...
class CauseStatistics:
    ...
    def calculate_frequency_distribution(
        ...
        if not results:
            ...
            return FrequencyDistribution(
                ...
                total_count=0,
                categories=[],
                metadata={'filters': {'start_date': str(start_date) if start_date else None,
        ...
        if include_confidence_intervals:
            ...
            def wilson_ci(count, total):
                ...
                margin = z_score * np.sqrt(p * (1 - p) / total + z_score**2 / (4 * total**2)) / denominator
        ...
        metadata = {
            ...
            'confidence_level': confidence_level if include_confidence_intervals else None
        }
    ...
    def calculate_temporal_trends(
        ...
        if not results:
            ...
            return TemporalTrend(
                data=empty_df,
                period_type=period,
                date_range=(date_result.min_date or date(1900, 1, 1),
    ...
    def calculate_cause_severity_crosstab(
        ...
        # Convert to DataFrame and pivot
        df = pd.DataFrame(results, columns=['cause_category', 'severity_level', 'count'])
    ...
    def calculate_hatch_maloperation_stats(
        ...
        hatch_keywords = [
            '%hatch%', '%cover%', '%opening%', '%closure%', '%securing%',
            '%watertight%', '%door%', '%access%'
        ]
