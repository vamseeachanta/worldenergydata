        ...
        if 'production_ranges' in config_data:
            ...
            for field_name, range_data in config_data['production_ranges'].items():
                if isinstance(range_data, dict):
                    ranges[field_name] = (range_data.get('min', 0), range_data.get('max', float('inf')))
...
class ProductionVolumeValidator:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.variation_threshold = self.config.get('variation_threshold', 5.0)  # 5x normal variation
    ...
    def _check_well_consistency(self, data: pd.DataFrame, well_id: Optional[str], result: ValidationResult) -> None:
...
class CompletenessChecker:
    ...
    def check_time_series_completeness(self, data: pd.DataFrame, date_column: str) -> ValidationResult:
        """
    ...
    def check_zero_values(self, data: pd.DataFrame, columns: List[str]) -> ValidationResult:
        """
    ...
    def check_required_fields(self, data: pd.DataFrame, required_fields: List[str]) -> ValidationResult:
        """
    ...
    def generate_completeness_report(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        for col in data.columns:
            ...
            zero_count = (data[col] == 0).sum() if pd.api.types.is_numeric_dtype(data[col]) else 0
            
            report['missing_values'][col] = missing_count
            report['zero_values'][col] = zero_count
            report['completeness_percentage'][col] = ((len(data) - missing_count) / len(data)) * 100
        
        report['overall_completeness'] = np.mean(list(report['completeness_percentage'].values()))
...
class OutlierDetector:
    ...
    def detect_outliers_zscore(self, data: pd.DataFrame, column: str, 
                               threshold: Optional[float] = None) -> ValidationResult:
        """
    ...
    def detect_outliers_iqr(self, data: pd.DataFrame, column: str,
                            multiplier: Optional[float] = None) -> ValidationResult:
        """
    ...
    def detect_multivariate_outliers(self, data: pd.DataFrame, columns: List[str]) -> ValidationResult:
        """
        ...
        if len(subset) < len(columns) + 1:
            result.add_warning('data', 'Insufficient data for multivariate outlier detection')
            return result
        ...
        if np.linalg.det(cov_matrix) == 0:
            result.add_warning('data', 'Singular covariance matrix - cannot compute Mahalanobis distance')
            return result
    ...
    def validate(self, data: Any, field_name: str = None) -> ValidationResult:
        ...
        if self.min_value is not None:
            ...
            if isinstance(below_min, bool) and below_min:
                result.add_error(field_name, f'Value {values} below minimum {self.min_value}')
        ...
        if self.max_value is not None:
            ...
            if isinstance(above_max, bool) and above_max:
                result.add_error(field_name, f'Value {values} above maximum {self.max_value}')
...
class ValidationRuleBuilder:
    ...
    def range_rule(self, field_name: str, min_value: Optional[float] = None,
                  max_value: Optional[float] = None) -> RangeValidationRule:
    ...
    def custom_rule(self, field_name: str, validator_func: Callable,
                   error_message: str) -> CustomValidationRule:
...
class DataQualityFramework:
    ...
    def run_quality_checks(self, data: pd.DataFrame) -> VerificationResult:
        ...
        if self.config.enable_completeness_check:
            ...
            zero_result = self.completeness_checker.check_zero_values(data, numeric_cols)
        ...
        quality_score = max(0, 1 - (total_issues / total_checks)) if total_checks > 0 else 1.0
    ...
    def generate_quality_report(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        report['completeness'] = self.completeness_checker.generate_completeness_report(data)
        ...
        if report['summary']['quality_score'] < 0.9:
            report['recommendations'].append('Data quality is below acceptable threshold (90%)')
        
        if report['summary']['total_errors'] > 0:
            report['recommendations'].append(f'Address {report["summary"]["total_errors"]} critical errors before analysis')
        
        if report['completeness']['overall_completeness'] < 95:
            report['recommendations'].append('Improve data completeness to at least 95%')
        
        return report
    
    def export_results(self, result: VerificationResult, path: Path, format: str = 'json') -> None:
        """
        ...
        if format == 'json':
            ...
            for warning in result.warnings:
                issues.append({
                    ...
                    'value': str(warning.value) if warning.value is not None else '',
