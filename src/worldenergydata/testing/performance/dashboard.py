...
class PerformanceDashboard:
    ...
    def create_regression_chart(self, test_name: str) -> go.Figure:
        ...
        if history.empty:
            return self._create_empty_figure(f"No history available for test: {test_name}")
        
        # Filter successful runs
        history = history[history['status'] == 'passed']
        
        if history.empty:
            return self._create_empty_figure(f"No successful runs for test: {test_name}")
    ...
    def generate_dashboard(self, output_path: Path):
        ...
        distribution_html = distribution_chart.to_html(include_plotlyjs=False, div_id="distribution")
        parallel_html = parallel_chart.to_html(include_plotlyjs=False, div_id="parallel")
