...
class PerformanceAnalyzer:
    ...
    def analyze_trends(self, days: int = 30) -> Dict[str, Any]:
        ...
        if trends.empty:
            return {
                'status': 'no_data',
                'message': 'No test execution data available'
            }
        ...
        # Duration trend
        duration_trend = np.polyfit(range(len(trends)), trends['avg_duration'].values, 1)[0]
        
        # Success rate trend
        success_trend = np.polyfit(range(len(trends)), trends['success_rate'].values, 1)[0]
        
        # Identify performance changes
        recent_avg = trends.tail(7)['avg_duration'].mean()
        historical_avg = trends.head(len(trends) - 7)['avg_duration'].mean() if len(trends) > 7 else recent_avg
    ...
    def identify_slow_tests(self, percentile: float = 90) -> pd.DataFrame:
        ...
        slow_tests['slowness_factor'] = slow_tests['avg_duration'] / stats['avg_duration'].median()
        slow_tests['variability'] = slow_tests['std_duration'] / slow_tests['avg_duration']
        
        # Sort by average duration
        slow_tests = slow_tests.sort_values('avg_duration', ascending=False)
        
        return slow_tests[['test_name', 'avg_duration', 'std_duration', 
                          'slowness_factor', 'variability', 'total_runs']]
    
    def detect_regressions(self, threshold_std: float = 2.0, 
            ...
            if not historical.empty:
                ...
                if recent_mean > historical_mean + (threshold_std * historical_std):
                    regressions.append({
                        ...
                        'std_deviations': float((recent_mean - historical_mean) / historical_std),
    ...
    def analyze_test_stability(self, test_name: str) -> Dict[str, Any]:
        ...
        older = durations.tail(len(durations) - 10) if len(durations) > 10 else durations
        ...
        return {
            ...
            'recent_trend': trend,
            'last_run': history.iloc[0]['timestamp'].isoformat() if not history.empty else None
        }
    ...
    def get_optimization_recommendations(self) -> List[Dict[str, Any]]:
        ...
        if not slow_tests.empty:
            ...
            recommendations.append({
                ...
                'potential_time_saved': float(top_slow['avg_duration'].sum() * 0.3)  # Assume 30% improvement
            })
        
        # 2. Identify flaky tests
        stats = self.db.get_test_statistics()
        if not stats.empty:
            flaky = stats[(stats['total_runs'] > 10) & (stats['success_rate'] < 95) & (stats['success_rate'] > 5)]
        ...
        if not stats.empty:
            ...
            if len(independent_tests) > 10:
                recommendations.append({
                    ...
                    'potential_time_saved': float(independent_tests['avg_duration'].sum() * 0.5)  # Assume 50% improvement
                })
    ...
    def calculate_parallel_efficiency(self, num_workers: int = 4) -> Dict[str, Any]:
        ...
        return {
            ...
            'load_balance_ratio': float(min(worker_loads) / max(worker_loads)) if max(worker_loads) > 0 else 1
        }