...
@cli.command()
@click.option(
    "--threshold", default=2.0, help="Standard deviations for regression detection"
)
...
def regressions(threshold, days):
    ...
    regressions = analyzer.detect_regressions(
        threshold_std=threshold, lookback_days=days
    )
...
def analyze(test_name):
    ...
    click.echo(
        f"Min/Max: {result.get('min_duration', 0):.3f}s / {result.get('max_duration', 0):.3f}s"
    )
...
def recommendations():
    ...
    for idx, rec in enumerate(recommendations, 1):
        ...
        click.secho(
            f"{idx}. [{rec['priority'].upper()}] {rec['title']}", fg=priority_color
        )
