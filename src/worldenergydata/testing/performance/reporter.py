...
class PerformanceReporter:
    ...
    def generate_text_report(self, days: int = 7) -> str:
        ...
        if recommendations:
            for idx, rec in enumerate(recommendations[:3], 1):
                ...
                if "potential_time_saved" in rec:
                    lines.append(
                        f"   Potential time saved: {rec['potential_time_saved']:.1f}s"
                    )
        else:
        ...
        if parallel["status"] == "calculated":
            lines.append(
                f"Serial execution time: {parallel['serial_execution_time']:.1f}s"
            )
            lines.append(
                f"Parallel execution time (4 workers): {parallel['parallel_execution_time']:.1f}s"
            )
    ...
    def generate_json_report(self, days: int = 7) -> Dict[str, Any]:
        ...
        report["slowest_tests"] = (
            slow_tests.to_dict("records") if not slow_tests.empty else []
        )
        ...
        # Parallelization
        report["parallelization"] = self.analyzer.calculate_parallel_efficiency(
            num_workers=4
        )
...
"""
                if "potential_time_saved" in rec:
                    html += (
                        f"<br>Potential time saved: {rec['potential_time_saved']:.1f}s"
                    )
                html += """
        </div>
"""
