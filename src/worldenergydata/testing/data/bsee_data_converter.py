...
class BSEEDataConverter:
    ...
    def generate_api_number(
        self, state_code: str = "177", county_code: str = "104"
    ) -> str:
        """
    ...
    def generate_well_data(
        self, num_wells: int = 5, water_depth_range: tuple = (1000, 7000)
    ) -> pd.DataFrame:
        """
    ...
    def generate_completion_data(
        self, api_numbers: Optional[List[str]] = None, num_completions: int = 5
    ) -> pd.DataFrame:
        """
        ...
        for idx, api_number in enumerate(api_numbers):
            ...
            row = {
                ...
                "COMPLETION_TYPE": random.choice(
                    ["CASED_HOLE", "OPEN_HOLE", "GRAVEL_PACK"]
                ),
                "SAND_CONTROL": random.choice(
                    ["NONE", "SCREEN", "GRAVEL_PACK", "FRAC_PACK"]
                ),
    ...
    def save_test_data(
        self, output_dir: Union[str, Path], prefix: str = "test"
    ) -> Dict[str, Path]:
        """
...
production['PRODUCTION_DATE'] = pd.to_datetime(
    production['PRODUCTION_DATE'],   # noqa: W291
    format='%Y%m'
)
```
