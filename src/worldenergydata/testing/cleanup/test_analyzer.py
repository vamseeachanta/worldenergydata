...
class TestAnalyzer:
    ...
    def _find_redundant_tests(self):
        ...
        for file_path, methods in self.test_methods.items():
            for method in methods:
                ...
                core_name = re.sub(
                    r"test_(.+?)(_\d+|_variant.*|_case.*|_version.*)?$", r"\1", method
                )
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        poorly_named = sum(
            1
            for methods in self.test_methods.values()
            for method in methods
            if len(method) < 10
            or not method.replace("test_", "").replace("_", "").isalnum()
        )
...
def analyze_and_cleanup(test_dir: str, dry_run: bool = True) -> Dict:
    ...
    return {"analysis": analysis, "cleanup_report": cleanup_report, "dry_run": dry_run}
