...
class ValidationResult:
    ...
    def add_error(self, field: str, message: str, value: Any = None, 
                  rule: str = None, row_index: Optional[int] = None):
        """Add a validation error."""
        self.errors.append(ValidationError(
            field=field, message=message, value=value, 
            rule=rule, severity='error', row_index=row_index
        ))
    ...
    def add_warning(self, field: str, message: str, value: Any = None,
        ...
        self.warnings.append(ValidationError(
            field=field, message=message, value=value,
            rule=rule, severity='warning', row_index=row_index
        ))
    ...
    def add_info(self, field: str, message: str, value: Any = None,
        ...
        self.info.append(ValidationError(
            field=field, message=message, value=value,
            rule=rule, severity='info', row_index=row_index
        ))
    ...
    def validate(self, data: Any) -> ValidationResult:
        ...
        for rule in self.rules:
            try:
                ...
                logger.error(f"Error applying rule {rule.__class__.__name__} to field {self.field_name}: {e}")
                result.add_error(
                    self.field_name,
        ...
        try:
            ...
            for rule in self.rules:
                try:
                    ...
                    logger.error(f"Error applying schema rule {rule.__class__.__name__}: {e}")
                    result.add_error(
                        "schema",
...
class ValidationSchema(ABC):
    ...
    def add_field_validator(self, field_name: str, validator: FieldValidator) -> 'ValidationSchema':
