...
class FieldAggregator(DataAggregator):
    ...
    def aggregate(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if result["total_well_count"] > 0:
            result["peak_oil_rate"] = result["oil_bbls"] / (
                result["total_well_count"] * 365
            )
            result["peak_gas_rate"] = result["gas_mcf"] / (
                result["total_well_count"] * 365
            )
    ...
    def fetch_field_leases(
        self, field_name: str, cfg: Optional[Dict[str, Any]] = None
    ) -> pd.DataFrame:
    ...
    def aggregate_with_fresh_data(
        self, field_names: List[str], cfg: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        ...
        for field_name in field_names:
            ...
            field_metrics = {
                "lease_count": (
                    len(lease_df["Lease Number"].unique())
                    if "Lease Number" in lease_df
                    else 0
                ),
                "has_production": not production_df.empty,
                "production_records": (
                    len(production_df) if not production_df.empty else 0
                ),
            }
    ...
    def get_field_summary(self, field_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            summary = {
                ...
                "lease_count": (
                    len(lease_df["Lease Number"].unique())
                    if "Lease Number" in lease_df and not lease_df.empty
                    else 0
                ),
                "area_codes": (
                    lease_df["Area Code"].unique().tolist()
                    if "Area Code" in lease_df and not lease_df.empty
                    else []
                ),
                "block_numbers": (
                    lease_df["Block Number"].unique().tolist()
                    if "Block Number" in lease_df and not lease_df.empty
                    else []
                ),
            ...
            if not production_df.empty:
                summary["production"] = {
                    ...
                    "total_oil_bbls": (
                        production_df["Oil Volume"].sum()
                        if "Oil Volume" in production_df
                        else 0
                    ),
                    "total_gas_mcf": (
                        production_df["Gas Volume"].sum()
                        if "Gas Volume" in production_df
                        else 0
                    ),
                }
    ...
    def calculate_field_economics(
        self, field: Field, price_deck: Dict[str, float]
    ) -> EconomicMetrics:
        """
