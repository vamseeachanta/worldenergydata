    ...
    def save_eWellAPMRawData_to_binary(self, cfg):
        ...
        for file_name in os.listdir(folder_path_zip):
            ...
            if file_name == "dsptsdelimit.ZIP":
                ...
                column_names = self._get_column_names_for_dsptsdelimit_file(
                    file_name_with_path
                )
            else:
                column_names = None
        ...
        try:
            with zipfile.ZipFile(file_path, "r") as zip_file:
                ...
                with zip_file.open(first_file) as csv_file:
                    ...
                    first_line = (
                        csv_file.readline().decode("utf-8", errors="ignore").strip()
                    )
                    ...
                    if header_indicators > len(first_line_values):
                        ...
                        logger.debug(
                            f"No headers detected in zip file: {file_path}, using target columns"
                        )
