...
from worldenergydata.bsee.data.by_block.block_data import BlockData
from worldenergydata.bsee.data.production import Production
from worldenergydata.bsee.data.scrapy_production_data import SpiderBsee
from worldenergydata.bsee.data.well import WellData
...
class ProductionUncleanCode:
    ...
    def get_production_data_by_api12(self, api12, cfg):
        ...
        production_yml = f_d_templates.get_data_from_existing_files(
            cfg["Analysis"].copy()
        )
        ...
        production_data = energy_engine(
            inputfile=None, cfg=production_yml, config_flag=False
        )
    ...
    def get_bottom_leases_by_block(self, cfg):
        ...
        cfg[cfg["basename"]].update(
            {"production": {"bottom_leases": bottom_lease_array}}
        )
    ...
    def get_production_data_for_each_lease(self, cfg):
        ...
        for bottom_lease in bottom_lease_array:
            production_data = self.get_production_data_by_bottom_lease(
                bottom_lease, cfg
            )
    ...
    def get_production_data_by_bottom_lease(self, bottom_lease, cfg):
        ...
        production_yml = f_d_templates.get_production_data_by_lease(
            cfg["Analysis"].copy()
        )

        settings = {
            "lease_number": bottom_lease,
            "label": "production_data_" + str(bottom_lease),
            "Duration": {"from": "01/1999", "to": "01/2024"},
        }
        master_settings = {
            "output_dir": "tests/modules/bsee/data/results/Data/well_production_data"
        }
