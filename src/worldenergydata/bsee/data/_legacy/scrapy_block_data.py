...
class BSEESpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        first_request_data = self.cfg["data_retrieval"]["block"]["website"][
            "form_data"
        ]["first_request"].copy()
    ...
    def step2(self, response):
        ...
        second_request_data = self.cfg["data_retrieval"]["block"]["website"][
            "form_data"
        ]["second_request"].copy()
...
class ScrapyRunnerBlock:
    ...
    def run_spider(self, cfg, input_item):
        ...
        deferred = self.runner.crawl(
            BSEESpider, input_item=input_item, cfg=cfg, data_store=data_store
        )
