...
logging.getLogger("scrapy").propagate = (
    False  # to avoid displaying log outputs in terminal
)
...
class SpiderBsee(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        first_request_data = self.cfg["data_retrieval"]["production"]["website"][
            "form_data"
        ]["first_request"]
    ...
    def step2(self, response):
        ...
        second_request_data = self.cfg["data_retrieval"]["production"]["website"][
            "form_data"
        ]["second_request"]
    ...
    def parse_csv_data(self, response):
        ...
        output_path = output_path = os.path.join(
            self.cfg["Analysis"]["result_folder"], "Data"
        )
...
class ScrapyRunnerProduction:
    ...
    def run_spider(self, cfg, input_item):
        ...
        deferred = self.runner.crawl(
            SpiderBsee, input_item=input_item, cfg=cfg, data_store=data_store
        )
