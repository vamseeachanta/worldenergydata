"""Pydantic schema for BSEE deepwater (permitted) structure records.

Validates and coerces raw data from the BSEE permitted structure dataset
(PermStrucRawData.zip).
"""
...
class DeepwaterStructureSchema(BaseModel):
    """Schema for a single BSEE deepwater / permitted structure record.

    Mirrors PlatformStructureSchema but includes permit-specific fields
    (PERMIT_NUMBER, BUS_ASC_NAME). Field names match the upstream BSEE
    CSV column headers.
    """
    ...
    # Optional fields (shared with platform)
    ...
    # Deepwater / permit-specific fields
