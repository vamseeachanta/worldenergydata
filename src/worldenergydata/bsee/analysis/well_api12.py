...
class WellAPI12:
    ...
    def run_well_analysis(self, cfg, data):
        ...
        for group_idx in range(0, len(well_groups)):
            ...
            for well_idx in range(0, len(group)):
                ...
                well_summary_df_group = pd.concat(
                    [well_summary_df_group, api12_analysis], ignore_index=True
                )
            ...
            file_name = os.path.join(
                cfg["Analysis"]["result_folder"], file_label + ".csv"
            )
            well_summary_df_group.to_csv(file_name, index=False)

            well_summary_df_groups = pd.concat(
                [well_summary_df_groups, well_summary_df_group], ignore_index=True
            )
    ...
    def well_timeline_analysis(self, cfg, well_summary_df_groups):
        ...
        column_keyword_cfg = {
            "date_column": "WELL_SPUD_DATE",
            "well_count_column": "WELL_SPUD_COUNT",
        }
        well_timeline_df = self.get_well_count_by_custom_cfg(
            well_timeline_df, well_summary_df_groups, column_keyword_cfg
        )
        column_keyword_cfg = {
            "date_column": "TOTAL_DEPTH_DATE",
            "well_count_column": "TOTAL_DEPTH_COUNT",
        }
        well_timeline_df = self.get_well_count_by_custom_cfg(
            well_timeline_df, well_summary_df_groups, column_keyword_cfg
        )
        column_keyword_cfg = {
            "date_column": "RIG_LAST_DATE_ON_WELL",
            "well_count_column": "RIG_LAST_DATE_COUNT",
        }
        well_timeline_df = self.get_well_count_by_custom_cfg(
            well_timeline_df, well_summary_df_groups, column_keyword_cfg
        )
        ...
        column_keyword_cfg = {
            ...
            "well_count_column": "PRODUCTION_START_COUNT",
        }
        well_timeline_df = self.get_well_count_by_custom_cfg(
            well_timeline_df, well_summary_df_groups, column_keyword_cfg
        )
        column_keyword_cfg = {
            "date_column": "LAST_PRODUCTION_DATE",
            "well_count_column": "PRODUCTION_END_COUNT",
        }
        well_timeline_df = self.get_well_count_by_custom_cfg(
            well_timeline_df, well_summary_df_groups, column_keyword_cfg
        )
        ...
        well_timeline_df = self.add_currently_producing_count(
            well_timeline_df, well_summary_df_groups
        )
    ...
    def get_well_count_by_custom_cfg(
        self, well_timeline_df, well_summary_df_groups, column_keyword_cfg
    ):
        ...
        well_timeline_df = pd.merge(
            well_timeline_df, df_temp, on=["date_time"], how="outer"
        )
        well_timeline_df = well_timeline_df.replace({np.nan: None})
        well_timeline_df.sort_values(by=["date_time"], inplace=True)
        well_timeline_df.reset_index(drop=True, inplace=True)
    ...
    def get_api12_analysis(self, cfg, well_data):
        ...
        api12_analysis = self.get_sidetracklabel_and_rig_rigdays(
            cfg, well_data, api12_analysis
        )
    ...
    def get_directional_surveys_by_api10(self, cfg):
        ...
        library_file_cfg = {"filepath": survey_data_path, "library_name": library_name}
        library_path = wwy.get_library_filepath(
        ...
        if api12_survey_df.empty:
            ...
            return
        api12_survey_df["API10"] = (
            api12_survey_df["API_WELL_NUMBER"].astype(str).str[:10]
        )
    ...
    def get_sidetracklabel_and_rig_rigdays(self, cfg, well_data, api12_analysis):
        ...
        api12_eWellWARRawData_mv_war_main = well_data[
            "api12_eWellWARRawData_mv_war_main"
        ]
        api12_eWellWARRawData_mv_war_main_prop = well_data[
            "api12_eWellWARRawData_mv_war_main_prop"
        ]

        if not api12_eWellAPDRawData.empty:
            api12_analysis["WELL_NM_ST_SFIX"] = api12_eWellAPDRawData[
                "WELL_NM_ST_SFIX"
            ].iloc[0]
            api12_analysis["WELL_NM_BP_SFIX"] = api12_eWellAPDRawData[
                "WELL_NM_BP_SFIX"
            ].iloc[0]
        ...
        if not api12_BoreholeRawData.empty:
            api12_analysis["WELL_NAME_SUFFIX"] = api12_BoreholeRawData[
                "WELL_NAME_SUFFIX"
            ].iloc[0]

        if not api12_eWellEORRawData.empty:
            api12_analysis["SUBSEA_TREE_HEIGHT_AML"] = api12_eWellEORRawData[
                "SUBSEA_TREE_HEIGHT_AML"
            ].max()

        if not api12_eWellWARRawData_mv_war_main_prop.empty:
            api12_analysis["MAX_DRILL_FLUID_WGT"] = float(
                api12_eWellWARRawData_mv_war_main_prop["DRILL_FLUID_WGT"].max()
            )
        ...
        if not api12_eWellWARRawData_mv_war_main.empty:
            api12_analysis["RIG_LAST_DATE_ON_WELL"] = (
                api12_eWellWARRawData_mv_war_main.WAR_END_DT.max()
            )

        if not api12_df.empty:
            api12_analysis["Water Depth (feet)"] = api12_df["Water Depth (feet)"].iloc[
                0
            ]
        ...
        api12_analysis.loc[
            df_row, ["Sidetrack No", "Bypass No", "Tree Height Above Mudline"]
        ] = [sidetrack_no, bypass_no, tree_elevation_aml]

        rig_analysis = well_rig_days.rig_analysis(
            cfg,
            api12_df,
            api12_eWellWARRawData_mv_war_main,
            api12_eWellWARRawData_mv_war_main_prop,
        ...
        api12_analysis["drilling_days_per_10000_ft"] = api12_analysis[
            "drilling_days_per_10000_ft"
        ].astype(float)
        api12_analysis.loc[df_row, "drilling_days_per_10000_ft"] = (
            drilling_days_per_10000_ft
        )
    ...
    def get_st_bp_tree_info(self, api12_df):
        ...
        bp_st_tree_info = api12_df[
            ["SN_EOR", "WELL_NM_ST_SFIX", "WELL_NM_BP_SFIX", "SUBSEA_TREE_HEIGHT_AML"]
        ].copy()
    ...
    def prepare_completion_data(self, completion_data):
        ...
        self.output_completions = completion_data.merge(
        ...
        self.output_completions["COMP_x_rel"] = (
            self.output_completions["COMP_x"] - field_x_ref
        )
        self.output_completions["COMP_y_rel"] = (
            self.output_completions["COMP_y"] - field_y_ref
        )
    ...
    def prepare_well_paths(self, directional_surveys, well_data):
        ...
        for api12 in API12_list:
            ...
            api12_dir_survey_df["md"] = api12_dir_survey_df["SURVEY_POINT_MD"].astype(float)
            ...
            try:
                ...
                label = (
                    self.output_data_api12_df[
                        self.output_data_api12_df.API10 == api10_value
                    ]["Well Name"].values[0]
                    + "-"
                    + self.output_data_api12_df[
                        self.output_data_api12_df.API10 == api10_value
    ...
    def plot_well_timeline_df(self, cfg, groups_dict):
        ...
        available_columns = [
            col for col in plot_columns if col in well_timeline_df.columns
        ]
        ...
        df_filtered = df_melted[
            (df_melted["Date"] >= "2007-01-01") & (df_melted["Date"] <= "2025-04-03")
        ].copy()
        ...
        df_filtered["type"] = (
            df_filtered["type"].map(label_mapping).fillna(df_filtered["type"])
        )
    ...
    def add_relative_WH_positions(self, api12, survey_xyz):
        api12_df = self.output_data_api12_df[
            self.output_data_api12_df.API12 == api12
        ].copy()
    ...
    def add_currently_producing_count(self, well_timeline_df, well_summary_df_groups):
        ...
        production_wells["START_PRODUCTION_DATE"] = pd.to_datetime(
            production_wells["START_PRODUCTION_DATE"], errors="coerce"
        )
        production_wells["LAST_PRODUCTION_DATE"] = pd.to_datetime(
            production_wells["LAST_PRODUCTION_DATE"], errors="coerce"
        )
