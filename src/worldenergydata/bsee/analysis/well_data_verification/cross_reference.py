...
class MappingConfig:
    ...
    def __post_init__(self):
        ...
        if not self.reporting:
            self.reporting = {
                "severity_thresholds": {"error": 10.0, "warning": 5.0, "info": 1.0}
            }
...
class ExcelBenchmarkReader:
    ...
    def read_file(
        ...
        try:
            df = pd.read_excel(
                file_path, sheet_name=sheet_name, parse_dates=parse_dates
            )
    ...
    def read_all_sheets(self, file_path: Union[str, Path]) -> Dict[str, pd.DataFrame]:
        ...
        try:
            ...
            for sheet_name in excel_file.sheet_names:
                ...
                logger.debug(
                    f"Read sheet '{sheet_name}' with {len(sheets[sheet_name])} rows"
                )
...
class FieldMapper:
    ...
    def map_dataframe(
        self, df: pd.DataFrame, skip_missing: bool = False
    ) -> pd.DataFrame:
        """
    ...
    def auto_map_columns(
        self, db_columns: List[str], excel_columns: List[str]
    ) -> Dict[str, str]:
        """
        ...
        for db_col in db_columns:
            ...
            for excel_col in excel_columns:
                if self.fuzzy_match(db_col, excel_col):
                    score = SequenceMatcher(
                        None, db_col.lower(), excel_col.lower()
                    ).ratio()
                    if score > best_score:
                        best_score = score
            ...
            if best_match:
                ...
                logger.debug(
                    f"Auto-mapped '{db_col}' to '{best_match}' (score: {best_score:.2f})"
                )
...
class ComparisonEngine:
    ...
    def compare_numeric(
        self, value1: float, value2: float, tolerance: Optional[float] = None
    ) -> ComparisonDetail:
        """
        ...
        return ComparisonDetail(
            is_match=is_match, difference=actual_diff, notes=f"Tolerance: {tolerance}"
        )
    ...
    def compare_string(
        ...
        if fuzzy or self.string_matching == "fuzzy":
            ...
            return ComparisonDetail(
                is_match=is_match, similarity_score=similarity, notes="Fuzzy matching"
            )
        else:
            is_match = str1 == str2

            return ComparisonDetail(is_match=is_match, notes="Exact matching")

    def compare_date(
        self, date1: datetime, date2: datetime, tolerance_seconds: int = 0
    ) -> ComparisonDetail:
        """
        ...
        return ComparisonDetail(
            is_match=is_match, difference=diff, notes=f"Difference: {diff} seconds"
        )
    ...
    def compare_dataframes(
        ...
        for col in value_columns:
            ...
            for idx, row in merged.iterrows():
                ...
                if pd.isna(src_val) and pd.isna(bench_val):
                ...
                elif isinstance(src_val, (int, float)) and isinstance(
                    bench_val, (int, float)
                ):
                    ...
                    if comparison.is_match:
                        ...
                        pct_diff = (
                            abs(comparison.difference / src_val * 100)
                            if src_val != 0
                            else 100
                        )
    ...
    def compare_aggregates(
        ...
        for _, row in merged.iterrows():
            val1 = (
                row[f"{agg_column}_{agg_func}_1"]
                if f"{agg_column}_{agg_func}_1" in row
                else row[f"{agg_column}_1"]
            )
            val2 = (
                row[f"{agg_column}_{agg_func}_2"]
                if f"{agg_column}_{agg_func}_2" in row
                else row[f"{agg_column}_2"]
            )
        ...
        return ComparisonDetail(
            is_match=all_match, notes=f"Aggregate comparison by {group_by}"
        )
...
class DiscrepancyReporter:
    ...
    def export_to_excel(
        self, discrepancies: List[Discrepancy], file_path: Union[str, Path]
    ) -> None:
        """
    ...
    def _get_top_discrepancies(
        self, discrepancies: List[Discrepancy], n: int = 10
    ) -> List[Dict[str, Any]]:
...
class CrossReferenceModule:
    ...
    def generate_consolidated_report(
        self, results: List[ComparisonResult], output_path: Union[str, Path]
    ) -> None:
        """
