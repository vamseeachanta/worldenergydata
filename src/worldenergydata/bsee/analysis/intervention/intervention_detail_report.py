...
from worldenergydata.bsee.analysis.intervention.activity_aggregator import (
    WARActivityAggregator, classify_activity,
)
...
_SVC_DN: dict[str, str] = {
    ...
    "workover_rig": "Workover Rig", "support_vessel": "Support Vessel",
...
_AN: dict[str, str] = {
    "EI": "Eugene Island", "SS": "Ship Shoal", "SP": "South Pass",
    "HI": "High Island", "SM": "South Marsh Island", "WD": "West Delta",
    "VR": "Vermilion", "ST": "South Timbalier", "MP": "Main Pass",
    "MC": "Mississippi Canyon", "GC": "Green Canyon", "WC": "West Cameron",
    "EC": "East Cameron", "GB": "Garden Banks", "VK": "Viosca Knoll",
    "GI": "Grand Isle", "WR": "Walker Ridge", "EW": "Ewing Bank",
    "MI": "Mustang Island", "EB": "East Breaks",
}
...
class InterventionDetailReport:
    ...
    def generate(self, output_path: str = "reports/bsee/intervention/intervention_by_service.html") -> str:
        ...
        secs = [self._s1(iv, n, y0, y1), self._s2(iv), self._s3(iv), self._s4(iv),
                self._s5(iv), self._s6(iv), self._s7(iv), self._s8(iv), self._s9(n, y0, y1)]
    ...
    def _s1(self, iv: pd.DataFrame, n: int, y0: int, y1: int) -> dict:
        ...
        bl.append(f"<strong>{n:,}</strong> intervention WAR records spanning "
                  f"<strong>{y0}-{y1}</strong>.")
        bl.append(f"Unique wells served: <strong>{iv['API_WELL_NUMBER'].nunique():,}</strong>.")
        if self._hb:
            bl.append(f"Unique operators: <strong>{iv['BUS_ASC_NAME'].nunique():,}</strong>.")
        ch: list[str] = []
        cnt = iv.groupby("RIG_TYPE").size().reindex(_SVC_TYPES, fill_value=0)
        lbl = [_SVC_DN.get(t, t) for t in cnt.index]
        fig = go.Figure(go.Pie(labels=lbl, values=cnt.values.tolist(),
        ...
        ch.append(_fhtml(fig, self._fi)); self._fi += 1
    ...
    def _s2(self, iv: pd.DataFrame) -> dict:
        ...
        ch: list[str] = []
        if iv.empty:
            return {"title": "Service Type Deep-Dive", "content": "<p>No data.</p>", "charts": []}
        for i, (sk, sn) in enumerate(_SVC_DN.items()):
            ...
            parts.append(f"<p>Records: <strong>{nr:,}</strong> | "
            ...
            yc = sub.groupby("YEAR").size().sort_index()
            fig = go.Figure(go.Scatter(x=yc.index.tolist(), y=yc.values.tolist(),
                                        mode="lines+markers",
                                        line=dict(color=_PAL[i % len(_PAL)], width=2)))
            fig.update_layout(title=f"{sn} -- Activity Trend",
                              xaxis_title="Year", yaxis_title="WAR Records", height=300)
            ch.append(_fhtml(fig, self._fi)); self._fi += 1
            if self._hb:
                top_op = sub.groupby("BUS_ASC_NAME").size().sort_values(ascending=False).head(5)
                parts.append(_tbl(["Operator", "Records"],
                                  [[op, f"{c:,}"] for op, c in top_op.items()]))
            top_rig = sub.groupby("RIG_NAME").size().sort_values(ascending=False).head(5)
            parts.append(_tbl(["Equipment", "Records"],
                              [[r, f"{c:,}"] for r, c in top_rig.items()]))
            top_area = sub.groupby("AREA_CODE").size().sort_values(ascending=False).head(5)
            parts.append(_tbl(["Area", "Records"],
                              [[f"{a} - {_AN.get(a, a)}", f"{c:,}"]
                               for a, c in top_area.items()]))
        return {"title": "Service Type Deep-Dive", "content": "\n".join(parts), "charts": ch}

    # S3: Service Type Comparison
    def _s3(self, iv: pd.DataFrame) -> dict:
        ch: list[str] = []
        if iv.empty:
            return {"title": "Service Type Comparison", "content": "<p>No data.</p>", "charts": []}
        yrs = sorted(iv["YEAR"].unique())
        ya = 2020 if 2020 in yrs else yrs[0]; yb = yrs[-1]
        ea = iv[iv["YEAR"] == ya].groupby("RIG_TYPE").size().reindex(_SVC_TYPES, fill_value=0)
        la = iv[iv["YEAR"] == yb].groupby("RIG_TYPE").size().reindex(_SVC_TYPES, fill_value=0)
        lbl = [_SVC_DN[t] for t in _SVC_TYPES]
        fig = go.Figure()
        fig.add_trace(go.Bar(x=lbl, y=ea.values.tolist(), name=str(ya), marker_color=_PAL[0]))
        fig.add_trace(go.Bar(x=lbl, y=la.values.tolist(), name=str(yb), marker_color=_PAL[1]))
        fig.update_layout(title=f"Records by Service Type: {ya} vs {yb}",
                          barmode="group", yaxis_title="WAR Records", height=400)
        ch.append(_fhtml(fig, self._fi)); self._fi += 1
        rows: list[list] = []
        for sk in _SVC_TYPES:
            sub = iv[iv["RIG_TYPE"] == sk]; nr = len(sub)
            ...
            rows.append([_SVC_DN[sk], f"{nr:,}", f"{nw:,}", f"{nop:,}",
                         f"{_camp(sub):.0f}", _AN.get(ta, ta), _yoy(sub)])
        content = _tbl(["Service Type", "Total Records", "Unique Wells",
                        "Unique Operators", "Avg Campaign Days", "Top Area", "Growth Rate"], rows)
    ...
    def _s4(self, iv: pd.DataFrame) -> dict:
        ch: list[str] = []
        if iv.empty:
            return {"title": "Geographic Distribution by Service Type",
                    "content": "<p>No data.</p>", "charts": []}
        ta = iv.groupby("AREA_CODE").size().sort_values(ascending=False).head(12).index.tolist()
        al = [f"{a} - {_AN.get(a, a)}" for a in ta]
        fig = go.Figure()
        for i, sk in enumerate(_SVC_TYPES):
            ac = iv[iv["RIG_TYPE"] == sk].groupby("AREA_CODE").size().reindex(ta, fill_value=0)
            fig.add_trace(go.Bar(x=al, y=ac.values.tolist(),
                                 name=_SVC_DN[sk], marker_color=_PAL[i % len(_PAL)]))
        fig.update_layout(title="Service Type Distribution Across GOM Areas",
                          barmode="stack", yaxis_title="WAR Records", height=450,
                          legend=dict(orientation="h", yanchor="bottom", y=1.02))
        ch.append(_fhtml(fig, self._fi)); self._fi += 1
        return {"title": "Geographic Distribution by Service Type", "content": "", "charts": ch}

    # S5: Operator-Service Matrix
    def _s5(self, iv: pd.DataFrame) -> dict:
        ch: list[str] = []
        if not self._hb or iv.empty:
            return {"title": "Operator-Service Matrix",
                    "content": "<p>No operator data.</p>", "charts": []}
        ops = iv.groupby("BUS_ASC_NAME").size().sort_values(ascending=False).head(15).index.tolist()
        sl = [_SVC_DN[s] for s in _SVC_TYPES]
        z = [[int(((iv["BUS_ASC_NAME"] == op) & (iv["RIG_TYPE"] == sk)).sum())
              for sk in _SVC_TYPES] for op in ops]
        fig = go.Figure(go.Heatmap(z=z, x=sl, y=ops, colorscale="Blues",
                                    texttemplate="%{z}"))
        fig.update_layout(title="Operator-Service Matrix (Top 15 Operators)",
    ...
    def _s6(self, iv: pd.DataFrame) -> dict:
        ...
        if iv.empty or "WAR_START_DT" not in iv.columns:
            return {"title": "Seasonal Patterns by Service Type",
                    "content": "<p>No data.</p>", "charts": []}
        tmp = iv.copy()
        tmp["MONTH"] = pd.to_datetime(tmp["WAR_START_DT"], format="mixed",
                                       dayfirst=False, errors="coerce").dt.month
        tmp = tmp[tmp["MONTH"].notna()]
        if tmp.empty:
            return {"title": "Seasonal Patterns by Service Type",
                    "content": "<p>No parseable dates.</p>", "charts": []}
        fig = go.Figure()
        for i, sk in enumerate(_SVC_TYPES):
            ...
            fig.add_trace(go.Bar(x=_MO, y=mc.values.tolist(),
                                 name=_SVC_DN[sk], marker_color=_PAL[i % len(_PAL)]))
        fig.update_layout(title="Monthly Distribution by Service Type",
                          barmode="group", yaxis_title="WAR Records", height=420,
                          legend=dict(orientation="h", yanchor="bottom", y=1.02))
        ch.append(_fhtml(fig, self._fi)); self._fi += 1
        rows: list[list] = []
        for sk in _SVC_TYPES:
            sub = tmp[tmp["RIG_TYPE"] == sk]
            if sub.empty:
                rows.append([_SVC_DN[sk], "N/A"]); continue
            pk = int(sub.groupby("MONTH").size().idxmax())
            rows.append([_SVC_DN[sk], _MO[pk - 1]])
        return {"title": "Seasonal Patterns by Service Type",
                "content": _tbl(["Service Type", "Peak Month"], rows), "charts": ch}

    # S7: Duration Analysis by Service Type
    def _s7(self, iv: pd.DataFrame) -> dict:
        ch: list[str] = []
        if iv.empty:
            return {"title": "Duration Analysis by Service Type",
                    "content": "<p>No data.</p>", "charts": []}
        fig = go.Figure(); rows: list[list] = []
        for i, sk in enumerate(_SVC_TYPES):
            sub = iv[iv["RIG_TYPE"] == sk]
            if sub.empty:
                rows.append([_SVC_DN[sk], "0", "0", "0", "0"]); continue
            dur = sub.groupby("API_WELL_NUMBER").size() * _WK
            fig.add_trace(go.Box(y=dur.values.tolist(), name=_SVC_DN[sk],
                                 marker_color=_PAL[i % len(_PAL)]))
            rows.append([_SVC_DN[sk], f"{dur.median():.0f}", f"{dur.mean():.0f}",
                         f"{dur.max():.0f}", f"{dur.count():,}"])
        fig.update_layout(title="Well Campaign Duration by Service Type",
                          yaxis_title="Estimated Days", height=420)
        ch.append(_fhtml(fig, self._fi)); self._fi += 1
        return {"title": "Duration Analysis by Service Type",
                "content": _tbl(["Service Type", "Median Days", "Mean Days",
                                  "Max Days", "Wells Served"], rows), "charts": ch}

    # S8: Market Share Trends
    def _s8(self, iv: pd.DataFrame) -> dict:
        ch: list[str] = []
        if iv.empty:
            return {"title": "Market Share Trends", "content": "<p>No data.</p>", "charts": []}
        yrs = sorted(iv["YEAR"].unique())
        # pre-compute yearly totals
        ytot = {yr: len(iv[iv["YEAR"] == yr]) for yr in yrs}
        fig = go.Figure()
        for i, sk in enumerate(_SVC_TYPES):
            vals = [len(iv[(iv["YEAR"] == yr) & (iv["RIG_TYPE"] == sk)]) / ytot[yr] * 100
                    if ytot[yr] else 0 for yr in yrs]
            fig.add_trace(go.Scatter(x=[int(y) for y in yrs], y=vals,
                                     name=_SVC_DN[sk], stackgroup="one",
                                     line=dict(color=_PAL[i % len(_PAL)])))
        fig.update_layout(title="Market Share by Service Type Over Time",
                          xaxis_title="Year", yaxis_title="Share (%)", height=420,
                          legend=dict(orientation="h", yanchor="bottom", y=1.02))
        ch.append(_fhtml(fig, self._fi)); self._fi += 1
        ...
        for sk in _SVC_TYPES:
            ...
            rows.append(row)
        return {"title": "Market Share Trends", "content": _tbl(hdr, rows), "charts": ch}

    # S9: Methodology
    def _s9(self, n: int, y0: int, y1: int) -> dict:
        return {"title": "Methodology", "content":
                "<ul>"
