...
class ComprehensiveActivityAnalyzer:
    ...
    def _analyze_well_depth(self) -> dict:
        empty = {
            "mean_total_md": 0.0, "max_total_md": 0.0,
    ...
    def _analyze_geological_era(self) -> dict:
        ...
        dist = sub.groupby("GEOLOGICAL_ERA", dropna=False).agg(
    ...
    def _analyze_cross_activity(self) -> dict:
        ...
        empty = {
            "drilling_count": 0, "intervention_count": 0, "ratio": 0.0,
        ...
        return {
            "drilling_count": d_count, "intervention_count": i_count,
            "ratio": float(ratio),
            "by_depth_class": self._cross_ratio(
                self._df, cat_col, "WATER_DEPTH_CLASS",
            ),
    ...
    @staticmethod
    def _cross_ratio(
        df: pd.DataFrame, cat_col: str, group_col: str,
    ) -> pd.DataFrame:
        ...
        table = pivot.pivot_table(
            index=group_col, columns=cat_col, values="cnt",
        ...
        table["ratio"] = table.apply(
            lambda r: r["drilling"] / r["intervention"]
            if r["intervention"] > 0 else 0.0,
            axis=1,
        )
    ...
    def _analyze_operator_portfolio(self) -> dict:
        ...
        top = self._df.groupby("BUS_ASC_NAME", dropna=False).agg(
            activity_count=("API_WELL_NUMBER", "count"),
            unique_wells=("API_WELL_NUMBER", "nunique"),
    ...
    def _analyze_duration_benchmarking(self) -> dict:
        _bench_cols = [
            "WATER_DEPTH_CLASS", "RIG_TYPE",
            "mean_days", "p25", "p50", "p75", "count",
        ]
        empty = {
