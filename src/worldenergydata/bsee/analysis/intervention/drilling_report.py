...
_DRILL_DN: dict[str, str] = {
    "drillship": "Drillship", "semi_submersible": "Semi-Submersible",
    "jack_up": "Jack-Up", "platform_rig": "Platform Rig",
    "tender_assisted": "Tender Assisted", "inland_barge": "Inland Barge",
...
_AP: dict[str, str] = {
    "EI": "Shelf", "SS": "Shelf", "SP": "Shelf", "HI": "Shelf",
    "SM": "Shelf", "WD": "Shelf", "VR": "Shelf", "ST": "Shelf",
    "MP": "Shelf", "WC": "Shelf", "EC": "Shelf", "GI": "Shelf",
    "MI": "Shelf", "EB": "Slope", "EW": "Slope",
    "MC": "Deepwater", "GC": "Deepwater", "GB": "Deepwater",
    "VK": "Deepwater", "WR": "Ultra-Deepwater",
}
_WDB = [(0, 500, "Shallow (<500 ft)"), (500, 1500, "Mid-water (500-1500 ft)"),
...
class DrillingAnalysisReport:
    ...
    def generate(self, output_path: str = "reports/bsee/intervention/drilling_analysis.html") -> str:
        """Generate full drilling analysis HTML report."""
        Path(output_path).parent.mkdir(parents=True, exist_ok=True)
        self._fi = 0
        d = self._drill; n = len(d)
        ...
        secs = [self._s1(n, y0, y1), self._s2(), self._s3(), self._s4(),
                self._s5(), self._s6(), self._s7(), self._s8()]
        if self._enhanced:
    ...
    def _s1(self, n: int, y0: int, y1: int) -> dict:
        ...
        bl.append(f"<strong>{n:,}</strong> drilling WAR records spanning "
                  f"<strong>{y0}-{y1}</strong>.")
        bl.append(f"<strong>{uw:,}</strong> unique wells drilled by "
                  f"<strong>{ur:,}</strong> unique rigs.")
        bl.append(f"Dominant rig type: <strong>{_DRILL_DN.get(top_rt, top_rt)}</strong>.")
        if self._has_bus:
            bl.append(f"Top operator: <strong>{d.groupby('BUS_ASC_NAME').size().idxmax()}</strong>.")
        return {"title": "Executive Summary", "content": self._bul(bl)}

    # S2: Drilling Fleet Composition
    def _s2(self) -> dict:
        ch: list[str] = []; d = self._drill
        if d.empty:
            return {"title": "Drilling Fleet Composition",
                    "content": "<p>No drilling data.</p>", "charts": []}
        ...
        f1.update_layout(title="Drilling Rig Type Distribution",
        ...
        return {"title": "Drilling Fleet Composition",
                "content": "<h3>Top 20 Rigs by Activity</h3>" + tbl, "charts": ch}

    # S3: Water Depth Analysis
    def _s3(self) -> dict:
        ch: list[str] = []; d = self._drill
        if d.empty or not self._has_wd:
            return {"title": "Water Depth Analysis",
                    "content": "<p>No water depth data available.</p>", "charts": []}
        wd = d[d["WATER_DEPTH"].notna()].copy()
        cov = len(wd) / len(d) * 100 if len(d) else 0
        note = (f"<p><em>Water depth available for {len(wd):,} of {len(d):,} "
        ...
        f1 = go.Figure(go.Bar(x=cc.index.tolist(), y=cc.values,
                               marker_color=[_PAL[i % len(_PAL)] for i in range(len(cc))],
                               text=[f"{v:,}" for v in cc.values], textposition="outside"))
        f1.update_layout(title="Water Depth Distribution", xaxis_title="Depth Category",
                         yaxis_title="Drilling Records", height=400)
        ch.append(_fhtml(f1, self._fi)); self._fi += 1
        rows = [[cat, f"{int(cc.get(cat, 0)):,}",
                 f"{int(cc.get(cat, 0))/len(wd)*100:.1f}%"] for cat in co]
        return {"title": "Water Depth Analysis",
    ...
    def _s4(self) -> dict:
        ch: list[str] = []; d = self._drill
        if d.empty:
            return {"title": "Drilling Activity by Year",
        ...
        for i, rt in enumerate(_DT):
            if rt not in piv.columns:
                continue
            f1.add_trace(go.Bar(x=piv.index.tolist(), y=piv[rt].tolist(),
                                name=_DRILL_DN.get(rt, rt), marker_color=_PAL[i % len(_PAL)]))
        totals = d.groupby("YEAR").size()
        f1.add_trace(go.Scatter(x=totals.index.tolist(), y=totals.values.tolist(),
                                name="Total", mode="lines+markers",
                                line=dict(color="#1F2937", width=2, dash="dot"), yaxis="y2"))
        f1.update_layout(title="Drilling Activity by Rig Type per Year", barmode="stack",
                         xaxis_title="Year", yaxis_title="WAR Records",
                         yaxis2=dict(title="Total", overlaying="y", side="right"), height=450,
        ...
        for yr in sorted(d["YEAR"].unique()):
            ...
            rows.append([str(yr), f"{c:,}", chg]); prev = c
        return {"title": "Drilling Activity by Year",
                "content": _tbl(["Year", "Records", "YoY Change"], rows), "charts": ch}

    # S5: Drilling Duration Analysis
    def _s5(self) -> dict:
        ch: list[str] = []; d = self._drill
        if d.empty:
            return {"title": "Drilling Duration Analysis",
                    "content": "<p>No data.</p>", "charts": []}
        well = d.groupby("API_WELL_NUMBER").agg(
            records=("RIG_NAME", "size"), rig_type=("RIG_TYPE", "first"))
        well["est_days"] = well["records"] * 7
        ad, md, xd = well["est_days"].mean(), well["est_days"].median(), well["est_days"].max()
        f1 = go.Figure(go.Histogram(x=well["est_days"].tolist(), nbinsx=30,
                                     marker_color=_CLR_D))
        f1.update_layout(title="Well Drilling Duration (Estimated Days)",
                         xaxis_title="Estimated Drilling Days",
                         yaxis_title="Number of Wells", height=400)
        ch.append(_fhtml(f1, self._fi)); self._fi += 1
        stats = (f"<p>Average: <strong>{ad:.0f}</strong> days | "
        ...
        rows = [[_DRILL_DN.get(str(rt), str(rt)), f"{r['mean']:.0f}",
                 f"{r['median']:.0f}", f"{int(r['count']):,}"] for rt, r in rd.iterrows()]
        return {"title": "Drilling Duration Analysis",
                "content": stats + _tbl(["Rig Type", "Avg Days", "Med Days", "Wells"], rows),
                "charts": ch}
    ...
    def _s6(self) -> dict:
        ch: list[str] = []; d = self._drill
        if d.empty:
            return {"title": "Geographic Analysis",
                    "content": "<p>No data.</p>", "charts": []}
        d = d.copy(); d["PROVINCE"] = d["AREA_CODE"].map(_AP).fillna("Other")
        po = ["Shelf", "Slope", "Deepwater", "Ultra-Deepwater", "Other"]
        piv = d.groupby(["YEAR", "PROVINCE"]).size().unstack(fill_value=0)
        clrs = {"Shelf": _PAL[0], "Slope": _PAL[1], "Deepwater": _PAL[2],
                "Ultra-Deepwater": _PAL[3], "Other": _PAL[4]}
        f1 = go.Figure()
        for prov in po:
            if prov not in piv.columns:
                continue
            f1.add_trace(go.Bar(x=piv.index.tolist(), y=piv[prov].tolist(),
                                name=prov, marker_color=clrs.get(prov, "#6B7280")))
        f1.update_layout(title="Drilling by Geological Province Over Time", barmode="stack",
                         xaxis_title="Year", yaxis_title="WAR Records", height=450,
                         legend=dict(orientation="h", yanchor="bottom", y=1.02))
        ch.append(_fhtml(f1, self._fi)); self._fi += 1
        ac = d.groupby("AREA_CODE").size().sort_values(ascending=False).head(15)
        rows = [[a, _AP.get(a, "Other"), f"{int(ac[a]):,}"] for a in ac.index]
        return {"title": "Geographic Analysis",
                "content": _tbl(["Area Code", "Province", "Records"], rows), "charts": ch}

    # S7: Operator Intelligence
    def _s7(self) -> dict:
        ch: list[str] = []; d = self._drill
        if d.empty or not self._has_bus:
            return {"title": "Operator Intelligence",
        ...
        f1 = go.Figure(go.Bar(y=top.index.tolist(), x=top.values, orientation="h",
                               marker_color=_CLR_D, text=[f"{v:,}" for v in top.values],
                               textposition="outside"))
        f1.update_layout(title="Top Drilling Operators", xaxis_title="WAR Records",
                         height=600, margin=dict(l=250))
        ch.append(_fhtml(f1, self._fi)); self._fi += 1
        rows = []
        for op in reversed(top.index):
            s = d[d["BUS_ASC_NAME"] == op]
            rows.append([op, f"{len(s):,}", _DRILL_DN.get(_mode(s["RIG_TYPE"]),
                         _mode(s["RIG_TYPE"])), _mode(s["AREA_CODE"]),
                         str(s["API_WELL_NUMBER"].nunique())])
        tbl = _tbl(["Operator", "Records", "Primary Rig Type", "Top Area", "Wells"], rows)
    ...
    def _s8(self) -> dict:
        d = self._drill
        if d.empty:
            return {"title": "Drilling Parameters Summary", "content": "<p>No data.</p>"}
        ...
        if self._has_bus:
            ...
            hhi_tbl = ("<h3>Operator Area Concentration (HHI)</h3>"
    ...
    def _s9(self, n: int, y0: int, y1: int) -> dict:
        dur_line = ("<li><strong>Duration:</strong> actual SPUD&#8594;TD from borehole data</li>"
                    if self._enhanced else
                    "<li><strong>Duration estimate:</strong> each WAR record ~ 1 week; per-well "
                    "duration = record count x 7 days</li>")
        enrich_line = ("<li><strong>Enrichment:</strong> enriched with borehole and paleowells data</li>"
                       if self._enhanced else "")
    ...
    def _se_duration(self) -> dict:
        ...
        stats = (f"<p>Actual mean: <strong>{mean_d:.1f}</strong> days | "
    ...
    def _se_insights(self) -> dict:
        cats = ["key_findings", "market_trends", "competitive_intelligence",
                "opportunity_areas", "risk_factors"]
        labels = {"key_findings": "Key Findings", "market_trends": "Market Trends",
        ...
        return {"title": "Key Insights", "content": body if body else "<p>No insights.</p>"}
