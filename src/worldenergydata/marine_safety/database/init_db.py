...
class DatabaseInitializer:
    ...
    def __init__(
        ...
        self.is_postgresql = db_url.startswith("postgresql://") or db_url.startswith(
            "postgres://"
        )
        ...
        schema_dir = (
            project_root / "specs" / "modules" / "analysis" / "marine" / "sub-specs"
        )
        ...
        logger.info(
            f"Database type: {'PostgreSQL' if self.is_postgresql else 'SQLite'}"
        )
        logger.info(f"Schema path: {self.schema_path}")
        logger.info(
            f"Mode: {'VERIFY ONLY' if verify_only else 'DRY RUN' if dry_run else 'DEPLOY'}"
        )
    ...
    def connect(self) -> None:
        ...
        try:
            if self.is_postgresql:
                ...
                url_parts = self.db_url.replace("postgresql://", "").replace(
                    "postgres://", ""
                )
    ...
    def read_schema_file(self) -> str:
        ...
        if not self.schema_path.exists():
            schema_type = (
                "SQLite" if (self.dev_mode or self.is_sqlite) else "PostgreSQL"
            )
    ...
    def create_schema(self) -> None:
        ...
        if self.is_sqlite:
            logger.warning(
                "SQLite mode: Some PostgreSQL-specific features will be skipped"
            )
        ...
        for i, (statement, description) in enumerate(statements, 1):
            try:
                ...
                if self.is_sqlite and self._should_skip_for_sqlite(statement):
                    skip_count += 1
                    logger.debug(
                        f"[{i}/{len(statements)}] Skipped (SQLite): {description}"
                    )
                    continue
    ...
    def seed_reference_data(self) -> None:
        ...
        if not self.dry_run:
            ...
            if type_count > 0 or source_count > 0:
                logger.warning(
                    f"Reference data already exists (types: {type_count}, sources: {source_count})"
                )
    ...
    def create_admin_user(
        self, username: str = "admin", password: Optional[str] = None
    ) -> None:
        """
        ...
        try:
            ...
            if password:
                create_sql = sql.SQL(
                    "CREATE USER {} WITH PASSWORD %s IN ROLE marine_safety_admin"
                ).format(sql.Identifier(username))
                self.execute_sql(
                    create_sql.as_string(self.conn), f"Creating user: {username}"
                )
                self.cursor.execute(create_sql, (password,))
            else:
                # Create user without password (must be set later)
                create_sql = sql.SQL(
                    "CREATE USER {} IN ROLE marine_safety_admin"
                ).format(sql.Identifier(username))
                self.execute_sql(
                    create_sql.as_string(self.conn), f"Creating user: {username}"
                )
                logger.warning(
                    f"User '{username}' created without password - set with ALTER USER"
                )
    ...
    def print_summary(self, verification: Dict[str, int]) -> None:
        ...
        logger.info(
            f"  Materialized Views:  {verification.get('materialized_views', 0):3d}"
        )
...
Examples:
  ...
  python init_db.py
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Print SQL statements without executing"
    )
    ...
    parser.add_argument(
        "--admin-password", type=str, help="Admin password (prompts if not provided)"
    )
...
def main() -> int:
    ...
    if not args.db_url:
        logger.error(
            "Database URL required: use --db-url or set DATABASE_URL environment variable"
        )
        return 1
