...
try:
    ...
    from worldenergydata.analysis.verification.models import VerificationResult
...
class WellDashboardExportManager:
    ...
    # BSEE row to DataFrame column mapping
    _BSEE_COLUMN_MAP: Dict[str, str] = {
        "Field/Block/Lease": "field_block_lease",
        "Well Name": "well_name",
        "Oil Production (BBL)": "oil_production",
        "Gas Production (MCF)": "gas_production",
        "Water Production (BBL)": "water_production",
        "Operating Days": "operating_days",
        "Oil Sales (BBL)": "oil_sales",
        "Gas Sales (MCF)": "gas_sales",
        "Revenue ($)": "revenue",
        "Operating Cost ($)": "operating_cost",
        "Net Income ($)": "net_income",
        "Cumulative Oil (BBL)": "cumulative_oil",
        "Cumulative Gas (MCF)": "cumulative_gas",
        "Quality Score": "quality_score",
    }

    # Standard BSEE 14-row format order
    _BSEE_ROW_ORDER: List[str] = [
        "Field/Block/Lease",
        "Well Name",
        "Oil Production (BBL)",
        "Gas Production (MCF)",
    ...
    def format_bsee_standard(self, well_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        Returns:
            BSEE formatted data dictionary with 14 standard rows
        """
        if well_data.empty:
            return self._create_empty_bsee_data()

        return self._map_columns_to_bsee_rows(well_data)

    def _create_empty_bsee_data(self) -> Dict[str, Any]:
        """Create empty BSEE data structure with all required rows."""
        return {row: [] for row in self._BSEE_ROW_ORDER}

    def _map_columns_to_bsee_rows(self, well_data: pd.DataFrame) -> Dict[str, Any]:
        """
        Map DataFrame columns to BSEE standard rows.

        Args:
            well_data: Well production DataFrame

        Returns:
            Dictionary with BSEE row names as keys and column data as values
        """
        bsee_data = {}
        default_values = [0] * len(well_data)

        for bsee_row in self._BSEE_ROW_ORDER:
            column_name = self._BSEE_COLUMN_MAP.get(bsee_row)
            bsee_data[bsee_row] = self._extract_column_data(
                well_data, column_name, default_values
            )
    ...
    def _extract_column_data(
        ...
        """
        Extract column data from DataFrame or return defaults.

        Args:
            well_data: Source DataFrame
            column_name: Column to extract
            default_values: Values to use if column not found

        Returns:
            List of column values or default values
        """
        if column_name and column_name in well_data.columns:
            return well_data[column_name].tolist()
