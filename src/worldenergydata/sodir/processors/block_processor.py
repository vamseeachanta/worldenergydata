...
class BlockProcessor:
        ...
        try:
            processed = {
                ...
                "valid_from": self._parse_date(
                    block_data.get("blcAreaPolyDateValidFrom")
                ),
                ...
                "licensees": self._process_licensees(
                    block_data.get("blcLicensees", [])
                ),
                ...
                "vertical_reference": block_data.get(
            ...
            logger.error(
                f"Error processing block {block_data.get('blcName', 'unknown')}: {str(e)}"
            )
            self.error_count += 1
            return None
    ...
    def process_batch(self, blocks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        logger.info(
            f"Processed {len(processed_blocks)}/{len(blocks)} blocks successfully"
        )
        ...
        for date_field in ["blcAreaPolyDateValidFrom", "blcAreaPolyDateValidTo"]:
            if date_field in block_data and block_data[date_field]:
                if not self._is_valid_date(block_data[date_field]):
                    errors.append(
                        f"Invalid date format in {date_field}: {block_data[date_field]}"
                    )
        ...
        if "geometry" in block_data and block_data["geometry"]:
            ...
            if "coordinates" in geometry and geometry["coordinates"]:
                ...
                if geometry.get("type") == "Polygon":
                    coords = (
                        geometry["coordinates"][0] if geometry["coordinates"] else []
                    )
                    if coords:
        ...
        if "coordinates" in block_data:
            ...
            if isinstance(coords, dict):
                ...
                if northing and easting:
                    ...
                    # Zone 31-35 cover Norway, centered around 3-21 degrees E
                    lon = (
                        3.0 + (zone - 31) * 6 + (easting - 500000) / 111320.0
                    )  # Rough approximation
                    return lon
