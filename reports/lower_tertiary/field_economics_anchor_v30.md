...
On public BSEE production + cost data, **Anchor** is **NPV-negative at 10%** life-to-date: terminal cumulative NPV **$-1,732.8 M** (sanctioned V30 model).

- **2 producing wells** (**15 total wellbores**), generating **$476 M** gross revenue.
- A **high-capex, deepwater** signature: **$4,161 M** of one-time D&C + facilities capital is the dominant driver of the NPV.
- The cumulative-NPV path bottomed at **$-1,797.0 M** in **2024** and has since recovered **$+64.2 M** as production paid back capital.
...
Cumulative discounted NPV evolution over field life, with critical well operations annotated. Terminal cumulative NPV = **$-1,732.8 M** (reconciles to sanctioned baseline $-1,732.8 M).

Cumulative NPV path (by year): `█▇▇▇▇▇▇▇▆▄▁▁`  _start $-145M → trough $-1,797M (2024) → latest $-1,733M_
...
Field terminal NPV decomposed into per-well contributions that sum exactly to the field total. Field NPV = **$-1,732.8 M**; sum of per-well net NPV = **$-1,732.8 M** (residual $0.0000).

| Rank | Well (API) | Name | Oil (MMbbl) | Gross well NPV ($MM) | Allocated shared cost ($MM) | Net well NPV ($MM) | % of field NPV |
|-----:|-----------|------|------------:|---------------------:|----------------------------:|-------------------:|-----------:|
| 1 | 608114075000 | AP001 | 3.64 | -80.2 | -855.2 | -935.4 | 54.0% |
| 2 | 608114075100 | AP002 | 3.28 | -27.4 | -770.0 | -797.4 | 46.0% |
...
NPV is linear in the oil price deck: each **+$1/bbl** on the realized oil price moves field NPV by **$+2.0 M**. Life-to-date NPV reaches **zero at a flat-equivalent realized WTI of $940/bbl**, versus the actual volume-weighted realized **$69/bbl** over the window.
