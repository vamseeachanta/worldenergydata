...
def compute():
    ...
    off_season_ranked = sorted(off_season_months,
    ...
    payload = {
        ...
        "overlay": {
            ...
            "lower_risk_windows_ranked": [
                {"month": mm["label"], "count": mm["count"]}
                for mm in off_season_ranked
            ],
        },
    }
    return payload
...
def render_md(p):
    ...
    L = []
    L.append("# Seasonal Intervention Risk Windows — Hurricane Season × WAR × Operations\n")
    L.append(f"*Issue #427 · child of #423 · sibling of #403 / #416 · data as of {DATE} · "
             "aggregate-only · **descriptive only (small N, no significance test)***\n")

    L.append("## TL;DR\n")
    L.append(f"- **{d['total_records']} WAR-derived operation records** carry a parseable "
             f"activity start date across the local BSEE sample CSVs.\n")
    L.append(f"- **{ov['records_in_hurricane_season']} ({ov['share_in_hurricane_season']:.0%})** "
    ...
    L.append(f"- **{ov['records_off_season']} ({ov['share_off_season']:.0%})** fall in the "
    ...
    if ov["lower_risk_windows_ranked"]:
        ...
        L.append("- **Lowest-storm-risk months with observed activity (data):** "
                 + ", ".join(f"{m['month']} ({m['count']})" for m in top) + ".\n")

    L.append("## Data vs assumptions — hard separation\n")
    L.append("| Element | Type | Source |")
    L.append("|---|---|---|")
    L.append("| Monthly/seasonal activity counts | **DATA** | local BSEE WAR CSVs (below) |")
    L.append("| Activity-type split | **DATA** | `WELL_ACTIVITY_CD` in the same CSVs |")
    L.append("| Hurricane-season window (Jun–Nov) & peak (Aug–Oct) | **ASSUMPTION (public)** | "
             "NOAA/NHC climatology |")
    L.append("")
    L.append("> **No HSE-incident-by-season cut** is included: the assembled "
    ...
    for lbl, info in p["data_sources"].items():
        L.append(f"| `{lbl}` | `{info['path']}` | {info['rows']} | {info['parseable_dates']} |")
    L.append("")
    ...
    for mm in d["months"]:
        ...
        L.append(f"| {mm['label']} | {mm['count']} | {mm['share']:.0%} | {hsf} | {pk} |")
    ...
    L.append("|---|---|---:|---:|---:|")
    for act, sp in sorted(d["activity_season_split"].items(), key=lambda x: -x[1]["total"]):
        mean = d["activity_meaning"].get(act, "")
        L.append(f"| `{act}` | {mean} | {sp['in_season']} | {sp['off_season']} | {sp['total']} |")
    L.append("")

    L.append("## Lower-risk operating windows (read-through)\n")
    L.append(f"- The Atlantic hurricane season ({hs['season_label']}; source: {hs['source']}) "
             "concentrates storm-evacuation, weather-deferral, and crew-fatigue overlap risk "
             f"in Jun–Nov, peaking {hs['peak_label']}.\n")
    L.append(f"- **Off-season (Dec–May)** carried **{ov['records_off_season']} of "
    ...
    L.append("- **Operational decision support:** where intervention scope is schedulable, "
             ...
             "(N≈" + f"{d['total_records']}" + ") to attach a quantified HSE-rate delta or a "
             "significance test.\n")

    L.append("## Caveats\n")
    L.append("- **Small N / sample extracts.** The local BSEE operations CSVs are truncated "
             "samples, not the full census; counts are illustrative of method, not a "
             "population estimate.\n")
    L.append("- **WAR_START_DT = reporting-period start**, a close proxy for when the "
             "operation was active that week; it is not a precise incident timestamp.\n")
    L.append("- **No HSE incident data** locally (0-byte db stub; no raw INCINV) — the "
             "incident-rate-by-season half of #427's scope is deferred to when INCINV lands.\n")
    L.append("- **No metocean current/wave overlay** included; that requires the #403 "
             "metocean datasets, out of scope for this local-only pass.\n")

    L.append("## Cross-links\n")
    L.append("- #403 — hurricane-mooring infrastructure (metocean/storm window source).\n")
    L.append("- #416 — Phase-1A WAR loaders + INCINV classification (loader reuse target).\n")
    L.append("- #423 — marketing-pipeline umbrella (parent).\n")

    L.append("## Reproduce\n")
    L.append("```\nPYTHONPATH=src python3 reports/gtm/seasonal_intervention_risk_windows.py\n```")
    L.append("Deterministic: emits the `.json` sidecar (all figures), this `.md`, and the "
             "`.html`. Every number above is in the JSON.\n")
    L.append(f"*Generated {p['meta']['generated_at']} · data as of {DATE} · aggregate-only.*\n")
    return "\n".join(L)
...
def render_html(p):
    ...
    bars = [f'<rect x="{band_x:.0f}" y="{top}" width="{band_w:.0f}" height="{h}" '
    ...
    for i, v in enumerate(counts):
        ...
        bh = int(h * v / mx)
        x = left + i * (bw + gap)
        color = "#dc2626" if m in peak_months else ("#d97706" if m in season_months else "#0e7490")
        bars.append(f'<rect x="{x}" y="{top+h-bh}" width="{bw}" height="{bh}" fill="{color}"/>'
    ...
    month_svg = (f'<svg width="{chart_w}" height="{top+h+30}" style="max-width:100%">'
                 f'{"".join(bars)}</svg>')
    ...
    for mm in d["months"]:
        ...
        month_rows += (f"<tr{bg}><td>{mm['label']}</td><td>{mm['count']}</td>"
    ...
    for act, sp in sorted(d["activity_season_split"].items(), key=lambda x: -x[1]["total"]):
        mean = d["activity_meaning"].get(act, "")
        act_rows += (f"<tr><td><code>{esc(act)}</code></td><td>{esc(mean)}</td>"
                     f"<td>{sp['in_season']}</td><td>{sp['off_season']}</td>"
                     f"<td>{sp['total']}</td></tr>")

    src_rows = ""
    for lbl, info in p["data_sources"].items():
        src_rows += (f"<tr><td><code>{esc(lbl)}</code></td><td><code>{esc(info['path'])}</code></td>"
                     f"<td>{info['rows']}</td><td>{info['parseable_dates']}</td></tr>")

    lrw = ", ".join(f"{m['month']} ({m['count']})"
                    for m in ov["lower_risk_windows_ranked"][:3])
...
def main():
    ...
    print(f"total_records={payload['data']['total_records']} "
