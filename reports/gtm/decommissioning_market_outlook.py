...
def build_forecast(inv):
    ...
    idle_15 = sum(
        v
        for y, v in inv["gom_ta_by_year"].items()
    ...
    for sk in ("low", "base", "high"):
        ...
        dwf = dw_share[sk]
        blended_unit = (1 - dwf) * cost["shelf_shallow"][sk] + dwf * cost["deepwater"][
            sk
        ]
...
def render_markdown(p):
    ...
    L.append(
        f"> **Issue**: [#424](https://github.com/vamseeachanta/worldenergydata/issues/424) "
        f"(child of [#423](https://github.com/vamseeachanta/worldenergydata/issues/423))  "
    )
    L.append("> **Branch**: `feat/autorun-2026-06-19`  ")
    L.append(
        f"> **Data as of**: {DATE} (BSEE borehole-status census; status dates through "
        f"{a['data_as_of_year']})  "
    )
    L.append(
    ...
    L.append(
        "A forward 5-year (2026-2030) Gulf of Mexico **decommissioning / plug-&-abandonment "
    ...
    for s in order:
        if s in d["gom_status"]:
            L.append(
                f"| `{s}` | {STATUS_MEANING.get(s, '')} | {d['gom_status'][s]:,} |"
            )
    L.append(f"| **Total** | | **{d['gom_wells']:,}** |\n")
    L.append(
        f"**Already decommissioned (PA):** {d['gom_status'].get('PA',0):,} GoM boreholes are "
    ...
    L.append(
        "Wells **not yet permanently abandoned** but in a status that draws down into P&A "
        "over the horizon — the addressable pipeline:\n"
    )
    L.append("| Eligible bucket | Count | Basis |")
    L.append("|---|---:|---|")
    L.append(
        f"| `TA` temporarily abandoned (idle) | {ei['TA_idle']:,} | idle iron — direct P&A candidates |"
    )
    L.append(
        f"| `COM` completed (producing/capable) | {ei['COM_completed']:,} | reach end-of-life over horizon |"
    )
    L.append(f"| `DSI` drilled & suspended | {ei['DSI']:,} | inactive |")
    L.append(f"| `AST` abandoned sidetrack | {ei['AST']:,} | residual |")
    L.append(
        f"| **Total eligible inventory** | **{ei['total_eligible']:,}** | model drawdown base |\n"
    )
    ...
    L.append(
        f"- **{ii['ta_idle_ge_10yr']:,}** TA wells have been idle **≥10 years** "
    ...
    L.append(
        "| GoM P&A | "
        + " | ".join(str(op["by_year_2015_2024"][str(y)]) for y in range(2015, 2025))
        + " |"
    )
    L.append("")
    L.append(
        f"- **2015-2024 mean**: {op['mean_2015_2024']:.1f} P&A / yr.  "
        f"**2020-2024 mean**: {op['mean_2020_2024']:.1f} / yr."
    )
    L.append(
        f"- Implied historical attrition pace vs. the eligible inventory: "
    ...
    L.append(
        "**Model**: eligible-inventory drawdown.  For each forecast year, "
    ...
    b = sc["base"]
    L.append(
        f"**Base case**: **{b['pa_wells_5yr_total']:,} GoM wells** plugged over 2026-2030 "
    ...
    L.append(
        f"**Sensitivity span**: the market ranges from **${sc['low']['market_size_5yr_usd_bn']:.2f}B** "
    ...
    L.append(
        f"- **Pipeline is large and aging**: {ei['total_eligible']:,} eligible wells, of which "
    ...
    L.append(
        "- **Steady multi-year demand**: at the base pace the GoM sustains several hundred "
        "P&A wells/year, a durable services market for well-plugging, rig/vessel, casing-cut, "
        "and conductor-removal contractors.\n"
    )
    L.append(
        "- **Capacity gap signal**: the observed PA rate "
    ...
    L.append(
        "Deterministic: emits the `.json` sidecar (all figures), this `.md`, and the `.html`. "
        "Every number above is in the JSON.\n"
    )
    L.append(
        f"*Generated {p['generated_at']} · data as of {DATE} · operator-aggregate only.*\n"
    )
    return "\n".join(L)
...
def render_html(p):
    ...
    for i, (y, v) in enumerate(pa_hist):
        ...
        x = i * (bw + gap) + 40
        bars.append(
            f'<rect x="{x}" y="{h-bh+20}" width="{bw}" height="{bh}" fill="#0e7490"/>'
    ...
    pa_svg = (
        f'<svg width="{len(pa_hist)*(bw+gap)+60}" height="{h+50}" '
    ...
    for i, (y, v) in enumerate(zip(yrs, b["pa_per_year"])):
        ...
        x = i * (bw + gap + 20) + 40
        fbars.append(
            f'<rect x="{x}" y="{h-bh+20}" width="{bw+8}" height="{bh}" fill="#d97706"/>'
    ...
    fc_svg = (
        f'<svg width="{len(yrs)*(bw+gap+20)+60}" height="{h+50}" '
    ...
    def srow(scn):
        ...
        return (
            f"<tr><td><b>{scn}</b></td><td>{s['attrition_pace']:.1%}</td>"
    ...
    for s in ["PA", "ST", "COM", "TA", "CNL", "DSI", "AST", "DRL", "APD"]:
        if s in d["gom_status"]:
            status_rows += (
                f"<tr><td><code>{s}</code></td><td>{esc(STATUS_MEANING.get(s,''))}</td>"
...
def main():
    ...
    print(
        "eligible:",
        fc["eligible_inventory"]["total_eligible"],
        "| base 5yr wells:",
        fc["scenarios"]["base"]["pa_wells_5yr_total"],
        "| base market $B:",
        fc["scenarios"]["base"]["market_size_5yr_usd_bn"],
    )
