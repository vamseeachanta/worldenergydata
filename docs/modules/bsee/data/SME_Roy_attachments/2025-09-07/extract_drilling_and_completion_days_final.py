# Filename: filter_war_files_by_lease.py (multi-lease support with filename tags)

import pandas as pd
import os

lease_file = os.path.join(os.path.dirname(__file__), "leases.csv")
# Load leases.csv
leases = pd.read_csv(lease_file,header=None, encoding="ISO-8859-1", dtype=str)[0].dropna().str.upper().str.strip().tolist()

# Tag for output filenames
lease_tag = "_".join([l.replace('/', '-') for l in leases])
