...
def build_report(lease):
    lease = lease.strip().upper()
    if not lease.startswith("G"):
        lease = "G" + lease

    # 1) main → SN_WAR & API
    main = load_clean("mv_war_main.txt")
    ...
    # 2) boreholes by API
    bore = load_clean("mv_war_boreholes_view.txt")
    ...
    prop = load_clean("mv_war_main_prop.txt")
    ...
    # 5) build Excel
    filename = f"{lease}_summary_formulas.xlsx"
    wb = Workbook()
    ws = wb.active
    ws.title = lease
