# Adversarial Plan Review Request: Issue #343

You are an independent adversarial reviewer. Assume the plan has defects until proven otherwise.
Do not praise. Do not restate the plan. Focus only on what is wrong, missing, risky, internally inconsistent, or insufficiently grounded.
Return APPROVE only if you affirmatively verify the plan is implementation-ready. When in doubt, return MINOR or MAJOR.
...
Issue: #343 feat(cost): build major-operator annual statement source registry and yearly coverage tracker
Goal: create a machine-readable registry of major operators, annual-statement discovery channels, and year-by-year coverage metadata that can support future disclosure collection.
Grounded facts already verified:
- sanction source evidence is currently embedded in src/worldenergydata/cost/data_collection/public_dataset.py
- disclosure provenance contract exists in src/worldenergydata/cost/data_collection/disclosure_ingest_contract.py
- derived YoY analytics exist in src/worldenergydata/cost/disclosure_analytics.py
- no operator statement registry module currently exists

## Review questions
1. Is the scope bounded enough to avoid drifting into scraping/backfill work?
2. Is the proposed registry contract concrete enough to implement and test?
3. Are there missing artifact paths, test requirements, or vocabulary decisions that should be fixed in-plan first?
4. Does the seed operator set and year-coverage concept align with current repo evidence?
5. Are any acceptance criteria qualitative rather than executable?
...
Key findings:
- <severity> <finding with citation>
