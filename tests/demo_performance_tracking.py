...
from worldenergydata.testing.performance.database import (
    PerformanceDatabase,
    TestExecutionRecord,
)
...
def create_sample_data(db: PerformanceDatabase):
    ...
    tests = [
        {
            "name": "test_bsee_production_processing",
            "base_duration": 2.5,
            "module": "tests/test_bsee.py",
        },
        {
            "name": "test_well_data_validation",
            "base_duration": 1.2,
            "module": "tests/test_bsee.py",
        },
        {
            "name": "test_npv_calculation",
            "base_duration": 0.8,
            "module": "tests/test_financial.py",
        },
        {
        ...
        {
            "name": "test_wind_data_processing",
            "base_duration": 1.8,
            "module": "tests/test_wind.py",
        },
        {
            "name": "test_validation_framework",
            "base_duration": 0.3,
...
def demo_performance_analysis():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for idx, row in slow_tests.iterrows():
            ...
            print(
                f"   Avg: {row['avg_duration']:.3f}s | Max: {row['max_duration']:.3f}s"
            )
        ...
        if stability["status"] == "analyzed":
            ...
            print(
                f"Average duration: {stability['avg_duration']:.3f}s ± {stability['std_duration']:.3f}s"
            )
        ...
        for workers in [1, 2, 4, 8]:
            ...
            if parallel["status"] == "calculated":
                print(
                    f"{workers} workers: {parallel['parallel_execution_time']:.1f}s "
