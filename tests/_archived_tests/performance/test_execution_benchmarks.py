            ...
            if operation in baseline:
                ...
                if current_time > baseline_time * (1 + tolerance):
                    regressions.append({
                        ...
                        'increase': (current_time - baseline_time) / baseline_time * 100
                    })
...
class TestExecutionBenchmarks:
    ...
    def test_calculation_benchmarks(self, recorder, sample_production_data):
        ...
        start = time.perf_counter()
        sample_production_data['ma_7'] = sample_production_data.groupby('well_id')['oil_production'].transform(
        ...
        start = time.perf_counter()
        sample_production_data['cumulative'] = sample_production_data.groupby('well_id')['oil_production'].cumsum()
        ...
        stats = {
            ...
            'quantiles': sample_production_data['oil_production'].quantile([0.25, 0.5, 0.75])
        }
    ...
    def test_export_benchmarks(self, recorder, tmp_path, sample_production_data):
        ...
        sample_production_data.to_excel(excel_file, index=False)
        excel_time = time.perf_counter() - start
        recorder.record('export_excel', excel_time, {'rows': len(sample_production_data)})
        ...
        sample_production_data.to_parquet(parquet_file, index=False)
        parquet_time = time.perf_counter() - start
        recorder.record('export_parquet', parquet_time, {'rows': len(sample_production_data)})
    ...
    def test_datetime_operations_benchmarks(self, recorder):
        ...
        start = time.perf_counter()
        df['timestamp_utc'] = df['timestamp'].dt.tz_localize('US/Central').dt.tz_convert('UTC')
