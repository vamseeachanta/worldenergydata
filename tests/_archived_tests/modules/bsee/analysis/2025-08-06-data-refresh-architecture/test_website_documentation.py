...
class BSEEDocumentationTester:
    ...
    def __init__(self):
        ...
        self.api_keywords = [
            ...
            "programmatic", "developer", "integration", "swagger",
            "openapi", "curl", "http request", "authentication"
        ]
        
        # BSEE-specific technical keywords
        self.bsee_tech_keywords = [
            "data access", "query interface", "web service", "download api",
            "bulk data", "automated access", "system integration",
            "data export", "file api", "batch processing"
        ]
    ...
    def _analyze_documentation_content(self, response, result):
        ...
        try:
            ...
            found_api_keywords = [keyword for keyword in self.api_keywords 
                                 if keyword in content]
            result['api_keywords_found'] = found_api_keywords
            
            # Look for BSEE technical keywords
            found_bsee_keywords = [keyword for keyword in self.bsee_tech_keywords
                                  if keyword in content]
            result['bsee_tech_keywords_found'] = found_bsee_keywords
            
            # Look for code examples
            code_indicators = ['<code>', '<pre>', 'curl', 'http://', 'https://', 
                              'json', 'xml', '<?xml', 'application/json']
            result['has_code_examples'] = any(indicator in content 
                                            for indicator in code_indicators)
            
            # Look for technical specifications
            tech_indicators = ['specification', 'protocol', 'format', 'schema',
                             'parameter', 'response', 'request', 'header']
            result['has_technical_specs'] = any(indicator in content 
    ...
    def _extract_relevant_links(self, soup, result):
        ...
        relevant_patterns = [
            r'api', r'rest', r'developer', r'docs', r'technical',
            r'integration', r'service', r'data.*access', r'query'
        ]
    ...
    def search_content_for_api_references(self, base_url: str) -> Dict:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                api_patterns = [
                    r'api\b', r'application programming interface',
                    r'web service', r'rest service', r'json service',
                    r'programmatic access', r'automated access'
                ]
                ...
                for link in nav_links:
                    ...
                    if any(word in link_text for word in 
                           ['developer', 'technical', 'api', 'integration']):
                        href = link.get('href', '')
...
class TestBSEEWebsiteDocumentation:
    ...
    def test_documentation_section_discovery(self):
        ...
        # Analyze findings
        api_relevant_sections = [section for section in doc_sections_found
                               if section['api_keywords_found'] or 
                                  section['documentation_type'] in ['api_reference', 'developer_guide']]
        ...
        summary = {
            ...
            'documentation_types_found': list(set(section['documentation_type'] 
        ...
        logger.info(f"Found {len(doc_sections_found)} accessible documentation sections")
    ...
    def test_main_website_content_analysis(self):
        ...
        summary = {
            'websites_analyzed': len([r for r in content_analysis_results if r['searched']]),
        ...
        logger.info(f"Content analysis complete: {len(all_api_mentions)} API mentions found")
        assert True  # Always passes, documents findings
    ...
    def test_discovered_links_analysis(self):
        ...
        for link_url in list(self.doc_tester.discovered_links)[:20]:
            try:
                ...
                if response.status_code == 200:
                    ...
                    api_indicators = ['api', 'rest', 'json', 'xml', 'endpoint', 'service']
                    analysis['api_relevant'] = any(indicator in content 
        ...
        self.doc_tester.document_finding(
            ...
            "Discovered links analysis complete",
            summary
        )
    ...
    def test_search_functionality_analysis(self):
        ...
        """
        search_terms = [
            "api", "web service", "rest", "json", "developer",
            "programmatic access", "integration", "technical documentation"
        ]
        ...
        for base_url in search_bases:
            for term in search_terms[:3]:  # Limit search terms to prevent excessive requests
                try:
                    ...
                    for search_url in search_patterns:
                        ...
                        if response.status_code == 200:
                            search_result = {
                                ...
                                'has_results': 'no results' not in response.text.lower()
                            }
        ...
        summary = {
            ...
            'searches_with_results': len([r for r in search_results if r.get('has_results', False)])
        }
        
        self.doc_tester.document_finding(
            'Search Functionality Summary',
            "Website search analysis complete",
            summary
        )
    ...
    def teardown_method(self):
        ...
        if hasattr(self, 'doc_tester') and self.doc_tester.findings:
            ...
            api_relevant_findings = len([f for f in self.doc_tester.findings 
                                       if 'api' in f['description'].lower() or 
                                          'developer' in f['description'].lower()])
            ...
            report_lines.append(f"- Links discovered: {len(self.doc_tester.discovered_links)}")
            report_lines.append("")
                    ...
                    if finding['details']:
                        for key, value in finding['details'].items():
                            if isinstance(value, list) and len(value) > 0:
                                report_lines.append(f"  - {key}: {', '.join(map(str, value[:5]))}")
            ...
            try:
                with open(report_path, 'w', encoding='utf-8') as f:
                    f.write(report)
                logger.info(f"Website documentation analysis report saved to {report_path}")
            except Exception as e:
