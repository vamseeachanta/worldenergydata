...
class BSEEDataConverter:
    ...
    def convert_to_bsee_production_format(df):
        ...
        result['Gor_scf_bbl'] = result['MON_G_PROD_VOL'] * 1000 / result['MON_O_PROD_VOL'].replace(0, 1)
        result['Wor_bbl_bbl'] = result['MON_W_PROD_VOL'] / result['MON_O_PROD_VOL'].replace(0, 1)
    ...
    def load_and_convert_test_data():
        ...
        test_data_path = Path('tests/fixtures/test_configs/bsee')
    ...
    def create_minimal_bsee_data(num_wells=2, num_months=12):
        ...
        df['PRODUCTION_DATETIME'] = pd.to_datetime(
            ...
            df['MER_MM'].astype(str).str.zfill(2) + '-01'
        )
...
if __name__ == "__main__":
    ...
    if converted is not None:
        ...
        converted.to_csv('tests/fixtures/test_configs/bsee/production_data_bsee_format.csv', index=False)
