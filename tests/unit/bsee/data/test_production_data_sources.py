...
class TestProductionDataFromSources:
    ...
    def setup(self):
        ...
        self.mock_cfg = {
            "basename": "bsee",
            "data": {
                "groups": [{"api12": ["123456789012"]}, {"api12": ["234567890123"]}]
            },
        }
    ...
    @patch(
        "worldenergydata.bsee.data.production.production_data_sources.production_from_zip"
    )
    ...
    def test_get_data_with_zip(self, mock_prod_zip):
        ...
        mock_prod_zip.get_production_data_by_wellapi12.assert_called_once_with(
            cfg, "123456789012"
        )
    ...
    def test_get_data_multiple_api12_per_group(self, mock_prod_zip):
        ...
        cfg = {
            "basename": "bsee",
            "data": {
                "groups": [{"api12": ["123456789012", "234567890123", "345678901234"]}]
            },
        }
