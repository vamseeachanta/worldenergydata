...
from tests.test_markers import smoke, unit
...
class TestBSEEAnalysis:
    ...
    def setup(self):
        ...
        self.mock_cfg = {
            "analysis": {"flag": True, "type": "production"},
        ...
        self.mock_data = {
            ...
            "production_data": pd.DataFrame(
                {"API12": ["123456789012", "234567890123"], "PRODUCTION": [1000, 2000]}
            ),
        }
    ...
    def test_run_analysis_for_all_wells(self, mock_prod_analysis, mock_well_analysis):
        ...
        well_groups = {
            "well_summary_df_groups": pd.DataFrame(
                {"API12": ["123456789012"], "WELL_NAME": ["WELL-1"]}
            )
        }
        prod_groups = {
        ...
        mock_prod_analysis.run_production_analysis.return_value = (
            self.mock_cfg,
            prod_groups,
        )
        ...
        with patch.object(self.analysis, "add_production_dates_to_well_summary"):
            result = self.analysis.run_analysis_for_all_wells(
                self.mock_cfg, self.mock_data
            )
    ...
    def test_add_production_dates_no_matching_wells(self, mock_logger, mock_well_api12):
        ...
        well_groups = {
            "well_summary_df_groups": pd.DataFrame(
                {"API12": ["999999999999"], "WELL_NAME": ["WELL-X"]}
            )
        }
        prod_groups = {
            "production_summary_df_groups": pd.DataFrame(
...
class TestBSEEAnalysisParameterized:
    ...
    @patch("worldenergydata.bsee.analysis.bsee_analysis.well_api12_analysis")
    @patch("worldenergydata.bsee.analysis.bsee_analysis.logger")
    def test_production_date_matching(
        self, mock_logger, mock_well_api12, analysis, api12, expected_matches
    ):
        ...
        prod_groups = {
            "production_summary_df_groups": (
                pd.DataFrame(prod_data) if prod_data else pd.DataFrame()
            )
        }
