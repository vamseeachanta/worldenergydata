...
def test_npv_output_files():
    ...
    expected_npv_columns = [
        ...
        "Total_Revenue_USD",
        "Total_OPEX_USD",
        "Total_Net_Cash_Flow_USD",
        "Analysis_Date",
        "Notes",
    ]
    assert (
        list(npv_summary_df.columns) == expected_npv_columns
    ), f"NPV summary columns mismatch. Got: {list(npv_summary_df.columns)}"

    # Assert NPV summary has exactly one row (one analysis)
    assert (
        len(npv_summary_df) == 1
    ), f"Expected 1 row in NPV summary, got {len(npv_summary_df)}"

    # Assert field name is correct
    assert (
        npv_summary_df["Field_Name"].iloc[0] == "goa_jack_stmalo"
    ), f"Field name mismatch: {npv_summary_df['Field_Name'].iloc[0]}"
    ...
    assert isinstance(
        npv_value, (int, float)
    ), f"NPV value should be numeric, got {type(npv_value)}"
    ...
    assert (
        abs(total_capex - 1460000000) < 100000000
    ), f"Total CAPEX should be ~$1.46B for Excel alignment, got {total_capex}"  # ±$100M tolerance
    ...
    assert (
        list(monthly_cashflows_df.columns) == expected_cashflow_columns
    ), f"Cash flows columns mismatch. Got: {list(monthly_cashflows_df.columns)}"
    ...
    assert (
        first_row["Cash_Flow_USD"] < 0
    ), f"Initial CAPEX should be negative, got {first_row['Cash_Flow_USD']}"
    assert (
        first_row["Description"] == "Initial CAPEX (Excel-aligned)"
    ), f"First row description should be 'Initial CAPEX (Excel-aligned)', got {first_row['Description']}"
    ...
    for _, row in monthly_rows.iterrows():
        assert (
            row["Description"] == "Monthly Net Cash Flow"
        ), f"Monthly row description should be 'Monthly Net Cash Flow', got {row['Description']}"
    ...
    assert (
        list(revenues_df.columns) == expected_revenue_columns
    ), f"Revenues columns mismatch. Got: {list(revenues_df.columns)}"

    # Assert revenues table has data rows
    assert (
        len(revenues_df) > 1
    ), "Revenues table should have at least one data row plus total row"
