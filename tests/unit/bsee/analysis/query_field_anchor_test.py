...
def run_application(input_file: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.error("Failed to execute engine with %s: %s", input_file, str(e))
        raise


def get_valid_pytest_output_file(pytest_output_file: Optional[str]) -> Optional[str]:
...
def test_config_file_loading() -> None:
    ...
    """
    config_files = ["query_field_anchor.yml", "query_field_anchor_simple.yml"]
