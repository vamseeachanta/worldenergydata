    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        ...
        writer.writerow(
            [
                ...
                "35.123",
                "-140.456",
                "Collision with cargo vessel in fog",
                "0",
                "3",
                "0",
                "Under investigation",
        ...
        writer.writerow(
            [
                ...
                "-12.456",
                "45.789",
                "Engine room explosion during bunker ops",
                "2",
                "5",
                "1",
                "Final report",
...
def test_imo_read_source(imo_gisis_csv, mock_session):
    ...
    importer = IMOGISISImporter(
        source_path=imo_gisis_csv, session=mock_session, validate_imo=False
    )
...
def test_imo_ship_type_mapping(imo_gisis_csv, mock_session):
    ...
    assert passenger_result in (
        "passenger",
        "passenger_ship",
        "other",
    )  # May map to other if not in mappings
...
def test_imo_parse_with_missing_imo_number(imo_gisis_csv, mock_session):
    ...
    importer = IMOGISISImporter(
        source_path=imo_gisis_csv, session=mock_session, validate_imo=True
    )
