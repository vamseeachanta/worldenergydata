...
def tsb_occurrence_csv(tmp_path):
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow([
            "OccNo", "OccDate", "OccTypeDisplayEng", "AccIncTypeDisplayEng",
            "TotalDeaths", "TotalMinorInjuries", "TotalSeriousInjuries", "TotalMissingIndividuals",
            "Summary", "ProvinceDisplayEng", "Latitude", "Longitude"
        ])
        writer.writerow([
            "TSB001", "2023-07-15 00:00:00.0000000", "Accident", "Grounding",
            "0", "1", "2", "0",
            "Vessel ran aground in shallow waters", "BRITISH COLUMBIA (BC)", "49.123", "-123.456"
        ])
        writer.writerow([
            "TSB002", "2023-08-20 14:30:00.0000000", "Incident", "Fire",
            "1", "3", "1", "0",
            "Fire in galley spread to accommodations", "NOVA SCOTIA (NS)", "44.567", "-63.890"
        ])
...
def tsb_vessels_csv(tmp_path):
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow([
            "OccNo", "VesselName", "VesselTypeDisplayEng", "IMO", "OfficialNo",
            "MMSI", "CallSign", "VesselFlagDisplayEng", "Length_Meters",
            "GrossTonnage", "YearBuilt", "NumberOfCrew", "NumberOfPass", "TotalPeopleOnBoard"
        ])
        writer.writerow([
            "TSB001", "Pacific Trader", "Cargo Ship", "IMO1234567", "CA123456",
            "316123456", "CFAB", "CANADA", "120.5", "8500", "2012", "18", "0", "18"
        ])
        writer.writerow([
            "TSB002", "Atlantic Star", "Fishing Vessel", "", "NS789012",
            "316789012", "VCDE", "CANADA", "25.3", "150", "2005", "6", "0", "6"
        ])
...
def test_tsb_vessels_loading(tsb_occurrence_csv, tsb_vessels_csv, mock_session):
    ...
    assert importer.vessels_by_occno["TSB002"][0]["VesselTypeDisplayEng"] == "Fishing Vessel"
...
def test_tsb_parse_grounding_record(tsb_occurrence_csv, tsb_vessels_csv, mock_session):
    ...
    assert parsed["incident_type"] in ("grounding", "collision", "other")  # Implementation may vary
