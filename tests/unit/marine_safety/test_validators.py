...
class TestCoordinateValidator:
    ...
    def test_valid_latitude_range(self):
        ...
        for lat in valid_latitudes:
            result = validator.validate_latitude(lat)
    ...
    def test_valid_longitude_range(self):
        ...
        for lon in valid_longitudes:
            result = validator.validate_longitude(lon)
            assert result.is_valid
            assert result.errors == []
    ...
    def test_coordinate_pair_validation(self):
        """Test validation of latitude/longitude pairs."""
        validator = CoordinateValidator()

        # Valid pair
        result = validator.validate_coordinate_pair(
            Decimal("29.123456"), Decimal("-94.123456")
        )
        assert result.is_valid

        # Invalid latitude
        result = validator.validate_coordinate_pair(
            Decimal("91.0"), Decimal("-94.123456")
        )
        assert not result.is_valid

        # Invalid longitude
        result = validator.validate_coordinate_pair(
            Decimal("29.123456"), Decimal("181.0")
        )
        assert not result.is_valid

    @pytest.mark.unit
    def test_coordinate_precision(self):
        """Test that coordinate precision is validated."""
        validator = CoordinateValidator(max_decimal_places=6)

        # Valid precision
        result = validator.validate_latitude(Decimal("29.123456"))
        assert result.is_valid

        # Excessive precision
        result = validator.validate_latitude(Decimal("29.12345678"))
        assert not result.is_valid

    @pytest.mark.unit
    def test_gulf_of_mexico_bounds(self):
        """Test validation of coordinates within Gulf of Mexico."""
        validator = CoordinateValidator()

        # Gulf of Mexico coordinates
        gulf_coords = [
            (Decimal("29.0"), Decimal("-94.0")),
            (Decimal("27.5"), Decimal("-90.0")),
            (Decimal("25.0"), Decimal("-85.0")),
        ]

        for lat, lon in gulf_coords:
            result = validator.validate_coordinate_pair(lat, lon)
            assert result.is_valid
