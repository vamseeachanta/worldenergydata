...
class TestLocationClassification:
    ...
    def test_get_location_statistics(self, hatch_maloperation_incidents):
        ...
        hatch_incidents = [
            inc
            for inc in hatch_maloperation_incidents
...
class TestRiskScoring:
    ...
    def test_catastrophic_incident_has_high_risk_score(
        self, hatch_maloperation_incidents
    ):
...
class TestRecommendationGeneration:
    ...
    def test_generate_recommendations_for_human_error(
        self, hatch_maloperation_incidents
    ):
    ...
    def test_generate_recommendations_for_maintenance_issues(
        self, hatch_maloperation_incidents
    ):
        ...
        assert len(recommendations) > 0
        assert any(
            "maintenance" in rec.lower() or "inspection" in rec.lower()
            for rec in recommendations
        )

    def test_generate_recommendations_for_equipment_failure(
        self, hatch_maloperation_incidents
    ):
        ...
        assert len(recommendations) > 0
        assert any(
            "replacement" in rec.lower() or "upgrade" in rec.lower()
            for rec in recommendations
        )

    def test_aggregate_recommendations_from_multiple_incidents(
        self, hatch_maloperation_incidents
    ):
