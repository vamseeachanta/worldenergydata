...
class TestDatasetSizeRequirements:
    ...
    def test_dataset_has_at_least_100_records(self):
        ...
        assert len(records) >= 100, (
            f"AC1 requires >= 100 records, got {len(records)}"
        )
    ...
    def test_dataset_includes_all_water_depth_bands(self):
        ...
        assert bands == set(WaterDepthBand), (
    ...
    def test_dataset_includes_hpht_records(self):
        ...
        assert hpht_count >= 3, (
            f"Expected >= 3 HPHT records, got {hpht_count}"
        )
    ...
    def test_high_confidence_records_present(self):
        ...
        high_or_medium = sum(
            1 for r in records
            if r.confidence in (Confidence.HIGH, Confidence.MEDIUM)
        )
        assert high_or_medium >= len(records) * 0.5, (
