...
class TestEngine:
    ...
    @pytest.mark.skip(
        reason="Complex mocking required - app_manager instantiated at import time"
    )
    ...
    @pytest.mark.skip(
        reason="CustomRouter not implemented in engine module - bsee_custom basename not supported"
    )
    ...
    def test_engine_with_invalid_basename(self):
        ...
        assert "Analysis for basename: invalid_basename not found" in str(
            exc_info.value
        )
    ...
    @pytest.mark.skip(
        reason="Complex mocking required - wwyaml instantiated at import time"
    )
    ...
    @pytest.mark.skip(
        reason="Complex mocking required - logger instantiated at import time"
    )
