...
def test_normalizes_colorado_ecmc_production_schema_to_screen_contract():
    frame = pd.DataFrame(
        [
            {
                "well_key": "051233249800",
                "field": "WATTENBERG",
                "test_year": 2025,
                "pressure_kind": "WHP_flowing_tubing",
                "pressure_psia": 99.7,
                "reference_depth_ft": 7994.0,
                "is_earliest_observation": True,
            },
            {
                "well_key": "051233249800",
    ...
    assert list(result["well_key"]) == ["051233249800", "051233249800"]
    assert list(result["field"]) == ["WATTENBERG", "WATTENBERG"]
    assert list(result["state"]) == ["CO", "CO"]
    assert list(result["source_name"]) == [
...
def test_load_observations_accepts_empty_warned_source_without_futurewarning(
    tmp_path, recwarn
):
    kansas_path = tmp_path / "kansas.parquet"
    colorado_path = tmp_path / "colorado.parquet"
    colorado_quality_path = tmp_path / "colorado_quality.json"
    pd.DataFrame(
        [
            {
                "well_key": "ks-1",
    ...
    ).to_parquet(colorado_path)
    colorado_quality_path.write_text(
        json.dumps({"source_warnings": ["no_positive_pressure_values"]}),
        encoding="utf-8",
    )
    ...
    observations, summary = load_observations(
        [
            ...
            {
                "name": "colorado_ecmc_production",
                "path": str(colorado_path),
                "quality_path": str(colorado_quality_path),
                "schema": "colorado_ecmc_production_v1",
                "state": "CO",
                "era": "production_report_late_life",
            },
    ...
    assert len(observations) == 1
    assert summary["loaded_row_counts"]["colorado_ecmc_production"] == 0
    assert summary["source_warnings"]["colorado_ecmc_production"] == [
        "no_positive_pressure_values"
    ]
    assert not [warning for warning in recwarn if warning.category is FutureWarning]
