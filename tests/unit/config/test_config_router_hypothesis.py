...
def source_config_strategy(draw):
    ...
    return {
        ...
        'update_frequency': draw(st.sampled_from(['daily', 'weekly', 'monthly', 'bi_monthly']))
    }
...
class TestConfigRouterProperties:
    ...
    @given(yaml_content=st.text(alphabet=st.characters(blacklist_categories=('Cc', 'Cs'), blacklist_characters='\x00')))
    ...
    @given(st.lists(st.sampled_from(['well', 'war', 'production', 'apm']), min_size=0, max_size=4))
