...
class TestRegionalMetrics:
    ...
    def test_to_dict_keys(self):
        m = _make_metrics()
        expected_keys = {
            "region", "total_fields", "total_oil_reserves_mmbbl",
            "total_gas_reserves_bcf", "average_recovery_factor",
            "average_water_depth_m", "average_field_size_mmboe",
            "drilling_efficiency", "discovery_rate",
            "production_efficiency", "economic_indicators",
        }
...
class TestNormalizeData:
    ...
    def test_percentile_normalization(self):
        analyzer = CrossRegionalAnalyzer()
        data = {"fields": _make_fields_df()}
        result = analyzer.normalize_data(data, "SODIR", NormalizationStrategy.PERCENTILE)
        assert "oil_reserves_mmbbl_normalized" in result["fields"].columns

    def test_log_transform_normalization(self):
        analyzer = CrossRegionalAnalyzer()
        data = {"fields": _make_fields_df()}
        result = analyzer.normalize_data(data, "SODIR", NormalizationStrategy.LOG_TRANSFORM)
        vals = result["fields"]["oil_reserves_mmbbl_normalized"]
        assert all(vals >= 0)
