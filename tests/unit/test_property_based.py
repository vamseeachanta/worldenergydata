...
def well_data_strategy(draw):
    ...
    return {
        "api_number": draw(
            st.text(min_size=10, max_size=14, alphabet=st.characters(categories=["Nd"]))
        ),
        ...
        "total_depth_ft": draw(
            st.floats(min_value=100, max_value=35000, allow_nan=False)
        ),
...
def config_dict_strategy(draw):
    ...
    basename = draw(
        st.text(min_size=1, max_size=50, alphabet=st.characters(categories=["L", "Nd"]))
    )
    ...
    if config["method"] == "bsee":
        config["bsee"] = {
            ...
            "api_numbers": draw(
                st.lists(st.text(min_size=10, max_size=14), min_size=1, max_size=5)
            ),
        }
...
class TestNPVPropertyBased:
    ...
    @given(cash_flows=cash_flows_strategy(), discount_rate=discount_rate_strategy())
    @settings(max_examples=100, deadline=None)
    def test_npv_calculation_properties(
        ...
        assert (
            abs(npv) <= total_abs + 1
        ), "NPV should be bounded by sum of absolute values"
    ...
    def test_npv_monotonicity(self, cash_flows: List[float], rates: List[float]):
        ...
        for i in range(len(npvs) - 1):
            assert (
                npvs[i] >= npvs[i + 1] - 1e-6
            ), "NPV should decrease with increasing rate"
    ...
    def test_npv_scaling_property(self, cash_flows: List[float]):
        ...
        assert np.isclose(
            npv2, npv1 * scale_factor, rtol=1e-10
        ), "NPV should scale linearly"
...
class TestProductionDataPropertyBased:
    ...
    def test_production_data_invariants(self, production_data: Dict):
        ...
        assert (
            len(oil) == production_data["months"]
        ), "Oil array length should match months"
        assert (
            len(gas) == production_data["months"]
        ), "Gas array length should match months"
        assert (
            len(water) == production_data["months"]
        ), "Water array length should match months"
        ...
        production_df=data_frames(
            columns=[
                column(
                    "oil_bbls",
                    dtype=float,
                    elements=st.floats(0, 50000, allow_nan=False),
                ),
                column(
                    "gas_mcf",
                    dtype=float,
                    elements=st.floats(0, 1000000, allow_nan=False),
                ),
                column(
                    "water_bbls",
                    dtype=float,
                    elements=st.floats(0, 100000, allow_nan=False),
                ),
            ],
    ...
    def test_production_dataframe_processing(self, production_df: pd.DataFrame):
        ...
        assert (
            production_df.shape[0] == original_shape[0]
        ), "Row count should be preserved"
        assert (
            production_df.shape[1] == original_shape[1] + 2
        ), "Should have 2 new columns"

        # Property 2: BOE calculation should be correct
        expected_boe = production_df["oil_bbls"] + production_df["gas_mcf"] / 6
        assert np.allclose(
            production_df["boe"], expected_boe
        ), "BOE calculation should be correct"

        # Property 3: Water cut should be between 0 and 1
        assert np.all(production_df["water_cut"] >= 0), "Water cut should be >= 0"
        assert np.all(
            production_df["water_cut"] <= 1 + 1e-10
        ), "Water cut should be <= 1"
...
class TestWellDataPropertyBased:
    ...
    @given(
        depths=st.lists(
            st.floats(
                min_value=0, max_value=35000, allow_nan=False, allow_infinity=False
            ),
            min_size=2,
            max_size=50,
        )
    ...
    def test_depth_statistics(self, depths: List[float]):
        ...
        assert (
            min_depth - 1e-10 <= mean_depth <= max_depth + 1e-10
        ...
        assert (
            min_depth - 1e-10 <= median_depth <= max_depth + 1e-10
...
class TestEnginePropertyBased:
    ...
    def test_config_validation(self, config: Dict):
        ...
        if "method" in config:
            assert (
                config["method"] in valid_methods
            ), f"Method should be one of {valid_methods}"
    ...
    @given(
        basename=st.text(
            min_size=1, max_size=50, alphabet=st.characters(categories=["L", "Nd"])
        ),
    ...
    def test_config_creation_invariants(self, basename: str, method: str):
        ...
        config = {
            "basename": basename,
            "method": method,
            "meta": {"basename": basename, "version": "1.0.0"},
        }

        # Property 1: Basename should be consistent
        assert (
            config["basename"] == config["meta"]["basename"]
...
class TestDataTransformationsPropertyBased:
    ...
    def test_cumulative_sum_properties(self, values: np.ndarray):
        ...
        assert np.isclose(
            cumsum[-1], np.sum(values), rtol=1e-6, atol=1e-6
        ...
        if len(values) > 1:
            ...
            assert np.allclose(
                recovered, values, rtol=1e-6, atol=1e-6
    ...
    @given(
        data=arrays(
            ...
            elements=st.floats(
                min_value=0, max_value=1e4, allow_nan=False, allow_infinity=False
            ),
        )
    )
    @settings(max_examples=30, deadline=None)
    def test_aggregation_properties(self, data: np.ndarray):
        ...
        assert np.isclose(
            total_from_rows, total_direct, rtol=1e-6, atol=1e-6
        ), "Sum of row sums should equal total"
        assert np.isclose(
            total_from_cols, total_direct, rtol=1e-6, atol=1e-6
        ...
        assert np.isclose(
            mean_of_means, overall_mean, rtol=1e-6, atol=1e-6
