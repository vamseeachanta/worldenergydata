...
class TestFieldAggregationConfig(unittest.TestCase):
    ...
    def test_config_initialization(self):
        ...
        config = FieldAggregationConfig(
            field_name="Test Field", aggregation_level="field"
        )
        ...
        self.assertEqual(
            config.comparison_metrics, ["production", "economics", "efficiency"]
        )
...
class TestFieldAggregationDashboard(unittest.TestCase):
    ...
    @patch(
        "worldenergydata.well_production_dashboard.field_aggregation.VerificationSystem"
    )
...
class TestFieldEconomicSummary(unittest.TestCase):
    ...
    @patch(
        "worldenergydata.well_production_dashboard.field_aggregation.DataQualityFramework"
    )
...
class TestFieldProductionChart(unittest.TestCase):
    ...
    def test_add_verification_overlay(self):
        ...
        chart_with_overlay = self.chart_builder.add_verification_overlay(
            base_chart, self.production_data["verification_score"]
        )
            ...
            wells.append(
                {
                    ...
                    "production_data": pd.DataFrame({"oil_bbls": [1000] * 12}),
                }
            )
