...
class TestProductionAPI12Analysis:
    ...
    def test_calculate_cumulative_production(self, analyzer, sample_production_data):
        ...
        if hasattr(analyzer, 'calculate_cumulative_production'):
            ...
            sample_production_data['CUMULATIVE_OIL'] = sample_production_data['OIL_VOLUME'].cumsum()
    ...
    def test_analyze_production_rates(self, analyzer, sample_production_data):
        ...
        sample_production_data['OIL_RATE_BOPD'] = sample_production_data['OIL_VOLUME'] / 30  # Monthly to daily
        sample_production_data['GAS_RATE_MCFD'] = sample_production_data['GAS_VOLUME'] / 30
    ...
    def test_handle_multiple_api12_wells(self, analyzer):
        ...
        well_stats = df.groupby('API_WELL_NUMBER')['OIL_VOLUME'].agg(['sum', 'mean', 'max'])
    ...
    def test_monthly_production_aggregation(self, analyzer, sample_production_data):
        ...
        # Convert to datetime
        sample_production_data['PRODUCTION_DATETIME'] = pd.to_datetime(sample_production_data['PRODUCTION_DATETIME'])
        
        # Extract month
        sample_production_data['MONTH'] = sample_production_data['PRODUCTION_DATETIME'].dt.month
    ...
    def test_production_decline_analysis(self, analyzer):
        ...
        production = [initial_rate * np.exp(-decline_rate * t) for t in range(time_points)]
    ...
    def test_gas_oil_ratio_calculation(self, analyzer, sample_production_data):
        ...
        # Calculate GOR
        sample_production_data['GOR'] = sample_production_data['GAS_VOLUME'] / sample_production_data['OIL_VOLUME']
    ...
    def test_water_cut_calculation(self, analyzer, sample_production_data):
        ...
        # Calculate water cut
        total_liquid = sample_production_data['OIL_VOLUME'] + sample_production_data['WATER_VOLUME']
        sample_production_data['WATER_CUT'] = sample_production_data['WATER_VOLUME'] / total_liquid * 100
    ...
    def test_production_forecasting(self, analyzer):
        ...
        future_X = np.arange(len(historical_data), len(historical_data) + 6).reshape(-1, 1)
        forecast = model.predict(future_X)
        
        assert len(forecast) == 6
        assert forecast[0] < historical_data['production'].iloc[0]  # Should show decline