...
def requires_network(func: Callable) -> Callable:
    ...
    return pytest.mark.skipif(
        not has_network(), reason="Network access required but not available"
    )(network(func))
...
def requires_data_file(filename: str) -> Callable:
    ...
    def decorator(func: Callable) -> Callable:
        ...
        return pytest.mark.skipif(
            not data_path.exists(), reason=f"Required data file {filename} not found"
        )(func)
...
def min_python_version(version: str) -> Callable:
    ...
    def decorator(func: Callable) -> Callable:
        ...
        return pytest.mark.skipif(
            current.major < major or (current.major == major and current.minor < minor),
