        ...
        try:
            ...
            if import_match:
                ...
                if module_file.exists():
                    ...
                    requested_imports = [item.strip() for item in imported_items_str.split(',')]
                    ...
                    if invalid_imports:
                        logger.info(f"  Removing invalid imports from {test_file.name}: {', '.join(invalid_imports)}")
                        ...
                        if valid_imports:
                            ...
                            for invalid_import in invalid_imports:
                                ...
                                test_pattern = rf'def (test_.*?{invalid_import}.*?)\(self.*?\):'
                                for match in re.finditer(test_pattern, content, re.IGNORECASE):
                                    ...
                                    content = content[:method_start] + commented_method + content[method_end:]
                        else:
                            # No valid imports, comment out the entire import
                            old_import = f"from {module_path_str} import {imported_items_str}"
                            new_import = f"# {old_import}  # Module has no exportable items"
                            content = content.replace(old_import, new_import)
                else:
                    # Module doesn't exist, comment out import
                    logger.info(f"  Module not found for {test_file.name}: {module_file}")
                    old_import = f"from {module_path_str} import {imported_items_str}"
