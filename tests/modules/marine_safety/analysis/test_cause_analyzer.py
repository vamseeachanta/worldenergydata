...
class TestIncidentCauseExtractor:
    ...
    def test_extract_handles_missing_data(self):
        ...
        assert (
            result.loc[
                result["incident_id"] == "INC002", "primary_cause_category"
            ].iloc[0]
...
class TestExtractCausesFromNarrative:
    ...
    def test_extract_causes_multiple(self):
        ...
        assert (
            CauseCategory.EQUIPMENT_FAILURE in categories
            or CauseCategory.MAINTENANCE_ISSUE in categories
        )
...
class TestIntegration:
    ...
    def test_end_to_end_cause_extraction(self, sample_raw_data):
        ...
        # Categorize
        extracted["cause_category"] = extracted["normalized_cause"].apply(
            map_to_cause_category
        )
        ...
        assert all(
            ...
            for cat in extracted["cause_category"]
            if pd.notna(cat)
        )
    ...
    def test_hatch_incident_detection_workflow(self):
        ...
        # Detect hatch issues
        data["has_hatch_issue"] = data["narrative"].apply(
            detect_hatch_opening_maloperation
        )
