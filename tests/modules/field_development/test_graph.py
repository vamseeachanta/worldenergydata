...
def test_subsea_tieback_builds_host_manifold_trees():
    c = FieldConcept(name="Penguins", concept_type=ConceptType.SUBSEA_TIEBACK,
                     tieback_distance_km=65.0, num_wells=4, num_manifolds=1)
...
def test_explicit_tree_type_overrides_concept_default():
    ...
    c = FieldConcept(name="X", concept_type=ConceptType.SEMISUB_FPS,
                     tree_type=TreeType.DRY, num_wells=2)
    g = concept_to_graph(c)
    assert _nodes_of(g, NodeType.MANIFOLD) == []
...
def test_subsea_to_shore_uses_terminal_and_no_export_sink():
    g = concept_to_graph(FieldConcept(name="Snohvit",
                                      concept_type=ConceptType.SUBSEA_TO_SHORE,
                                      num_wells=2))
...
def test_every_node_has_a_symbol():
    g = concept_to_graph(FieldConcept(name="X", concept_type=ConceptType.SPAR,
                                      num_wells=2))
