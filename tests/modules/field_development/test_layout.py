...
def test_host_to_manifold_pixel_distance_equals_tieback_times_scale():
    scale = 50.0
    c = FieldConcept(name="Mensa", concept_type=ConceptType.SUBSEA_TIEBACK,
                     tieback_distance_km=12.0, num_wells=3, num_manifolds=1)
...
def test_render_returns_svg_with_north_arrow_and_scale_bar():
    svg = render_layout(FieldConcept(name="X", concept_type=ConceptType.FPSO,
                                     num_wells=2))
...
def test_render_includes_field_name_escaped():
    svg = render_layout(FieldConcept(name="A & B <field>",
                                     concept_type=ConceptType.FPSO, num_wells=1))
...
def test_deterministic_same_concept_same_svg():
    c = FieldConcept(name="X", concept_type=ConceptType.SEMISUB_FPS,
                     num_wells=5, num_manifolds=2, tieback_distance_km=0.0)
    assert render_layout(c) == render_layout(c)


def test_default_scale_constant_used_when_unspecified():
    c = FieldConcept(name="X", concept_type=ConceptType.SUBSEA_TIEBACK,
                     tieback_distance_km=10.0, num_wells=1, num_manifolds=1)
