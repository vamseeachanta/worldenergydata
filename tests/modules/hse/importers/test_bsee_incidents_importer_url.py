...
class TestBSEEIncidentsImporterURL:
    ...
    def test_importer_initialization(self, db_session):
        ...
        assert importer.BSEE_WELL_DATA_URL == 'https://www.data.bsee.gov/Well/Files/APDRawData.zip'
    ...
    def test_fetch_data_downloads_from_url(self, db_session, mock_zip_data, mock_processed_data):
        ...
        with patch.object(importer.scraper, 'download_zip_to_memory', return_value=mock_zip_data) as mock_download, \
             patch.object(importer.processor, 'process_well_data', return_value=mock_processed_data) as mock_process:
    ...
    def test_fetch_data_processes_dict_with_data_key(self, db_session, mock_zip_data):
        ...
        with patch.object(importer.scraper, 'download_zip_to_memory', return_value=mock_zip_data), \
             patch.object(importer.processor, 'process_well_data', return_value=mock_output):
    ...
    def test_fetch_data_processes_direct_dataframe(self, db_session, mock_zip_data):
        ...
        mock_output = {
            'file1.csv': pd.DataFrame([{'id': '2', 'value': 'direct'}])
        }
    ...
    def test_normalize_data_inherited_from_parent(self, db_session, mock_zip_data, mock_processed_data):
...
class TestBSEEIncidentsImporterURLIntegration:
    ...
    def test_no_duplicate_import(self, db_session):
        ...
        db_session.query.return_value.filter.return_value.first.return_value = existing_incident

        mock_data = {
            'file1.csv': pd.DataFrame([
                {'incident_id': 'INC-001', 'incident_date': '2024-01-15',
                 'operator_name': 'Shell', 'incident_type': 'injury',
                 'severity': 'recordable', 'facility': 'Platform A',
                 'lat': '28.5', 'lon': '-89.2'}
            ])
        }

        with patch.object(importer.scraper, 'download_zip_to_memory', return_value=b'mock'), \
             patch.object(importer.processor, 'process_well_data', return_value=mock_data):
