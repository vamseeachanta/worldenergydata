...
class TestBSEEStatisticsImporterURL:
    ...
    def test_importer_initialization(self, db_session):
        ...
        assert importer.BSEE_PRODUCTION_DATA_URL == 'https://www.data.bsee.gov/Production/Files/ProductionRawData.zip'
    ...
    def test_fetch_data_downloads_from_url(self, db_session, mock_zip_data, mock_processed_data):
        ...
        with patch.object(importer.scraper, 'download_zip_to_memory', return_value=mock_zip_data) as mock_download, \
             patch.object(importer.processor, 'process_production_data', return_value=mock_processed_data) as mock_process:
    ...
    def test_fetch_data_uses_longer_timeout(self, db_session, mock_zip_data, mock_processed_data):
        """Test fetch_data uses production data type for longer timeout (1200s)."""
        importer = BSEEStatisticsImporterURL(db_session)

        with patch.object(importer.scraper, 'download_zip_to_memory', return_value=mock_zip_data) as mock_download, \
             patch.object(importer.processor, 'process_production_data', return_value=mock_processed_data):
    ...
    def test_normalize_data_converts_string_integers(self, db_session, mock_zip_data, mock_processed_data):
    ...
    def test_normalize_data_sets_incident_type_equipment_failure(self, db_session, mock_zip_data, mock_processed_data):
    ...
    def test_normalize_data_maps_report_date_to_incident_date(self, db_session, mock_zip_data, mock_processed_data):
    ...
    def test_importer_handles_multiple_csv_files(self, db_session, mock_zip_data):
        ...
        mock_data = {
            'production_2024_01.csv': pd.DataFrame([{'incident_id': 'STAT-001', 'report_date': '2024-01-15', 'operator_name': 'Shell', 'severity': 'recordable', 'operational_period': '30', 'total_incidents': '5', 'fatality_count': '0', 'lost_time_count': '1', 'recordable_count': '2', 'near_miss_count': '1', 'minor_count': '1'}]),
            'production_2024_02.csv': pd.DataFrame([{'incident_id': 'STAT-002', 'report_date': '2024-02-15', 'operator_name': 'BP', 'severity': 'minor', 'operational_period': '28', 'total_incidents': '3', 'fatality_count': '0', 'lost_time_count': '0', 'recordable_count': '1', 'near_miss_count': '1', 'minor_count': '1'}])
        }

        with patch.object(importer.scraper, 'download_zip_to_memory', return_value=mock_zip_data), \
             patch.object(importer.processor, 'process_production_data', return_value=mock_data):
...
class TestBSEEStatisticsImporterURLPerformance:
    ...
    def test_large_file_handling(self, db_session):
        ...
        with patch.object(importer.scraper, 'download_zip_to_memory', return_value=b'mock'), \
             patch.object(importer.processor, 'process_production_data', return_value=large_data):
