...
class BuildLeaseReportFinal:
    ...
    def build_report(self, cfg):
        ...
        api12_filtered_borehole_df = borehole_df[
            borehole_df["API_WELL_NUMBER"].isin(api_list)
        ].copy()
        ...
        sn_war_mud_weight_df = sn_war_mud_weight_df[
            ["SN_WAR", "DRILL_FLUID_WGT"]
        ].rename(columns={"DRILL_FLUID_WGT": "Max Mud Weight (ppg)"})

        # Merge and select max mud weight per API
        merged_df = lease_war_df[["SN_WAR", "API_WELL_NUMBER"]].drop_duplicates()
        merged_df = merged_df.merge(
            api12_filtered_borehole_df, on="API_WELL_NUMBER", how="left"
        )
        merged_df = merged_df.merge(sn_war_mud_weight_df, on="SN_WAR", how="left")
        merged_df["Max Mud Weight (ppg)"] = pd.to_numeric(
            merged_df["Max Mud Weight (ppg)"], errors="coerce"
        )
        max_weight_indices = merged_df.groupby("API_WELL_NUMBER")[
            "Max Mud Weight (ppg)"
        ].idxmax()
        merged_df = merged_df.loc[max_weight_indices].reset_index(drop=True)

        # Convert dates to datetime
        merged_df["WELL_SPUD_DATE"] = pd.to_datetime(
            merged_df["WELL_SPUD_DATE"], errors="coerce"
        )
        merged_df["TOTAL_DEPTH_DATE"] = pd.to_datetime(
            merged_df["TOTAL_DEPTH_DATE"], errors="coerce"
        )
