...
class DrillingCompletionDays:
    ...
    def _load_lease_data(self):
        ...
        lease_path = self.cfg.get("filepath", {}).get(
        ...
        self.lease_df = pd.read_csv(
            lease_path, header=None, encoding="utf-8-sig", dtype=str
        )
        ...
        self.leases = (
            self.lease_df["LEASE_NUM"].str.replace("^G", "", regex=True).tolist()
        )
        logger.info(
            f"Loaded {len(self.leases)} lease numbers from leases.csv (stripped of 'G')"
        )
        ...
        self.lease_info = (
            ...
            .assign(
                LEASE_NUM=lambda df: df["LEASE_NUM"]
                .str.upper()
    ...
    def _load_war_data(self):
        ...
        self.main_war["API_WELL_NUMBER"] = (
            self.main_war["API_WELL_NUMBER"].astype(str).str.zfill(10)
        )
        self.main_war["SN_WAR"] = self.main_war["SN_WAR"].astype(str).str.strip()
        self.main_war["WELL_NAME"] = self.main_war["WELL_NAME"].fillna("")
        self.main_war["WAR_START_DT"] = pd.to_datetime(
            self.main_war["WAR_START_DT"], errors="coerce"
        )
        self.main_war["WAR_END_DT"] = pd.to_datetime(
            self.main_war["WAR_END_DT"], errors="coerce"
        )

        # Filter by lease
        self.main_war["SURF_LEASE_NUM"] = self.main_war["SURF_LEASE_NUM"].astype(str)
        self.main_war_filtered = self.main_war[
            self.main_war["SURF_LEASE_NUM"].isin(self.leases)
        ].copy()
        logger.info(
            f"Filtered WAR records: {len(self.main_war_filtered)} out of {len(self.main_war)}"
        )

        # Load boreholes data
        boreholes_path = war_files.get(
        ...
        self.boreholes["API_WELL_NUMBER"] = (
            self.boreholes["API_WELL_NUMBER"].astype(str).str.zfill(10)
        )
        self.boreholes["WELL_SPUD_DATE"] = pd.to_datetime(
            self.boreholes["WELL_SPUD_DATE"], errors="coerce"
        )
        self.boreholes["TOTAL_DEPTH_DATE"] = pd.to_datetime(
            self.boreholes["TOTAL_DEPTH_DATE"], errors="coerce"
        )
        self.boreholes["BH_TOTAL_MD"] = pd.to_numeric(
            self.boreholes["BH_TOTAL_MD"], errors="coerce"
        )
        self.boreholes["WELL_BORE_TVD"] = pd.to_numeric(
            self.boreholes["WELL_BORE_TVD"], errors="coerce"
        )

        # Load properties data
        prop_path = war_files.get(
        ...
        self.main_prop["DRILL_FLUID_WGT"] = pd.to_numeric(
            self.main_prop["DRILL_FLUID_WGT"], errors="coerce"
        )
    ...
    def _process_analysis(self):
        ...
        # Add depth info
        depth_summary = (
            self.boreholes.groupby("API_WELL_NUMBER")[["BH_TOTAL_MD", "WELL_BORE_TVD"]]
            .max()
            .reset_index()
        )
        ...
        main_merge = (
            self.main_war[["SN_WAR", "API_WELL_NUMBER"]].dropna().drop_duplicates()
        )
        main_merge["API_WELL_NUMBER"] = (
            main_merge["API_WELL_NUMBER"].astype(str).str.zfill(10)
        )
        ...
        def adjust_spud(api, td):
            war_dates = self.main_war_filtered[
                self.main_war_filtered["API_WELL_NUMBER"] == api
            ][["WAR_START_DT", "WAR_END_DT"]].dropna()
            ...
            if gap_idx:
                ...
                if last_gap_idx + 1 < len(war_dates):
                    ...
                    early_days = (
                        war_dates.loc[:last_gap_idx, "WAR_END_DT"]
                        - war_dates.loc[:last_gap_idx, "WAR_START_DT"]
                    ).dt.days.sum()
        ...
        for _, row in spud_td.iterrows():
            ...
            td = row["TOTAL_DEPTH_DATE"]
            completions = (
                self.main_war_filtered[
                    (self.main_war_filtered["API_WELL_NUMBER"] == api)
                    & (self.main_war_filtered["WAR_START_DT"] > td)
                ][["WAR_START_DT", "WAR_END_DT"]]
                .dropna()
                .sort_values(by="WAR_START_DT")
            )
            ...
            for i in range(1, len(completions)):
                if completions.loc[i, "GAP"] > COMPLETION_GAP_THRESHOLD:
                    ...
                    segment_days += (
                        segment["WAR_END_DT"] - segment["WAR_START_DT"]
                    ).dt.days.sum()
                    start_idx = i
            # Add final segment
        ...
        completion_summary = pd.DataFrame(
            completion_segments, columns=["API_WELL_NUMBER", "COMPLETION_DAYS"]
        )
        ...
        final["WELL_NAME"] = final["API_WELL_NUMBER"].map(
            self.main_war_filtered.dropna(subset=["WELL_NAME"])
        ...
        api_to_lease = (
            ...
            .assign(
                SURF_LEASE_NUM=lambda df: df["SURF_LEASE_NUM"]
                .str.upper()
                .str.replace("^G", "", regex=True)
                .str.strip()
            )
        ...
        for api in list(final["API_WELL_NUMBER"].unique())[:5]:
            ...
            logger.info(
                f"API: {api} → Lease#: {lease_num} → Name: {lease_meta.get('LEASE_NAME', '')}, Depth: {lease_meta.get('WATER_DEPTH', '')}"
            )

        final["LEASE_NAME"] = final["SURF_LEASE_NUM"].map(
            lambda x: self.lease_info.get(x, {}).get("LEASE_NAME", "")
        )
        final["WATER_DEPTH"] = final["SURF_LEASE_NUM"].map(
            lambda x: self.lease_info.get(x, {}).get("WATER_DEPTH", "")
        )
        ...
        final["TOTAL_DEPTH_DATE"] = pd.to_datetime(
            final["TOTAL_DEPTH_DATE"]
        ).dt.strftime("%m/%d/%Y")
        final = final[
            [
                "LEASE_NAME",
        ...
        final["SPUD_DATE_SORT"] = pd.to_datetime(
            final["WELL_SPUD_DATE"], errors="coerce"
        )
        final = final.sort_values(by=["LEASE_NAME", "SPUD_DATE_SORT"]).drop(
            columns=["SPUD_DATE_SORT"]
        )
        ...
        if os.path.exists(output_path):
            ...
            output_filename = (
                f"drilling_and_completion_days_by_api_validation_{timestamp}.xlsx"
            )
            output_path = os.path.join(result_path, output_filename)
