...
def test_npv_analysis_components():
    ...
    print("\nTest 3: Revenue calculation")
    total_oil_revenue = (
        sum(production_data["oil_production_bbl"])
        * economic_params["oil_price_per_bbl"]
    )
    total_gas_revenue = (
        sum(production_data["gas_production_mcf"])
        * economic_params["gas_price_per_mcf"]
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--debug", action="store_true", help="Enable interactive debugging"
    )
    parser.add_argument(
        "--test",
        choices=["all", "engine", "direct", "components"],
        default="all",
