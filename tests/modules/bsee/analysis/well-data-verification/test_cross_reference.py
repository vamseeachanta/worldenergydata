...
class TestFieldMapper:
    ...
    def test_map_dataframe_columns(self):
        ...
        config = MappingConfig({"well_id": "Well_ID", "oil_volume": "Oil_Prod"})
        mapper = FieldMapper(config)

        df = pd.DataFrame(
            {"Well_ID": ["W001", "W002"], "Oil_Prod": [100, 200], "Other_Col": [1, 2]}
        )
    ...
    def test_handle_missing_columns(self):
        ...
        df = pd.DataFrame({"Well_ID": ["W001"], "Oil_Prod": [100]})
...
class TestComparisonEngine:
    ...
    def test_aggregate_comparison(self):
        ...
        df1 = pd.DataFrame({"lease": ["L1", "L1", "L2"], "oil": [100, 150, 200]})

        df2 = pd.DataFrame({"lease": ["L1", "L2"], "oil": [250, 200]})

        result = engine.compare_aggregates(
            df1, df2, group_by="lease", agg_column="oil", agg_func="sum"
        )
...
class TestDiscrepancyReporter:
    ...
    def test_filter_discrepancies_by_severity(self):
        ...
        warnings_and_above = reporter.filter_by_severity(
            discrepancies, min_severity="warning"
        )
...
class TestCrossReferenceModule:
    ...
    def test_full_cross_reference_workflow(self, tmp_path):
        ...
        mapping_config = MappingConfig(
            {"well_id": "Well_ID", "oil_volume": "Oil_Prod", "gas_volume": "Gas_Prod"}
        )

        # Run cross-reference
        module = CrossReferenceModule(mapping_config)
        result = module.cross_reference(
            db_data, excel_path, key_column="well_id", numeric_tolerance=0.01
        )
    ...
    def test_cross_reference_with_yaml_config(self, tmp_path):
        ...
        pd.DataFrame({"Well_Identifier": ["W001"], "Oil_BBL": [1000]}).to_excel(
            excel_path, index=False
        )

        db_data = pd.DataFrame({"well_id": ["W001"], "oil_volume": [1050]})
    ...
    def test_batch_cross_reference(self, tmp_path):
        ...
        for i in range(3):
            ...
            pd.DataFrame({"Well_ID": [f"W00{i}"], "Oil": [1000 * (i + 1)]}).to_excel(
                excel_path, index=False
            )
            excel_files.append(excel_path)

        db_data = pd.DataFrame(
            {"well_id": ["W000", "W001", "W002"], "oil_volume": [1000, 2010, 3000]}
        )
    ...
    def test_generate_consolidated_report(self, tmp_path):
        ...
        results = [
            ComparisonResult(
                100, 90, [Discrepancy("W001", "oil", 100, 110, 10, 10.0, "error")]
            ),
            ComparisonResult(
                50, 48, [Discrepancy("W002", "gas", 1000, 1005, 5, 0.5, "info")]
            ),
        ]
