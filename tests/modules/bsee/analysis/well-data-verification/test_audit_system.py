...
class TestAuditDatabase:
    ...
    def test_transaction_management(self, audit_db):
        ...
        with audit_db.transaction() as tx:
            ...
            tx.execute(
                ...
                (str(uuid.uuid4()), "TEST_EVENT", "user1", datetime.now().isoformat(), json.dumps({"test": True}))
            )
            
        # Verify data was committed
        result = audit_db.query("SELECT * FROM audit_events WHERE event_type = ?", ("TEST_EVENT",))
        assert len(result) == 1
    ...
    def test_database_backup(self, audit_db, tmp_path):
        ...
        audit_db.execute(
            "INSERT INTO audit_events (event_id, event_type, user_id, timestamp, details) VALUES (?, ?, ?, ?, ?)",
            (str(uuid.uuid4()), "BACKUP_TEST", "user1", datetime.now().isoformat(), json.dumps({}))
        )
        ...
        cursor = conn.execute("SELECT * FROM audit_events WHERE event_type = 'BACKUP_TEST'")
