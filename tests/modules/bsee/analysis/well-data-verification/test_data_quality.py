...
class TestCompletenessChecker:
    ...
    def test_detect_missing_months(self):
        ...
        data = pd.DataFrame(
            {"date": dates, "oil_volume": [100, 110, 120, 130], "well_id": ["W1"] * 4}
        )
    ...
    def test_detect_null_values(self):
        ...
        result = checker.check_required_fields(
            data, ["well_id", "lease_num", "oil_volume"]
        )
...
class TestOutlierDetector:
    ...
    def test_zscore_outlier_detection(self):
        ...
        data = pd.DataFrame(
            {"oil_volume": all_data, "well_id": [f"W{i}" for i in range(len(all_data))]}
        )
    ...
    def test_multivariate_outlier_detection(self):
        ...
        result = detector.detect_multivariate_outliers(
            data, ["oil_volume", "gas_volume"]
        )
    ...
    def test_configurable_thresholds(self):
        ...
        config = {"zscore_threshold": 2, "iqr_multiplier": 1.0}
        ...
        data = pd.DataFrame(
            {
                "value": [
                    ...
                    18,
                    25,
                ]  # 25 might be outlier with strict threshold
            }
        )
...
class TestValidationRuleBuilder:
    ...
    def test_create_range_rule(self):
        ...
        data = pd.DataFrame({"oil_volume": [100, 5000, -10, 15000]})
    ...
    def test_create_pattern_rule(self):
        ...
        data = pd.DataFrame({"well_id": ["W001", "W002", "INVALID", "W99"]})
    ...
    def test_create_custom_rule(self):
        ...
        data = pd.DataFrame({"count": [2, 4, 5, 8, 9]})
    ...
    def test_combine_rules(self):
        ...
        rules = [
            ...
            builder.custom_rule(
                "value", lambda x: x % 10 == 0, "Must be multiple of 10"
            ),
        ]

        combined_rule = builder.combine_rules(rules)

        data = pd.DataFrame({"value": [10, 20, 150, None, 35]})
...
class TestDataQualityFramework:
    ...
    def test_configurable_quality_framework(self):
        ...
        config = QualityConfig(
            ...
            production_ranges={"oil_volume": (0, 5000), "gas_volume": (0, 50000)},
        )
        ...
        assert (
            not result.is_valid or result.has_warnings
        )  # Modified: accept warnings too
    ...
    def test_export_quality_results(self, tmp_path):
        ...
        data = pd.DataFrame(
            {"well_id": ["W1", "W2", "W3"], "oil_volume": [100, 200, 300]}
        )
