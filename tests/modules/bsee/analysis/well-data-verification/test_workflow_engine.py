...
class TestWorkflowState:
    ...
    def test_state_transitions(self):
        ...
        # Valid transitions
        assert WorkflowState.can_transition(WorkflowState.NOT_STARTED, WorkflowState.IN_PROGRESS)
        assert WorkflowState.can_transition(WorkflowState.IN_PROGRESS, WorkflowState.PAUSED)
        assert WorkflowState.can_transition(WorkflowState.PAUSED, WorkflowState.IN_PROGRESS)
        assert WorkflowState.can_transition(WorkflowState.IN_PROGRESS, WorkflowState.COMPLETED)
        assert WorkflowState.can_transition(WorkflowState.IN_PROGRESS, WorkflowState.FAILED)
        
        # Invalid transitions
        assert not WorkflowState.can_transition(WorkflowState.COMPLETED, WorkflowState.IN_PROGRESS)
        assert not WorkflowState.can_transition(WorkflowState.FAILED, WorkflowState.COMPLETED)
...
class TestWorkflowSession:
    ...
    def test_session_creation(self):
        ...
        session = WorkflowSession(
            session_id="TEST-001",
            user="test_user",
            workflow_type="well_verification"
        )
...
class TestWorkflowValidator:
    ...
    def test_validate_input_data(self):
        ...
        invalid_data = {
            "date_range": {"start": "2024-01-01"}  # Missing required field
        }
...
def sample_workflow_data():
    ...
    return {
        ...
        "well_data": pd.DataFrame({
            ...
            "PRODUCTION_DATE": pd.to_datetime(["2024-01-01", "2024-01-01", "2024-01-01"]),
