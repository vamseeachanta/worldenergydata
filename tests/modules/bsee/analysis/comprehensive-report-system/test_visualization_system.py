...
from worldenergydata.modules.bsee.reports.comprehensive.visualizations.economic_charts import (
    EconomicChart,
)
from worldenergydata.modules.bsee.reports.comprehensive.visualizations.geographic_charts import (
    GeographicChart,
)
from worldenergydata.modules.bsee.reports.comprehensive.visualizations.production_charts import (
    ProductionChart,
)
from worldenergydata.modules.bsee.reports.comprehensive.visualizations.well_performance_charts import (
...
class TestProductionCharts(unittest.TestCase):
    ...
    @patch(
        "worldenergydata.modules.bsee.reports.comprehensive.visualizations.production_charts.go"
    )
    ...
    def test_cumulative_production_chart(self, mock_go):
        ...
        chart = self.chart_builder.create_cumulative_production(
            self.sample_data, product_type="oil"
        )
...
class TestWellPerformanceCharts(unittest.TestCase):
    ...
    @patch(
        "worldenergydata.modules.bsee.reports.comprehensive.visualizations.well_performance_charts.go"
    )
    ...
    def test_heat_map_creation(self, mock_go):
        ...
        matrix_data = self.well_data.pivot_table(
            values="production_rate", index="well_id", columns="efficiency"
        )

        chart = self.chart_builder.create_heat_map(
            matrix_data, title="Well Performance Heat Map"
        )
...
class TestEconomicCharts(unittest.TestCase):
    ...
    @patch(
        "worldenergydata.modules.bsee.reports.comprehensive.visualizations.economic_charts.go"
    )
...
class TestGeographicCharts(unittest.TestCase):
    ...
    def test_well_location_map(self, mock_go):
        ...
        chart = self.chart_builder.create_well_map(
            self.location_data, color_by="status", size_by="production"
        )
    ...
    def test_field_boundary_map(self, mock_go):
        ...
        chart = self.chart_builder.create_field_boundary_map(
            self.location_data, field_boundaries
        )
    ...
    def test_production_density_map(self, mock_go):
        ...
        chart = self.chart_builder.create_production_density_map(
            self.location_data, resolution=10
        )
...
class TestInteractiveDashboard(unittest.TestCase):
    ...
    def _create_dashboard_data(self):
        ...
        return {
            ...
            "well_status": pd.DataFrame(
                {"status": ["Active", "Shut-in", "P&A"], "count": [20, 3, 2]}
            ),
        }
    ...
    def test_dashboard_layout_creation(self, mock_subplots):
        ...
        dashboard = self.dashboard_builder.create_dashboard(
            self.dashboard_data, layout="grid", rows=2, cols=2
        )
    ...
    @patch(
        "worldenergydata.modules.bsee.reports.comprehensive.visualizations.dashboard_builder.go"
    )
    def test_drill_down_capability(self, mock_go):
        ...
        chart = self.dashboard_builder.create_drilldown_chart(
            hierarchical_data, initial_level="block"
        )
    ...
    def test_real_time_update_capability(self):
        ...
        update_config = self.dashboard_builder.configure_realtime_updates(
            update_interval=5000, data_source="api", max_points=100  # 5 seconds
        )
    ...
    def test_export_to_html(self):
        ...
        with patch.object(self.chart_builder, "export_to_html") as mock_export:
            ...
            result = self.chart_builder.export_to_html(
                chart, "test_dashboard.html", include_plotlyjs="cdn"
            )
    ...
    def test_batch_export(self):
        ...
        with patch.object(self.chart_builder, "export_batch") as mock_batch:
            ...
            result = self.chart_builder.export_batch(
                charts, output_dir="exports/", format="png"
            )
