...
class TestComplianceVisualizationGeneration:
    ...
    def setup_method(self):
        ...
        self.template = ComplianceTemplate(
            template_name="test_compliance", template_path=self.template_path
        )
    ...
    def test_compliance_dashboard_content(self):
        ...
        assert (
            "indicator" in dashboard_html.lower() or "gauge" in dashboard_html.lower()
        )
    ...
    def test_milestone_timeline_generation_with_data(self):
        ...
        milestone_tracking = {"milestones": milestones}
    ...
    def test_chart_html_structure(self):
        ...
        for chart_name, chart_html in charts.items():
            ...
            # Should not contain genuine script errors. The bare substring
            # "error" is unsafe here: plotly's embedded default template
            # legitimately serializes the "error_x"/"error_y" trace styling
            # keys. Check for real error indicators instead.
            lower = chart_html.lower()
    ...
    def test_chart_data_integration(self):
        ...
        assert (
            "95" in dashboard_html or "95.0" in dashboard_html
        )  # Production compliance
        assert (
            "85" in dashboard_html
            or "0.85" in dashboard_html
            or "85.0" in dashboard_html
        )  # Environmental score
        assert (
            "92" in dashboard_html
            or "0.92" in dashboard_html
            or "92.0" in dashboard_html
        )  # Safety score
    ...
    def test_chart_customization_options(self):
        ...
        assert (
            "50" in dashboard_html or "0.5" in dashboard_html
...
class TestComplianceDashboardSpecific:
    ...
    def setup_method(self):
        ...
        self.temp_dir = Path(tempfile.mkdtemp())
        self.template = ComplianceTemplate(
            template_name="dashboard_test", template_path=self.temp_dir
        )
