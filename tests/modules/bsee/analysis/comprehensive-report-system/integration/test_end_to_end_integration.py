...
from worldenergydata.modules.bsee.reports.comprehensive.exporters.base import ReportExporter
...
class TestEndToEndIntegration:
                ...
                for lease in leases_per_field[field]:
                    ...
                    for i in range(num_wells):
                        well = {
                            ...
                            'spud_date': datetime(2020, 1, 1) + timedelta(days=np.random.randint(0, 1095)),
                            'first_production': datetime(2021, 1, 1) + timedelta(days=np.random.randint(0, 730))
                        }
                        wells.append(well)
                        well_id += 1
            ...
            if well['status'] == 'ACTIVE':
                ...
                for month in range(1, 13):
                    ...
                    prod_record['revenue_usd'] = (
                        ...
                        prod_record['gas_volume_mcf'] * prod_record['gas_price_usd_mcf']
                    )
        ...
        config = {
            ...
            'aggregation_settings': {
                ...
                'metrics_to_aggregate': [
                    ...
                    'revenue_usd', 'operating_cost_usd', 'netback_usd'
                ],
            ...
            'template_settings': {
                ...
                'economic': {'enabled': True, 'include_npv': True, 'discount_rate': 0.1},
    ...
    def test_complete_workflow_with_all_components(self, sample_hierarchical_data, report_config, temp_output_dir):
        ...
        assert all(block in organizational_units['blocks'] for block in ['MC 123', 'MC 456'])
    ...
    def test_data_consistency_across_hierarchy(self, sample_hierarchical_data, report_config):
        ...
        organizational_units = controller.build_hierarchy(wells_df, production_df)
        aggregation_results = controller.run_aggregation(organizational_units, production_df)
    ...
    def test_template_data_integration(self, sample_hierarchical_data, report_config):
        ...
        for template_name, template_instance in templates.items():
            ...
            if template_name == 'compliance':
                ...
                assert 'operational' in rendered.lower() or 'efficiency' in rendered.lower()
    ...
    def test_performance_with_moderate_dataset(self, report_config):
        ...
        organizational_units = controller.build_hierarchy(wells_df, production_df)
        aggregation_results = controller.run_aggregation(organizational_units, production_df)
        report = controller.generate_report('economic', aggregation_results, organizational_units)
        
        end_time = time.time()
        processing_time = end_time - start_time
        
        # Should process 100 wells in under 60 seconds
        assert processing_time < 60, f"Processing took {processing_time:.2f} seconds, should be under 60"
