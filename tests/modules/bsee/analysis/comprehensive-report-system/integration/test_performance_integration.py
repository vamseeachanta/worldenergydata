...
from worldenergydata.modules.bsee.reports.comprehensive.performance.cache import (
    CacheManager,
)
from worldenergydata.modules.bsee.reports.comprehensive.performance.parallel_processor import (
    ParallelProcessor,
)
...
class TestPerformanceIntegration:
    ...
    def test_performance_with_caching_disabled(self):
        ...
        perf_config = PerformanceConfiguration(
            cache_enabled=False, parallel_enabled=True
        )
        controller = PerformanceReportController(perf_config=perf_config)
    ...
    def test_performance_with_parallel_disabled(self):
        ...
        perf_config = PerformanceConfiguration(
            cache_enabled=True, parallel_enabled=False
        )
        controller = PerformanceReportController(perf_config=perf_config)
    ...
    def test_performance_statistics(self, controller):
        ...
        assert (
            stats["controller_metrics"]["cache_hits"]
            + stats["controller_metrics"]["cache_misses"]
            > 0
        )

        # Configuration should match
        assert (
            stats["configuration"]["cache_enabled"]
            == controller.perf_config.cache_enabled
        )
        assert (
            stats["configuration"]["parallel_enabled"]
            == controller.perf_config.parallel_enabled
        )
    ...
    def test_end_to_end_performance_improvement(self, controller, sample_data):
        ...
        standard_config = PerformanceConfiguration(
            cache_enabled=False, parallel_enabled=False
        )
