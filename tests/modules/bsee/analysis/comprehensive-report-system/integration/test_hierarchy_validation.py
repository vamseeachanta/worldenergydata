...
class TestHierarchyValidation:
    ...
    def hierarchical_test_data(self):
        ...
        hierarchy = {
            "MC 123": {
                ...
                "Field_B": {"LEASE003": ["W006", "W007"], "LEASE004": ["W008"]},
            },
            "MC 456": {
                "Field_C": {"LEASE005": ["W009", "W010", "W011"], "LEASE006": ["W012"]},
                "Field_D": {"LEASE007": ["W013", "W014"]},
            },
        }
            ...
            for field, leases in fields.items():
                for lease, wells in leases.items():
                    for well_id in wells:
                        ...
                        base_oil = (
                            well_counter * 1000
                        )  # Each well produces incrementally more oil
                        ...
                        for month in range(1, 7):  # 6 months of data
                            record = {
                                ...
                                "oil_volume_bbl": base_oil
                                * month,  # Increases each month
        ...
        expected_aggregations = {
            "well_totals": df.groupby("well_id")
            .agg(
                {
                    "oil_volume_bbl": "sum",
                    "gas_volume_mcf": "sum",
        ...
        return {"data": df, "hierarchy": hierarchy, "expected": expected_aggregations}
    ...
    def test_complete_hierarchy_consistency(self, hierarchical_test_data):
        ...
        for metric in ["gas_volume_mcf", "water_volume_bbl", "revenue_usd"]:
            ...
            assert abs(well_total - grand_total) < 0.01, f"{metric} well total mismatch"
            assert (
                abs(lease_total - grand_total) < 0.01
            ), f"{metric} lease total mismatch"
            assert (
                abs(field_total - grand_total) < 0.01
            ), f"{metric} field total mismatch"
            assert (
                abs(block_total - grand_total) < 0.01
    ...
    def test_aggregator_hierarchy_validation(self, hierarchical_test_data):
        ...
        try:
            ...
            if lease_result is not None:
                assert "oil_volume_bbl" in str(lease_result) or hasattr(
                    lease_result, "oil_volume_bbl"
                )
        ...
        try:
            ...
            if field_result is not None:
                assert "oil_volume_bbl" in str(field_result) or hasattr(
                    field_result, "oil_volume_bbl"
                )
        except (AttributeError, TypeError):
            pass
        ...
        try:
            ...
            if block_result is not None:
                assert "oil_volume_bbl" in str(block_result) or hasattr(
                    block_result, "oil_volume_bbl"
                )
        except (AttributeError, TypeError):
            pass
    ...
    def test_revenue_calculation_consistency(self, hierarchical_test_data):
        ...
        total_revenue_calculated = (
            data["oil_volume_bbl"] * data["oil_price_usd"]
        ).sum() + (data["gas_volume_mcf"] * data["gas_price_usd_mcf"]).sum()

        assert (
            abs(total_revenue_direct - total_revenue_calculated) < 0.01
    ...
    def test_hierarchy_with_missing_data(self, hierarchical_test_data):
        ...
        field_a_total = data_no_lease[data_no_lease["field"] == "Field_A"][
            "oil_volume_bbl"
        ].sum()

        assert (
            field_a_total > 0
        ), "Field should still have production from remaining lease"
        ...
        assert (
            abs(grand_total - block_totals) < 0.01
