...
from src.worldenergydata.modules.bsee.reports.comprehensive.templates.economic_template import (
    EconomicTemplate, EconomicAnalysis, RevenueBreakdown, CostAnalysis, 
    ProfitabilityMetrics, NPVAnalysis, ROIMetrics, SensitivityAnalysis,
    WaterfallComponent, EconomicForecast
)
from src.worldenergydata.modules.bsee.reports.comprehensive.models import (
    ProductionMetrics, EconomicMetrics
)
from src.worldenergydata.modules.bsee.reports.comprehensive.hierarchical_aggregator import (
    PriceDeck, CostStructure
)
...
class TestEconomicTemplate:
    ...
    def economic_template(self):
        ...
        return EconomicTemplate(
            template_name="economic_report_test",
            version="1.0.0"
        )
    ...
    def test_build_economic_context_from_production(self, economic_template, sample_production_metrics):
        """Test building economic context from production metrics"""
        context = economic_template.build_economic_context_from_production(sample_production_metrics)
    ...
    def test_build_economic_context_from_economics(self, economic_template, sample_economic_metrics):
        """Test building economic context from economic metrics"""
        context = economic_template.build_economic_context_from_economics(sample_economic_metrics)
...
class TestRevenueBreakdown:
    ...
    def test_revenue_breakdown_per_bbl_metrics(self, price_deck, production_data):
        ...
        total_boe = production_data['oil_bbls'] + (production_data['gas_mcf'] / 6) + production_data['ngl_bbls']
        expected_revenue_per_boe = breakdown.total_revenue / total_boe
        
        assert breakdown.revenue_per_boe == pytest.approx(expected_revenue_per_boe, rel=1e-2)
...
class TestCostAnalysis:
    ...
    def revenue_data(self):
        ...
        return {
            'total_revenue': 101250000.0
        }
    
    def test_cost_analysis_initialization(self, cost_structure, production_data, revenue_data):
    ...
    def test_cost_analysis_per_boe_metrics(self, cost_structure, production_data, revenue_data):
...
class TestProfitabilityMetrics:
    ...
    def revenue_breakdown(self):
        ...
        return RevenueBreakdown(
            oil_revenue=75000000.0,
            gas_revenue=21000000.0,
            ngl_revenue=5250000.0
        )
...
class TestROIMetrics:
    ...
    def test_payback_period_calculation(self):
        ...
        assert roi_metrics.payback_period_years == pytest.approx(expected_payback, rel=1e-2)
...
class TestSensitivityAnalysis:
    ...
    def test_price_sensitivity_analysis(self, base_case_metrics):
        ...
        oil_sensitivity = sensitivity.analyze_oil_price_sensitivity(
            base_case_metrics,
            price_range=[-20, -10, 0, 10, 20]  # Percentage changes
        )
        
        assert len(oil_sensitivity) == 5
        assert oil_sensitivity[2]['price_change_pct'] == 0  # Base case
        assert oil_sensitivity[0]['npv'] < oil_sensitivity[4]['npv']  # Lower price < Higher price
    ...
    def test_production_sensitivity_analysis(self, base_case_metrics):
        ...
        prod_sensitivity = sensitivity.analyze_production_sensitivity(
            base_case_metrics,
            volume_range=[-30, -15, 0, 15, 30]  # Percentage changes
        )
        
        assert len(prod_sensitivity) == 5
        assert prod_sensitivity[2]['volume_change_pct'] == 0  # Base case
        assert prod_sensitivity[0]['npv'] < prod_sensitivity[4]['npv']  # Lower volume < Higher volume
    ...
    def test_cost_sensitivity_analysis(self, base_case_metrics):
        ...
        cost_sensitivity = sensitivity.analyze_cost_sensitivity(
            base_case_metrics,
            cost_range=[-25, -10, 0, 10, 25]  # Percentage changes
        )
...
class TestEconomicForecast:
    ...
    def test_revenue_forecast(self, historical_production):
        ...
        for i in range(1, len(revenue_forecast)):
            assert revenue_forecast[i]['total_revenue'] >= revenue_forecast[i-1]['total_revenue']
...
class TestProductionEconomicsAnalysis:
    ...
    def test_production_economics_context_creation(self, economic_template_with_production):
    ...
    def test_goby_revenue_calculations_integration(self, economic_template_with_production):
...
class TestWellLevelEconomicAnalysis:
    ...
    def test_individual_well_analysis(self, well_production_data):
        ...
        for well_id, well_production in well_production_data.items():
            ...
            well_context = template.build_economic_context_from_production(well_production)
            ...
            assert goby_metrics["row_1_oil_production_bbls"] == well_production.oil_production_bbls
            assert goby_metrics["row_2_gas_production_mcf"] == well_production.gas_production_mcf
    ...
    def test_well_npv_calculation(self, well_production_data):
        ...
        for well_id, well_production in well_production_data.items():
            ...
            cash_flows = [-5000000] + [annual_net_income] * 10  # Initial investment + 10 years
    ...
    def test_well_roi_metrics(self, well_production_data):
        ...
        for well_id, well_production in well_production_data.items():
            ...
            template.add_roi_metrics(initial_investment, annual_net_income, project_years=15)
...
class TestFieldLevelEconomicIntegration:
    ...
    def test_field_per_well_metrics(self, field_metrics_data):
        ...
        field_perf = field_economics["field_performance"]
        expected_avg_revenue_per_well = field_metrics_data["gross_revenue"] / field_metrics_data["total_wells"]
        
        assert field_perf["avg_revenue_per_well"] == pytest.approx(expected_avg_revenue_per_well, rel=1e-2)
