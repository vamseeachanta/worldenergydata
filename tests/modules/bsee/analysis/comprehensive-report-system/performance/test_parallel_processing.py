...
class TestParallelProcessing:
    ...
    def test_hierarchical_parallel_processing(
        self, parallel_processor, sample_org_units
    ):
    ...
    def test_parallel_aggregation(self, parallel_processor, sample_org_units):
        ...
        assert (
            results[("sum", "oil_volume_bbl")]
            == sample_org_units["oil_volume_bbl"].sum()
        )
        assert (
            results[("mean", "gas_volume_mcf")]
            == sample_org_units["gas_volume_mcf"].mean()
        )
    ...
    def test_performance_scaling(self, sample_org_units):
        ...
        assert (
            processing_times[2] <= processing_times[1]
        )  # 4 workers at least as fast as 2
    ...
    def test_memory_efficient_processing(self, parallel_processor):
        ...
        large_data = pd.DataFrame(
            {"id": range(10000), "value": np.random.random(10000)}
        )

        # Process with memory limits
        result = processor.process_memory_efficient(
            large_data, max_memory_mb=100, batch_size=1000
        )
    ...
    def test_parallel_template_rendering(self, parallel_processor):
        ...
        template_data = {"metrics": {"oil": 1000, "gas": 500}, "period": "2024-Q1"}
