...
class TestEnvironmentalMetricsAggregation:
    ...
    def test_environmental_status_classifications(self):
        ...
        poor = EnvironmentalMetrics(
            spill_incidents=5, total_spill_volume_bbls=50.0, environmental_violations=2
        )
...
class TestEnvironmentalComplianceTracking:
    ...
    def test_environmental_compliance_tracking_scores(self):
        ...
        assert env_compliance["environmental_status"] in [
            "Excellent",
            "Good",
            "Fair",
            "Poor",
    ...
    def test_environmental_compliance_aggregation_multiple_entities(self):
        ...
        total_spill_volume = sum(
            [m.total_spill_volume_bbls for m in metrics_list]
        )  # 15.0
        total_emissions = sum([m.air_emissions_tons for m in metrics_list])  # 325.0
        avg_environmental_score = sum(
            [m.calculate_environmental_score() for m in metrics_list]
        ) / len(metrics_list)
    ...
    def test_environmental_compliance_threshold_analysis(self):
        ...
        test_cases = [
            {
                "metrics": EnvironmentalMetrics(
                    spill_incidents=0, air_emissions_tons=50.0
                ),
                "expected_status": "Excellent",
            },
            {
                "metrics": EnvironmentalMetrics(
                    spill_incidents=1, air_emissions_tons=80.0
                ),
                "expected_status": "Good",
            },
            {
                "metrics": EnvironmentalMetrics(
                    spill_incidents=2, air_emissions_tons=120.0
                ),
