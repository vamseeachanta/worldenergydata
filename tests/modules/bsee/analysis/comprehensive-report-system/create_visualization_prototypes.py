...
def create_field_level_visualizations():
    ...
    fig.add_trace(
        go.Bar(
            x=fields, y=water_depths, name="Water Depth (ft)", marker_color="lightblue"
        ),
        row=1,
        col=1,
    )
    ...
    fig.add_trace(
        go.Bar(x=fields, y=well_counts, name="Number of Wells", marker_color="green"),
        row=1,
        col=2,
    )
    ...
    fig.add_trace(
        go.Bar(
            x=fields, y=avg_construction_days, name="Avg Days", marker_color="orange"
        ),
        row=2,
        col=1,
    )
    ...
    fig.update_layout(
        title_text="Field-Level Data Visualizations", height=700, showlegend=False
    )

    # Save the figure
    output_path = Path(
        "tests/modules/bsee/analysis/comprehensive-report-system/results"
    )
    output_path.mkdir(parents=True, exist_ok=True)
    fig.write_html(str(output_path / "field_level_visualizations.html"))

    print(
        f"Field-level visualizations saved to: {output_path / 'field_level_visualizations.html'}"
    )
...
def create_well_level_visualizations():
    ...
    fig2 = make_subplots(
        ...
        cols=2,
        subplot_titles=(
            "Well Construction Days Distribution",
    ...
    fig2.add_trace(
        go.Box(y=construction_days, name="Construction Days", marker_color="lightblue"),
        row=1,
        col=1,
    )
    ...
    fig2.update_layout(
        title_text="Well-Level Performance Metrics", height=400, showlegend=False
    )
    ...
    print(
        f"Well performance saved to: {output_path / 'well_performance_visualization.html'}"
    )
...
def create_production_visualization_prototype():
    ...
    for i, well in enumerate(wells):
        ...
        fig.add_trace(
            go.Scatter(
                x=dates, y=production, mode="lines", name=well, line=dict(width=2)
            )
        )
    ...
    print(
        f"Production visualization saved to: {output_path / 'production_visualization.html'}"
    )
