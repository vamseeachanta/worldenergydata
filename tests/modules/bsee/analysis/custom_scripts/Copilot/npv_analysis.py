...
def load_actual_excel_data():
    ...
    try:
        ...
        discount_rates = [
            ...
            0.15,
            0.19,
        ]  # Based on the Excel analysis output
...
def calculate_custom_npv_scenarios():
    ...
    if cash_flows is None or discount_rates is None:
        ...
        cash_flows = [
            -4300000000,  # Initial CAPEX (more realistic based on Excel data)
            300000000,
            350000000,
            500000000,
            850000000,
...
def save_results_to_file(npv_collection):
    ...
    df = pd.DataFrame(
        [
            {"Discount_Rate": rate, "Discount_Rate_Percent": f"{rate:.1%}", "NPV": npv}
            for rate, npv in npv_collection
        ]
    )
