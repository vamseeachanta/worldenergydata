...
class JStMNPVAnalyzer:
    ...
    def load_excel_data(self):
        ...
        try:
            df = pd.read_excel(
                self.excel_file, sheet_name="NPV w Mo'ly data chart", engine="openpyxl"
            )
            return df
    ...
    def extract_financial_metrics(self, df):
        ...
        for col in df.columns:
            try:
                ...
                potential_npv = numeric_col[
                    (numeric_col.abs() > 1000000) & (numeric_col.abs() < 1e12)
                ]
    ...
    def perform_sensitivity_analysis(self, base_scenario, base_rate=0.10):
        ...
        for change in [-0.3, -0.2, -0.1, 0.1, 0.2, 0.3]:
            adjusted_flows = [base_cash_flows[0]] + [
                cf * (1 + change) for cf in base_cash_flows[1:]
            ]
            new_npv = npf.npv(base_rate, adjusted_flows)
            pct_change = (
                ((new_npv - base_npv) / abs(base_npv) * 100) if base_npv != 0 else 0
            )
    ...
    def generate_comprehensive_report(self):
        ...
        discount_rates = sorted(
            list(set(excel_rates[:10] + [0.05, 0.08, 0.10, 0.12, 0.15, 0.20]))
        )
        ...
        if excel_metrics:
            ...
            print(
                f"Positive NPVs: {excel_metrics['positive_npv_count']} ({excel_metrics['positive_npv_count']/excel_metrics['npv_count']*100:.1f}%)"
            )
            print(
                f"Negative NPVs: {excel_metrics['negative_npv_count']} ({excel_metrics['negative_npv_count']/excel_metrics['npv_count']*100:.1f}%)"
            )
        ...
        for scenario_name, results in scenario_results.items():
            ...
            for calc in results["calculations"]:
                if calc["discount_rate"] in [0.08, 0.10, 0.15]:
                    ...
                    print(
                        f"  NPV @ {calc['discount_rate']:.0%}: ${calc['npv']:>12,.0f} ({npv_status})"
                    )
        ...
        for key, data in sensitivity["sensitivities"].items():
            if "revenue" in key:
                print(
                    f"  {data['description']:>12}: ${data['npv']:>15,.0f} ({data['npv_change_pct']:+.1f}%)"
                )
        ...
        for scenario_name, results in scenario_results.items():
            npv_10pct = next(
                (
                    ...
                    if abs(c["discount_rate"] - 0.10) < 0.001
                ),
                None,
            )
            if npv_10pct:
