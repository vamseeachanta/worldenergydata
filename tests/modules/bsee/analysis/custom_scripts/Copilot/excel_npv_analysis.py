    ...
    try:
        ...
        for sheet_name in xl_file.sheet_names:
            ...
            if financial_data_found:
                print(
                    f"Found {len(financial_data_found)} cells with financial keywords:"
                )
                for item in financial_data_found[:10]:  # Show first 10
                    print(
                        f"  Row {item['row']}, Col {item['col']}: {item['value']} (keyword: {item['keyword']})"
                    )
                ...
                try:
                    ...
                    if not potential_rates.empty:
                        for val in potential_rates.dropna():
                            rate_data.append(
                                {"column": col, "value": val, "type": "Decimal rate"}
                            )

                    # Look for percentage rates (between 1 and 50)
                    potential_pct_rates = numeric_col[
                        (numeric_col >= 1) & (numeric_col <= 50)
                    ]
                    if not potential_pct_rates.empty:
                        for val in potential_pct_rates.dropna():
                            rate_data.append(
                                {"column": col, "value": val, "type": "Percentage rate"}
                            )
    ...
    for sheet_name, analysis in sheets_analysis.items():
        ...
        if npv_data:
            ...
            for npv_item in npv_data:
                print(
                    f"  NPV: ${npv_item['value']:,.2f} (from column: {npv_item['column']})"
                )
        ...
        if rate_data:
            ...
            for rate_item in rate_data:
                ...
                print(
                    f"  Rate: {rate_val:.4f} ({rate_val*100:.2f}%) from column: {rate_item['column']}"
                )
        ...
        if cash_flows:
            ...
            for i, cf in enumerate(cash_flows[:3]):  # Show first 3
                print(
                    f"  Series {i+1} (Column: {cf['column']}): {len(cf['values'])} periods"
                )
                ...
                try:
                    ...
                    print(
                        f"    Calculated NPV @ {discount_rate*100:.1f}%: ${npv_calc:,.2f}"
                    )
...
def save_excel_analysis_results(npv_results, sheets_analysis):
    ...
    if npv_results:
        sorted_results = sorted(
            npv_results.items(), key=lambda x: x[1]["npv"], reverse=True
        )

        print("\nTop 5 Highest NPVs:")
        for i, (key, data) in enumerate(sorted_results[:5]):
            print(
                f"  {i+1}. ${data['npv']:>15,.2f} @ {data['discount_rate']:.1%} ({key})"
            )
