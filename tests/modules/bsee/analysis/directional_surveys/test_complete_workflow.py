...
class TestDirectionalSurveysWorkflow:
    ...
    def test_complete_directional_surveys_workflow(self):
        ...
        assert hasattr(self.well_api12, 'output_data_well_path'), "output_data_well_path not created"
        assert len(self.well_api12.output_data_well_path) > 0, "No well path data generated"
        assert 608124000400 in self.well_api12.output_data_well_path, "API12 608124000400 not found"
        ...
        print(f"   ✓ X range: {well_path_data['x_coor'].min():.1f} to {well_path_data['x_coor'].max():.1f} ft")
        print(f"   ✓ Y range: {well_path_data['y_coor'].min():.1f} to {well_path_data['y_coor'].max():.1f} ft")
        print(f"   ✓ Z range: {well_path_data['z_coor'].min():.1f} to {well_path_data['z_coor'].max():.1f} ft")
    ...
    def test_process_survey_xyz_method(self):
        ...
        print(f"   ✓ Final displacement: X={result['x_coor'].iloc[-1]:.1f}ft, Y={result['y_coor'].iloc[-1]:.1f}ft")
    ...
    def test_add_relative_WH_positions_method(self):
        ...
        assert abs(actual_x_offset - expected_x_offset) < 0.01, "X offset not applied correctly"
        assert abs(actual_y_offset - expected_y_offset) < 0.01, "Y offset not applied correctly"
    ...
    def test_actual_3d_plot_generation(self):
        ...
        try:
            ...
            expected_plot_file = os.path.join(
                self.temp_dir, 
                'test_directional_surveys_well_paths.png'
            )
