...
class TestWellAPI12DirectionalSurveys:
    ...
    def setup_method(self):
        ...
        self.well_api12.output_data_api12_df = self.mock_well_data[
            "merged_api12_df"
        ].copy()
        ...
        with (
            ...
            try:
                self.well_api12.prepare_well_paths(
                    self.mock_directional_surveys, self.mock_well_data
                )
    ...
    def test_process_survey_xyz_vertical_well(self):
        ...
        assert all(
            result["x_coor"] == 0
        ), "Vertical well should have zero x displacement"
        assert all(
            result["y_coor"] == 0
        ), "Vertical well should have zero y displacement"

        # Z coordinates should approximately equal MD for vertical well
        np.testing.assert_array_almost_equal(
            result["z_coor"].values, result["md"].values, decimal=1
        )
    ...
    def test_process_survey_xyz_azimuth_wraparound(self):
        ...
        assert (
            result["az_diff"].iloc[1] == 20
        ), "Azimuth wraparound should be handled correctly"
    ...
    def test_process_survey_xyz_mathematical_properties(self):
        ...
        assert all(
            build_rates >= 0
        ), "Build rates should be non-negative for increasing inclination"

        # Verify that coordinates are monotonically increasing in some direction
        assert (
            result["z_coor"].iloc[-1] > result["z_coor"].iloc[0]
        ), "Final TVD should be greater than initial"

    def test_process_survey_xyz_edge_cases(self):
        """Test process_survey_xyz with edge cases"""
        # Single point survey
        single_point = pd.DataFrame({"md": [0], "inc": [0], "az": [0]})
    ...
    def test_process_survey_xyz_data_types(self):
        ...
        assert not np.isinf(
            result["x_coor"]
        ).any(), "x_coor should not have infinite values"
        assert not np.isinf(
            result["y_coor"]
        ).any(), "y_coor should not have infinite values"
        assert not np.isinf(
            result["z_coor"]
        ).any(), "z_coor should not have infinite values"
    ...
    def test_add_relative_WH_positions_preserves_other_columns(self):
        ...
        for col in unchanged_columns:
            pd.testing.assert_series_equal(
                result[col], survey_xyz[col], check_names=False
            )
    ...
    def test_add_relative_WH_positions_zero_offset(self):
        ...
        self.well_api12.output_data_api12_df = zero_offset_well_data[
            "merged_api12_df"
        ].copy()
        self.well_api12.output_data_api12_df["xyz"] = None
        ...
        pd.testing.assert_series_equal(
            result["x_coor"], survey_xyz["x_coor"], check_names=False
        )
        pd.testing.assert_series_equal(
            result["y_coor"], survey_xyz["y_coor"], check_names=False
        )
    ...
    def test_add_relative_WH_positions_negative_offsets(self):
        ...
        # Update the instance data
        self.well_api12.output_data_api12_df = negative_offset_well_data[
            "merged_api12_df"
        ].copy()
        self.well_api12.output_data_api12_df["xyz"] = None

        survey_xyz = pd.DataFrame(
            {"x_coor": [0, 100, 200], "y_coor": [0, 50, 100], "z_coor": [0, 1000, 2000]}
        )
    ...
    def test_add_relative_WH_positions_data_independence(self):
        ...
        assert not result["x_coor"].equals(
            original_survey["x_coor"]
        ), "Result should be different from original"
        assert not result["y_coor"].equals(
            original_survey["y_coor"]
        ), "Result should be different from original"
    ...
    def test_plot_field_wells_file_saving(self):
        ...
        with (
            ...
            mock_fig.savefig.assert_called_once_with(
                expected_filename, bbox_inches="tight", dpi=800
            )
    ...
    def test_plot_field_wells_well_labeling(self):
        ...
        with (
            ...
            assert (
                "label" in call_args[1]
            ), "plot3D should be called with label parameter"
    ...
    def test_plot_field_wells_multiple_wells(self):
        ...
        second_well_data = pd.DataFrame(
            {
                ...
                "API10": [
                    6081240040
                ],  # Different API10 to ensure both wells are plotted
        ...
        self.well_api12.output_data_api12_df = pd.concat(
            [self.well_api12.output_data_api12_df, second_well_data], ignore_index=True
        )
        ...
        with (
            ...
            assert (
                mock_ax.plot3D.call_count >= 1
            ), f"plot3D should be called at least once, but was called {mock_ax.plot3D.call_count} times"
    ...
    def test_plot_field_wells_error_handling(self):
        ...
        # Setup mock well path data
        mock_well_path_data = {
            608124000999: pd.DataFrame(
                {  # API12 not in output_data_api12_df
                    "x_coor": [0.0, 100.0, 200.0],
                    "y_coor": [0.0, 50.0, 100.0],
                    "z_coor": [0.0, 1000.0, 2000.0],
                }
