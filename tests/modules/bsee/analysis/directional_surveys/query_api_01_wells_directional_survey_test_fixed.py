...
def create_test_data_from_yaml():
    ...
    directional_surveys = pd.DataFrame({
        ...
        'SURVEY_POINT_MD': [0, 1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000],
        ...
        'WELL_N_S_CODE': ['N'] * 10,
        'WELL_E_W_CODE': ['E'] * 10,
        'SURVEY_POINT_TVD': [0, 999.5, 1996.8, 2988.1, 3970.2, 4940.5, 5896.8, 6837.2, 7759.8, 8662.5]
    })
...
def run_application_fixed():
    ...
    try:
        ...
        assert 608124000400 in well_api12.output_data_well_path, "API12 608124000400 not processed"
        ...
        print(f"✓ X (Easting) range: {well_path_data['x_coor'].min():.1f} to {well_path_data['x_coor'].max():.1f} ft")
        print(f"✓ Y (Northing) range: {well_path_data['y_coor'].min():.1f} to {well_path_data['y_coor'].max():.1f} ft")
        print(f"✓ Z (TVD) range: {well_path_data['z_coor'].min():.1f} to {well_path_data['z_coor'].max():.1f} ft")
        ...
        print(f"✓ Data stored: {len(stored_data['data'])} points with label '{stored_data['label']}'")
