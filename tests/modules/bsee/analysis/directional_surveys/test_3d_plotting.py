...
def test_actual_3d_plotting():
    ...
    try:
        ...
        if len(well_api12.output_data_well_path) > 0:
            print(
                f"   ✓ Well path data created for {len(well_api12.output_data_well_path)} wells"
            )

            well_path = well_api12.output_data_well_path[608124000400]
            print(f"   ✓ Survey points: {len(well_path)}")
            print(
                f"   ✓ X range: {well_path['x_coor'].min():.1f} to {well_path['x_coor'].max():.1f} ft"
            )
            print(
                f"   ✓ Y range: {well_path['y_coor'].min():.1f} to {well_path['y_coor'].max():.1f} ft"
            )
            print(
                f"   ✓ Z range: {well_path['z_coor'].min():.1f} to {well_path['z_coor'].max():.1f} ft"
            )
