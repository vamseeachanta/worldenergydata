...
print(
    "🔍 Sample SURF_LEASE_NUMs from WAR file:",
    main_war["SURF_LEASE_NUM"].dropna().unique()[:10],
)
...
boreholes["WELL_SPUD_DATE"] = pd.to_datetime(
    boreholes["WELL_SPUD_DATE"], errors="coerce"
)
boreholes["TOTAL_DEPTH_DATE"] = pd.to_datetime(
    boreholes["TOTAL_DEPTH_DATE"], errors="coerce"
)
...
def adjust_spud(api, td):
    war_dates = main_war_filtered[main_war_filtered["API_WELL_NUMBER"] == api][
...
spud_td = pd.DataFrame(
    rows,
    columns=["API_WELL_NUMBER", "WELL_SPUD_DATE", "TOTAL_DEPTH_DATE", "DRILLING_DAYS"],
)
...
final["LEASE_NAME"] = final["SURF_LEASE_NUM"].map(
    lambda x: lease_info.get(x, {}).get("LEASE_NAME", "")
)
final["WATER_DEPTH"] = final["SURF_LEASE_NUM"].map(
    lambda x: lease_info.get(x, {}).get("WATER_DEPTH", "")
)
...
final.to_excel("drilling_and_completion_days_by_api_test_output.xlsx", index=False)
print("✅ drilling_and_completion_days_by_api_test_output.xlsx written.")
