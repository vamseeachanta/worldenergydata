...
class ComparisonDataLoader:
    ...
    def load_lease_method_data(self, file_path: Union[str, Path]) -> pd.DataFrame:
        ...
        if missing_required:
            raise ValueError(
                f"Missing required columns in lease method data: {missing_required}"
            )
        ...
        df_renamed["api_number"] = pd.to_numeric(
            df_renamed["api_number"], errors="coerce"
        )
    ...
    def load_api12_method_data(self, file_path: Union[str, Path]) -> pd.DataFrame:
        ...
        if missing_required:
            raise ValueError(
                f"Missing required columns in API12 method data: {missing_required}"
            )
    ...
    def _clean_lease_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["drilling_days_lease"] = pd.to_numeric(
            df["drilling_days_lease"], errors="coerce"
        )
        df["completion_days_lease"] = pd.to_numeric(
            df["completion_days_lease"], errors="coerce"
        )
    ...
    def _clean_api12_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Ensure drilling and completion days are numeric
        df["drilling_days_api12"] = pd.to_numeric(
            df["drilling_days_api12"], errors="coerce"
        )
        df["completion_days_api12"] = pd.to_numeric(
            df["completion_days_api12"], errors="coerce"
        )
...
class ComparisonAnalyzer:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized ComparisonAnalyzer with thresholds: "
            f"drilling={drilling_days_threshold}d, completion={completion_days_threshold}d, "
            f"percentage={percentage_threshold}%"
        )

    def match_wells_by_api(
        self, lease_data: pd.DataFrame, api12_data: pd.DataFrame
    ) -> pd.DataFrame:
        """
        ...
        """
        logger.info(
            f"Matching wells: {len(lease_data)} lease wells vs {len(api12_data)} API12 wells"
        )
        ...
        if len(matched) == 0:
            logger.warning(
                "No wells matched between datasets. Check API number formats."
            )

        return matched

    def calculate_drilling_days_differences(
        ...
        df["drilling_days_percent_diff"] = df.apply(
            lambda row: self._calculate_percentage_difference(
                row["drilling_days_lease"], row["drilling_days_api12"]
            ),
            axis=1,
        )
    ...
    def calculate_completion_days_differences(
        self, matched_data: pd.DataFrame
    ) -> pd.DataFrame:
        """
        ...
        df["completion_days_percent_diff"] = df.apply(
            lambda row: self._calculate_percentage_difference(
                row["completion_days_lease"], row["completion_days_api12"]
            ),
            axis=1,
        )
    ...
    def apply_discrepancy_flags(self, comparison_data: pd.DataFrame) -> pd.DataFrame:
        ...
        def determine_flag(row):
            ...
            if (
                drilling_abs_diff > self.drilling_days_threshold * 2
                or completion_abs_diff > self.completion_days_threshold * 2
                or drilling_percent_diff > self.percentage_threshold * 2
                or completion_percent_diff > self.percentage_threshold * 2
            ):
                return "ERROR"

            # REVIEW: Moderate differences
            elif (
                drilling_abs_diff > self.drilling_days_threshold
                or completion_abs_diff > self.completion_days_threshold
                or drilling_percent_diff > self.percentage_threshold
                or completion_percent_diff > self.percentage_threshold
            ):
                return "REVIEW"
    ...
    def _add_summary_statistics(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(
            f"Summary statistics calculated: "
