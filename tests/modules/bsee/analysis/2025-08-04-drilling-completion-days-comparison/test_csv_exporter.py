...
class TestCSVExporter:
    ...
    def test_individual_method_csv_export(
        self, sample_lease_data, sample_api12_data, temp_output_dir
    ):
    ...
    def test_timestamped_file_naming(self, sample_comparison_data, temp_output_dir):
        ...
        lease_filename = exporter.generate_timestamped_filename(
            "drilling_days_lease_method", "csv"
        )
        api12_filename = exporter.generate_timestamped_filename(
            "drilling_days_api12_method", "csv"
        )
        comparison_filename = exporter.generate_timestamped_filename(
            "drilling_days_comparison", "csv"
        )
    ...
    def test_csv_metadata_headers(self, sample_comparison_data, temp_output_dir):
        ...
        exporter.export_comparison_results(
            sample_comparison_data, output_file, include_metadata=True
        )
    ...
    def test_data_integrity_validation(self, sample_comparison_data):
        ...
        tiber_matches = standardized_df[
            standardized_df["API12_number"] == "608084001500"
        ]
        ...
        assert (
            len(tiber_matches) > 0
        ), f"No matching rows found for API 608084001500 in {standardized_df['API12_number'].values}"
    ...
    def test_handle_missing_data_in_csv(self):
        ...
        assert (
            pd.isna(standardized_df.iloc[1]["lease_method_drilling_days"])
            or standardized_df.iloc[1]["lease_method_drilling_days"] == "N/A"
        )
    ...
    def test_excel_compatibility(self, sample_comparison_data, temp_output_dir):
        ...
        if drilling_diff_col in df.columns:
            ...
            assert not any(
                np.isinf(pd.to_numeric(df[drilling_diff_col], errors="coerce")).fillna(
                    False
                )
            )

    def test_pandas_roundtrip_compatibility(
        self, sample_comparison_data, temp_output_dir
    ):
    ...
    def test_file_versioning_and_overwrite_protection(
        self, sample_comparison_data, temp_output_dir
    ):
        ...
        timestamp_filename = exporter.generate_timestamped_filename(
            base_filename, "csv"
        )
    ...
    def test_results_directory_creation(self, sample_comparison_data):
        ...
        try:
            exporter.export_comparison_results(
                sample_comparison_data, output_file, create_dirs=True
            )
...
class TestCSVExporterIntegration:
    ...
    def test_end_to_end_workflow_csv_export(self):
        ...
        excel_files = list(
            results_dir.glob("drilling_and_completion_days_by_api_validation_*.xlsx")
        )
