...
class DrillingDaysComparisonTest:
    ...
    def execute_method1_test(self) -> bool:
        ...
        try:
            ...
            expected_output = self.test_directory / "results" / "drilling_and_completion_days_by_api.xlsx"
    ...
    def execute_method2_test(self) -> bool:
        ...
        try:
            ...
            method2_config = self.test_directory / "query_api_01_wells_api12_rig_days_Tiber.yml"
            ...
            # Verify output file was created
            expected_output = self.test_directory / "results" / "well_summ_goa_tiber.csv"
            if expected_output.exists():
    ...
    def load_method_outputs(self) -> Tuple[Optional[pd.DataFrame], Optional[pd.DataFrame]]:
        """
        ...
        try:
            ...
            if self.method1_results and os.path.exists(self.method1_results):
                ...
                print(f"Method 1: Loaded {len(method1_df)} records from {self.method1_results}")
            ...
            if self.method2_results and os.path.exists(self.method2_results):
                ...
                print(f"Method 2: Loaded {len(method2_df)} records from {self.method2_results}")
    ...
    def standardize_columns(self, df1: pd.DataFrame, df2: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
        """
    ...
    def execute_test_with_retry(self, method_name: str, execute_func) -> Tuple[bool, str]:
        """
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if success:
                    ...
                    error_msg = f"{method_name} execution failed on attempt {attempt + 1}"
                ...
                error_msg = f"{method_name} execution error on attempt {attempt + 1}: {str(e)}"
                if attempt < max_retries:
                    print(f"{error_msg}, retrying...")
                    continue
                else:
    ...
    def compare_drilling_days_data(self, df1: pd.DataFrame, df2: pd.DataFrame) -> pd.DataFrame:
        """
        ...
        comparison_df = pd.merge(
            df1[['api12', 'well_name', 'drilling_days', 'completion_days']].add_suffix('_method1'),
            df2[['api12', 'well_name', 'drilling_days', 'completion_days']].add_suffix('_method2'),
        ...
        comparison_df['drilling_days_diff'] = (
            pd.to_numeric(comparison_df['drilling_days_method1'], errors='coerce') - 
            pd.to_numeric(comparison_df['drilling_days_method2'], errors='coerce')
        )
        
        comparison_df['completion_days_diff'] = (
            pd.to_numeric(comparison_df['completion_days_method1'], errors='coerce') - 
        ...
        comparison_df['drilling_days_pct_diff'] = (
            comparison_df['drilling_days_diff'] / 
            pd.to_numeric(comparison_df['drilling_days_method2'], errors='coerce').replace(0, pd.NA) * 100
        )
        
        comparison_df['completion_days_pct_diff'] = (
            comparison_df['completion_days_diff'] / 
            pd.to_numeric(comparison_df['completion_days_method2'], errors='coerce').replace(0, pd.NA) * 100
        )
        
        # Add status flags based on thresholds from config
        thresholds = self.config.get('comparison', {}).get('thresholds', {})
        drilling_abs_threshold = thresholds.get('drilling_days', {}).get('absolute_difference', 5)
        drilling_pct_threshold = thresholds.get('drilling_days', {}).get('percentage_difference', 10)
        completion_abs_threshold = thresholds.get('completion_days', {}).get('absolute_difference', 5)
        completion_pct_threshold = thresholds.get('completion_days', {}).get('percentage_difference', 10)
        
        def get_status(row):
            drilling_abs_diff = abs(row['drilling_days_diff']) if pd.notna(row['drilling_days_diff']) else 0
            drilling_pct_diff = abs(row['drilling_days_pct_diff']) if pd.notna(row['drilling_days_pct_diff']) else 0
            completion_abs_diff = abs(row['completion_days_diff']) if pd.notna(row['completion_days_diff']) else 0
            completion_pct_diff = abs(row['completion_days_pct_diff']) if pd.notna(row['completion_days_pct_diff']) else 0
            
            # Check if either method has missing data
            if (pd.isna(row['drilling_days_method1']) or pd.isna(row['drilling_days_method2']) or
                pd.isna(row['completion_days_method1']) or pd.isna(row['completion_days_method2'])):
                return 'ERROR'
            
            # Check if differences exceed thresholds
            if (drilling_abs_diff > drilling_abs_threshold or drilling_pct_diff > drilling_pct_threshold or
                completion_abs_diff > completion_abs_threshold or completion_pct_diff > completion_pct_threshold):
                return 'REVIEW'
    ...
    def generate_markdown_comparison_table(self, comparison_df: pd.DataFrame) -> str:
        ...
        table_config = self.config.get('reports', {}).get('markdown', {}).get('table_format', {})
        ...
        markdown_lines = [
            ...
            "|" + ("-" * 14) + "|" + ("-" * 11) + "|" + ("-" * 19) + "|" + ("-" * 19) + "|" + ("-" * 15) + "|" + ("-" * 21) + "|" + ("-" * 21) + "|" + ("-" * 17) + "|" + ("-" * 8) + "|"
        ]
        
        # Add table rows
        for _, row in comparison_df.iterrows():
            # Use method1 API12 if available, otherwise method2
            api12 = row['api12_method1'] if pd.notna(row['api12_method1']) else row['api12_method2']
            well_name = row['well_name_method1'] if pd.notna(row['well_name_method1']) else row['well_name_method2']
            ...
            well_name_short = well_name[:9] + "..." if len(str(well_name)) > 12 else str(well_name)
...
def test_drilling_days_comparison():
    ...
    assert results['success'] is True, f"Comparison failed: {results.get('error', 'Unknown error')}"
    ...
    if summary.get('method1_records', 0) > 0 and summary.get('method2_records', 0) > 0:
        ...
        print("Warning: One or both methods produced no records - this may indicate missing test data")
...
def test_method1_execution():
    ...
    assert os.path.exists(comparison_test.method1_results), f"Method 1 output file not found: {comparison_test.method1_results}"
...
def test_method2_execution():
    ...
    assert os.path.exists(comparison_test.method2_results), f"Method 2 output file not found: {comparison_test.method2_results}"
...
def test_markdown_report_generation():
    ...
    # Validate results
    assert results['success'] is True, f"Report generation failed: {results.get('error', 'Unknown error')}"
    ...
    assert os.path.exists(results['markdown_report_path']), f"Report file not created: {results['markdown_report_path']}"
    ...
    assert "# Drilling Days Comparison Report" in markdown_content, "Report title missing"
