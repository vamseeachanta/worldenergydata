...
class CSVExporter:
    ...
    def prepare_standardized_format(
    ...
    def export_comparison_results(
        ...
        if include_metadata:
            self._write_csv_with_metadata(
                standardized_df, output_path, "Drilling Days Comparison Results"
            )
        else:
    ...
    def export_lease_method_data(
        ...
        if include_metadata:
            self._write_csv_with_metadata(
                lease_data, output_path, "Lease Method Drilling and Completion Days"
            )
        else:
    ...
    def export_api12_method_data(
        ...
        if include_metadata:
            self._write_csv_with_metadata(
                api12_data, output_path, "API12 Method Drilling and Completion Days"
            )
        else:
    ...
    def export_all_files(
        ...
        if use_timestamps:
            comparison_filename = self.generate_timestamped_filename(
        ...
        if lease_data is not None:
            if use_timestamps:
                lease_filename = self.generate_timestamped_filename(
        ...
        if api12_data is not None:
            if use_timestamps:
                api12_filename = self.generate_timestamped_filename(
        ...
        logger.info(
            f"All CSV files exported successfully. Files: {list(exported_files.keys())}"
        )
    ...
    def _write_csv_with_metadata(
        self, df: pd.DataFrame, output_path: Path, description: str
    ) -> None:
        """
    ...
    def _generate_notes(self, row: pd.Series) -> str:
        ...
        if pd.notna(drilling_diff_val) and str(drilling_diff_val) != "N/A":
            try:
                ...
                if drilling_diff > 10:
                    notes.append(
                        f"Large drilling days difference ({drilling_diff:.0f}d)"
                    )
                elif drilling_diff > 5:
                    notes.append(
                        f"Moderate drilling days difference ({drilling_diff:.0f}d)"
                    )
            except (ValueError, TypeError):
                pass
        ...
        if pd.notna(completion_diff_val) and str(completion_diff_val) != "N/A":
            try:
                ...
                if completion_diff > 6:
                    notes.append(
                        f"Large completion days difference ({completion_diff:.0f}d)"
                    )
                elif completion_diff > 3:
                    notes.append(
                        f"Moderate completion days difference ({completion_diff:.0f}d)"
                    )
            except (ValueError, TypeError):
                pass
