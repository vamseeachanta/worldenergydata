...
def verify_csv_integrity():
    ...
    try:
        ...
        logger.info(f"✓ Comparison CSV read successfully: {len(comparison_df)} rows, {len(comparison_df.columns)} columns")
        ...
        missing_cols = [col for col in expected_columns if col not in comparison_df.columns]
        if missing_cols:
        ...
        if len(comparison_df) > 0:
            ...
            logger.info(f"Sample data - API: {first_row['API12_number']}, Well: {first_row['Well_name']}")
            logger.info(f"  Lease drilling days: {first_row['lease_method_drilling_days']}")
            logger.info(f"  API12 drilling days: {first_row['api12_method_drilling_days']}")
    ...
    if lease_file:
        ...
        try:
            ...
            logger.info(f"✓ Lease method CSV read successfully: {len(lease_df)} rows, {len(lease_df.columns)} columns")
        except Exception as e:
    ...
    if api12_file:
        ...
        try:
            ...
            logger.info(f"✓ API12 method CSV read successfully: {len(api12_df)} rows, {len(api12_df.columns)} columns")
        except Exception as e:
    ...
    try:
        ...
        numeric_cols = ['lease_method_drilling_days', 'api12_method_drilling_days', 
                       'Drilling_days_difference', 'Drilling_days_percent_diff']
        
        for col in numeric_cols:
            if col in comparison_df.columns:
                # Check for infinity values (should be replaced with 'N/A')
                has_inf = any(comparison_df[col].astype(str).str.contains('inf', case=False, na=False))
                if has_inf:
                    logger.warning(f"⚠ Column {col} contains infinity values that may cause Excel issues")
                else:
    ...
    try:
        ...
        # Compare structure
        if len(roundtrip_df) == len(comparison_df) and len(roundtrip_df.columns) == len(comparison_df.columns):
    ...
    try:
        if len(comparison_df) > 0:
            ...
            for idx, row in comparison_df.iterrows():
                lease_drilling = pd.to_numeric(row['lease_method_drilling_days'], errors='coerce')
                api12_drilling = pd.to_numeric(row['api12_method_drilling_days'], errors='coerce')
                calc_diff = pd.to_numeric(row['Drilling_days_difference'], errors='coerce')
                
                if pd.notna(lease_drilling) and pd.notna(api12_drilling) and pd.notna(calc_diff):
                    expected_diff = lease_drilling - api12_drilling
                    if abs(calc_diff - expected_diff) > 0.01:  # Allow small floating point differences
                        logger.warning(f"⚠ Row {idx}: drilling difference calculation may be incorrect")
