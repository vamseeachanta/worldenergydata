...
class TestNetworkFailureScenarios:
    ...
    def test_http_error_codes(self):
        ...
        for error_code in error_codes_to_test:
            with patch.object(self.scraper.session, "get") as mock_get:
                ...
                assert result["status"] == "error"
                assert (
                    str(error_code) in result["error"]
                    or "http" in result["error"].lower()
                )
                assert result["data_source"] == "well_data"
    ...
    def test_partial_download_failures(self):
        ...
        with patch.object(self.scraper.session, "get") as mock_get:
            mock_response = Mock()
            mock_response.raise_for_status = Mock()
            mock_response.iter_content.side_effect = ChunkedEncodingError(
                "Connection broken"
            )
            mock_get.return_value = mock_response
            ...
            assert result["status"] == "error"
            assert (
                "connection broken" in result["error"].lower()
                or "chunked" in result["error"].lower()
            )
    ...
    def test_ssl_certificate_failures(self):
        ...
        with patch.object(self.scraper.session, "get") as mock_get:
            ...
            assert result["status"] == "error"
            assert (
                "ssl" in result["error"].lower()
                or "certificate" in result["error"].lower()
            )
        ...
        with patch.object(self.scraper.session, "get") as mock_get:
            ...
            assert (
                "ssl" in result["error"].lower()
                or "handshake" in result["error"].lower()
            )
