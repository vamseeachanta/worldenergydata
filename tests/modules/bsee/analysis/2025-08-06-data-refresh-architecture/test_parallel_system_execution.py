...
class TestParallelSystemExecution:
    ...
    def setup_method(self):
        ...
        self.legacy_test_path = (
            ...
            / "data"
            / "refresh"
            / "data_refresh_test.py"
        )
        self.enhanced_test_path = (
            self.project_root
            / "tests"
            / "modules"
            / "bsee"
    ...
    def test_both_systems_exist_and_accessible(self):
        ...
        # Test files exist
        assert (
            self.legacy_test_path.exists()
        ), f"Legacy test not found: {self.legacy_test_path}"
        assert (
            self.enhanced_test_path.exists()
        ), f"Enhanced test not found: {self.enhanced_test_path}"

        # Test configs exist
        assert (
            self.legacy_config_path.exists()
        ), f"Legacy config not found: {self.legacy_config_path}"
        assert (
            self.enhanced_config_path.exists()
        ), f"Enhanced config not found: {self.enhanced_config_path}"
    ...
    def test_legacy_system_execution_with_dependency_handling(self):
        ...
        if result.returncode != 0:
            ...
            has_dependency_error = any(
                indicator in error_output for indicator in dependency_indicators
            )

            if has_dependency_error:
                # This is expected - the error is about missing dependencies
                assert (
                    "assetutilities" in error_output
                ), "Should specifically mention missing assetutilities"
            else:
                # Some other error - should still be informative
                assert (
                    len(result.stderr) > 0
                ), "Error occurred but no error message provided"
    ...
    def test_enhanced_system_execution_with_dependency_handling(self):
        ...
        ), "Enhanced system did not complete execution"
...
"""
            ...
            ), f"Legacy mock failed: {legacy_result.stderr}"
            assert (
                enhanced_result.returncode == 0
            ), f"Enhanced mock failed: {enhanced_result.stderr}"
    ...
    def test_configuration_file_independence(self):
        ...
        ), "Enhanced config should be a dictionary"

        # Enhanced config should have enhanced-specific settings
        if "enhanced_mode" in enhanced_config:
            assert (
                enhanced_config["enhanced_mode"] is True
            ), "Enhanced config should have enhanced_mode: True"
    ...
    def test_git_bash_environment_compatibility(self):
        ...
        for var in git_bash_vars:
            ...
            if value is not None:
                ...
                assert isinstance(
                    value, str
                ), f"Environment variable {var} should be string"

        # Test that Python executable is accessible
        python_exe = sys.executable
        assert os.path.exists(
            python_exe
        ), f"Python executable not accessible: {python_exe}"
...
"""
        ...
        try:
            ...
            assert (
                result.returncode == 0
            ), f"Python script execution failed: {result.stderr}"
...
'''
        ...
        try:
            ...
            assert (
                result.returncode == 0
            ), "Error handling test should complete successfully"
            assert (
                "EXPECTED_ERROR:" in result.stdout
            ), "Should report the expected error"
            assert (
                "nonexistent_module_12345" in result.stdout
            ), "Should mention the specific missing module"
    ...
    def test_both_systems_work_in_git_bash_without_conflicts(self):
        ...
        print(
            f"Environment: Windows={is_windows}, Python={python_version.major}.{python_version.minor}"
        )
        ...
        if legacy_result.returncode == 0:
            ...
            if "assetutilities" in legacy_result.stderr.lower():
                print(
                    "SUCCESS: Legacy system correctly reported missing assetutilities dependency"
                )
                legacy_errors_handled = True
            else:
                print(
                    f"WARNING: Legacy system failed with unexpected error: {legacy_result.stderr[:200]}..."
                )
                legacy_errors_handled = (
                    True  # Still counts as handled if it provides an error message
                )
        ...
        if enhanced_result.returncode == 0:
            ...
            if "assetutilities" in enhanced_result.stderr.lower():
                print(
                    "SUCCESS: Enhanced system correctly reported missing assetutilities dependency"
                )
                enhanced_errors_handled = True
            else:
                print(
                    f"WARNING: Enhanced system failed with unexpected error: {enhanced_result.stderr[:200]}..."
                )
                enhanced_errors_handled = (
                    True  # Still counts as handled if it provides an error message
                )
        ...
        if legacy_result.returncode != 0:
            assert (
                len(legacy_result.stderr) > 0
            ), "Legacy system failed but provided no error message"

        if enhanced_result.returncode != 0:
            assert (
                len(enhanced_result.stderr) > 0
            ), "Enhanced system failed but provided no error message"
        ...
        assert (
            legacy_config_content != enhanced_config_content
        ), "Config files should be different"
