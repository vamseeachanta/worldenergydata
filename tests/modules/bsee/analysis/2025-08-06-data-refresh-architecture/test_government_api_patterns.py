...
class TestGovernmentAPIPatterns:
    ...
    def test_standard_api_patterns(self):
        ...
        logger.info(f"Standard API pattern testing complete: {len(successful_endpoints)}/{total_tests} successful")
    ...
    def test_arcgis_rest_patterns(self):
        ...
        self.pattern_tester.document_finding(
            'ArcGIS REST Summary',
            f"ArcGIS REST pattern testing complete",
            summary
        )
    ...
    def test_data_download_patterns(self):
        ...
        self.pattern_tester.document_finding(
            ...
            "Data download pattern testing complete",
            summary
        )
    ...
    def test_authentication_patterns(self):
        ...
        logger.info(f"Authentication testing complete: {len(endpoints_requiring_auth)} endpoints require auth")
        assert True  # Always passes, documents findings
    ...
    def teardown_method(self):
        ...
        if hasattr(self, 'pattern_tester') and self.pattern_tester.findings:
            ...
            successful_tests = len([r for r in self.pattern_tester.test_results 
            ...
            report_lines.append(f"- Success rate: {(successful_tests/total_tests*100):.1f}%" if total_tests > 0 else "- Success rate: 0%")
            report_lines.append("")
            ...
            if self.pattern_tester.test_results:
                ...
                for result in self.pattern_tester.test_results:
                    if result['accessible'] and result['status_code'] == 200:
                        ...
                        report_lines.append(f"- Response Time: {result['response_time']:.2f}s")
                        if result['api_indicators']:
                            report_lines.append(f"- API Indicators: {result['api_indicators']}")
                        report_lines.append("")
