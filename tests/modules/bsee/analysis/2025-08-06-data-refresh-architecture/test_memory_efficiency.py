...
class TestMemoryEfficiency:
    ...
    def test_in_memory_processing_large_files(self):
        ...
        with patch.object(
            self.scraper, "_download_file_to_memory", return_value=large_zip_buffer
        ):
    ...
    def test_streaming_processing_without_local_storage(self):
        ...
        mock_response.iter_content.return_value = [
            test_content[i : i + 8192] for i in range(0, len(test_content), 8192)
        ]
    ...
    def test_no_large_files_stored_in_repository(self):
        ...
        try:
            ...
            with patch.object(
                self.scraper, "_download_file_to_memory", return_value=large_zip
            ):
                source = self.scraper.data_sources["well_data"]
                ...
                assert (
                    len(large_files) == 0
                ), f"Large files created during processing: {large_files}"
                ...
                mock_result = {
                    ...
                    "data": {
                        ...
                        "files": {},
                        "dataframes": {
                            "test.csv": result.get("dataframes", {}).get(
                                "test_data.csv"
                            )
                        },
                ...
                saved_files = self.scraper.save_processed_data(
                    mock_result, temp_dir, legacy_compatible=True
                )
                ...
                for file_path in saved_files:
                    ...
                    assert (
                        file_size_mb < 50
                    ), f"Saved file too large: {file_path} ({file_size_mb:.1f}MB)"
    ...
    def test_memory_consumption_optimization(self):
        ...
        with patch("gc.collect") as mock_gc:
            ...
            with patch.object(
                self.scraper, "_download_file_to_memory", return_value=mock_zip
            ):
                with patch.object(
                    self.scraper,
                    "_process_zip_in_memory",
    ...
    def test_temporary_file_cleanup(self):
        ...
        assert (
            len(new_temp_files) == 0
        ), f"Temporary files not cleaned up: {new_temp_files}"
    ...
    def test_memory_efficiency_and_repository_constraints_integration(self):
        ...
        scraper_methods = [
            method
            for method in dir(self.scraper)
...
if __name__ == "__main__":
    ...
    pytest.main(
        [
            __file__ + "::TestMemoryEfficiency::test_in_memory_processing_large_files",
            "-v",
        ]
    )
