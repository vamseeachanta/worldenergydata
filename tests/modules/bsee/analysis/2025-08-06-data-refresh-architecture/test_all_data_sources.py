...
class DataSourceTester:
        ...
        config = {
            ...
            'parameters': {
                'filepath': {
                    ...
                    'production': {'bin': self.data_sources['production']['output_dir']},
    ...
    def test_single_source(self, source_name):
        ...
        try:
            ...
            if success:
                logger.success(f"✓ {source['description']} processing completed successfully")
            else:
            ...
            try:
                with open(file_path, 'rb') as f:
                    ...
                    if isinstance(data, pd.DataFrame):
                        logger.info(f"  ✓ {bin_file}: {file_size:,} bytes, shape={data.shape}")
                        valid_files += 1
                    else:
                        logger.info(f"  ✓ {bin_file}: {file_size:,} bytes, type={type(data).__name__}")
                        valid_files += 1
    ...
    def print_summary(self):
        ...
        for source_name, result in self.test_results.items():
            ...
            if result.get('success', False):
                ...
                logger.success(f"✓ {source_desc}: PASSED ({files_count} files processed)")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--source', choices=['well', 'production', 'war', 'all'], 
                       default='well',
