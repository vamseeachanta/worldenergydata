...
from src.worldenergydata.modules.bsee.data.enhanced.data_refresh_chunked import DataRefreshChunked
...
def test_chunked_refresh():
    ...
    cfg = {
        ...
        'parameters': {
            'filepath': {
                'apm': {
                    'bin': 'data/test/bin/apd'
                },
                'production': {
                    'bin': 'data/test/bin/production'
                },
                'war': {
                    'bin': 'data/test/bin/war'
                }
            }
        }
    }
    ...
    logger.info(f"  Data types: well={cfg['data']['well']}, production={cfg['data']['production']}, war={cfg['data']['war']}")
...
def test_bandwidth_savings():
    ...
    for scenario in scenarios:
        ...
        logger.info(f"  Bandwidth saved: {savings:.0f} MB ({(savings/total_without_cache)*100:.1f}%)")
        
        # Time savings (assuming 10 Mbps connection)
        time_saved_minutes = (savings * 8) / (10 * 60)  # Convert MB to minutes at 10 Mbps
