...
def test_timeout_configuration():
    ...
    assert (
        scraper.TIMEOUTS["production"] == 1200
    ), "Production data timeout should be 20 minutes"
    ...
    for data_type in ["well", "production", "war"]:
        ...
        if "error" not in info:
            ...
            if data_type == "production" and size_mb > 10:
                assert (
                    scraper.TIMEOUTS["production"] >= 1200
                ), "Production timeout too small for file size"
            if data_type == "war" and size_mb > 50:
                assert (
                    scraper.TIMEOUTS["war"] >= 2400
                ), "WAR timeout too small for file size"
        else:
...
def test_download_with_timeout_simulation():
    ...
    for data_type in ["well", "production"]:
        ...
        if "size_mb" in info:
            ...
            logger.info(
                f"  Min time needed (at 1 MB/s): {min_required_time:.0f} seconds"
            )
