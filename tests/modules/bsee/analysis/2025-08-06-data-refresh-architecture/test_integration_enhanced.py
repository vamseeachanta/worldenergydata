...
class IntegrationTester:
    ...
    def test_initialization(self):
        ...
        try:
            ...
            assert hasattr(
                data_refresh, "memory_processor"
            ), "memory_processor not initialized"
            assert hasattr(
                data_refresh, "config_router"
            ), "config_router not initialized"
    ...
    def test_file_processing(self, source_type="well"):
        ...
        expected_files = {
            "well": [
                "mv_apddata_all.txt",
                "mv_apdcasing_all.txt",
                "mv_apdperforation_all.txt",
            ],
            ...
            try:
                with open(file_path, "rb") as f:
                    ...
                    if hasattr(data, "shape"):
                        logger.info(
                            f"  {bin_file}: {file_size/1024:.1f} KB, shape: {data.shape}"
                        )
                    else:
