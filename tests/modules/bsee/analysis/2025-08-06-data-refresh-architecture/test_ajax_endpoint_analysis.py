...
class AJAXEndpointAnalyzer:
    ...
    def _analyze_javascript_code(self, soup, result):
        ...
        for script in scripts:
            if script.string:
                ...
                url_matches = re.findall(r'url\s*:\s*["\']([^"\']+)["\']', script_content, re.IGNORECASE)
                for url_match in url_matches:
                    if any(ext in url_match.lower() for ext in ['.aspx', '.ashx', '.asmx', '/api/', 'json']):
                        full_url = urljoin(result['url'], url_match)
                        result['json_endpoints_found'].append(full_url)
                
                # Look for fetch() calls
                fetch_matches = re.findall(r'fetch\s*\(\s*["\']([^"\']+)["\']', script_content, re.IGNORECASE)
    ...
    def _analyze_callback_mechanisms(self, page_content, result):
        ...
        viewstate_match = re.search(r'__VIEWSTATE["\']?\s*value\s*=\s*["\']([^"\']+)["\']', page_content)
        if viewstate_match:
            callback_info = {
        ...
        event_validation = re.search(r'__EVENTVALIDATION["\']?\s*value\s*=\s*["\']([^"\']+)["\']', page_content)
        if event_validation:
            callback_info = {
    ...
    def analyze_network_patterns(self, page_content: str) -> Dict:
        ...
        for match in json_matches[:10]:  # Limit to first 10
            try:
                json.loads(match)
                result['json_data_structures'].append(match[:100] + '...' if len(match) > 100 else match)
...
class TestAJAXEndpointAnalysis:
    ...
    def test_web_interface_ajax_analysis(self):
            ...
            if result['analysis_successful']:
                ...
                self.ajax_analyzer.document_finding(
                    ...
                    f"Analyzed {interface_url}",
                    {
                        ...
                        'callback_mechanisms_count': len(result['callback_mechanisms_found']),
                        'devexpress_controls_count': len(result['devexpress_controls_found'])
                    }
                )
        ...
        summary = {
            ...
            'interfaces_with_ajax': len([i for i in analyzed_interfaces if i['ajax_calls_found']]),
            'interfaces_with_devexpress': len([i for i in analyzed_interfaces if i['devexpress_controls_found']])
        }
        
        self.ajax_analyzer.document_finding(
            'AJAX Analysis Summary',
            f"Web interface AJAX analysis complete",
            summary
        )
        
        logger.info(f"AJAX analysis complete: {total_ajax_calls} AJAX calls, {total_endpoints} endpoints found")
    ...
    def test_discovered_endpoint_validation(self):
        ...
        logger.info(f"Endpoint validation complete: {len(validation_result['working_endpoints'])} working endpoints")
        assert True  # Always passes, documents findings
    ...
    def test_devexpress_callback_analysis(self):
        ...
        summary = {
            ...
            'unique_control_types': list(set([ctrl for finding in devexpress_findings 
    ...
    def test_network_pattern_analysis(self):
        ...
        for interface_url in self.ajax_analyzer.target_interfaces:
            try:
                ...
                if response.status_code == 200:
                    network_analysis = self.ajax_analyzer.analyze_network_patterns(response.text)
                    ...
                    self.ajax_analyzer.document_finding(
                        ...
                        {
                            'async_patterns_count': len(network_analysis['async_patterns_found']),
                            'loading_indicators_count': len(network_analysis['data_loading_indicators']),
                            'json_structures_count': len(network_analysis['json_data_structures'])
                        }
                    )
    ...
    def teardown_method(self):
        ...
        if hasattr(self, 'ajax_analyzer') and self.ajax_analyzer.findings:
            ...
            report_lines.append(f"- Total research findings: {len(self.ajax_analyzer.findings)}")
            report_lines.append("")
            ...
            if self.ajax_analyzer.callback_mechanisms:
                ...
                for callback in self.ajax_analyzer.callback_mechanisms[:10]:
                    report_lines.append(f"- **Type**: {callback.get('type', 'unknown')}")
                    report_lines.append(f"  - **Target**: {callback.get('target', callback.get('mechanism', 'N/A'))}")
                    report_lines.append("")
