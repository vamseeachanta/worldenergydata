...
class BSEEDataScraper:
    ...
    def download_and_process(self, data_source: str, force_refresh: bool = False) -> Dict:
        """
        ...
        try:
            ...
            logger.info(f"Successfully processed {source.display_name} in {processing_time:.1f}s")
            ...
            result = {
                ...
                'total_records': sum(len(df) for df in processed_data.get('dataframes', {}).values() if df is not None)
            }
    ...
    def _download_file_to_memory(self, source: BSEEDataSource) -> BinaryIO:
        ...
        for attempt in range(self.max_retries + 1):
            try:
                logger.info(f"Downloading {source.display_name} (attempt {attempt + 1}/{self.max_retries + 1})")
                
                # Stream download to handle large files efficiently
                response = self.session.get(source.url, stream=True, timeout=self.timeout)
                ...
                if content_length:
                    ...
                    if size_mb > self.max_memory_mb:
                        warnings.warn(f"File size ({size_mb:.1f}MB) exceeds memory limit ({self.max_memory_mb}MB)")
                    ...
                    if chunk:  # Filter out keep-alive chunks
                        ...
                        if downloaded % (1024 * 1024) == 0:  # Every MB
                            ...
                            if content_length:
                                ...
                                logger.debug(f"Downloaded {mb_downloaded:.1f}MB ({percent:.1f}%)")
                ...
                if attempt < self.max_retries:
                    ...
                    raise RequestException(f"Failed to download after {self.max_retries + 1} attempts: {e}")
            
            except Exception as e:
                if attempt < self.max_retries:
                    wait_time = 2 ** attempt
                    logger.warning(f"Download error (attempt {attempt + 1}): {e}. Retrying in {wait_time}s...")
                    time.sleep(wait_time)
                else:
                    raise
    ...
    def _process_zip_in_memory(self, zip_buffer: BinaryIO, source: BSEEDataSource) -> Dict:
        """
            ...
            with zipfile.ZipFile(zip_buffer, 'r') as zip_ref:
                ...
                for file_name in file_list:
                    ...
                    if file_ext in ['.txt', '.csv', '.dat', '.tsv']:
                        try:
                            file_data = self._process_data_file(zip_ref, file_name, source)
                            if file_data is not None:
                                ...
                                if file_data.get('is_structured', False):
                                    df = self._create_dataframe_from_file(file_data, source)
                                    if df is not None:
                ...
                processed_data['metadata']['file_types'] = list(processed_data['metadata']['file_types'])
                
                logger.info(f"Processed {processed_data['metadata']['data_files_found']} data files")
    ...
    def _process_data_file(self, zip_ref: zipfile.ZipFile, file_name: str, source: BSEEDataSource) -> Optional[Dict]:
        """
        ...
        try:
            with zip_ref.open(file_name, 'r') as file_handle:
                ...
                is_structured = self._is_structured_data(non_empty_lines, source.delimiter)
                ...
                if is_structured and len(non_empty_lines) > 1:  # Has header + data
                    ...
                    file_data['estimated_columns'] = len(non_empty_lines[0].split(source.delimiter))
    ...
    def _is_structured_data(self, lines: List[str], delimiter: str) -> bool:
        ...
        delimiter_counts = [line.count(delimiter) for line in lines[:10]]  # Check first 10 lines
        ...
        consistent_lines = sum(1 for count in non_zero_counts if abs(count - avg_count) <= 2)
        
        return consistent_lines / len(non_zero_counts) >= 0.7  # 70% consistency threshold
    
    def _create_dataframe_from_file(self, file_data: Dict, source: BSEEDataSource) -> Optional[pd.DataFrame]:
        """
        ...
        try:
            ...
            logger.warning(f"Could not create DataFrame from {file_data['file_name']}: {e}")
            return None
    ...
    def _needs_refresh(self, data_source: str) -> bool:
        ...
        if source.update_frequency == 'daily':
            refresh_interval = timedelta(hours=6)  # Refresh every 6 hours for daily updates
        elif source.update_frequency == 'bi-monthly':
            refresh_interval = timedelta(days=7)   # Refresh weekly for bi-monthly updates
        else:
            refresh_interval = timedelta(hours=12)  # Default: 12 hours
        
        return datetime.now() - last_download > refresh_interval
    
    def save_processed_data(self, processed_data: Dict, output_dir: str, legacy_compatible: bool = True) -> List[str]:
        """
        ...
        if processed_data['status'] != 'success':
            raise ValueError(f"Cannot save failed processing result: {processed_data.get('error', 'Unknown error')}")
        ...
        if legacy_compatible:
            ...
            if not dataframes:
                logger.warning(f"No DataFrames found in processed data for {data_source}")
                # Create a consolidated DataFrame if individual files aren't available
                all_data = []
                for file_name, file_data in processed_data_dict.get('files', {}).items():
                    if file_data.get('is_structured', False) and 'content' in file_data:
                        try:
                            # Parse the content into DataFrame
                            df = self._create_dataframe_from_file(file_data, 
                                self.data_sources[data_source] if data_source in self.data_sources else None)
                            if df is not None:
                                all_data.append(df)
                        except Exception as e:
                            logger.warning(f"Failed to create DataFrame from {file_name}: {e}")
    ...
    def save_processed_data_legacy_format(self, processed_data: Dict, output_dir: str) -> List[str]:
        """
    ...
    def get_statistics(self) -> Dict:
        ...
        return {
            **self.stats,
            'success_rate': (self.stats['downloads_successful'] / max(self.stats['downloads_attempted'], 1)) * 100,
            'average_processing_time': self.stats['total_processing_time'] / max(self.stats['downloads_successful'], 1),
