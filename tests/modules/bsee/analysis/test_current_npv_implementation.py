...
class TestCurrentNPVImplementation:
    ...
    def setup_method(self):
        ...
        self.sample_revenue_df = pd.DataFrame({
            ...
            'Avg Price (USD/bbl)': ['$65.00', '$68.00', '$70.00', '$72.00', '$75.00'],
            'Revenue (USD)': ['$6,500,000.00', '$6,460,000.00', '$6,300,000.00', '$6,120,000.00', '$6,000,000.00']
        })
    ...
    def test_current_npv_calculation_baseline(self):
        ...
        with patch('pandas.DataFrame.to_csv'), \
            ...
            npv_result = self.analyzer.perform_npv_calculation(self.test_cfg, self.sample_revenue_df)
        ...
        print(f"Discount Rate: {self.test_cfg['economics']['cost']['discount_rate_annual']*100}%")
    ...
    def test_cash_flow_construction(self):
        ...
        with patch.object(self.analyzer, 'perform_npv_calculation') as mock_npv:
            ...
            assert 'Net Cash Flow' in result_df.columns, "Net Cash Flow should be calculated"
            
            # Check OPEX calculation
            expected_opex_0 = result_df['Monthly Oil Production'].iloc[0] * self.test_cfg['economics']['cost']['OPEX']
            actual_opex_0 = result_df['OPEX'].iloc[0]
            assert abs(expected_opex_0 - actual_opex_0) < 0.01, f"OPEX calculation mismatch: expected {expected_opex_0}, got {actual_opex_0}"
    ...
    def test_discount_rate_application(self):
        ...
        simple_cash_flows = [-1000000, 300000, 300000, 300000, 300000]  # Simple 4-year project
        discount_rate = 0.10
        ...
        print(f"% Difference: {abs(npf_result - excel_npv) / abs(excel_npv) * 100:.2f}%")
    ...
    def test_period_timing_assumptions(self):
        ...
        net_cash_flows = [100000, 95000, 90000, 85000, 80000]  # Sample monthly net cash flows
        ...
        print(f"Operating Cash Flows (Periods 1-5): {[f'${cf:,.0f}' for cf in net_cash_flows]}")
    ...
    def test_current_implementation_full_workflow(self):
        ...
        with patch('pandas.DataFrame.to_csv'), \
            ...
            mock_excel.return_value = pd.DataFrame({
                'col_2': [np.nan, np.nan, 65.0, 68.0, 70.0, 72.0, 75.0]  # Mock BRENT prices
            })
    ...
    def test_excel_benchmark_comparison(self):
        ...
        excel_aligned_cfg['economics']['cost']['discount_rate_annual'] = excel_discount_rate
        excel_aligned_cfg['economics']['cost']['CAPEX'] = excel_capex
        
        with patch('pandas.DataFrame.to_csv'), \
             patch('os.path.join', return_value='mock_path'):
            
            current_npv = self.analyzer.perform_npv_calculation(excel_aligned_cfg, self.sample_revenue_df)
