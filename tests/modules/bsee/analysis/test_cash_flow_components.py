...
class TestCashFlowComponents:
    ...
    def sample_production_data(self):
        ...
        production_volumes = [1000, 1200, 1100, 900, 850, 800, 750, 700, 650, 600, 550, 500,
    ...
    def sample_oil_prices(self):
        ...
        return [65.0, 66.5, 68.0, 70.0, 72.0, 69.0, 67.0, 65.0, 63.0, 62.0, 61.0, 60.0,
                58.0, 59.0, 61.0, 63.0, 65.0, 67.0, 68.0, 66.0, 64.0, 62.0, 60.0, 59.0]
    ...
    def test_revenue_calculation_basic(self, sample_production_data, sample_oil_prices):
        ...
        assert revenues[0] == expected_first_revenue, f"Expected ${expected_first_revenue}, got ${revenues[0]}"
    ...
    def test_opex_calculation_basic(self, sample_production_data, config_with_economics):
        ...
        assert opex_values[0] == expected_first_opex, f"Expected ${expected_first_opex}, got ${opex_values[0]}"
    ...
    def test_net_cash_flow_calculation(self, sample_production_data, sample_oil_prices, config_with_economics):
        ...
        assert net_cash_flows[0] == expected_ncf, f"Expected ${expected_ncf}, got ${net_cash_flows[0]}"
        
        # Verify all net cash flows are positive (assuming oil price > OPEX/bbl)
        assert all(ncf > 0 for ncf in net_cash_flows), "All net cash flows should be positive given price > OPEX"
    
    def test_zero_production_edge_case(self):
        """Test edge case with zero production."""
        zero_production = pd.DataFrame({
            'PRODUCTION_DATETIME': pd.date_range(start='2018-01-01', periods=3, freq='ME'),
    ...
    def test_negative_oil_price_edge_case(self):
        ...
        production = pd.DataFrame({
            'PRODUCTION_DATETIME': pd.date_range(start='2020-04-01', periods=3, freq='ME'),
        ...
        assert revenues[1] == 1000 * (-37.0), "Revenue calculation should handle negative prices"
        
        # OPEX should still be positive
        assert opex_values[1] > 0, "OPEX should remain positive even with negative oil price"
        
        # Net cash flow should be very negative
        assert ncf_values[1] < revenues[1], "Net cash flow should be more negative than revenue"
    
    def test_missing_data_edge_case(self):
        """Test edge case with missing production or price data."""
        production = pd.DataFrame({
            'PRODUCTION_DATETIME': pd.date_range(start='2018-01-01', periods=5, freq='ME'),
        ...
        assert pd.isna(ncf_values[4]), "Missing production should result in NaN net cash flow"
    
    def test_cash_flow_dataframe_structure(self, sample_production_data, sample_oil_prices, config_with_economics):
        ...
        expected_columns = ['Month', 'Date', 'Production_BBL', 'Oil_Price_USD_per_BBL', 
                          'Revenue_USD', 'OPEX_USD', 'Net_Cash_Flow_USD']
        assert list(cash_flow_df.columns) == expected_columns, "Cash flow DataFrame should have expected columns"
        
        # Verify data types
        assert cash_flow_df['Revenue_USD'].dtype in [np.float64, float], "Revenue should be numeric"
        assert cash_flow_df['OPEX_USD'].dtype in [np.float64, float], "OPEX should be numeric"
        assert cash_flow_df['Net_Cash_Flow_USD'].dtype in [np.float64, float], "Net cash flow should be numeric"
        ...
        for _, row in cash_flow_df.iterrows():
            ...
            assert abs(row['Revenue_USD'] - expected_revenue) < 0.01, "Revenue calculation mismatch"
            assert abs(row['OPEX_USD'] - expected_opex) < 0.01, "OPEX calculation mismatch"
            assert abs(row['Net_Cash_Flow_USD'] - expected_ncf) < 0.01, "Net cash flow calculation mismatch"
    ...
    def test_cash_flow_with_capex_period_zero(self, config_with_economics):
        ...
        monthly_ncf = [45000, 54000, 49500, 40500, 38250, 36000, 33750, 31500, 
                      29250, 27000, 24750, 22500, 21600, 20700, 19800, 18900,
                      18000, 17100, 16200, 15300, 14400, 13500, 12600, 11700]
        ...
        assert all(cf > 0 for cf in cash_flows[1:]), "All operational cash flows should be positive"
        
        # Verify total cash flow
        total_operational_cf = sum(cash_flows[1:])
        net_total = sum(cash_flows)
        
        assert total_operational_cf > 0, "Total operational cash flow should be positive"
        assert net_total < total_operational_cf, "Net total should be less than operational due to CAPEX"
...
class TestProductionAPI12CashFlowMethods:
    ...
    def test_revenue_table_generation_structure(self, analyzer, config_with_economics):
        ...
        production_df = pd.DataFrame({
            'PRODUCTION_DATETIME': pd.date_range(start='2018-01-01', periods=12, freq='ME'),
        ...
        expected_columns = ['Month', 'Monthly Oil Production', 'Avg Price (USD/bbl)', 'Revenue (USD)']
        ...
        # Verify structure
        assert list(revenue_table.columns) == expected_columns, "Revenue table should have expected columns"
        assert len(revenue_table) == 12, "Revenue table should have 12 months"
        assert all(revenue_table['Revenue (USD)'] > 0), "All revenues should be positive"
