...
class ExcelVsTask8Comparison:
    ...
    def load_dataframes(self):
        ...
        logger.info(
            f"Excel CSV loaded: {len(df_excel)} rows, {len(df_excel.columns)} columns"
        )

        # Load Task 8 data
        df_task8 = pd.read_csv(self.task8_csv_path)
        logger.info(
            f"Task 8 CSV loaded: {len(df_task8)} rows, {len(df_task8.columns)} columns"
        )
    ...
    def align_dataframes(self, df_excel, df_task8):
        ...
        logger.info(
            f"Aligned DataFrames to {min_rows} rows and {len(column_order)} columns"
        )
    ...
    def calculate_variances(self, df_excel, df_task8, columns):
        ...
        for col in columns:
            ...
            try:
                ...
                percent_diff = (absolute_diff / task8_vals * 100).replace(
    ...
    def create_comparison_report(self, df_excel, df_task8, variance_results):
        ...
        for metric in key_metrics:
            if metric in variance_results:
                ...
                report_lines.append(
                    f"| {metric} | {excel_total:,.2f} | {task8_total:,.2f} | {abs_diff:,.2f} | {pct_diff:.2f}% |"
                )
        ...
        if "Monthly_production_BBL" in variance_results:
            ...
            report_lines.append(
                f"- Difference: {prod_var['absolute_diff_sum']:,.0f} BBL ({prod_var['absolute_diff_sum']/prod_var['task8_sum']*100:.1f}%)"
            )
            ...
            report_lines.append(
                f"- Excel average per period: {excel_avg_prod:,.0f} BBL"
            )
            report_lines.append(
                f"- Task 8 average per period: {task8_avg_prod:,.0f} BBL"
            )
            ...
            if excel_avg_prod < 200000:  # Likely daily data
                ...
                report_lines.append(
                    f"- If multiplied by 30.4: {excel_avg_prod * 30.4:,.0f} BBL/month"
                )
        ...
        if "Oil_price_USD" in df_excel.columns and "Oil_price_USD" in df_task8.columns:
            ...
            report_lines.append(
                f"- Price difference: ${abs(excel_prices.mean() - task8_prices.mean()):.2f}/BBL"
            )
    ...
    def save_comparison_results(self, report, df_excel, df_task8, variance_results):
        ...
        report_path = os.path.join(
            self.output_folder, f"excel_vs_task8_comparison_{timestamp}.md"
        )
        ...
        variance_path = os.path.join(
            self.output_folder, f"excel_vs_task8_variance_{timestamp}.csv"
        )
        ...
        comparison_path = os.path.join(
            self.output_folder, f"excel_vs_task8_sidebyside_{timestamp}.csv"
        )
    ...
    def run_comparison(self):
        ...
        # Align DataFrames
        df_excel_aligned, df_task8_aligned, columns = self.align_dataframes(
            df_excel, df_task8
        )

        # Calculate variances
        variance_results = self.calculate_variances(
            df_excel_aligned, df_task8_aligned, columns
        )
        ...
        if "Monthly_production_BBL" in variance_results:
            ...
            print(
                f"  Variance: {abs(prod_var['absolute_diff_sum']/prod_var['task8_sum']*100):.1f}%"
            )
...
def test_excel_vs_task8_comparison():
    ...
    if not Path(comparator.excel_csv_path).exists():
        pytest.skip(f"Excel CSV not found: {comparator.excel_csv_path}")
    if not Path(comparator.task8_csv_path).exists():
        pytest.skip(f"Task 8 CSV not found: {comparator.task8_csv_path}")
