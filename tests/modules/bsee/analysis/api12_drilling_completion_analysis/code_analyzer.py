    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.FunctionDef):
            func_info = {
                ...
                "decorators": [
                    decorator.id if hasattr(decorator, "id") else str(decorator)
...
def analyze_data_sources(script_path: str) -> Dict[str, List[str]]:
    ...
    data_sources = {"database_queries": [], "file_operations": [], "api_calls": []}
...
def analyze_drilling_completion_logic(script_path: str) -> Dict[str, List[str]]:
    ...
    key_var_patterns = [r"API\w*", r"WELL\w*", r"LEASE\w*", r"DATE\w*", r"DAYS\w*"]
...
def compare_methodologies(
    lease_script_path: str, api12_script_path: str
) -> Dict[str, Any]:
    """
...
def generate_methodology_summary(
    ...
    summary = {
        "lease_method": {
            ...
            "drilling_completion_logic": analyze_drilling_completion_logic(
                lease_script_path
            ),
        ...
        "key_differences": identify_key_differences(
            lease_script_path, api12_script_path
        ),
    }
