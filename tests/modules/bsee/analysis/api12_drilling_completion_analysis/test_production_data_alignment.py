...
class TestProductionDataAlignment:
    ...
    def test_excel_production_data_identification(self, excel_file_path):
        ...
        df = pd.read_excel(excel_file_path, sheet_name="NPV w Mo'ly data chart", engine='openpyxl')
        
        # Production data should be in row 22 (JSM Total AVGMoly)
        production_label = df.iloc[22, 1]  # Column 1 should have the label
        assert isinstance(production_label, str), f"Expected string label, got {type(production_label)}"
        assert "JSM" in production_label and "Total" in production_label, f"Expected JSM Total label, got '{production_label}'"
    ...
    def test_excel_production_data_extraction(self, excel_file_path):
        ...
        assert len(production_data) >= 12, f"Expected at least 12 months of production, got {len(production_data)}"
        
        # Validate production ranges (reasonable production volumes)
        assert all(prod > 1000 for prod in production_data), "Some production values seem too low"
        assert all(prod < 200000 for prod in production_data), "Some production values seem too high"
        
        print(f"Extracted {len(production_data)} production values: {production_data[:5]}...{production_data[-5:]}")
    ...
    def test_production_data_with_known_values(self, excel_file_path):
        ...
        expected_first_values = [19123.987142857142, 32642.056878306877, 49933.10769230769]
        
        assert len(production_data) >= 3, f"Need at least 3 values for validation, got {len(production_data)}"
        ...
        for i, expected_prod in enumerate(expected_first_values):
            ...
            assert abs(actual_prod - expected_prod) < 1.0, f"Production {i}: expected {expected_prod:.2f}, got {actual_prod:.2f}"
    ...
    def test_production_period_alignment_with_prices(self, excel_file_path):
        ...
        for col_idx in range(2, min(df.shape[1], 60)):
            ...
            if (pd.notna(prod_val) and isinstance(prod_val, (int, float)) and prod_val > 0 and
                pd.notna(price_val) and isinstance(price_val, (int, float)) and 20 < price_val < 200):
                production_data.append(prod_val)
                price_data.append(price_val)
        
        # Validate alignment
        assert len(production_data) == len(price_data), f"Production-price mismatch: {len(production_data)} vs {len(price_data)}"
        assert len(production_data) >= 12, f"Expected at least 12 aligned periods, got {len(production_data)}"
    ...
    def test_production_data_scaling_calibration(self, excel_file_path):
        ...
        assert 5000 < avg_monthly_prod < 200000, f"Average monthly production {avg_monthly_prod:.0f} seems outside expected range"
        ...
        if avg_monthly_prod < 1000:  # Data might be in thousands
            ...
            print(f"Scaled production (x{scaling_factor}): average {scaled_avg:.0f} barrels/month")
        else:
            scaling_factor = 1.0
    ...
    def test_production_data_length_consistency(self, excel_file_path):
        ...
        for col_idx in range(2, min(df.shape[1], 60)):
            ...
            if (pd.notna(month_val) and 
                pd.notna(price_val) and isinstance(price_val, (int, float)) and 20 < price_val < 200 and
                pd.notna(prod_val) and isinstance(prod_val, (int, float)) and prod_val > 0):
                months.append(str(month_val))
                prices.append(price_val)
                production.append(prod_val)
        
        # Validate consistency
        assert len(months) == len(prices) == len(production), f"Length mismatch: months={len(months)}, prices={len(prices)}, production={len(production)}"
        assert len(months) >= 12, f"Expected at least 12 consistent periods, got {len(months)}"
    ...
    def test_production_data_missing_value_handling(self, excel_file_path):
        ...
        valid_production = [val for val in all_production_cells if pd.notna(val) and isinstance(val, (int, float)) and val > 0]
        invalid_production = [val for val in all_production_cells if pd.isna(val) or not isinstance(val, (int, float)) or val <= 0]
    ...
    def test_production_data_extraction_function(self, excel_file_path):
        ...
        assert len(extracted_production) >= 12, f"Expected at least 12 production values, got {len(extracted_production)}"
        assert all(isinstance(p, (int, float)) for p in extracted_production), "All production values should be numeric"
        
        # Verify some expected values
        assert abs(extracted_production[0] - 19123.987142857142) < 1.0, f"First production value mismatch"
        assert abs(extracted_production[1] - 32642.056878306877) < 1.0, f"Second production value mismatch"
        
        print(f"Integration test passed: extracted {len(extracted_production)} production values")
    ...
    def test_production_data_revenue_calculation_alignment(self, excel_file_path):
        ...
        assert len(aligned_data) >= 12, f"Expected at least 12 aligned data points, got {len(aligned_data)}"
        ...
        for i, data_point in enumerate(aligned_data):
            ...
            assert abs(actual_revenue - expected_revenue) < 0.01, f"Revenue calculation error at period {i}"
        ...
        assert avg_monthly_revenue > 500000, f"Average monthly revenue ${avg_monthly_revenue:,.0f} seems too low"
        assert avg_monthly_revenue < 10000000, f"Average monthly revenue ${avg_monthly_revenue:,.0f} seems too high"
        
        print(f"Revenue alignment test passed: average monthly revenue ${avg_monthly_revenue:,.0f}")
