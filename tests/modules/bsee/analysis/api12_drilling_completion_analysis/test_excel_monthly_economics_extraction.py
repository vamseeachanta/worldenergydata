...
class ExcelMonthlyEconomicsExtractor:
            ...
            for idx, label in enumerate(df_excel.iloc[:, 0]):
                if pd.notna(label):
                    ...
                    if "well" in label_str and "count" in label_str:
                        ...
                        "npv" in label_str and idx > 30
                    ):  # NPV calculations usually later in sheet
                        data_rows["npv"] = idx
            ...
            for data_type, row_idx in data_rows.items():
                if row_idx is not None:
                    ...
                    if data and any(v > 0 for v in data):
                        ...
                        if non_zero_data:
                            logger.info(
                                f"Extracted {data_type}: {len(data)} values, range: {min(non_zero_data):,.2f} - {max(non_zero_data):,.2f}"
                            )
                        else:
                            logger.info(
                                f"Extracted {data_type}: {len(data)} values (all zeros)"
                            )
    ...
    def create_monthly_economics_dataframe(self, timeline, extracted_data):
        ...
        for i in range(len(timeline)):
            ...
            wells_total = int(
                initial_wells + (final_wells - initial_wells) * well_progression
            )
            ...
            df_data["Cumulative_cash_flow_after_OPEX"].append(
                cumulative_cash_flow + cumulative_capex
            )
            df_data["Cumulative_NPV"].append(
                0.0
            )  # Placeholder - would need NPV calculation
    ...
    def run_extraction(self):
        ...
        logger.info(
            f"Total production: {df_economics['Monthly_production_BBL'].sum():,.0f} BBL"
        )
...
def test_excel_monthly_economics_extraction():
    ...
    if not Path(extractor.excel_file_path).exists():
        pytest.skip(f"Excel data file not found: {extractor.excel_file_path}")
    df_economics, csv_path = extractor.run_extraction()

    # Validate extraction
    assert len(df_economics) > 0, "Should extract economic data"
    assert (
        "Monthly_production_BBL" in df_economics.columns
    ), "Should have production column"
    assert "Oil_price_USD" in df_economics.columns, "Should have oil price column"
    assert (
        df_economics["Monthly_production_BBL"].sum() > 0
