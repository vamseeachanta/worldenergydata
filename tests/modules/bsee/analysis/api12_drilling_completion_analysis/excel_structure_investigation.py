...
def investigate_excel_structure():
    ...
    try:
        ...
        for row_idx in range(min(20, len(df))):
            ...
            row_desc = (
                df.iloc[row_idx, 0] if pd.notna(df.iloc[row_idx, 0]) else "[empty]"
            )
        ...
        for row_idx in range(3, min(15, len(df))):
            ...
            if len(numeric_vals) > 20:  # Row has substantial data
                ...
                print(
                    f"         Range: {min(numeric_vals):,.1f} - {max(numeric_vals):,.1f}"
                )
            ...
            for col_idx in range(len(df.columns)):
                ...
                if pd.notna(val) and isinstance(val, (int, float)):
                    if abs(abs(val) - target_npv) < target_npv * 0.1:  # Within 10%
                        print(
                            f"Found potential NPV at Row {row_idx}, Col {col_idx}: {val:,.2f}"
                        )
