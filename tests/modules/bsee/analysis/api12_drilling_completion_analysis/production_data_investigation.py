    ...
    try:
        ...
        for row_idx in candidate_rows:
            ...
            for scale in scaling_factors:
                ...
                print(
                    f"  Scale {scale:>7,}x: NPV=${npv_result:>15,.0f} | Var={variance_pct:>6.1f}% | Rev=${total_revenue:>12,.0f} | OPEX=${total_opex:>12,.0f}"
                )
        ...
        if len(row_12_data) >= len(brent_prices):
            ...
            for year in range(annual_periods):
                ...
                year_price = (
                    sum(brent_prices[start_idx:end_idx]) / months_per_year
                )  # Average price
        ...
        print(
            "3. Check if there are other cost components not included in our calculation"
        )
        print(
            "4. Verify the exact NPV formula used in Excel (different period timing?)"
        )
