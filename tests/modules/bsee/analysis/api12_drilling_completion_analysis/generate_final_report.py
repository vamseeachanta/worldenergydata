...
import pandas as pd
from enhanced_report_generator import generate_enhanced_comprehensive_report
from report_generator import (
    compile_comprehensive_report,
    export_analysis_data,
    save_report_to_file,
)
from root_cause_analyzer import load_actual_comparison_data
...
def main():
    ...
    print(
        f"Average drilling difference: {comparison_data['drilling_diff'].mean():.1f} days"
    )
    print(
        f"Average completion difference: {comparison_data['completion_diff'].mean():.1f} days"
    )
    print(
        f"Wells with extreme differences (>200 days): {len(comparison_data[comparison_data['total_diff'] > 200])}"
    )
    print(
        f"Report generation completed at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    )
    print("=" * 60)
