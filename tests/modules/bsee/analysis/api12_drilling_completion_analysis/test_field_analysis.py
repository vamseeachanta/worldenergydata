...
class TestFieldAnalysis:
    ...
    def test_field_analysis_with_actual_data(self):
        ...
        assert summary['summary']['total_fields'] > 5  # We know there are multiple fields
        
        # Test that major fields are present
        expected_fields = ['Anchor', 'Cascade', 'Chinook', 'Stones', 'St Malo']
        for field in expected_fields:
            if field in summary['summary']['fields']:  # Some fields might not be in this dataset
        ...
        for field_name, field_data in summary['field_analysis'].items():
            ...
            for i, well in enumerate(field_data['selected_wells']):
                print(f"    Well {i+1}: API12 {well['api12']}, Total Diff: {well['total_diff']}")
    ...
    def test_get_wells_by_specific_fields(self):
        ...
        from tests.modules.bsee.analysis.api12_drilling_completion_analysis.data_loader import calculate_differences
    ...
    def test_export_field_analysis(self):
        ...
        from tests.modules.bsee.analysis.api12_drilling_completion_analysis.well_selector import export_field_analysis_to_csv
        import tempfile
        import os
