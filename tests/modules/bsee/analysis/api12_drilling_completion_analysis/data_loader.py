...
def standardize_data(
    lease_df: pd.DataFrame, api12_df: pd.DataFrame
    ...
    if "api12" in lease_std.columns:
        lease_std["api12"] = pd.to_numeric(
            lease_std["api12"], errors="coerce", downcast=None
        ).astype("int64")
    if "api12" in api12_std.columns:
        api12_std["api12"] = pd.to_numeric(
            api12_std["api12"], errors="coerce", downcast=None
        ).astype("int64")
...
def convert_data_types_lease(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "API_WELL_NUMBER" in df_converted.columns:
        df_converted["API_WELL_NUMBER"] = pd.to_numeric(
            df_converted["API_WELL_NUMBER"], errors="coerce", downcast=None
        ).astype("int64")
...
def convert_data_types_api12(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "API12" in df_converted.columns:
        df_converted["API12"] = pd.to_numeric(
            df_converted["API12"], errors="coerce", downcast=None
        ).astype("int64")
...
def parse_rigdays_milestone(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for idx, milestone_str in df_parsed["rigdays_by_milestone"].items():
        ...
        try:
            ...
            # Extract values
            df_parsed.loc[idx, "milestone_drilling_days"] = milestone_data.get(
                "drilling_days", np.nan
            )
            df_parsed.loc[idx, "milestone_completion_days"] = milestone_data.get(
                "completion_days", np.nan
            )
            df_parsed.loc[idx, "milestone_rig_days"] = milestone_data.get(
                "rig_days", np.nan
            )
