...
def generate_executive_summary(
    comparison_data: pd.DataFrame, methodology_data: Dict[str, Any]
) -> Dict[str, Any]:
    """
    ...
    return {
        ...
        "methodology_comparison": {
            "lease_method": methodology_data.get("lease_method", {}).get(
                "approach", "Timeline-based analysis"
            ),
            "api12_method": methodology_data.get("api12_method", {}).get(
                "approach", "Milestone-based analysis"
            ),
        },
...
def identify_root_causes(
    ...
    high_completion_diffs = comparison_data[
        comparison_data["completion_diff"].abs() > 30
    ]
    ...
    return {
        ...
        "high_difference_analysis": {
            ...
            "fields_most_affected": comparison_data.groupby("field_name")["total_diff"]
            .mean()
            .nlargest(3)
            .index.tolist(),
        },
...
"""
    ...
    # Field summary table
    field_summary = (
        comparison_data.groupby("field_name")
        .agg(
            {
    ...
    return {
        "agreement_statistics": {
            ...
            "mean_absolute_error": float(
                np.mean(np.abs(comparison_data["total_diff"]))
            ),
            "root_mean_squared_error": float(
                np.sqrt(np.mean(comparison_data["total_diff"] ** 2))
            ),
            "correlation_coefficient": float(
                np.corrcoef(
                    lease_drilling + lease_completion, api12_drilling + api12_completion
                )[0, 1]
            ),
        },
        "difference_distribution": {
            "drilling_diff_range": [
                float(drilling_diffs.min()),
                float(drilling_diffs.max()),
            ],
            "completion_diff_range": [
                float(completion_diffs.min()),
                float(completion_diffs.max()),
            ],
            "total_diff_range": [
                float(comparison_data["total_diff"].min()),
                float(comparison_data["total_diff"].max()),
            ],
        },
        "outlier_analysis": {
...
""".format(
        lease_zero_drill=len(
            comparison_data[comparison_data["lease_drilling_days"] == 0]
        ),
        lease_zero_comp=len(
            comparison_data[comparison_data["lease_completion_days"] == 0]
        ),
        api12_zero_drill=len(
            comparison_data[comparison_data["api12_drilling_days"] == 0]
        ),
        api12_zero_comp=len(
            comparison_data[comparison_data["api12_completion_days"] == 0]
        ),
        ...
        lease_extreme=len(
            comparison_data[comparison_data["lease_drilling_days"] > 200]
        ),
        api12_extreme=len(
            comparison_data[comparison_data["api12_drilling_days"] > 200]
        ),
    ...
    correlations = [
        (
            ...
            comparison_data["lease_drilling_days"].corr(
                comparison_data["api12_drilling_days"]
            ),
            "Method agreement on drilling duration",
        ),
        (
            "Lease Completion vs API12 Completion",
            comparison_data["lease_completion_days"].corr(
                comparison_data["api12_completion_days"]
            ),
        ...
        (
            ...
            (
                comparison_data["lease_drilling_days"]
                + comparison_data["lease_completion_days"]
            ).corr(
                comparison_data["api12_drilling_days"]
                + comparison_data["api12_completion_days"]
            ),
...
def export_analysis_data(
    comparison_data: pd.DataFrame, methodology_data: Dict[str, Any], output_path: str
) -> bool:
    """
