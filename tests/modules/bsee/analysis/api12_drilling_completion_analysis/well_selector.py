...
def get_wells_by_field(
    comparison_df: pd.DataFrame, field_name: str, n_wells: int = 2
) -> pd.DataFrame:
    """
...
def analyze_wells_by_field(comparison_df: pd.DataFrame) -> Dict[str, Dict[str, Any]]:
    ...
    for field_name in field_names:
        ...
        field_stats = {
            "total_wells": len(
                comparison_df[comparison_df["field_name"] == field_name]
            ),
...
def filter_wells_by_criteria(
    comparison_df: pd.DataFrame, criteria: Dict[str, Tuple[str, float]]
) -> pd.DataFrame:
    """
...
def rank_wells_by_difference(
    comparison_df: pd.DataFrame, ascending: bool = False
) -> pd.DataFrame:
    """
    ...
    """
    return comparison_df.sort_values("total_diff", ascending=ascending).reset_index(
        drop=True
    )


def validate_well_selection(
    comparison_df: pd.DataFrame, selected_wells: List[int]
) -> bool:
    """
...
def generate_well_comparison_summary(
...
def export_field_analysis_to_csv(
    field_analysis: Dict[str, Any], output_path: str
) -> None:
    """
