...
class TestNPVIntegrationWorkflow:
    ...
    def test_npv_workflow_configuration_validation(self, sample_config):
        ...
        assert 'discount_rate_annual' in sample_config['economics']['cost'], "Discount rate required"
        ...
        assert 0 < economics['discount_rate_annual'] < 1, "Discount rate should be between 0 and 1"
        
        print(f"Configuration validation passed: CAPEX=${economics['CAPEX']:,.0f}, "
    ...
    def test_production_api12_analysis_initialization(self):
        ...
        assert hasattr(analyzer, 'generate_revenue_table'), "generate_revenue_table method required"
        assert hasattr(analyzer, 'perform_npv_calculation'), "perform_npv_calculation method required"
        assert hasattr(analyzer, 'perform_excel_aligned_npv_calculation'), "perform_excel_aligned_npv_calculation method required"
        
        # Test that methods are callable
        assert callable(analyzer.generate_revenue_table), "generate_revenue_table should be callable"
        assert callable(analyzer.perform_npv_calculation), "perform_npv_calculation should be callable"
        assert callable(analyzer.perform_excel_aligned_npv_calculation), "perform_excel_aligned_npv_calculation should be callable"
        
        print("ProductionAPI12Analysis initialization successful")
    
    def test_revenue_table_generation_workflow(self, sample_config, sample_production_data):
        ...
        try:
            ...
            revenue_df = analyzer.generate_revenue_table(sample_config, sample_production_data)
            ...
            if not revenue_df.empty:
                ...
                print(f"Revenue table generated: {len(revenue_df)} rows, {len(revenue_df.columns)} columns")
    ...
    def test_npv_calculation_workflow(self, sample_config):
        ...
        try:
            ...
            assert npv_result < 0, "NPV should be negative for this scenario (high CAPEX)"
            
            print(f"NPV calculation successful: ${npv_result:,.2f}")
            
            # Test Excel-aligned NPV calculation
            try:
                excel_aligned_npv = analyzer.perform_excel_aligned_npv_calculation(sample_config, revenue_df)
                
                assert isinstance(excel_aligned_npv, (int, float)), "Excel-aligned NPV should be numeric"
                
                print(f"Excel-aligned NPV calculation successful: ${excel_aligned_npv:,.2f}")
                
                # Compare the two methods
                variance_pct = abs(npv_result - excel_aligned_npv) / abs(excel_aligned_npv) * 100 if excel_aligned_npv != 0 else float('inf')
    ...
    def test_output_file_generation_workflow(self, sample_config, temp_results_dir):
        ...
        for filename in expected_files:
            ...
            if filename == 'npv_summary_test_npv_integration.csv':
                ...
                cashflow_data = {
                    ...
                    'Cash_Flow_USD': [-1460000000] + [9800000 * (0.95**i) for i in range(24)],
                    'Description': ['Initial CAPEX (Excel-aligned)'] + ['Monthly Net Cash Flow'] * 24
                }
        ...
        assert len(created_files) == len(expected_files), f"Expected {len(expected_files)} files, created {len(created_files)}"
        print(f"Output file generation workflow successful: {len(created_files)} files created")
    ...
    def test_backward_compatibility_configuration(self):
        ...
        for config_name, config in [('original', original_config), ('updated', updated_config)]:
    ...
    def test_npv_accuracy_requirements_validation(self):
        ...
        from tests.modules.bsee.analysis.test_npv_accuracy_validation import NPVBenchmarkValidator
        ...
        if excel_benchmark != 0:
            ...
            print(f"  Target: ≤20%")
            print(f"  Status: {'PASS' if variance_result['within_20pct_threshold'] else 'NEEDS IMPROVEMENT'}")
            ...
            if current_variance <= 20.0:
                ...
                print(f"[WARN] NPV accuracy requirement NOT MET (current: {current_variance:.1f}%, target: <=20%)")
    ...
    def test_complete_integration_workflow(self, sample_config, temp_results_dir):
        ...
        try:
            created_files = self.test_output_file_generation_workflow(sample_config, temp_results_dir)
        ...
        passed_steps = sum(1 for result in workflow_results.values() if isinstance(result, str) and result == 'PASS')
        
        for step, result in workflow_results.items():
            status_icon = "[PASS]" if result == 'PASS' else "[SKIP]" if 'SKIP' in str(result) else "[FAIL]"
        ...
        assert success_rate >= 0.3, f"Workflow success rate too low: {success_rate*100:.0f}%"
