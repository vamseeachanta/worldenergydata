    ...
    try:
        ...
        for row_idx in range(df.shape[0]):
            ...
            for col_idx in range(df.shape[1]):
                ...
                if pd.notna(cell_val) and isinstance(cell_val, str):
                    if any(
                        keyword in str(cell_val).upper()
                        for keyword in ["BRENT", "OIL PRICE", "PRICE", "WTI"]
                    ):
                        print(
                            f"  Found price indicator at Row {row_idx}, Col {col_idx}: {cell_val}"
                        )
        ...
        for row_idx in range(df.shape[0]):
            ...
            if pd.notna(label):
                ...
                if any(
                    keyword in label_str
                    for keyword in ["TOTAL", "JSM", "PRODUCTION", "BBL", "MONTHLY"]
                ):
                    # Get data range
                    data_vals = []
                    for c in range(2, min(10, df.shape[1])):
                    ...
                    if data_vals:
                        print(
                            f"  Row {row_idx}: {label} -> Min: {min(data_vals):,.0f}, Max: {max(data_vals):,.0f}, Avg: {np.mean(data_vals):,.0f}"
                        )
        ...
        for sheet in xl_file.sheet_names:
            if any(
                keyword in sheet.upper()
                for keyword in ["PRICE", "BRENT", "DATA", "INPUT"]
            ):
                print(f"\nChecking sheet: {sheet}")
                df_sheet = pd.read_excel(
                    excel_file_path, sheet_name=sheet, engine="openpyxl"
                )
