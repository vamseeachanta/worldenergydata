...
class TestOilPriceExtraction:
    ...
    def test_brent_price_row_identification(self, excel_file_path):
        ...
        assert isinstance(brent_label, str), f"Expected string label, got {type(brent_label)}"
        assert "BRENT" in brent_label.upper(), f"Expected BRENT label, got '{brent_label}'"
    ...
    def test_brent_price_extraction_basic(self, excel_file_path):
        ...
        for col_idx in range(2, min(df.shape[1], 60)):  # Start from column 2, reasonable limit
        ...
        assert len(brent_prices) >= 24, f"Expected at least 24 months of prices, got {len(brent_prices)}"
        
        # Validate price ranges (reasonable oil prices)
        assert all(20 < price < 200 for price in brent_prices), "Some prices are outside reasonable range (20-200)"
        
        print(f"Extracted {len(brent_prices)} BRENT prices: {brent_prices[:5]}...{brent_prices[-5:]}")
    ...
    def test_brent_price_extraction_with_validation(self, excel_file_path):
        ...
        expected_first_prices = [62.34, 47.76, 58.1, 55.89, 59.52]  # First 5 prices from Excel
        
        assert len(brent_prices) >= 5, f"Need at least 5 prices for validation, got {len(brent_prices)}"
        ...
        for i, expected_price in enumerate(expected_first_prices):
            ...
            assert abs(actual_price - expected_price) < 0.01, f"Price {i}: expected {expected_price}, got {actual_price}"
    ...
    def test_month_header_extraction(self, excel_file_path):
        ...
        found_months = [header for header in month_headers if any(month in header for month in month_names)]
        assert len(found_months) > 0, f"No recognizable month names found in headers: {month_headers[:10]}"
    ...
    def test_price_data_alignment_with_months(self, excel_file_path):
        ...
        for col_idx in range(2, min(df.shape[1], 60)):
            ...
            if (pd.notna(month_val) and 
                pd.notna(price_val) and 
                isinstance(price_val, (int, float)) and 
                20 < price_val < 200):
                months.append(str(month_val))
                prices.append(price_val)
        
        # Validate alignment
        assert len(months) == len(prices), f"Month-price mismatch: {len(months)} months vs {len(prices)} prices"
        assert len(months) >= 12, f"Expected at least 12 months of data, got {len(months)}"
        
        # Create aligned DataFrame for testing
        price_data = pd.DataFrame({
            'Month': months,
            'BRENT_Price_USD_per_BBL': prices
        })
        
        assert price_data['BRENT_Price_USD_per_BBL'].mean() > 30, "Average oil price seems too low"
        assert price_data['BRENT_Price_USD_per_BBL'].mean() < 120, "Average oil price seems too high"
        
        print(f"Successfully aligned {len(price_data)} month-price pairs")
        print(f"Price range: ${price_data['BRENT_Price_USD_per_BBL'].min():.2f} - ${price_data['BRENT_Price_USD_per_BBL'].max():.2f}")
    ...
    def test_fallback_price_handling(self):
        ...
        fallback_prices = [60.0, 65.0, 70.0, 68.0, 72.0] * 5  # 25 months of fallback data
        
        assert len(fallback_prices) >= 20, "Fallback prices should cover at least 20 months"
        assert all(30 < price < 100 for price in fallback_prices[:5]), "Fallback prices should be reasonable"
    ...
    def test_price_extraction_function_integration(self, excel_file_path):
        ...
        assert len(extracted_prices) >= 12, f"Expected at least 12 prices, got {len(extracted_prices)}"
        assert all(isinstance(p, (int, float)) for p in extracted_prices), "All prices should be numeric"
        
        # Verify some expected values
        assert abs(extracted_prices[0] - 62.34) < 0.01, f"First price mismatch: expected 62.34, got {extracted_prices[0]}"
        assert abs(extracted_prices[1] - 47.76) < 0.01, f"Second price mismatch: expected 47.76, got {extracted_prices[1]}"
