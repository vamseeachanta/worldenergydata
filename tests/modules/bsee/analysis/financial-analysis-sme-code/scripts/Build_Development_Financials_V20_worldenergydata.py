...
def _matrix_to_timeseries(df: pd.DataFrame) -> pd.DataFrame:
    ...
    well_key = (
        "WELL_NAME" if "WELL_NAME" in df.columns else (id_cols[0] if id_cols else None)
    )
    if not well_key:
    ...
    long["BBLS_PER_DAY"] = pd.to_numeric(long["BBLS_PER_DAY"], errors="coerce").fillna(
        0.0
    )
    ...
    ts = (
        long.pivot_table(
            index="YearMonth", columns=well_key, values="OIL_BBL", aggfunc="sum"
        )
        .sort_index()
        .reset_index()
    )
    ...
    for sh in xls.sheet_names:
        ...
        if mcol:
            ...
            num_cols = [
                c
                for c in df.columns
                if c != "YearMonth" and pd.api.types.is_numeric_dtype(df[c])
            ]
            out[sh] = pd.concat([df[["YearMonth"]], df[num_cols]], axis=1).dropna(
                subset=["YearMonth"]
            )
...
def merge_prod_for_dev(
    prod_by_sheet: dict, leases_df: pd.DataFrame, dev_name: str
) -> pd.DataFrame:
    L = leases_df.copy()
    L.columns = [c.upper() for c in L.columns]
    lease_names = set(
        L.loc[
            L["DEV_NAME"].astype(str).str.upper() == str(dev_name).strip().upper(),
            "LEASE_NAME",
        ]
        .dropna()
        .astype(str)
    ...
    if not frames:
        if dev_name in prod_by_sheet:
            ...
            return pd.DataFrame(columns=["YearMonth"]).assign(
                YearMonth=pd.Series([], dtype="datetime64[ns]")
            )
    prod = frames[0]
    for f in frames[1:]:
...
def load_dc_both(dnc_path: str, leases_df: pd.DataFrame):
    ...
    for sh in xls.sheet_names:
        ...
        # Monthly rows
        have_drill = any(
            c in df.columns for c in ["DRILL_DAYS", "DRILLING_DAYS", "DRILL DAYS"]
        )
        have_comp = any(
            c in df.columns for c in ["COMP_DAYS", "COMPLETION_DAYS", "COMP DAYS"]
        )
    ...
    totals = (
        pd.concat(totals_frames, ignore_index=True) if totals_frames else pd.DataFrame()
    )
...
def build_day_maps_for_dev(
    ...
    def _dev_filter(df):
        ...
        d = d[
            d["DEV_NAME"].astype(str).str.strip().str.upper()
            == str(dev_name).strip().upper()
        ]
        return d

    mon = _dev_filter(monthly)
    tot = _dev_filter(totals)

    wells = set()
    wells_with_totals = set()
    if tot is not None and not tot.empty:
        wk = None
        for cand in ["WELL_NAME", "WELL", "API_WELL_NUMBER", "API"]:
            if cand in tot.columns:
                wk = cand
                break
        if wk:
    ...
    if mon is not None and not mon.empty:
        ...
        for cand in ["WELL_NAME", "WELL", "API_WELL_NUMBER", "API"]:
            if cand in mon.columns:
                ...
                break
        if wk:
            wells |= set(mon[wk].astype(str).str.strip().tolist())
    ...
    def alloc_forward(start_ts, n_days):
        out = {}
        if pd.isna(start_ts) or n_days <= 0:
            return out
    ...
    def alloc_backward(end_month, n_days):
        out = {}
        if end_month is None or n_days <= 0:
            return out
    ...
    if mon is not None and not mon.empty:
        ...
        for _, r in mon.iterrows():
            w = str(r.get(well_col, "")).strip()
            if pd.isna(r.get("YearMonth", pd.NaT)) or not w or w in wells_with_totals:
                continue
            m = (
                pd.to_datetime(r.get("YearMonth", pd.NaT), errors="coerce")
                .to_period("M")
                .to_timestamp()
            )
            dd = float(pd.to_numeric(r.get("DRILL_DAYS", 0), errors="coerce") or 0.0)
            cd = float(pd.to_numeric(r.get("COMP_DAYS", 0), errors="coerce") or 0.0)
            if dd > 0:
                drill_map[w][m] = drill_map[w].get(m, 0) + dd
            if cd > 0:
                comp_map[w][m] = comp_map[w].get(m, 0) + cd
...
def load_assumptions(xlsx_path: str) -> dict:
    ...
    pcol = (
        pick_col(df, ["PARAMETER", "PARAM", "KEY", "NAME"], required=False)
        or df.columns[0]
    )
...
def load_wti_map(path: str, base: float) -> dict:
    ...
    pcol = (
        pick_col(df, ["WTI", "PRICE", "WTI_PRICE", "WTI_USD_BBL"], required=False)
        or df.columns[1]
    )
    ...
    mp = {}
    for _, r in df.iterrows():
        mp[pd.to_datetime(r["YearMonth"]).to_period("M").to_timestamp()] = (
            float(r[pcol]) if pd.notna(r[pcol]) else base
        )
    return mp
...
def mirr_from_monthly(
    cf: np.ndarray, r_fin_month: float, r_reinv_month: float
) -> float:
    n = cf.size
    if n <= 1:
        return np.nan
...
def main():
    ...
    leases["DEV_TYPE_EFF"] = (
        leases["DEV_TYPE"] if "DEV_TYPE" in leases.columns else leases["DEV_SYSTEM"]
    )
    ...
    MODU_usd = _get_rate_usd(
        A,
        usd_keys=[
            "MODU_LOADED_DAYRATE_USD",
            "MODU_Loaded_dayrate_USD",
            "MODU_LOADED_DAYRATE",
        ],
        k_keys=["MODU_Loaded_dayrate", "MODU_LOADED_DAYRATE_MM"],
    )
    DRY_usd = _get_rate_usd(
        A,
        usd_keys=[
            "DRY_LOADED_DAYRATE_USD",
            "Dry_Loaded_dayrate_USD",
            "DRY_LOADED_DAYRATE",
        ],
    ...
    subsea_pump_mm = float(
        A.get("Subsea_Pump_MM", A.get("Pump_pkg_per_5_wells_MM", 0.0))
    )
    ...
    if dry_sys_per_prod_usd is not None and pd.notna(dry_sys_per_prod_usd):
        ...
        dry_sys_per_prod_mm = float(
            A.get(
                "Dry_Well_System_Per_Producer_MM",
                A.get(
                    "Dry_Well_System_Per_Producer",
    ...
    reinv_ann = float(
        A.get("MIRR_Reinvest_Rate_annual", A.get("Reinvest_Rate_Annual", disc_ann))
    )
    fin_ann = float(
        A.get("MIRR_Finance_Rate_annual", A.get("Finance_Rate_Annual", disc_ann))
    )
    ...
    price_model_used = (
        "Monthly WTI file" if len(wti_map) > 0 else f"Flat WTI ${flat_wti:.2f}/bbl"
    )
    ...
    def build_input_totals_map(df_totals, df_monthly):
        ...
        if df_totals is not None and not df_totals.empty:
            ...
            for _, r in df_totals.iterrows():
                ...
                if not w:
                    continue
                dd = float(
                    pd.to_numeric(r.get("DRILL_DAYS", 0), errors="coerce") or 0.0
                )
                cd = float(pd.to_numeric(r.get("COMP_DAYS", 0), errors="coerce") or 0.0)
                spud = pd.to_datetime(r.get("WELL_SPUD_DATE", pd.NaT), errors="coerce")
                td = pd.to_datetime(r.get("TOTAL_DEPTH_DATE", pd.NaT), errors="coerce")
        ...
        if df_monthly is not None and not df_monthly.empty:
            well_col = None
            for cand in ["WELL_NAME", "WELL", "API_WELL_NUMBER", "API"]:
                if cand in df_monthly.columns:
                    well_col = cand
                    break
            grp = (
                df_monthly.groupby(well_col, dropna=True).agg(
                    DRILL_DAYS=pd.NamedAgg(column="DRILL_DAYS", aggfunc="sum"),
            ...
            for _, r in grp.iterrows():
                ...
                if w not in res:
                    res[w] = (
                        float(r["DRILL_DAYS"] or 0.0),
                        float(r["COMP_DAYS"] or 0.0),
                        pd.NaT,
                        pd.NaT,
                    )
        return res
    ...
    for dev_name, g in leases.groupby("DEV_NAME"):
        ...
        latest_prod = (
            prod_df["YearMonth"].max()
            if "YearMonth" in prod_df.columns and not prod_df.empty
            else global_start
        )
        months_alloc = []
        ...
        overall_end = (
            max([x for x in [latest_prod, latest_dc] if x is not None])
            if (latest_prod or latest_dc)
            else global_start
        )
        ...
        for w in sorted(all_wells):
            ...
            for m, v in drill_map.get(w, {}).items():
                if m in out.index:
                    out.loc[m, dcol] += float(v)
            for m, v in comp_map.get(w, {}).items():
                if m in out.index:
                    out.loc[m, ccol] += float(v)
            drill_tot += out[dcol]
            comp_tot += out[ccol]
        ...
        out["Revenue_Net"] = (
            rev_gross - royalty_amt - severance_amt + out["Revenue_Adjustments"]
        )
        ...
        for m in out.index:
            rate = (
                MODU_usd
                if (system == "subsea" or (fo_dev is None or m < fo_dev))
                else DRY_usd
            )
        ...
        injector_count = (
            (producers_count // 7)
            if (system == "subsea" and producers_count >= 7)
            else 0
        )
        ...
        if fo_dev is not None and host_usd > 0:
            ...
            win = pd.date_range(
                fo_dev - pd.offsets.DateOffset(months=total_host_months),
                fo_dev - pd.offsets.DateOffset(months=1),
                freq="MS",
            )
            if len(win) > 0:
                ...
                for mm in win:
                    if mm in out.index:
                        out.loc[mm, "CAPEX_Facilities"] += per_m

        # SURF allocation
        if (
            system == "subsea"
            and surf_usd_total > 0
            and surf_prefo > 0
            and fo_dev is not None
        ):
            win = pd.date_range(
                fo_dev - pd.offsets.DateOffset(months=surf_prefo),
                fo_dev - pd.offsets.DateOffset(months=1),
                freq="MS",
            )
            if len(win) > 0:
        ...
        if system != "subsea" and dry_sys_per_prod_usd > 0 and producers_count > 0:
            for c in producer_cols:
                ...
                if fm is not None:
                    ...
                    win = pd.date_range(
                        fm - pd.offsets.DateOffset(months=12),
                        fm - pd.offsets.DateOffset(months=1),
                        freq="MS",
                    )
                    if len(win) > 0:
                        per_m = alloc / len(win)
        ...
        pump_count = (
            (producers_count // 8)
            if (system == "subsea" and producers_count >= 8)
            else 0
        )
        pump_total_usd = pump_count * subsea_pump_mm * 1_000_000.0
        anchor_fo = None
        if pump_count > 0:
            prod_fo_sorted = sorted(
                [
                    m
                    for m in [first_nonzero_month(out[c]) for c in producer_cols]
                    if m is not None
                ]
            )
        ...
        if pump_total_usd > 0 and anchor_fo is not None and surf_prefo > 0:
            win = pd.date_range(
                anchor_fo - pd.offsets.DateOffset(months=surf_prefo),
                anchor_fo - pd.offsets.DateOffset(months=1),
                freq="MS",
            )
            if len(win) > 0:
        ...
        out["Net_Cash_Flow"] = (
            out["Revenue_Net"] - out["OPEX"] - out["CAPEX"] + out["Tax_Savings"]
        )
        ...
        if fo_dev is not None:
            pre_mask = out.index < fo_dev
            post_mask = out.index >= fo_dev
        else:
            pre_mask = out.index < out.index.min()
            post_mask = ~pre_mask
        dnc_pre_gross = float(
            out.loc[pre_mask, ["CAPEX_Drill", "CAPEX_Comp"]].sum().sum()
        )
        dnc_post_gross = float(
            out.loc[post_mask, ["CAPEX_Drill", "CAPEX_Comp"]].sum().sum()
        )
        ...
        for w in sorted(set(list(drill_map.keys()) + list(comp_map.keys()))):
            ...
            dd_in, cd_in, spud, td = input_totals_all.get(
                w, (np.nan, np.nan, pd.NaT, pd.NaT)
            )
            fo = fo_by_well.get(w, None)
            qc_rows.append(
                {
                    ...
                    "TD": td,
                    "FO_MONTH": fo,
                    "Drill Days — Input": dd_in,
                    "Drill Days — Alloc": dd_alloc,
                    "Drill Days — Diff": (
                        (dd_alloc - dd_in) if pd.notna(dd_in) else np.nan
                    ),
                    "Comp Days — Input": cd_in,
                    "Comp Days — Alloc": cd_alloc,
                    "Comp Days — Diff": (
                        (cd_alloc - cd_in) if pd.notna(cd_in) else np.nan
                    ),
                }
            )
        ...
        proj_rows.append(
            {
                ...
                "Facilities SURF USD": float(
                    surf_usd_total if system == "subsea" else 0.0
                ),
                "Subsea Pumps USD": float(
                    pump_total_usd if system == "subsea" else 0.0
                ),
                "Dry Well Systems USD": float(
                    dry_sys_total_usd if system != "subsea" else 0.0
                ),
                ...
                "Injector Wells": int(
                    (len(producer_cols) // 7)
                    if (system == "subsea" and len(producer_cols) >= 7)
                    else 0
                ),
                "Pumps Count (8/prod)": int(
                    (len(producer_cols) // 8)
                    if (system == "subsea" and len(producer_cols) >= 8)
                    else 0
                ),
                "DEV SYSTEM USED": system,
                "ECON PATH USED": (
                    "dry: MODU->DRY at FO" if system != "subsea" else "subsea: MODU"
                ),
                "FO Month": fo_dev,
            }
        )
        ...
        dd_in_dev = float(
            np.nansum(
                [
                    input_totals_all.get(w, (np.nan, np.nan, np.nan, np.nan))[0]
                    for w in wells_in_dev
                ]
            )
        )
        cd_in_dev = float(
            np.nansum(
                [
                    input_totals_all.get(w, (np.nan, np.nan, np.nan, np.nan))[1]
                    for w in wells_in_dev
                ]
            )
        )
        dd_alloc_dev = float(
            sum([sum(drill_map.get(w, {}).values()) for w in wells_in_dev])
        )
        cd_alloc_dev = float(
            sum([sum(comp_map.get(w, {}).values()) for w in wells_in_dev])
        )
        qc_dev_rows.append(
            {
                "DEV_NAME": dev_name,
    ...
    with pd.ExcelWriter(OUT_XLSX, engine="openpyxl") as writer:
        ...
        readme = pd.DataFrame(
            {
                "README — Financial Analysis V20 Output Workbook": [
                    None,
                    "File Name:",
                    "Generated By:",
                    "Version:",
                ...
                "": [
                    ...
                    "Build_Development_Financials_V20.py",
                    "V20",
                    _dt.datetime.now().strftime("%B %d, %Y"),
                ],
            }
        )
        ...
        qc_dev_df = pd.DataFrame(
            qc_dev_rows,
            columns=[
                "DEV_NAME",
                "Drill Days — Input (Dev)",
                "Drill Days — Alloc (Dev)",
                "Drill Days — Diff (Dev)",
                "Comp Days — Input (Dev)",
                "Comp Days — Alloc (Dev)",
                "Comp Days — Diff (Dev)",
...
def _populate_readme_sheet(wb, out_name):
    ...
    ws["A2"] = "Generated On"
    ws["B2"] = _dt.now().strftime("%B %d, %Y %H:%M")
    ws["A3"] = "File Name"
    ws["B3"] = out_name
...
def _read_mirr_rates_from_assumptions(path="leases_assumptions.xlsx"):
    ...
    try:
        ...
        sh = (
            wb["Inputs_Assumptions"]
            if "Inputs_Assumptions" in wb.sheetnames
            else wb.active
        )
...
def _apply_v20_formatting(out_name, discount_rate_annual):
    ...
    rates = {
        ...
        "MIRR Reinvest Rate (Annual)": (
            reinvest_b6 if reinvest_b6 is not None else discount_rate_annual
        ),
        "MIRR Finance Rate (Annual)": (
            finance_b7 if finance_b7 is not None else discount_rate_annual
        ),
    }
    ...
    if proj_sheet in wb.sheetnames and exec_sheet in wb.sheetnames:
        ...
        to_delete = sorted(
            [hmap_exec[h] for h in move_headers if h in hmap_exec], reverse=True
        )
    ...
    wb.save(out_name)
    print(f"SUCCESS: V20 formatting applied to {out_name}")
