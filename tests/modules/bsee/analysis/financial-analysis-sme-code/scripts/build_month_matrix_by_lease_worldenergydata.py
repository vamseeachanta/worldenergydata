...
def choose_excel_engine() -> str:
    ...
    try:
        ...
        if _ensure_pkg("openpyxl"):
            return "openpyxl"
    raise SystemExit(
        "ERROR: Neither 'xlsxwriter' nor 'openpyxl' is available and installation failed. "
...
def resolve_group_column(leases_df: pd.DataFrame, group_col: str) -> str:
    if group_col and str(group_col).upper() != "AUTO":
        ...
        if cand in leases_df.columns:
            return cand
        raise SystemExit(
            f"ERROR: group-col '{group_col}' not found in leases columns: {list(leases_df.columns)}"
        )
...
def load_leases_with_names(path: str) -> pd.DataFrame:
    ...
    lease_num_col = (
        next((c for c in df.columns if "LEASE" in c and "NUM" in c), None)
        or df.columns[0]
    )
    lease_name_col = next(
        (c for c in df.columns if "LEASE" in c and "NAME" in c), lease_num_col
    )
...
def main():
    ap = argparse.ArgumentParser(
        prog="build_month_matrix_by_lease_worldenergydata.py",
    ...
    ap.add_argument(
        "--dir",
        default="../../../../../../data/modules/bsee/zip/historical_production_yearly",
        help="Directory containing OGOR zip files (default: repository OGORA dir)",
    )
    ap.add_argument(
        "--leases",
        default="AUTO",
        help="Leases file (.xlsx/.xls/.csv). If 'AUTO', search in --dir then CWD.",
    )
    # ONLY CHANGE: Add _worldenergydata suffix to output
    ap.add_argument(
        "--out",
        default="../results/multi_year_lease_matrix_with_charts_worldenergydata.xlsx",
        help="Output Excel path (default: results directory)",
    )
    ap.add_argument(
        "--pattern",
        default="ogora20??delimit.zip",
        help="Glob for OGOR zips (default shown)",
    )
    args = ap.parse_args()
    # Fallbacks for older CLIs
    if not hasattr(args, "group_mode"):
        args.group_mode = "leases"
    ...
    if str(args.leases).upper() == "AUTO":
        ...
        chosen = None
        for c in candidates:
            # Check current directory first (where script is)
            if os.path.exists(c):
                chosen = c
                break
            # Then check the OGORA directory
        ...
        if not chosen:
            raise SystemExit(
                "ERROR: Could not find leases.xlsx/.xls/.csv in current directory or --dir. Pass --leases <path>."
            )
        args.leases = chosen
        print(f"Using leases file: {args.leases}")
    else:
        if not os.path.exists(args.leases):
            raise SystemExit(f"ERROR: Leases file not found: {args.leases}")
    ...
    # ONLY CHANGE: Filter to 2014-2025 range to match original SME script
    def year_key(p):
        m = YEAR_IN_FN.search(os.path.basename(p))
        return int(m.group(1)) if m else 0

    # Filter paths to only include 2014-2025
    filtered_paths = []
    for p in paths:
        year = year_key(p)
        if 2014 <= year <= 2025:
    ...
    if not paths:
        raise SystemExit(f"No files matched pattern in 2014-2025 range: {search_glob}")
    ...
    for p in paths:
        ...
        df["YEAR_MONTH"] = (
            ...
            + "-"
            + df["PROD_MONTH"].astype(str).str.zfill(2)
        )
        ...
        g = (
            df.groupby(
                ["LEASE_NUM", "WELL_NAME", "API_WELL_NUMBER", "YEAR_MONTH"],
                dropna=False,
            )
        ...
        g["BBLS_PER_DAY"] = g.apply(
            lambda r: (
                (r["OIL_SUM"] / r["DAYS_SUM"])
                if r["DAYS_SUM"] and r["DAYS_SUM"] > 0
                else 0
            ),
            axis=1,
        )
    ...
    if not parts:
        raise SystemExit(
            "No matching rows found across the OGOR files for the given leases."
        )
    ...
    with pd.ExcelWriter(args.out, engine=excel_engine) as writer:
        ...
        if args.group_mode == "group":
            ...
            for sheet_name in sorted(group_to_leases.keys()):
                ...
                if sub.empty:
                    pd.DataFrame(columns=["WELL_NAME", "API_WELL_NUMBER"]).to_excel(
                ...
                for ln in sorted(group_leases):
                    mapping_rows.append(
                        {
                            "LEASE_NUM": ln,
                            "GROUP_SHEET": sheet_name,
                            "SHEET_WRITTEN": safe_name,
                        }
                    )
            ...
            if mapping_rows:
                pd.DataFrame(mapping_rows).to_excel(
                    writer, sheet_name="QA_Lease_to_Group", index=False
                )
            ...
            if "LEASE_NAME" in leases_df.columns:
                ...
                name_col = (
                    "DEV_NAME" if "DEV_NAME" in leases_df.columns else "LEASE_NUM"
                )
            ...
            if mapping_rows:
                pd.DataFrame(mapping_rows).to_excel(
                    writer, sheet_name="QA_LeaseName_to_Leases", index=False
                )

    print(f"SUCCESS: Wrote multi-year matrix to {args.out}")
