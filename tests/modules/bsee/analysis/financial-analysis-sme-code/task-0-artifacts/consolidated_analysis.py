...
class FinancialAnalyzer:
    ...
    def _convert_matrix_to_timeseries(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        melted["Production"] = pd.to_numeric(
            melted["Production"], errors="coerce"
        ).fillna(0)
    ...
    def _calculate_lease_financials(
        self,
        lease_name: str,
        group_name: str,
        production: Dict,
        ...
        results["Drilling_CAPEX"] = self._allocate_drilling_costs(
            lease_dc, results.index
        )
        results["Completion_CAPEX"] = self._allocate_completion_costs(
            lease_dc, results.index
        )
        ...
        # Calculate EBITDA
        results["EBITDA"] = (
            results["Net_Revenue"] - results["OPEX"] - results["Total_CAPEX"]
        )
        ...
        # Calculate NPV
        discount_rate = lease_assumptions.get(
            "DISCOUNT_RATE", self.config["discount_rate"]
        )
    ...
    def _allocate_drilling_costs(
        self, dc_data: pd.DataFrame, months: pd.DatetimeIndex
    ) -> pd.Series:
    ...
    def _allocate_completion_costs(
        self, dc_data: pd.DataFrame, months: pd.DatetimeIndex
    ) -> pd.Series:
