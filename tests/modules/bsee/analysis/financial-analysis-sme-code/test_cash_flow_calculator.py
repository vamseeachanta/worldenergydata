...
from src.worldenergydata.modules.bsee.analysis.financial.cash_flow_calculator import (
    CashFlowCalculator,
    FinancialParameters,
    DevelopmentType,
    calculate_npv,
...
class TestCashFlowCalculator:
    ...
    def test_calculate_revenue(self, calculator, sample_production_data):
        ...
        pd.testing.assert_series_equal(result['Gross_Oil_bbls'], expected_gross, check_names=False)
        
        # Check revenue calculation
        expected_revenue_gross = expected_gross * 80.0
        pd.testing.assert_series_equal(result['Revenue_Gross'], expected_revenue_gross, check_names=False)
        ...
        pd.testing.assert_series_equal(result['Revenue_Net'], expected_revenue_net, check_names=False)
    ...
    def test_calculate_opex(self, calculator, sample_production_data):
        ...
        pd.testing.assert_series_equal(result['OPEX_Var'], expected_var_opex, check_names=False)
        ...
        pd.testing.assert_series_equal(result['OPEX'], expected_total_opex, check_names=False)
    
    def test_calculate_capex_drilling(self, calculator, sample_production_data, sample_drilling_data):
    ...
    def test_calculate_net_cash_flow(self, calculator, sample_production_data, sample_drilling_data):
        ...
        pd.testing.assert_series_equal(result['Tax_Savings'], expected_tax_savings, check_names=False)
        
        # Check net cash flow calculation
        expected_ncf = (result['Revenue_Net'] - result['OPEX'] - 
                       result['CAPEX'] + result['Tax_Savings'])
        pd.testing.assert_series_equal(result['Net_Cash_Flow'], expected_ncf, check_names=False)
        
        # Check cumulative cash flow
        expected_cumulative = expected_ncf.cumsum()
        pd.testing.assert_series_equal(result['Cum_Cash_Flow'], expected_cumulative, check_names=False)
