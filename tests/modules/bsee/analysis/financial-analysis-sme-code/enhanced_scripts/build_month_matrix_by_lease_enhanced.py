...
def choose_excel_engine() -> str:
    ...
    try:
        ...
        if _ensure_pkg("openpyxl"):
            return "openpyxl"
    raise SystemExit("ERROR: Neither 'xlsxwriter' nor 'openpyxl' is available and installation failed. "
...
def resolve_group_column(leases_df: pd.DataFrame, group_col: str) -> str:
    if group_col and str(group_col).upper() != "AUTO":
        ...
        if cand in leases_df.columns:
            return cand
        raise SystemExit(f"ERROR: group-col '{group_col}' not found in leases columns: {list(leases_df.columns)}")
    ...
    if str(args.leases).upper() == "AUTO":
        ...
        for c in candidates:
            ...
            if os.path.exists(p1):
                ...
                break
        if not chosen:
            raise SystemExit("ERROR: Could not find leases.xlsx/.xls/.csv in current directory or --dir. Pass --leases <path>.")
        args.leases = chosen
