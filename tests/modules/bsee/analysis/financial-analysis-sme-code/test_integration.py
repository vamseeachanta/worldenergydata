...
class TestFinancialAnalyzer:
    ...
    def test_load_data(self, analyzer):
        ...
        with patch.object(analyzer.data_loader, 'load_production_data') as mock_prod:
            with patch.object(analyzer.data_loader, 'load_drilling_data') as mock_drill:
                mock_prod.return_value = pd.DataFrame({'API': [1, 2], 'oil': [100, 200]})
                mock_drill.return_value = pd.DataFrame({'API': [1, 2], 'days': [30, 45]})
    ...
    def test_calculate_financial_metrics(self, analyzer):
        ...
        with patch.object(analyzer.cash_flow_calculator, 'calculate_npv') as mock_npv:
            with patch.object(analyzer.cash_flow_calculator, 'calculate_mirr') as mock_mirr:
...
class TestEndToEndIntegration:
    ...
    def test_cli_execution(self, test_data_dir, tmp_path):
        ...
        from src.worldenergydata.modules.bsee.analysis.financial.cli_interface import main
        ...
        with patch('sys.argv', ['cli_interface.py'] + test_args):
            with patch('src.worldenergydata.modules.bsee.analysis.financial.analyzer.FinancialAnalyzer.run_analysis') as mock_run:
                mock_run.return_value = MagicMock(success=True, output_file=str(output_file))
    ...
    def test_data_flow_integration(self, test_data_dir):
        ...
        with patch.object(analyzer.data_loader, 'load_production_data') as mock_prod:
            with patch.object(analyzer.drilling_loader, 'load_drilling_data') as mock_drill:
                with patch.object(analyzer.lease_grouper, 'group_leases') as mock_group:
                    with patch.object(analyzer.cash_flow_calculator, 'calculate_monthly_cash_flow') as mock_calc:
                        # Setup mock returns
                        mock_prod.return_value = pd.DataFrame({
                            'API': [1, 2],
                            'oil_bbls': [1000, 2000],
                            'year_month': pd.date_range('2020-01', periods=2, freq='MS')
                        })
    ...
    def test_memory_usage(self):
        ...
        with patch.object(analyzer, 'load_data') as mock_load:
            ...
            assert memory_increase < 2000, f"Memory usage increased by {memory_increase}MB"
...
class TestConfigurationLoading:
    ...
    def test_yaml_config_loading(self, tmp_path):
        ...
        from src.worldenergydata.modules.bsee.analysis.financial.config_loader import load_config
    ...
    def test_config_validation(self):
        ...
        from src.worldenergydata.modules.bsee.analysis.financial.config_loader import validate_config
...
class TestSampleDataExecution:
    ...
    def test_output_format_validation(self, tmp_path):
        ...
        assert loaded_wb.sheetnames[0] == "README" or loaded_wb.sheetnames[1] == "README"