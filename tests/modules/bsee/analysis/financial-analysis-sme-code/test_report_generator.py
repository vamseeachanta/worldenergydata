...
class TestReportGenerator:
        ...
        return pd.DataFrame(
            {
                ...
                "Cum_Cash_Flow": pd.Series(
                    [-20000] * 3 + [-420000] + [-220000] + [80000] * 7
                ).cumsum(),
            }
        ).set_index("YearMonth")
    ...
    def test_format_executive_summary_data(
        self, report_generator, sample_development_data
    ):
        """Test formatting of executive summary data"""
        exec_data = report_generator._format_executive_summary_data(
    ...
    def test_format_project_summary_data(
        self, report_generator, sample_development_data
    ):
        """Test formatting of project summary data"""
        proj_data = report_generator._format_project_summary_data(
            sample_development_data
        )
    ...
    def test_multi_development_report(self, tmp_path):
        ...
        with patch.object(generator, "_save_workbook"):
            with patch.object(generator, "_apply_v20_formatting"):
                with patch("openpyxl.Workbook"):
                    result = generator.generate_report(
                        development_data=developments, output_path=str(output_file)
                    )
