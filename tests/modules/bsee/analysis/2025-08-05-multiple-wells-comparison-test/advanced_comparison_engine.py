...
class ComparisonConfig:
    ...
    results_directory: str = "tests/modules/bsee/analysis/multiple_wells_comparison_test/results"
...
class OutlierDetector:
    ...
    def detect_outliers_statistical(self, data: pd.Series, method: str = 'zscore') -> Dict[str, Any]:
        """
        ...
        return {
            ...
            'method': method,
            'threshold': self.config.outlier_threshold_std if method != 'iqr' else self.config.outlier_threshold_iqr,
            'outlier_count': len(outlier_indices),
            'outlier_percentage': (len(outlier_indices) / len(clean_data)) * 100 if len(clean_data) > 0 else 0
        }
    
    def detect_outliers_clustering(self, df: pd.DataFrame, features: List[str]) -> Dict[str, Any]:
        """
        ...
        dbscan = DBSCAN(eps=self.config.clustering_eps, min_samples=self.config.clustering_min_samples)
...
class StatisticalAnalyzer:
    ...
    def analyze_distributions(self, lease_data: pd.Series, api12_data: pd.Series, 
        ...
        """
        # Basic statistics
        lease_stats = self._calculate_descriptive_stats(lease_data, f"lease_{metric_name}")
        api12_stats = self._calculate_descriptive_stats(api12_data, f"api12_{metric_name}")
    ...
    def _calculate_descriptive_stats(self, data: pd.Series, name: str) -> Dict[str, float]:
    ...
    def _perform_statistical_tests(self, data1: pd.Series, data2: pd.Series) -> Dict[str, Any]:
        ...
        try:
            ...
            results['ttest'] = {
                ...
                'significant': ttest_result.pvalue < (1 - self.config.statistical_confidence_level)
            }
            
            # Mann-Whitney U test (non-parametric)
            mannwhitney_result = stats.mannwhitneyu(clean_data1, clean_data2, alternative='two-sided')
            results['mannwhitney'] = {
                'statistic': mannwhitney_result.statistic,
                'pvalue': mannwhitney_result.pvalue,
                'significant': mannwhitney_result.pvalue < (1 - self.config.statistical_confidence_level)
            }
            ...
            results['kolmogorov_smirnov'] = {
                ...
                'significant': ks_result.pvalue < (1 - self.config.statistical_confidence_level)
            }
    ...
    def _analyze_differences(self, data1: pd.Series, data2: pd.Series) -> Dict[str, Any]:
        ...
        return {
            ...
            'agreement_within_10_percent': (np.abs(percentage_diffs) <= 10).sum() if not percentage_diffs.empty else 0
        }
    
    def calculate_effect_size(self, data1: pd.Series, data2: pd.Series) -> Dict[str, float]:
        ...
        # Cohen's d
        pooled_std = np.sqrt(((len(clean_data1) - 1) * clean_data1.var() + 
                             (len(clean_data2) - 1) * clean_data2.var()) / 
                            (len(clean_data1) + len(clean_data2) - 2))
        
        cohens_d = (clean_data1.mean() - clean_data2.mean()) / pooled_std if pooled_std > 0 else 0
        
        # Glass's delta
        glass_delta = (clean_data1.mean() - clean_data2.mean()) / clean_data1.std() if clean_data1.std() > 0 else 0
...
class AdvancedComparisonEngine:
    ...
    def perform_comprehensive_comparison(self, lease_df: pd.DataFrame, 
                                       api12_df: pd.DataFrame) -> Tuple[List[ComparisonResult], StatisticalSummary]:
        """
        ...
        if self.config.enable_detailed_logging:
            self.logger.info(f"Starting comprehensive comparison: {len(lease_df)} lease wells, {len(api12_df)} API12 wells")
        ...
        statistical_summary = self._generate_statistical_summary(aligned_data, comparison_results)
        ...
        if self.config.enable_detailed_logging:
            self.logger.info(f"Comparison completed in {self.processing_stats['processing_time_seconds']:.2f} seconds")
        
        return comparison_results, statistical_summary
    
    def _match_and_align_data(self, lease_df: pd.DataFrame, api12_df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Merge on API12
        aligned_data = pd.merge(lease_clean, api12_clean, on='API12', how='outer', indicator=True)
        ...
        if self.config.enable_detailed_logging:
            self.logger.info(f"Data alignment: {both_count} matches, {lease_only_count} lease-only, {api12_only_count} API12-only")
        
        return aligned_data
    
    def _perform_individual_comparisons(self, aligned_data: pd.DataFrame) -> List[ComparisonResult]:
        ...
        for _, row in aligned_data.iterrows():
            try:
                ...
                if self.config.enable_detailed_logging:
                    self.logger.warning(f"Failed to compare well {row.get('API12', 'Unknown')}: {str(e)}")
    ...
    def _determine_well_status(self, drilling_diff: Optional[float], completion_diff: Optional[float],
                              drilling_pct_diff: Optional[float], completion_pct_diff: Optional[float]) -> Tuple[str, List[str]]:
    ...
    def _calculate_well_significance(self, lease_drilling: Optional[float], api12_drilling: Optional[float],
                                   lease_completion: Optional[float], api12_completion: Optional[float]) -> Dict[str, float]:
    ...
    def _generate_statistical_summary(self, aligned_data: pd.DataFrame, 
                                    comparison_results: List[ComparisonResult]) -> StatisticalSummary:
        ...
        drilling_outliers = self.outlier_detector.detect_outliers_statistical(
            pd.Series([r.drilling_diff for r in comparison_results if r.drilling_diff is not None]),
            method='zscore'
        )
        
        completion_outliers = self.outlier_detector.detect_outliers_statistical(
            pd.Series([r.completion_diff for r in comparison_results if r.completion_diff is not None]),
            method='zscore'
        )
        ...
        return StatisticalSummary(
            ...
            successful_matches=len([r for r in comparison_results if r.overall_status in ['OK', 'REVIEW', 'ERROR']]),
    ...
    def _perform_correlation_analysis(self, aligned_data: pd.DataFrame) -> Dict[str, float]:
        """Perform correlation analysis between methods."""
        correlations = {}
        
        # Drilling days correlation
        if 'Drilling_Days_Lease' in aligned_data.columns and 'Drilling_Days_API12' in aligned_data.columns:
            drilling_corr = aligned_data[['Drilling_Days_Lease', 'Drilling_Days_API12']].corr().iloc[0, 1]
            correlations['drilling_days'] = drilling_corr if not pd.isna(drilling_corr) else 0.0
        
        # Completion days correlation
        if 'Completion_Days_Lease' in aligned_data.columns and 'Completion_Days_API12' in aligned_data.columns:
            completion_corr = aligned_data[['Completion_Days_Lease', 'Completion_Days_API12']].corr().iloc[0, 1]
            correlations['completion_days'] = completion_corr if not pd.isna(completion_corr) else 0.0
        
        return correlations
    
    def export_detailed_results(self, comparison_results: List[ComparisonResult], 
                              statistical_summary: StatisticalSummary) -> Dict[str, str]:
        ...
        csv_path = Path(self.config.results_directory) / f"advanced_comparison_results_{timestamp}.csv"
        ...
        json_path = Path(self.config.results_directory) / f"statistical_summary_{timestamp}.json"
...
if __name__ == "__main__":
    ...
    print(f"Configuration: outlier_threshold={config.outlier_threshold_std}, discrepancy_threshold={config.discrepancy_absolute_threshold} days")