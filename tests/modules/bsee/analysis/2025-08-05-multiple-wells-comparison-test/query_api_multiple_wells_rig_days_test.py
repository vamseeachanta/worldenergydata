...
class MultipleWellsComparisonFramework:
    ...
    def __init__(self, results_dir: Optional[str] = None):
        ...
        self.processor = (
            MultipleWellsDataProcessor(chunk_size=30)
            if COMPARISON_PROCESSOR_AVAILABLE
            else None
        )
        self.timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")

    def run_api12_method_analysis(
        self, config_file: str = "query_api_multiple_wells_rig_days.yml"
    ) -> str:
        """
    ...
    def run_lease_method_analysis(
        self, config_file: str = "drilling_completion_days.yml"
    ) -> str:
        """
    ...
    def generate_comparison_report(self, comparison_data: Dict) -> str:
        ...
        report_content = self._create_markdown_report_content(
            results, stats, processing_stats
        )

        # Save report
        report_path = (
            self.results_dir / f"multiple_wells_comparison_report_{self.timestamp}.md"
        )
    ...
    def _create_markdown_report_content(
        self, results: List[Dict], stats: Dict, processing_stats: Dict
    ) -> str:
        """
...
"""
        ...
        if not error_review_wells.empty:
            ...
            attention_table = error_review_wells[
                ...
                20
            )  # Limit to top 20 to avoid overwhelming output
...
"""
    ...
    def export_detailed_results(self, comparison_data: Dict) -> Tuple[str, str]:
        ...
        csv_path = (
            self.results_dir
            / f"multiple_wells_comparison_detailed_{self.timestamp}.csv"
        )
        ...
        json_path = (
            self.results_dir
            / f"multiple_wells_comparison_summary_{self.timestamp}.json"
        )
...
def test_batch_processing_capability():
    ...
    assert (
        "Top 10" in report_content
    )  # Should focus on top discrepancies, not all wells
