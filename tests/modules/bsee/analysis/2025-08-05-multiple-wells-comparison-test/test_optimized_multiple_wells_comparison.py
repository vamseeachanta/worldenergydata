...
@pytest.mark.skipif(
    not OPTIMIZER_INTEGRATION_AVAILABLE,
    reason="Optimized comparison framework not available",
)
class TestOptimizedMultipleWellsComparisonFramework:
    ...
    def framework(self, temp_dir):
        ...
        performance_constraints = ResourceConstraints(
            max_chunk_size=20, enable_gc_optimization=True  # Smaller chunks for testing
        )
    ...
    def test_memory_optimization_integration(self, framework, sample_datasets):
        ...
        lease_data["large_numbers"] = np.random.randint(
            0, 100, len(lease_data)
        )  # Can be int8
        lease_data["categories"] = np.random.choice(
            ["A", "B", "C"], len(lease_data)
        )  # Can be category
        ...
        initial_memory = (
            (
                lease_data.memory_usage(deep=True).sum()
                + api12_data.memory_usage(deep=True).sum()
            )
            / 1024
            / 1024
        )
...
class TestOptimizedFrameworkIntegration:
    ...
    def test_120_plus_wells_integration(self):
        ...
        performance_constraints = ResourceConstraints(
            max_chunk_size=40, memory_warning_threshold=0.7, enable_gc_optimization=True
        )
        ...
        framework = OptimizedMultipleWellsComparisonFramework(
            performance_constraints=performance_constraints, report_config=report_config
        )
        ...
        assert (
            performance_metrics["average_memory_efficiency"] >= 0
        ...
        print(
            f"✓ Performance: {performance_metrics['wells_per_second']:.1f} wells/second"
        )
        print(
            f"✓ Memory efficiency: {performance_metrics['average_memory_efficiency']:.1f}%"
        )
    ...
    def test_performance_scalability(self):
        ...
        for size in dataset_sizes:
            ...
            framework = OptimizedMultipleWellsComparisonFramework(
                ...
                report_config=ReportConfig(include_charts=False, enable_appendix=False),
            )
            ...
            performance_results.append(
                {
                    ...
                    "wells_per_second": results["performance_metrics"][
                        "wells_per_second"
                    ],
        ...
        for i, result in enumerate(performance_results):
            ...
            print(
                f"Dataset {result['dataset_size']} wells: "
