...
class TestSodirIntegration(unittest.TestCase):
    ...
    def test_complete_data_collection_workflow(self):
        ...
        with patch.object(SodirAPIClient, 'get_blocks', return_value=self.mock_blocks_data), \
             patch.object(SodirAPIClient, 'get_wellbores', return_value=self.mock_wellbores_data), \
             patch.object(SodirAPIClient, 'get_fields', return_value=self.mock_fields_data):
    ...
    def test_data_processing_pipeline(self):
        ...
        for field in processed_fields:
            ...
            self.assertAlmostEqual(field['original_oil_bbl'], expected_oil_bbl, delta=1000)
    ...
    def test_storage_and_retrieval(self):
        ...
        # Verify files exist
        raw_blocks_file = Path(self.temp_dir) / 'raw' / f'blocks_{datetime.now().strftime("%Y%m%d")}.json'
