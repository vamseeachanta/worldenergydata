...
class SodirData:
        ...
        try:
            ...
            if validate:
                ...
                if self.config.get('collection', {}).get('strict_validation', False):
                    processed_data = [d for d in processed_data if d.get('_valid', True)]
            ...
            logger.info(f"Successfully collected {len(processed_data)} {dataset_type} records")
    ...
    def _fetch_data(
        ...
        # Fetch data
        return api_methods[dataset_type](**params) if params else api_methods[dataset_type]()
    ...
    def validate_discovery(self, discovery_data: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        raw_format = {
            ...
            'dscRecoverableCondensate': discovery_data.get('recoverable_condensate_msm3')
        }
    ...
    def validate_wellbore(self, wellbore_data: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        if not self.validator.validate_wellbore_name(wellbore_data['wellbore_name']):
            return False, [f"Invalid wellbore name format: {wellbore_data['wellbore_name']}"]
        return True, []
