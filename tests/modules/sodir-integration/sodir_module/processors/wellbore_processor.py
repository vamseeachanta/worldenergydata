...
class WellboreProcessor:
    ...
    def process(self, wellbore_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            processed = {
                ...
                'status_normalized': self.normalize_wellbore_status(wellbore_data.get('wlbStatus')),
                'purpose': wellbore_data.get('wlbPurpose', ''),
                'purpose_normalized': self.normalize_purpose(wellbore_data.get('wlbPurpose')),
                ...
                'total_depth_ft': self._convert_depth_to_feet(wellbore_data.get('wlbTotalDepth')),
                'kelly_bush_elevation_m': self._safe_float(wellbore_data.get('wlbKellyBushElevation')),
                'kelly_bush_elevation_ft': self._convert_depth_to_feet(wellbore_data.get('wlbKellyBushElevation')),
                'water_depth_m': self._safe_float(wellbore_data.get('wlbWaterDepth')),
                'water_depth_ft': self._convert_depth_to_feet(wellbore_data.get('wlbWaterDepth')),
                
                # Dates
                'completion_date': self._parse_date(wellbore_data.get('wlbCompletionDate')),
                'entry_date': self._parse_date(wellbore_data.get('wlbEntryDate')),
                'drilling_start_date': self._parse_date(wellbore_data.get('wlbDrillingStartDate')),
                'drilling_end_date': self._parse_date(wellbore_data.get('wlbDrillingEndDate')),
                ...
                'bottom_hole_temperature_c': self._safe_float(wellbore_data.get('wlbBottomHoleTemperature')),
                'max_inclination_deg': self._safe_float(wellbore_data.get('wlbMaxInclination')),
            ...
            if processed['drilling_start_date'] and processed['drilling_end_date']:
                processed['drilling_duration_days'] = self._calculate_duration(
                    processed['drilling_start_date'],
                    processed['drilling_end_date']
                )
            ...
            logger.error(f"Error processing wellbore {wellbore_data.get('wlbName', 'unknown')}: {str(e)}")
            self.error_count += 1
            return None
    ...
    def process_batch(self, wellbores: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        logger.info(f"Processed {len(processed_wellbores)}/{len(wellbores)} wellbores successfully")
        ...
        for field in depth_fields:
            if field in wellbore_data and wellbore_data[field] is not None:
                try:
                    ...
                    errors.append(f"Invalid depth value in {field}: {wellbore_data[field]}")
