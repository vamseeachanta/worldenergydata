...
class SodirBatchProcessor:
    ...
    def process_data_transformation(
        ...
        try:
            ...
            for chunk_result, error in results:
                if error:
                    batch_result.failed_items += len(chunk_result) if chunk_result else self.config.batch_size
                    batch_result.errors.append(str(error))
                else:
                    batch_result.processed_items += len(chunk_result) if chunk_result else 0
                    if chunk_result:
            ...
            if self.config.use_parallel and len(formats) > 1:
                ...
                with concurrent.futures.ThreadPoolExecutor(
                    ...
                    for future in concurrent.futures.as_completed(futures):
                        ...
                        try:
                            output_file = future.result(timeout=self.config.timeout_seconds)
                            if output_file:
                                batch_result.output_files.append(str(output_file))
                                batch_result.processed_items += 1
                            else:
                                batch_result.failed_items += 1
                                batch_result.errors.append(f"Failed to export {format_type}")
                        except Exception as e:
                            batch_result.failed_items += 1
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        successful = sum(1 for b in self.batch_history if b.status == BatchStatus.COMPLETED)
        ...
        return {
            ...
            'failed_batches': sum(1 for b in self.batch_history if b.status == BatchStatus.FAILED),
            'partial_batches': sum(1 for b in self.batch_history if b.status == BatchStatus.PARTIAL),
