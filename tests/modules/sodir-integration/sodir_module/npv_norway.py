...
class NorwegianFinancialParameters:
    ...
    petroleum_tax_rate: float = 0.78  # Total petroleum tax rate (special tax + corporate tax)
    ...
    net_revenue_interest: float = 1.0  # Revenue share after royalties (Norway has no royalties)
...
class NorwayNPVCalculator:
    ...
    def calculate_npv(self, production_profile: pd.DataFrame,
                     capex_schedule: Optional[List[float]] = None,
                     opex_override: Optional[pd.DataFrame] = None) -> CashFlowResult:
        """
        ...
        taxable_income = revenue_df['total_revenue'] - opex_df['total_opex'] - depreciation_df['depreciation']
        ...
        fcf = revenue_df['total_revenue'] - opex_df['total_opex'] - capex_df['capex'] - tax_df['total_tax']
        ...
        metrics = {
            'pi_ratio': self._calculate_profitability_index(fcf, capex_df['capex'].sum()),
            'breakeven_oil_price': self._calculate_breakeven_price(production_profile, cash_flows),
            'government_take': tax_df['total_tax'].sum() / revenue_df['total_revenue'].sum() if revenue_df['total_revenue'].sum() > 0 else 0
        }
    ...
    def calculate_revenue(self, production_profile: pd.DataFrame) -> pd.DataFrame:
        ...
        revenue['total_revenue'] = revenue['total_revenue'] * self.params.net_revenue_interest
    ...
    def calculate_taxes(self, taxable_income: float) -> Dict[str, float]:
        ...
        taxes['effective_rate'] = taxes['total_tax'] / taxable_income if taxable_income > 0 else 0
    ...
    def calculate_depreciation(self, capex_df: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        depreciation_schedule = pd.DataFrame(index=capex_df.index, columns=['depreciation'])
                ...
                if capex > 0:
                    ...
                    for j in range(self.params.depreciation_years):
                        if idx + j < len(depreciation_schedule):
                            depreciation_schedule.iloc[idx + j, 0] += annual_depreciation
    ...
    def sensitivity_analysis(self, production_profile: pd.DataFrame,
                           parameters: List[str] = None,
                           variations: List[float] = None) -> Dict[str, List[float]]:
        """
        ...
        original_params = {
            ...
            'capex': self.params.capex_schedule.copy() if self.params.capex_schedule else []
        }
        ...
        for param in parameters:
            ...
            for variation in variations:
                # Apply variation
                if param == 'oil_price':
                    self.params.oil_price_usd = original_params['oil_price'] * (1 + variation / 100)
                elif param == 'discount_rate':
                    self.params.discount_rate = original_params['discount_rate'] * (1 + variation / 100)
                elif param == 'opex':
                    self.params.opex_per_boe = original_params['opex'] * (1 + variation / 100)
                elif param == 'capex' and original_params['capex']:
                    self.params.capex_schedule = [c * (1 + variation / 100) for c in original_params['capex']]
    ...
    def _build_capex_dataframe(self, capex_schedule: List[float], num_years: int) -> pd.DataFrame:
    ...
    def _calculate_npv_from_cashflows(self, cash_flows: pd.Series, discount_rate: float) -> float:
    ...
    def _calculate_profitability_index(self, cash_flows: pd.Series, total_capex: float) -> float:
    ...
    def _calculate_breakeven_price(self, production_profile: pd.DataFrame, 
