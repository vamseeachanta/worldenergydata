...
class ComparisonResult:
    ...
    def summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'key_differences': {
                metric: diff for metric, diff in self.metrics_diff.items() 
                if abs(diff) > 0.1  # Significant differences only
            },
            'statistically_significant': [
                metric for metric, p_value in self.statistical_significance.items()
                if p_value < 0.05
            ],
...
class CrossRegionalAnalyzer:
    ...
    def normalize_data(self, data: Dict[str, pd.DataFrame], 
                      source: str = 'SODIR',
                      strategy: NormalizationStrategy = NormalizationStrategy.STANDARD) -> Dict[str, pd.DataFrame]:
        """
        ...
        if 'fields' in data:
            normalized['fields'] = self._normalize_fields(data['fields'], source, strategy)
        
        # Normalize wellbore data
        if 'wellbores' in data:
            normalized['wellbores'] = self._normalize_wellbores(data['wellbores'], source)
        
        # Normalize production data
        if 'production' in data:
            normalized['production'] = self._normalize_production(data['production'], source)
        
        return normalized
    
    def _normalize_fields(self, fields_df: pd.DataFrame, source: str,
                         strategy: NormalizationStrategy) -> pd.DataFrame:
        """Normalize fields data"""
        ...
        normalized.columns = [column_mapping.get(col, col) for col in normalized.columns]
        ...
        numerical_cols = ['oil_reserves_mmbbl', 'gas_reserves_bcf', 'water_depth_m', 'recovery_factor']
        ...
        for col in numerical_cols:
            if col in normalized.columns:
                if strategy == NormalizationStrategy.STANDARD:
                    ...
                    if max_val > min_val:
                        normalized[f'{col}_normalized'] = (normalized[col] - min_val) / (max_val - min_val)
        ...
        normalized['total_boe_mmboe'] = normalized['oil_reserves_mmbbl'] + normalized['gas_reserves_bcf'] / 6
        normalized['gas_oil_ratio'] = normalized['gas_reserves_bcf'] / (normalized['oil_reserves_mmbbl'] + 1)  # Avoid division by zero
    ...
    def _normalize_wellbores(self, wellbores_df: pd.DataFrame, source: str) -> pd.DataFrame:
        """Normalize wellbore data"""
        normalized = wellbores_df.copy()
        
        # Standardize column names
        if 'depth_m' not in normalized.columns and 'total_depth_ft' in normalized.columns:
        ...
        if 'drilling_days' in normalized.columns and 'depth_m' in normalized.columns:
            normalized['drilling_rate_m_per_day'] = normalized['depth_m'] / (normalized['drilling_days'] + 1)
        
        return normalized
    
    def _normalize_production(self, production_df: pd.DataFrame, source: str) -> pd.DataFrame:
        """Normalize production data"""
        ...
        if 'water_production_mmbbl' in normalized.columns and 'oil_production_mmbbl' in normalized.columns:
            total_liquid = normalized['water_production_mmbbl'] + normalized['oil_production_mmbbl']
            normalized['water_cut'] = normalized['water_production_mmbbl'] / (total_liquid + 0.001)
        
        return normalized
    
    def calculate_metrics(self, data: Dict[str, pd.DataFrame], region: str) -> RegionalMetrics:
        """
        ...
        total_oil = fields_df['oil_reserves_mmbbl'].sum() if 'oil_reserves_mmbbl' in fields_df else 0
        total_gas = fields_df['gas_reserves_bcf'].sum() if 'gas_reserves_bcf' in fields_df else 0
        avg_recovery = fields_df['recovery_factor'].mean() if 'recovery_factor' in fields_df else 0
        avg_depth = fields_df['water_depth_m'].mean() if 'water_depth_m' in fields_df else 0
    ...
    def _calculate_production_efficiency(self, production_df: pd.DataFrame, 
                                        fields_df: pd.DataFrame) -> float:
        ...
        if 'production_boe' in production_df:
            total_production = production_df.groupby('field_name')['production_boe'].sum().mean()
            return total_production
        
        return 0.0
    
    def _calculate_economic_indicators(self, fields_df: pd.DataFrame, 
                                      production_df: pd.DataFrame) -> Dict[str, float]:
        """Calculate economic indicators"""
        indicators = {}
        
        # Resource value (simplified)
        if not fields_df.empty:
            oil_value = fields_df.get('oil_reserves_mmbbl', pd.Series([0])).sum() * 80  # $80/bbl assumption
            gas_value = fields_df.get('gas_reserves_bcf', pd.Series([0])).sum() * 3  # $3/mcf assumption
    ...
    def compare_regions(self, sodir_metrics: RegionalMetrics, 
                       bsee_metrics: RegionalMetrics) -> ComparisonResult:
        """
        ...
        metrics_diff = {
            'recovery_factor_diff': sodir_metrics.average_recovery_factor - bsee_metrics.average_recovery_factor,
            'water_depth_diff_m': sodir_metrics.average_water_depth_m - bsee_metrics.average_water_depth_m,
            'field_size_diff_mmboe': sodir_metrics.average_field_size_mmboe - bsee_metrics.average_field_size_mmboe,
            'drilling_efficiency_diff': sodir_metrics.drilling_efficiency - bsee_metrics.drilling_efficiency,
            'discovery_rate_diff': sodir_metrics.discovery_rate - bsee_metrics.discovery_rate,
            'production_efficiency_diff': sodir_metrics.production_efficiency - bsee_metrics.production_efficiency
        }
        
        # Calculate percentage differences
        for key in metrics_diff:
            base_metric = key.replace('_diff', '').replace('_m', '').replace('_mmboe', '')
            sodir_val = getattr(sodir_metrics, base_metric if hasattr(sodir_metrics, base_metric) else key.replace('_diff', ''), 0)
            bsee_val = getattr(bsee_metrics, base_metric if hasattr(bsee_metrics, base_metric) else key.replace('_diff', ''), 0)
        ...
        statistical_significance = self._calculate_statistical_significance(sodir_metrics, bsee_metrics)
    ...
    def _calculate_statistical_significance(self, sodir_metrics: RegionalMetrics, 
                                          bsee_metrics: RegionalMetrics) -> Dict[str, float]:
        ...
        significance['recovery_factor'] = 0.01 if abs(sodir_metrics.average_recovery_factor - 
                                                     bsee_metrics.average_recovery_factor) > 0.1 else 0.5
        significance['water_depth'] = 0.01 if abs(sodir_metrics.average_water_depth_m - 
                                                 bsee_metrics.average_water_depth_m) > 500 else 0.5
        significance['field_size'] = 0.01 if abs(sodir_metrics.average_field_size_mmboe - 
                                                bsee_metrics.average_field_size_mmboe) > 100 else 0.5
        
        return significance
    
    def _generate_insights(self, metrics_diff: Dict[str, float], 
                         sodir_metrics: RegionalMetrics,
                         bsee_metrics: RegionalMetrics) -> List[str]:
        """Generate insights from comparison"""
        insights = []
        
        # Recovery factor insights
        if metrics_diff.get('recovery_factor_diff', 0) > 0.1:
            insights.append(f"Norwegian fields show {metrics_diff['recovery_factor_diff']:.1%} higher recovery factors, "
                          "potentially due to advanced EOR techniques or reservoir characteristics")
        elif metrics_diff.get('recovery_factor_diff', 0) < -0.1:
            insights.append(f"US Gulf fields show {abs(metrics_diff['recovery_factor_diff']):.1%} higher recovery factors")
        
        # Water depth insights
        if metrics_diff.get('water_depth_diff_m', 0) > 500:
            insights.append("US Gulf operations are significantly deeper water on average, "
                          "requiring more advanced subsea technology")
        elif metrics_diff.get('water_depth_diff_m', 0) < -500:
            insights.append("Norwegian operations are in significantly deeper water on average")
        
        # Field size insights
        if sodir_metrics.average_field_size_mmboe > bsee_metrics.average_field_size_mmboe * 2:
            insights.append("Norwegian fields are substantially larger on average, "
                          "potentially offering better economies of scale")
        elif bsee_metrics.average_field_size_mmboe > sodir_metrics.average_field_size_mmboe * 2:
            insights.append("US Gulf fields are substantially larger on average")
        
        # Drilling efficiency
        if metrics_diff.get('drilling_efficiency_diff', 0) > 10:
            insights.append("Norwegian drilling operations show higher efficiency, "
                          f"averaging {sodir_metrics.drilling_efficiency:.0f} meters per day")
        
        # Gas vs oil focus
        sodir_gor = sodir_metrics.total_gas_reserves_bcf / (sodir_metrics.total_oil_reserves_mmbbl + 1)
        bsee_gor = bsee_metrics.total_gas_reserves_bcf / (bsee_metrics.total_oil_reserves_mmbbl + 1)
        
        if sodir_gor > bsee_gor * 2:
            insights.append("Norwegian Continental Shelf is more gas-focused compared to oil-dominant US Gulf")
        elif bsee_gor > sodir_gor * 2:
            insights.append("US Gulf is more gas-focused compared to Norwegian operations")
        
        return insights
    
    def find_comparable_fields(self, sodir_fields: pd.DataFrame, 
        ...
        for fields_df in [sodir_fields, bsee_fields]:
            if 'size_class' not in fields_df.columns and 'total_boe_mmboe' in fields_df.columns:
            ...
            if 'water_depth_range' not in fields_df.columns and 'water_depth_m' in fields_df.columns:
        ...
        for _, sodir_field in sodir_fields.iterrows():
            ...
            if best_match is not None and best_score > 0.5:  # Threshold for matching
                matches.append({
                    ...
                    'matched_criteria': self._get_matched_criteria(sodir_field, best_match, criteria)
                })
    ...
    def _calculate_similarity(self, field1: pd.Series, field2: pd.Series, 
    ...
    def _get_matched_criteria(self, field1: pd.Series, field2: pd.Series, 
    ...
    def compare_production_efficiency(self, sodir_production: pd.DataFrame,
                                     bsee_production: pd.DataFrame) -> Dict[str, Any]:
        """
        ...
        if not sodir_production.empty and 'cumulative_oil' in sodir_production and 'recoverable_oil' in sodir_production:
            sodir_recovery = sodir_production['cumulative_oil'].sum() / sodir_production['recoverable_oil'].sum()
            efficiency['recovery_rate_sodir'] = sodir_recovery
        
        if not bsee_production.empty and 'cumulative_oil' in bsee_production and 'recoverable_oil' in bsee_production:
            bsee_recovery = bsee_production['cumulative_oil'].sum() / bsee_production['recoverable_oil'].sum()
        ...
        if 'wells_count' in sodir_production and 'cumulative_oil' in sodir_production:
            sodir_well_prod = sodir_production['cumulative_oil'].sum() / sodir_production['wells_count'].sum()
            efficiency['oil_per_well_sodir'] = sodir_well_prod
        
        if 'wells_count' in bsee_production and 'cumulative_oil' in bsee_production:
            bsee_well_prod = bsee_production['cumulative_oil'].sum() / bsee_production['wells_count'].sum()
        ...
        if 'oil_per_well_sodir' in efficiency and 'oil_per_well_bsee' in efficiency:
            efficiency['wells_productivity_ratio'] = efficiency['oil_per_well_sodir'] / efficiency['oil_per_well_bsee']
    ...
    def perform_statistical_tests(self, sodir_data: pd.DataFrame, 
        ...
        for var in variables:
            if var in sodir_data.columns and var in bsee_data.columns:
                ...
                if len(sodir_values) > 1 and len(bsee_values) > 1:
                    ...
                    u_stat, p_value_u = stats.mannwhitneyu(sodir_values, bsee_values, alternative='two-sided')
                    
                    # Levene's test for variance
                    levene_stat, p_value_levene = stats.levene(sodir_values, bsee_values)
