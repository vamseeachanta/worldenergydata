    ...
    def predict(self, time_periods: Union[int, List[int]]) -> np.ndarray:
        ...
        for t in time_periods:
            if self.model_type == "exponential":
                ...
                pred = self.initial_rate / ((1 + self.b_factor * self.decline_rate * t) ** (1/self.b_factor))
...
class ForecastResult:
    ...
    def to_dataframe(self) -> pd.DataFrame:
        ...
        df = pd.DataFrame({
            'period': self.forecast_periods if self.forecast_periods else range(len(self.forecast_values)),
...
class ProductionForecaster:
    ...
    def fit_decline_curve(self, production_data: pd.Series,
                         model_type: str = 'exponential') -> DeclineCurve:
        """
    ...
    def _fit_exponential_decline(self, time: np.ndarray, production: np.ndarray,
                                initial_rate: float) -> DeclineCurve:
    ...
    def _fit_hyperbolic_decline(self, time: np.ndarray, production: np.ndarray,
                               initial_rate: float) -> DeclineCurve:
    ...
    def _fit_harmonic_decline(self, time: np.ndarray, production: np.ndarray,
                             initial_rate: float) -> DeclineCurve:
    ...
    def _fit_linear_decline(self, time: np.ndarray, production: np.ndarray,
                          initial_rate: float) -> DeclineCurve:
    ...
    def forecast_production(self, historical_data: pd.DataFrame,
        ...
        if method == 'decline_curve':
            ...
            forecast = self._forecast_with_decline_curve(production, forecast_years, 'exponential')
        ...
        if len(production) > forecast_years:
            ...
            accuracy_metrics = {
                'mape': mean_absolute_percentage_error(test[:len(temp_forecast)], temp_forecast[:len(test)]),
                'rmse': np.sqrt(mean_squared_error(test[:len(temp_forecast)], temp_forecast[:len(test)]))
            }
        else:
        ...
        return ForecastResult(
            ...
            forecast_periods=list(range(len(production), len(production) + forecast_years))
        )
    
    def _forecast_with_decline_curve(self, production: pd.Series, 
                                    forecast_years: int,
    ...
    def _forecast_with_moving_average(self, production: pd.Series,
                                     forecast_years: int,
    ...
    def _forecast_with_polynomial(self, production: pd.Series,
                                 forecast_years: int,
    ...
    def _calculate_confidence_intervals(self, historical: pd.Series,
                                       forecast: np.ndarray,
    ...
    def ensemble_forecast(self, historical_data: pd.DataFrame,
        ...
        for model in models:
            try:
                if model in ['exponential', 'hyperbolic', 'harmonic']:
                    ...
                    production = historical_data.iloc[:, 0] if historical_data.shape[1] > 0 else historical_data
    ...
    def validate_forecast(self, forecast: np.ndarray, 
    ...
    def scenario_forecast(self, historical_data: pd.DataFrame,
    ...
    def calculate_eur(self, production_profile: pd.Series,
                     decline_curve: Optional[DeclineCurve] = None,
                     economic_limit: float = 0.0) -> float:
        """
