...
class TestBSEEPipeline:
    ...
    def test_production_api12_full_pipeline(self, production_data, well_data, tmp_path):
        ...
        config = {
            ...
            'query': {
                'flag': True,
                'api_list': ['177154051100', '177154051200']
            }
        }
        ...
        assert all(str(api) in config['query']['api_list'] for api in filtered_data['API_WELL_NUMBER'].unique())
        
        # 2. Test data aggregation
        monthly_production = filtered_data.groupby(['API_WELL_NUMBER', 'PRODUCTION_DATE']).agg({
            'OIL_VOLUME': 'sum',
            'GAS_VOLUME': 'sum',
            'WATER_VOLUME': 'sum'
        }).reset_index()
        ...
        # 3. Test calculations
        monthly_production['GOR'] = monthly_production['GAS_VOLUME'] / monthly_production['OIL_VOLUME'].replace(0, 1)
        monthly_production['WOR'] = monthly_production['WATER_VOLUME'] / monthly_production['OIL_VOLUME'].replace(0, 1)
        ...
        for api in api_list_as_int:
            api_data = monthly_production[monthly_production['API_WELL_NUMBER'] == api].copy()
            if len(api_data) > 0:
    ...
    def test_drilling_completion_days_analysis(self, custom_config, test_data_dir):
        ...
        if custom_config.get('drilling_n_completion_days', {}).get('flag', False):
            ...
            for well in wells:
                ...
                if len(well_prod) > 0:
                    well_prod['PRODUCTION_DATE'] = pd.to_datetime(well_prod['PRODUCTION_DATE'])
    ...
    def test_field_comparison_analysis(self, production_data, well_data):
        ...
        field_production = production_data.merge(
            well_data[['API_WELL_NUMBER', 'FIELD_NAME']], 
            on='API_WELL_NUMBER'
        )
        ...
        assert len(field_summary) > 0
        assert all(field in field_summary.index for field in ['ANCHOR', 'JULIA', 'JACK', 'ST MALO'])
    ...
    def test_economic_analysis_with_production(self, production_data):
        ...
        monthly = production_data.groupby(pd.Grouper(key='PRODUCTION_DATE', freq='M')).agg({
            'OIL_VOLUME': 'sum'
        })
