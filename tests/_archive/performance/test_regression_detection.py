...
class PerformanceBaseline:
    ...
    def __init__(self, baseline_file: Path = None):
        self.baseline_file = baseline_file or Path(
            "tests/performance/baselines/baseline.json"
        )
...
class RegressionDetector:
    ...
    def check_regression(
        self, operation: str, metrics: Dict, tolerance: str = "normal"
    ) -> Optional[Dict]:
        ...
        regression_info = {"operation": operation, "metrics": {}}
        ...
        if self.regressions:
            ...
            for reg in self.regressions:
                ...
                for metric, info in reg["metrics"].items():
                    ...
                    report.append(
                        f"    Change:   +{info['change_percent']:.1f}% (tolerance: {info['tolerance_percent']:.0f}%)"
                    )
        else:
...
class TestRegressionDetection:
    ...
    def test_baseline_creation(self, baseline_manager):
        ...
        baseline_manager.update_baseline(
            "test_operation",
            {"execution_time": 1.0, "memory_delta": 10.0, "peak_memory": 50.0},
        )
    ...
    def test_regression_detection(self, baseline_manager, detector):
        ...
        # Set baseline
        baseline_manager.update_baseline(
            "slow_operation", {"execution_time": 1.0, "memory_delta": 10.0}
        )

        # Test with regression (30% slower)
        regression = detector.check_regression(
            "slow_operation",
            {"execution_time": 1.3, "memory_delta": 10.0},
            tolerance="normal",
        )  # 20% tolerance
        ...
        no_regression = detector.check_regression(
            "slow_operation",
            {"execution_time": 1.15, "memory_delta": 10.0},
            tolerance="normal",
        )
    ...
    def test_improvement_detection(self, baseline_manager, detector):
        ...
        # Set baseline
        baseline_manager.update_baseline(
            "improved_operation", {"execution_time": 2.0, "memory_delta": 20.0}
        )

        # Test with significant improvement (50% faster)
        detector.check_regression(
            "improved_operation",
            {"execution_time": 1.0, "memory_delta": 10.0},
            tolerance="normal",
        )
    ...
    def test_measure_operation(self, detector):
        ...
        result, metrics = detector.measure_operation(
            "sample_op", sample_operation, 10000
        )
    ...
    def test_trend_analysis(self, tmp_path):
        ...
        for i, date in enumerate(dates):
            ...
            with open(file_path, "w") as f:
                json.dump({"date": date.isoformat(), "metrics": metrics}, f)
        ...
        exec_degradation_rate = (
            df["execution_time"].iloc[-1] / df["execution_time"].iloc[0] - 1
        ) * 100
        memory_degradation_rate = (
            df["memory_usage"].iloc[-1] / df["memory_usage"].iloc[0] - 1
        ) * 100
        ...
        if exec_degradation_rate > 20:
            warnings.warn(
                f"Significant performance degradation detected: {exec_degradation_rate:.1f}%"
            )
