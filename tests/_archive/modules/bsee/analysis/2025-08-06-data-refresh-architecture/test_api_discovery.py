...
class TestAPIDiscoveryDocumentation:
    ...
    def test_document_research_methodology(self):
        ...
        self.discovery.document_finding(
            "Methodology", "API discovery methodology established", methodology
        )
    ...
    def test_base_url_accessibility(self):
        ...
        for base_url in self.discovery.base_urls:
            accessible, status_code, reason = self.discovery.test_url_accessibility(
                base_url
            )
            ...
            if accessible:
                ...
                self.discovery.document_finding(
                    "Base URL Access", f"{base_url} is accessible", result_details
                )
            else:
                self.discovery.document_finding(
                    "Base URL Access", f"{base_url} is not accessible", result_details
                )
        ...
        logger.info(
            f"Documented accessibility for {len(self.discovery.base_urls)} base URLs"
        )
    ...
    def test_common_api_pattern_detection(self):
        ...
        for base_url in self.discovery.base_urls:
            for api_pattern in self.discovery.api_patterns:
                ...
                self.discovery.document_finding(
                    "API Pattern Testing", f"Tested {test_url}", result
                )

        # Document summary of API pattern testing
        summary = {
            "total_patterns_tested": len(self.discovery.base_urls)
            * len(self.discovery.api_patterns),
            "accessible_endpoints": len(api_endpoints_found),
            "found_endpoints": api_endpoints_found,
        }

        self.discovery.document_finding(
            "API Pattern Summary", "Common API pattern testing completed", summary
        )
    ...
    def test_documentation_section_analysis(self):
        ...
        for base_url in self.discovery.base_urls:
            for doc_pattern in self.discovery.doc_patterns:
                ...
                if result["accessible"] and result["status_code"] == 200:
                    if (
                        result["has_api_indicators"]
                        or "documentation" in result["content_type"]
                    ):
                        doc_sections_found.append(test_url)

                self.discovery.document_finding(
                    "Documentation Analysis", f"Analyzed {test_url}", result
                )

        # Document documentation analysis summary
        summary = {
            "total_doc_patterns_tested": len(self.discovery.base_urls)
            * len(self.discovery.doc_patterns),
            "documentation_sections_found": len(doc_sections_found),
            "found_sections": doc_sections_found,
        }

        self.discovery.document_finding(
            "Documentation Summary", "Documentation section analysis completed", summary
        )

        logger.info(
            f"Analyzed {summary['total_doc_patterns_tested']} documentation patterns"
        )
