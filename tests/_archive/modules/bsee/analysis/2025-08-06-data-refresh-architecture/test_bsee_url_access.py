    ...
    def validate_url_access(self, data_source: str) -> Dict:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                result["supports_range_requests"] = "bytes" in headers.get(
                    "accept-ranges", ""
                )

                # Validate content type is ZIP
                if (
                    "zip" not in result["content_type"].lower()
                    and "application/octet-stream" not in result["content_type"].lower()
                ):
                    result["error_details"] = (
                        f"Unexpected content type: {result['content_type']}"
                    )
                ...
                if result["content_length_mb"] > 0:
                    ...
                    if size_ratio < 0.1 or size_ratio > 10:  # Allow 10x variation
                        result["error_details"] = (
                            f"Unexpected file size: {result['content_length_mb']:.1f}MB (expected ~{expected_size_mb}MB)"
                        )
    ...
    def _test_download_stability(self, url: str) -> bool:
        ...
        try:
            ...
            if response.status_code in [
                200,
                206,
            ]:  # 200 if range not supported, 206 if partial
    ...
    def validate_portal_page(self) -> Dict:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                result["total_download_links"] = len(
                    [l for l in links if ".zip" in l["href"].lower()]
                )

                # Check if all expected links are present
                expected_sources = set(self.data_sources.keys())
                found_sources = set(download_links.keys())
                result["expected_links_present"] = expected_sources.issubset(
                    found_sources
                )
                ...
                if not result["expected_links_present"]:
                    ...
                    result["error_details"] = (
                        f"Missing download links for: {', '.join(missing)}"
                    )
            ...
            if response.status_code in [200, 206]:
                ...
                try:
                    ...
                    if b"PK\x01\x02" in sample_data or b"PK\x03\x04" in sample_data:
                        ...
                        if response.status_code == 200:  # Full file
                            with zipfile.ZipFile(
                                ...
                                total_compressed = sum(
                                    info.compress_size for info in zip_ref.infolist()
                                )
                        ...
                        result["error_details"] = (
                            "No ZIP file signatures found in sample"
                        )
                ...
                result["error_details"] = (
                    f"HTTP {response.status_code}: Could not download sample"
                )
...
class TestBSEEURLAccess:
    ...
    def test_well_data_url_access(self):
        ...
        assert result[
            "accessible"
        ], f"Well data URL not accessible: {result['error_details']}"
        assert (
        ...
        assert (
            result["content_length_mb"] > 1
        ), "Well data file too small (likely empty)"
        ...
        logger.info(
            f"Well data URL validated successfully: {result['content_length_mb']:.1f}MB"
        )
    ...
    def test_production_data_url_access(self):
        ...
        assert result[
            "accessible"
        ], f"Production data URL not accessible: {result['error_details']}"
        assert (
            result["status_code"] == 200
        ), f"Expected 200, got {result['status_code']}"
        ...
        assert (
            result["content_length_mb"] < 500
        ), "Production data file too large (>500MB)"

        # Download should be stable
        assert result["download_stable"], "Production data download not stable"

        logger.info(
            f"Production data URL validated successfully: {result['content_length_mb']:.1f}MB"
        )
    ...
    def test_war_data_url_access(self):
        ...
        assert result[
            "accessible"
        ], f"WAR data URL not accessible: {result['error_details']}"
        assert (
            result["status_code"] == 200
        ), f"Expected 200, got {result['status_code']}"
        ...
        logger.info(
            f"WAR data URL validated successfully: {result['content_length_mb']:.1f}MB"
        )
    ...
    def test_all_urls_comprehensive_validation(self):
        ...
        summary = {
            ...
            "all_sources_accessible": accessible_count
            == len(self.url_validator.data_sources),
            "average_response_time": sum(
                r["response_time"] for r in all_results.values()
            )
            / len(all_results),
        }

        self.url_validator.document_finding(
            "Comprehensive URL Validation", f"All BSEE data sources validated", summary
        )

        # All URLs should be accessible for successful implementation
        assert summary[
            "all_sources_accessible"
        ], f"Only {accessible_count}/3 data sources accessible"
        ...
        logger.info(
            f"All 3 BSEE data sources validated successfully: {total_size_mb:.1f}MB total"
        )
    ...
    def test_portal_page_validation(self):
        ...
        assert result[
            "accessible"
        ], f"Portal page not accessible: {result['error_details']}"
        assert (
            result["status_code"] == 200
        ), f"Expected 200, got {result['status_code']}"
        ...
        assert (
            result["total_download_links"] > 0
        ), "No download links found on portal page"

        # Expected links may not all be present (page structure may change)
        # But we should find at least some of our target files
        assert (
            len(result["download_links_found"]) > 0
        ), "None of the expected download links found"

        logger.info(
            f"Portal page validated: {len(result['download_links_found'])}/3 expected links found"
        )
    ...
    def teardown_method(self):
        ...
        if hasattr(self, "url_validator") and self.url_validator.findings:
            ...
            accessible_urls = len(
                [
                    f
                    for f in self.url_validator.findings
            ...
            report_lines.append(
                f"- Total validations performed: {len(self.url_validator.findings)}"
            )
