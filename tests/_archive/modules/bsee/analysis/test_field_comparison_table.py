...
class FieldComparisonTableGenerator:
    ...
    def extract_excel_method_data(self):
        ...
        try:
            ...
            if len(production_data) == 0:
                logger.warning(
                    "No production data found in Excel, using synthetic data"
                )
            ...
            # Align data lengths
            min_length = (
                min(len(production_data), len(oil_prices))
                if len(oil_prices) > 0
                else len(production_data)
            )
            ...
            total_revenue = sum(
                prod * price for prod, price in zip(production_data, oil_prices)
            )
            ...
            avg_daily_production = (
                total_production / num_months if num_months > 0 else 0
            )

            logger.info(
                f"Excel method data extracted: {num_months} months of production"
            )
    ...
    def _create_synthetic_worldenergydata_data(self):
        ...
        total_production = (
            85000000  # Higher production from BSEE comprehensive data (85M BBL)
        )
        avg_oil_price = (
            58.45  # Different price source/period reflecting broader market data
        )
    ...
    def generate_comparison_table(self, excel_data, worldenergydata_data):
        ...
        parameters = [
            (
                "Number of months of production",
                excel_data["num_months"],
                worldenergydata_data["num_months"],
                False,
                False,
            ),
            (
                "Production Start Month",
                excel_data["production_start"],
                worldenergydata_data["production_start"],
                False,
                False,
            ),
            (
                "Production End Month",
                excel_data["production_end"],
                worldenergydata_data["production_end"],
                False,
                False,
            ),
            (
                "Total production in BBL",
                excel_data["total_production_bbl"],
                worldenergydata_data["total_production_bbl"],
                False,
                True,
            ),
            (
                "Average oil price in USD",
                excel_data["avg_oil_price_usd"],
                worldenergydata_data["avg_oil_price_usd"],
                True,
                False,
            ),
            (
                "Total revenue in USD",
                excel_data["total_revenue_usd"],
                worldenergydata_data["total_revenue_usd"],
                True,
                False,
            ),
            (
                "Number of Wells - total",
                excel_data["total_wells"],
                worldenergydata_data["total_wells"],
                False,
                False,
            ),
            (
                "Number of Wells - producing",
                excel_data["producing_wells"],
                worldenergydata_data["producing_wells"],
                False,
                False,
            ),
            (
                "Total average daily Production by month",
                excel_data["avg_daily_production_per_month"],
                worldenergydata_data["avg_daily_production_per_month"],
                False,
                True,
            ),
        ]
        ...
        for param_name, excel_val, world_val, is_currency, is_bbl in parameters:
            ...
            table_lines.append(
                f"| {param_name} | {excel_formatted} | {world_formatted} |"
            )

        # Add analysis section
        table_lines.extend(["", "## Analysis Summary", "", "### Key Differences", ""])
        ...
        price_diff = (
            worldenergydata_data["avg_oil_price_usd"] - excel_data["avg_oil_price_usd"]
        )
        price_diff_pct = (price_diff / excel_data["avg_oil_price_usd"]) * 100

        revenue_diff = (
            worldenergydata_data["total_revenue_usd"] - excel_data["total_revenue_usd"]
        )
...
class TestFieldComparisonTable:
    ...
    def test_excel_data_extraction(self):
        ...
        required_fields = [
            ...
            "total_wells",
            "producing_wells",
            "avg_daily_production_per_month",
        ]
        ...
        assert (
            20 <= excel_data["avg_oil_price_usd"] <= 200
        ), "Oil price should be reasonable range"
        ...
        print(
            f"Production period: {excel_data['production_start']} to {excel_data['production_end']}"
        )
        ...
        print(
            f"Avg daily production per month: {excel_data['avg_daily_production_per_month']:,.0f} BBL"
        )
    ...
    def test_worldenergydata_extraction(self):
        ...
        assert (
            20 <= world_data["avg_oil_price_usd"] <= 200
        ), "Oil price should be reasonable range"
        ...
        print(
            f"Production period: {world_data['production_start']} to {world_data['production_end']}"
        )
        ...
        print(
            f"Avg daily production per month: {world_data['avg_daily_production_per_month']:,.0f} BBL"
        )
    ...
    def test_comparison_parameters_calculation(self):
        ...
        for param_key, param_name in parameters_to_test:
            ...
            if isinstance(excel_val, (int, float)) and isinstance(
                ...
                print(
                    f"{param_name}: Excel={excel_val:,.2f}, World={world_val:,.2f}, Diff={diff_pct:+.1f}%"
                )
            else:
    ...
    def test_comparison_table_save_and_verify(self):
        ...
        assert os.path.exists(
            filepath
        ), f"Comparison table file not created: {filepath}"
        ...
        assert (
            saved_content == table_content
        ), "Saved content doesn't match generated content"
