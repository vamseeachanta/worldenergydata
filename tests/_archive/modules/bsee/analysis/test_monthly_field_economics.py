...
class MonthlyFieldEconomicsGenerator:
        ...
        try:
            ...
            try:
                df_excel = pd.read_excel(
                    self.excel_file_path, sheet_name=self.excel_sheet, engine="openpyxl"
                )
            except ValueError:
                try:
                    df_excel = pd.read_excel(
                        self.excel_file_path, sheet_name=self.excel_sheet, engine="xlrd"
                    )
                except:
                    df_excel = pd.read_excel(
                        self.excel_file_path, sheet_name=self.excel_sheet
                    )
            ...
            for col_idx in range(
                2, min(df_excel.shape[1], 80)
            ):  # Extended range for more data
            ...
            return self._generate_synthetic_production_profile(
                60
            ), self._generate_synthetic_oil_prices(60)
    ...
    def _generate_synthetic_production_profile(self, months):
        ...
        for month in range(months):
            ...
            production.append(
                max(monthly_prod + noise, monthly_prod * 0.1)
            )  # Minimum 10% of initial
    ...
    def _extend_production_data(self, production_data, target_months):
        ...
        for month in range(current_months, target_months):
            monthly_prod = last_production * (1 - decline_rate) ** (
                month - current_months + 1
            )
            extended_data.append(
                max(monthly_prod, last_production * 0.05)
            )  # Minimum 5% of last
    ...
    def generate_monthly_economics_dataframe(self):
        ...
        start_date = datetime(
            2014, 8, 1
        )  # Jack St. Malo started production August 2014
        ...
        for month_idx in range(months_count):
            ...
            monthly_npv_contribution = (net_revenue - monthly_capex) / (
                (1 + self.discount_rate / 12) ** month_idx
            )
        ...
        logger.info(
            f"Generated monthly economics DataFrame with {len(df)} months of data"
        )
    ...
    def _calculate_monthly_wells(self, months_count):
        ...
        for month in range(months_count):
            ...
            if month < 6:
                total_wells = self.initial_wells + (
                    month * 1
                )  # Add 1 well per month initially
                producing_wells = max(
                    total_wells - 2, self.initial_wells - 2
                )  # Some wells not producing initially
            elif month < 24:
                ...
                producing_wells = (
                    total_wells - 1
                )  # Usually 1 well offline for maintenance
            else:
                # Mature field
                total_wells = self.max_wells
                producing_wells = (
                    total_wells - 2
                )  # 2 wells typically offline in mature field
...
class TestMonthlyFieldEconomics:
    ...
    def test_excel_data_extraction(self):
        ...
        assert len(production_data) == len(
            oil_prices
        ), "Production and price data should be same length"

        # Validate data ranges
        assert all(
            p > 0 for p in production_data
        ), "All production values should be positive"
        assert all(
            20 <= price <= 200 for price in oil_prices
    ...
    def test_monthly_economics_calculation(self):
        ...
        assert (
            df["Monthly_production_BBL"].sum() > 0
        ), "Should have positive total production"
        assert df["Cumulative_NPV"].iloc[-1] != 0, "Should have non-zero final NPV"

        # Validate cumulative calculations
        assert df[
            "Cumulative_revenue"
        ].is_monotonic_increasing, "Cumulative revenue should be increasing"
        assert df[
            "Cumulative_OPEX"
        ].is_monotonic_increasing, "Cumulative OPEX should be increasing"
        assert df[
            "Cumulative_CAPEX"
        ].is_monotonic_increasing, "Cumulative CAPEX should be increasing"
    ...
    def test_dataframe_column_validation(self):
        ...
        for col_name, expected_type in expected_columns.items():
            ...
            if expected_type == str:
                ...
                if col_name == "Month-Year":
                    assert all(
                        len(str(val)) == 7 and "-" in str(val) for val in df[col_name]
                    ), f"{col_name} should be in YYYY-MM format"
            elif expected_type in [float, int]:
                assert df[col_name].dtype in [
                    "float64",
                    "int64",
                ], f"{col_name} should be numeric"
                assert (
                    not df[col_name].isna().any()
                ), f"{col_name} should not have NaN values"

        # Validate business logic
        assert (
            df["Oil_sales"] == df["Monthly_production_BBL"] * df["Oil_price_USD"]
        ).all(), "Oil sales calculation should be correct"
        assert (
            df["Net_revenue_after_OPEX"] == df["Oil_sales"] - df["OPEX_monthly"]
        ).all(), "Net revenue calculation should be correct"
        assert (
            df["Wells_producing"].le(df["Wells_total"]).all()
        ), "Producing wells should not exceed total wells"
        ...
        print(
            f"+ DataFrame covers {df['Month-Year'].iloc[0]} to {df['Month-Year'].iloc[-1]}"
        )
    ...
    def test_csv_export_functionality(self):
        ...
        assert (
            abs(original_final_npv - loaded_final_npv) < 1000
        ), "NPV should be preserved in CSV"
    ...
    def test_data_accuracy_and_completeness(self):
        ...
        assert np.allclose(
            df["OPEX_monthly"], calculated_opex, rtol=0.01
        ), "OPEX calculation should be accurate"

        # Cumulative values should be consistent
        assert np.allclose(
            df["Cumulative_revenue"], df["Oil_sales"].cumsum()
        ), "Cumulative revenue should be accurate"
        assert np.allclose(
            df["Cumulative_OPEX"], df["OPEX_monthly"].cumsum()
        ), "Cumulative OPEX should be accurate"
        assert np.allclose(
            df["Cumulative_CAPEX"], df["CAPEX_monthly"].cumsum()
        ), "Cumulative CAPEX should be accurate"

        # Daily production rate should be monthly production / 30
        expected_daily_rate = df["Monthly_production_BBL"] / 30
        assert np.allclose(
            df["Daily_production_rate_BBL_per_day"], expected_daily_rate
        ), "Daily production rate should be accurate"

        # Well counts should be realistic
        assert (
            df["Wells_total"].max() <= 35
        ), "Total wells should be realistic for Jack St. Malo"
        ...
        print(
            f"+ Data spans {len(df)} months from {df['Month-Year'].iloc[0]} to {df['Month-Year'].iloc[-1]}"
        )
        ...
        print(
            f"Average monthly production: {df['Monthly_production_BBL'].mean():,.0f} BBL"
        )
