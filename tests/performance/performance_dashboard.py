...
class PerformanceDashboard:
    ...
    def load_regression_report(self, report_file: str = "tests/performance/regression_report.txt") -> str:
    ...
    def generate_html_dashboard(self, 
                               ...
                               regression_report: Optional[str] = None) -> str:
        """Generate HTML dashboard with performance metrics"""
        
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        
        # Process benchmark data if available
        benchmark_html = self._generate_benchmark_section(benchmark_data) if benchmark_data else ""
        
        # Process regression report
        regression_html = self._format_regression_report(regression_report) if regression_report else ""
...
"""
    ...
    def _get_avg_time(self, benchmark_data: Dict) -> str:
        ...
        times = [b.get('stats', {}).get('mean', 0) for b in benchmark_data['benchmarks']]
        if times:
    ...
    def _generate_recommendations(self, benchmark_data: Dict, regression_report: str) -> str:
        ...
        if benchmark_data and 'benchmarks' in benchmark_data:
            ...
            if slow_tests:
                recommendations.append(f"• Optimize slow tests: {', '.join(slow_tests[:3])}")
        
        # Check for regressions
        regression_count = self._count_regressions(regression_report)
        if regression_count > 0:
            recommendations.append(f"• Address {regression_count} performance regressions immediately")
