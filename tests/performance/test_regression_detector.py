...
class TestPerformanceRegressionDetector:
    ...
    def detector(self, tmp_path):
        ...
        return PerformanceRegressionDetector(
            baseline_path=str(tmp_path / "baselines"), threshold_percent=20.0
        )
    ...
    def test_update_baselines_smart(self, detector):
        ...
        original_regression_baseline = detector.baselines["test_regression"][
            "avg_execution_time"
        ]
        ...
        assert (
            detector.baselines["test_regression"]["avg_execution_time"]
            == original_regression_baseline
        )
...
class TestRegressionMonitor:
    ...
    def test_start_end_test(self, monitor):
        ...
        assert (
            len(monitor.detector.current_metrics["test_func"]["execution_times"]) == 1
        )
    ...
    def test_regression_warning(self, monitor):
        ...
        monitor.detector.baselines["test_func"] = {"avg_execution_time": 0.001}
...
class TestRegressionDetectorIntegration:
    ...
    def test_integration_with_benchmark(self, benchmark, tmp_path):
        ...
        detector.record_metric(
            "sample_function", benchmark.stats["mean"], memory_usage=None
        )
        ...
        detector.record_metric(
            "sample_function", benchmark.stats["mean"] * 1.5  # 50% slower
        )
