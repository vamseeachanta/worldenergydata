...
class PerformanceRegressionDetector:
    ...
    def __init__(
        self,
        baseline_path: str = "tests/performance/baselines",
    ...
    def detect_regression(self, test_name: str) -> Tuple[bool, Dict]:
        ...
        if current["memory_usage"] and baseline.get("avg_memory_usage"):
            ...
            mem_percent_change = (
                (current_avg_mem - baseline_avg_mem) / baseline_avg_mem
            ) * 100
            details["memory_percent_change"] = mem_percent_change
            details["has_memory_regression"] = (
                mem_percent_change > self.threshold_percent
            )
    ...
    def analyze_all_tests(self) -> Dict:
        ...
        for test_name in self.current_metrics:
            ...
            test_result = {"test": test_name, "details": details}
    ...
    def update_baselines_smart(self):
        ...
        for test_name in self.current_metrics:
            ...
            self.baselines[test_name] = {
                ...
                "std_execution_time": (
                    statistics.stdev(current["execution_times"])
                    if len(current["execution_times"]) > 1
                    else 0
                ),
            ...
            if current["memory_usage"]:
                self.baselines[test_name]["avg_memory_usage"] = statistics.mean(
