...
class DiscoveryProcessor:
    ...
    def process(self, discovery_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            processed = {
                ...
                "discovery_year": self._safe_int(
                    discovery_data.get("dscDiscoveryYear")
                ),
                # Classification
                "hydrocarbon_type": self.normalize_hc_type(
                    discovery_data.get("dscHcType")
                ),
                ...
                "recoverable_oil_mmbbl": self._convert_to_mmbbl(
                    discovery_data.get("dscRecoverableOil")
                ),
                "recoverable_gas_bcf": self._convert_to_bcf(
                    discovery_data.get("dscRecoverableGas")
                ),
                "recoverable_ngl_mmbbl": self._convert_to_mmbbl(
                    discovery_data.get("dscRecoverableNGL")
                ),
                "recoverable_condensate_mmbbl": self._convert_to_mmbbl(
                    discovery_data.get("dscRecoverableCondensate")
                ),
                # Resource estimates in original units (million Sm3)
                "recoverable_oil_msm3": self._safe_float(
                    discovery_data.get("dscRecoverableOil")
                ),
                "recoverable_gas_bsm3": self._safe_float(
                    discovery_data.get("dscRecoverableGas")
                ),
                "recoverable_ngl_msm3": self._safe_float(
                    discovery_data.get("dscRecoverableNGL")
                ),
                "recoverable_condensate_msm3": self._safe_float(
                    discovery_data.get("dscRecoverableCondensate")
                ),
            ...
            logger.error(
                f"Error processing discovery {discovery_data.get('dscName', 'unknown')}: {str(e)}"
            )
            self.error_count += 1
            return None
    ...
    def process_batch(self, discoveries: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        logger.info(
            f"Processed {len(processed_discoveries)}/{len(discoveries)} discoveries successfully"
        )
    ...
    def validate(self, discovery_data: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        if "dscDiscoveryYear" in discovery_data and discovery_data["dscDiscoveryYear"]:
            try:
                ...
                errors.append(
                    f"Invalid year value: {discovery_data['dscDiscoveryYear']}"
                )
        ...
        for field in resource_fields:
            if field in discovery_data and discovery_data[field] is not None:
                try:
                    ...
                    errors.append(
                        f"Invalid numeric value in {field}: {discovery_data[field]}"
                    )

        # Check that at least one resource type has a value
        has_resources = any(
            discovery_data.get(field) is not None
            and self._safe_float(discovery_data.get(field)) > 0
            for field in resource_fields
        )
    ...
    def _assess_commerciality(self, processed_data: Dict[str, Any]) -> bool:
        ...
        if status and any(
            term in status for term in ["DEVELOP", "PRODUC", "COMMERCIAL"]
        ):
            return True
