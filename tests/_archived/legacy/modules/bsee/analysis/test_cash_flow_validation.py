...
class CashFlowValidator:
    ...
    def extract_excel_production(self) -> List[float]:
        ...
        for col_idx in range(2, min(self.excel_data.shape[1], 60)):
            ...
            if pd.notna(prod_val) and isinstance(prod_val, (int, float)) and prod_val > 0:
                production.append(prod_val)
        
        return production
    
    def calculate_python_cash_flows(self, production: List[float], prices: List[float], 
                                  opex_per_bbl: float = 20.0) -> Dict:
        ...
        revenues = [prod * price for prod, price in zip(aligned_production, aligned_prices)]
    ...
    def calculate_variance_analysis(self, python_results: Dict, excel_benchmarks: Dict) -> Dict:
        ...
        min_periods = min(len(python_results.get('revenues', [])), 
        ...
        for i in range(min_periods):
            ...
            revenue_variance = abs(python_rev - excel_rev) / excel_rev * 100 if excel_rev != 0 else 0
            
            python_opex = python_results['opex'][i]
            excel_opex = excel_benchmarks['opex'][i]
            opex_variance = abs(python_opex - excel_opex) / excel_opex * 100 if excel_opex != 0 else 0
            
            python_ncf = python_results['net_cash_flows'][i]
            excel_ncf = excel_benchmarks['net_cash_flows'][i]
            ncf_variance = abs(python_ncf - excel_ncf) / excel_ncf * 100 if excel_ncf != 0 else 0
        ...
        variance_analysis['total_revenue_variance'] = abs(python_total_rev - excel_total_rev) / excel_total_rev * 100 if excel_total_rev != 0 else 0
        
        python_total_opex = sum(python_results['opex'])
        excel_total_opex = sum(excel_benchmarks['opex'])
        variance_analysis['total_opex_variance'] = abs(python_total_opex - excel_total_opex) / excel_total_opex * 100 if excel_total_opex != 0 else 0
        
        python_total_ncf = sum(python_results['net_cash_flows'])
        excel_total_ncf = sum(excel_benchmarks['net_cash_flows'])
        variance_analysis['total_ncf_variance'] = abs(python_total_ncf - excel_total_ncf) / excel_total_ncf * 100 if excel_total_ncf != 0 else 0
        
        # Summary statistics
        period_rev_variances = [pv['revenue_variance_pct'] for pv in variance_analysis['period_variances']]
        period_ncf_variances = [pv['ncf_variance_pct'] for pv in variance_analysis['period_variances']]
        
        variance_analysis['summary'] = {
            'avg_revenue_variance_pct': np.mean(period_rev_variances) if period_rev_variances else 0,
            'max_revenue_variance_pct': max(period_rev_variances) if period_rev_variances else 0,
            'avg_ncf_variance_pct': np.mean(period_ncf_variances) if period_ncf_variances else 0,
            'max_ncf_variance_pct': max(period_ncf_variances) if period_ncf_variances else 0,
            'periods_analyzed': min_periods
        }
        
        return variance_analysis
    
    def generate_comparison_report(self, python_results: Dict, excel_benchmarks: Dict, 
                                 variance_analysis: Dict) -> str:
        ...
        report.append(f"Average Revenue Variance: {summary.get('avg_revenue_variance_pct', 0):.2f}%")
        report.append(f"Maximum Revenue Variance: {summary.get('max_revenue_variance_pct', 0):.2f}%")
        report.append(f"Average Net Cash Flow Variance: {summary.get('avg_ncf_variance_pct', 0):.2f}%")
        report.append(f"Maximum Net Cash Flow Variance: {summary.get('max_ncf_variance_pct', 0):.2f}%")
        report.append("")
        ...
        if python_results and excel_benchmarks:
            ...
            report.append(f"Revenue Variance: {variance_analysis.get('total_revenue_variance', 0):.2f}%")
            report.append("")
            ...
            report.append(f"Net Cash Flow Variance: {variance_analysis.get('total_ncf_variance', 0):.2f}%")
        ...
        report.append("-" * 60)
        report.append(f"{'Period':<8} {'Rev Var %':<10} {'OPEX Var %':<12} {'NCF Var %':<10}")
        report.append("-" * 60)
        
        period_variances = variance_analysis.get('period_variances', [])
        for pv in period_variances[:5]:  # Show first 5 periods
            report.append(f"{pv['period']:<8} {pv['revenue_variance_pct']:<10.2f} "
    ...
    def create_visual_comparison_data(self, python_results: Dict, excel_benchmarks: Dict) -> pd.DataFrame:
        ...
        for i in range(min_periods):
            comparison_data.append({
                ...
                'Python_NCF': python_results['net_cash_flows'][i],
                'Excel_NCF': excel_benchmarks['net_cash_flows'][i],
                'Revenue_Variance_Pct': abs(python_results['revenues'][i] - excel_benchmarks['revenues'][i]) / excel_benchmarks['revenues'][i] * 100 if excel_benchmarks['revenues'][i] != 0 else 0,
                'NCF_Variance_Pct': abs(python_results['net_cash_flows'][i] - excel_benchmarks['net_cash_flows'][i]) / excel_benchmarks['net_cash_flows'][i] * 100 if excel_benchmarks['net_cash_flows'][i] != 0 else 0
            })
...
class TestCashFlowValidation:
    ...
    def test_excel_price_extraction(self, validator):
        ...
        assert all(20 < price < 200 for price in prices), "Some prices outside reasonable range"
        
        # Verify known values
        assert abs(prices[0] - 62.34) < 0.01, f"First price mismatch: expected 62.34, got {prices[0]}"
    ...
    def test_excel_production_extraction(self, validator):
        ...
        assert len(production) >= 12, f"Expected at least 12 production values, got {len(production)}"
        assert all(prod > 1000 for prod in production), "Some production values seem too low"
        
        # Verify known values
        assert abs(production[0] - 19123.987142857142) < 1.0, f"First production mismatch"
    ...
    def test_python_cash_flow_calculation(self, validator):
        ...
        results = validator.calculate_python_cash_flows(production, prices, opex_per_bbl)
    ...
    def test_variance_analysis_calculation(self, validator):
        ...
        variance_analysis = validator.calculate_variance_analysis(python_results, excel_benchmarks)
    ...
    def test_comparison_report_generation(self, validator):
        ...
        variance_analysis = validator.calculate_variance_analysis(python_results, excel_benchmarks)
        report = validator.generate_comparison_report(python_results, excel_benchmarks, variance_analysis)
    ...
    def test_visual_comparison_data_creation(self, validator):
        ...
        comparison_df = validator.create_visual_comparison_data(python_results, excel_benchmarks)
    ...
    def test_full_validation_workflow(self, validator):
        ...
        python_results = validator.calculate_python_cash_flows(production, prices, opex_per_bbl=20.0)
        
        # Create mock Excel benchmarks (in real implementation, these would come from Excel)
        excel_benchmarks = {
            'revenues': [prod * price * 1.05 for prod, price in zip(python_results['production'], python_results['prices'])],  # 5% higher
            'opex': [prod * 21.0 for prod in python_results['production']],  # $1 higher per barrel
            'net_cash_flows': []
        }
        excel_benchmarks['net_cash_flows'] = [rev - opex for rev, opex in zip(excel_benchmarks['revenues'], excel_benchmarks['opex'])]
        ...
        print(f"Full validation completed for {variance_analysis['summary']['periods_analyzed']} periods")
    ...
    def test_validation_with_target_variance_threshold(self, validator):
        ...
        excel_benchmarks = {
            'revenues': [prod * price * 1.05 for prod, price in zip(python_results['production'], python_results['prices'])],  # 5% higher (well within 20% threshold)
            'opex': [prod * 19.5 for prod in python_results['production']],  # Slightly lower OPEX ($0.50 less)
            'net_cash_flows': []
        }
        excel_benchmarks['net_cash_flows'] = [rev - opex for rev, opex in zip(excel_benchmarks['revenues'], excel_benchmarks['opex'])]
        ...
        assert avg_ncf_variance <= TARGET_VARIANCE_THRESHOLD, f"Average NCF variance {avg_ncf_variance:.2f}% exceeds threshold {TARGET_VARIANCE_THRESHOLD}%"
        assert max_ncf_variance <= TARGET_VARIANCE_THRESHOLD * 1.5, f"Maximum NCF variance {max_ncf_variance:.2f}% exceeds 1.5x threshold"
