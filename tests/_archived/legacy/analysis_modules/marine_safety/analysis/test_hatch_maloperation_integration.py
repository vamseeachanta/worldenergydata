...
class TestHatchAnalyzerWithLLM:
    ...
    def sample_incidents(self) -> pd.DataFrame:
        ...
        return pd.DataFrame({
            ...
            'SEVERITY': ['MINOR', 'SERIOUS', 'MINOR', 'SERIOUS', 'CRITICAL',
                        'MINOR', 'SERIOUS', 'MINOR', 'CRITICAL', 'MINOR'],
    ...
    def test_hatch_analyzer_with_llm_enabled(self, sample_incidents):
        ...
        hatch_cases = [r for r in results['classifications'] if r['is_hatch_maloperation']]
    ...
    def test_hatch_analyzer_llm_vs_regex_comparison(self, sample_incidents):
        ...
        assert len(llm_results['classifications']) == len(regex_results['classifications'])

        # LLM should catch at least as many cases as regex
        llm_positives = sum(1 for r in llm_results['classifications'] if r['is_hatch_maloperation'])
        regex_positives = sum(1 for r in regex_results['classifications'] if r['is_hatch_maloperation'])
    ...
    def test_hybrid_detection_fallback(self, sample_incidents):
        ...
        original_classify = analyzer.llm_classifier.classify if hasattr(analyzer, 'llm_classifier') else None

        def mock_classify_with_failure(text):
            if "collision" in text.lower():
                raise Exception("LLM service temporarily unavailable")
            return original_classify(text) if original_classify else {'label': 'other', 'confidence': 0.5}

        if hasattr(analyzer, 'llm_classifier'):
            with patch.object(analyzer.llm_classifier, 'classify', side_effect=mock_classify_with_failure):
                results = analyzer.analyze(sample_incidents)
    ...
    def test_llm_confidence_threshold(self, sample_incidents):
        ...
        high_positives = sum(1 for r in high_results['classifications'] if r['is_hatch_maloperation'])
        low_positives = sum(1 for r in low_results['classifications'] if r['is_hatch_maloperation'])
    ...
    def test_batch_processing_consistency(self, sample_incidents):
        ...
        for batch_r, indiv_r in zip(batch_results['classifications'], individual_results):
...
class TestLLMErrorHandling:
    ...
    def problematic_incidents(self) -> pd.DataFrame:
        ...
        return pd.DataFrame({
            ...
            'DATE': [None, '2024-01-01', 'invalid-date', '2024-01-03', '2024-01-04'],
            'NARRATIVE': [
                None,                    # None value
                "",                      # Empty string
    ...
    def test_partial_llm_failure_handling(self, problematic_incidents):
        ...
        if hasattr(analyzer, 'llm_classifier'):
            with patch.object(analyzer.llm_classifier, 'classify',
...
class TestAnalysisReportGeneration:
    ...
    def classified_incidents(self) -> pd.DataFrame:
        ...
        return pd.DataFrame({
            'INCIDENT_ID': range(1, 11),
            'DATE': [(datetime.now() - timedelta(days=i*30)).strftime('%Y-%m-%d') for i in range(10)],
            'NARRATIVE': [
                "hatch not secured",
                "collision",
                "access hatch open",
            ...
            'SEVERITY': ['SERIOUS'] * 10,
            'VESSEL_TYPE': ['BULK CARRIER'] * 10,
            'IS_HATCH_MALOPERATION': [True, False, True, False, True, False, True, False, True, False],
            'CONFIDENCE': [0.95, 0.85, 0.92, 0.88, 0.90, 0.87, 0.93, 0.86, 0.91, 0.84],
        })
...
class TestConfigurationOptions:
    ...
    def test_disable_llm_configuration(self, basic_incidents):
        ...
        assert all('confidence' not in r or r['confidence'] == 1.0
    ...
    def test_batch_size_configuration(self, basic_incidents):
        ...
        assert len(small_results['classifications']) == len(large_results['classifications'])
...
pytestmark = pytest.mark.skipif(
    not pytest.config.getoption("--with-llm", default=False),
