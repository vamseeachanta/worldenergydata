...
class SurveyProcessor:
    ...
    def process(self, survey_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            processed = {
                ...
                'acquisition_start_date': self._parse_date(survey_data.get('srvAcquisitionStartDate')),
                'acquisition_end_date': self._parse_date(survey_data.get('srvAcquisitionEndDate')),
                'acquisition_year': self._safe_int(survey_data.get('srvAcquisitionYear')),
                ...
                'coverage_percentage': self._safe_float(survey_data.get('srvCoveragePercent')),
                
                # Technical parameters
                'source_type': survey_data.get('srvSourceType'),
                'source_volume': self._safe_float(survey_data.get('srvSourceVolume')),
                'source_pressure': self._safe_float(survey_data.get('srvSourcePressure')),
                'streamer_length_m': self._safe_float(survey_data.get('srvStreamerLength')),
                ...
                'processing_status': self.normalize_processing_status(survey_data.get('srvProcessingStatus')),
                ...
                'block_coverage': self._parse_blocks(survey_data.get('srvBlockCoverage')),
                'geometry': survey_data.get('geometry'),
                
                # Data availability
                'data_owner': survey_data.get('srvDataOwner'),
                'data_available': self._parse_boolean(survey_data.get('srvDataAvailable')),
                'confidentiality_expiry': self._parse_date(survey_data.get('srvConfidentialityExpiry')),
            ...
            if processed['acquisition_start_date'] and processed['acquisition_end_date']:
            ...
            if processed['acquisition_year']:
                processed['survey_age_years'] = datetime.now().year - processed['acquisition_year']
            ...
            if processed['line_km'] and processed['area_km2'] and processed['area_km2'] > 0:
            ...
            logger.error(f"Error processing survey {survey_data.get('srvName', 'unknown')}: {str(e)}")
            self.error_count += 1
            return None
    ...
    def process_batch(self, surveys: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        logger.info(f"Processed {len(processed_surveys)}/{len(surveys)} surveys successfully")
    ...
    def validate(self, survey_data: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        if start_date and end_date:
            try:
                ...
                if start and end and start > end:
                    errors.append(f"Acquisition end date ({end}) before start date ({start})")
            except Exception:
                pass
        ...
        if 'srvAcquisitionYear' in survey_data and survey_data['srvAcquisitionYear']:
            try:
                ...
                errors.append(f"Invalid year value: {survey_data['srvAcquisitionYear']}")
                
        # Validate technical parameters
        if 'srvStreamerCount' in survey_data and survey_data['srvStreamerCount'] is not None:
            try:
                ...
                errors.append(f"Invalid streamer count value: {survey_data['srvStreamerCount']}")
