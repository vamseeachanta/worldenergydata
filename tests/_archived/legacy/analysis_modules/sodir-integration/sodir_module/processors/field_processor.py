...
class FieldProcessor:
    ...
    def process(self, field_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            processed = {
                ...
                'ownership_share': self._safe_float(field_data.get('fldOwnershipShare')),
                
                # Discovery and production dates
                'discovery_year': self._safe_int(field_data.get('fldDiscoveryYear')),
                'production_start_year': self._safe_int(field_data.get('fldProductionStartYear')),
                'cessation_year': self._safe_int(field_data.get('fldCessationYear')),
                
                # Original reserves (in place)
                'original_reserves_oil_mmbbl': self._convert_to_mmbbl(field_data.get('fldOriginalReservesOil')),
                'original_reserves_gas_bcf': self._convert_to_bcf(field_data.get('fldOriginalReservesGas')),
                'original_reserves_ngl_mmbbl': self._convert_to_mmbbl(field_data.get('fldOriginalReservesNGL')),
                'original_reserves_condensate_mmbbl': self._convert_to_mmbbl(field_data.get('fldOriginalReservesCondensate')),
                
                # Remaining reserves
                'remaining_reserves_oil_mmbbl': self._convert_to_mmbbl(field_data.get('fldRemainingReservesOil')),
                'remaining_reserves_gas_bcf': self._convert_to_bcf(field_data.get('fldRemainingReservesGas')),
                'remaining_reserves_ngl_mmbbl': self._convert_to_mmbbl(field_data.get('fldRemainingReservesNGL')),
                'remaining_reserves_condensate_mmbbl': self._convert_to_mmbbl(field_data.get('fldRemainingReservesCondensate')),
                
                # Recoverable reserves
                'recoverable_oil_mmbbl': self._convert_to_mmbbl(field_data.get('fldRecoverableOil')),
                'recoverable_gas_bcf': self._convert_to_bcf(field_data.get('fldRecoverableGas')),
                'recoverable_ngl_mmbbl': self._convert_to_mmbbl(field_data.get('fldRecoverableNGL')),
                'recoverable_condensate_mmbbl': self._convert_to_mmbbl(field_data.get('fldRecoverableCondensate')),
                
                # Production totals
                'cumulative_oil_production_mmbbl': self._convert_to_mmbbl(field_data.get('fldCumulativeOilProduction')),
                'cumulative_gas_production_bcf': self._convert_to_bcf(field_data.get('fldCumulativeGasProduction')),
                'cumulative_ngl_production_mmbbl': self._convert_to_mmbbl(field_data.get('fldCumulativeNGLProduction')),
                'cumulative_condensate_production_mmbbl': self._convert_to_mmbbl(field_data.get('fldCumulativeCondensateProduction')),
            ...
            if processed['recoverable_oil_mmbbl'] and processed['cumulative_oil_production_mmbbl']:
                processed['oil_production_efficiency'] = round(
                    processed['cumulative_oil_production_mmbbl'] / processed['recoverable_oil_mmbbl'],
                    3
                )
            else:
            ...
            if processed['discovery_year']:
                processed['field_age_years'] = datetime.now().year - processed['discovery_year']
                
            # Calculate production duration
            if processed['production_start_year']:
                if processed['cessation_year']:
                    processed['production_duration_years'] = processed['cessation_year'] - processed['production_start_year']
                else:
                    processed['production_duration_years'] = datetime.now().year - processed['production_start_year']
                    
            self.processed_count += 1
            return processed
            
        except Exception as e:
            logger.error(f"Error processing field {field_data.get('fldName', 'unknown')}: {str(e)}")
            self.error_count += 1
            return None
    ...
    def process_batch(self, fields: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        logger.info(f"Processed {len(processed_fields)}/{len(fields)} fields successfully")
    ...
    def validate(self, field_data: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        reserve_fields = [
            'fldOriginalReservesOil', 'fldOriginalReservesGas',
            'fldRemainingReservesOil', 'fldRemainingReservesGas',
        ...
        for field in reserve_fields:
            if field in field_data and field_data[field] is not None:
                try:
                    ...
                    errors.append(f"Invalid numeric value in {field}: {field_data[field]}")
                    
        # Validate logical relationships
        if all(field in field_data for field in ['fldOriginalReservesOil', 'fldRemainingReservesOil']):
            try:
                original = float(field_data['fldOriginalReservesOil'] or 0)
                remaining = float(field_data['fldRemainingReservesOil'] or 0)
                if remaining > original:
                    errors.append(f"Remaining reserves ({remaining}) exceed original reserves ({original})")
            except (ValueError, TypeError):
                pass
    ...
    def _calculate_recovery_factor(self, original: Optional[float], remaining: Optional[float]) -> Optional[float]:
        """
