...
class SodirCacheOptimizer:
    ...
    def warm_cache(self, api_client: Any) -> Dict[str, int]:
        ...
        for endpoint, params in self.common_patterns:
            try:
                ...
                logger.info(f"Warmed cache for {endpoint} with {warmed[endpoint]} items")
    ...
    def get_statistics(self) -> CacheStats:
        ...
        eviction_penalty = max(0, 1.0 - (self.stats.evictions / max(1, self.stats.total_requests)))
        self.stats.cache_efficiency_score = (hit_rate * 0.5 + size_efficiency * 0.3 + eviction_penalty * 0.2)
    ...
    def _evict_lfu(self) -> bool:
        ...
        for key, entry in self.cache.items():
            ...
            if entry.priority > 5 and self.current_size_bytes < self.max_size_bytes * 0.9:
                continue
