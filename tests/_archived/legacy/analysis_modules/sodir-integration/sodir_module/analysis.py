...
class SodirDataLoader:
    ...
    def load_data(self, data_types: Optional[List[str]] = None) -> Dict[str, pd.DataFrame]:
...
class SodirAnalysis:
    ...
    def analyze_field(self, field_name: str) -> FieldAnalysisResult:
        ...
        field_data = self.data['fields'][self.data['fields']['field_name'] == field_name]
        if field_data.empty:
    ...
    def analyze_temporal_trends(self) -> Dict[str, Any]:
        ...
        if 'production' in self.data:
            ...
            if 'year' in production_df.columns:
                results['production_trends'] = {
                    'oil_trend': production_df.groupby('year')['oil_production_mmbbl'].sum().to_dict(),
                    'gas_trend': production_df.groupby('year')['gas_production_bcf'].sum().to_dict()
                }
        ...
        if 'fields' in self.data:
            ...
            if 'production_start' in fields_df.columns:
                ...
                fields_df['years_producing'] = current_year - fields_df['production_start']
                
                results['maturity_progression'] = {
                    'early': len(fields_df[fields_df['years_producing'] < 10]),
                    'mature': len(fields_df[(fields_df['years_producing'] >= 10) & 
    ...
    def _calculate_reserves(self, field_data: pd.DataFrame, 
                          production_data: Optional[pd.DataFrame]) -> Dict[str, float]:
        ...
        if production_data is not None and not production_data.empty:
            ...
            reserves['remaining_oil_mmbbl'] = reserves['original_oil_mmbbl'] - cumulative_oil
            reserves['remaining_gas_bcf'] = reserves['original_gas_bcf'] - cumulative_gas
            reserves['depletion_percentage'] = (cumulative_oil / reserves['original_oil_mmbbl']) * 100
        
        return reserves
    
    def _calculate_economics(self, field_data: pd.DataFrame, 
                           production_data: Optional[pd.DataFrame]) -> Dict[str, float]:
        ...
        economics = {
            ...
            'estimated_revenue_mmusd': field_data['recoverable_oil_mmbbl'].iloc[0] * oil_price
        }
        
        if production_data is not None:
            future_production = field_data['recoverable_oil_mmbbl'].iloc[0] - production_data['oil_production_mmbbl'].sum()
    ...
    def _summarize_results(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        summary = {
            'fields_analyzed': len([k for k in results.keys() if k != 'portfolio' and k != 'temporal']),
...
class FieldAnalyzer:
    ...
    def analyze(self, field_data: pd.DataFrame, wellbore_data: pd.DataFrame, 
    ...
    def _analyze_production(self, production_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            ...
            'years_producing': len(production_data['year'].unique()) if 'year' in production_data else 0
        }
...
class PortfolioAnalyzer:
    ...
    def _calculate_portfolio_metrics(self, fields_data: pd.DataFrame) -> Dict[str, float]:
    ...
    def _calculate_concentration(self, fields_data: pd.DataFrame) -> float:
        ...
        total_boe = (fields_data['recoverable_oil_mmbbl'] + 
                    fields_data['recoverable_gas_bcf'] / 6).sum()
        
        if total_boe == 0:
            return 0
        
        market_shares = (fields_data['recoverable_oil_mmbbl'] + 
                        fields_data['recoverable_gas_bcf'] / 6) / total_boe
    ...
    def _assess_risk_profile(self, fields_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            'water_depth_risk': 'high' if fields_data['water_depth_m'].mean() > 1000 else 'moderate',
...
class TemporalAnalyzer:
    ...
    def analyze(self, data: Dict[str, pd.DataFrame], resolution: str = 'yearly') -> Dict[str, Any]:
        """Analyze temporal trends in data"""
        return {
            'production_trends': self._analyze_production_trends(data.get('production')),
    ...
    def _analyze_production_trends(self, production_data: Optional[pd.DataFrame]) -> Dict[str, Any]:
        """Analyze production trends over time"""
        if production_data is None or production_data.empty:
            return {}
        
        return {
            'oil_trend': 'declining' if self._is_declining(production_data, 'oil_production_mmbbl') else 'stable',
            'gas_trend': 'declining' if self._is_declining(production_data, 'gas_production_bcf') else 'stable'
        }
    ...
    def _analyze_discovery_trends(self, fields_data: Optional[pd.DataFrame]) -> Dict[str, Any]:
    ...
    def _analyze_activity_trends(self, wellbore_data: Optional[pd.DataFrame]) -> Dict[str, Any]:
...
class MetricsCalculator:
    ...
    def calculate_field_metrics(self, field_data: pd.DataFrame, 
                               wellbore_data: pd.DataFrame,
                               production_data: pd.DataFrame) -> Dict[str, float]:
        ...
        if 'production_start' in field_data.columns:
            years_producing = datetime.now().year - field_data['production_start'].iloc[0]
        ...
        if not wellbore_data.empty and 'drilling_days' in wellbore_data.columns:
            metrics['drilling_efficiency'] = 1.0 / (wellbore_data['drilling_days'].mean() / 30)
        
        # Production efficiency
        if not production_data.empty:
            if 'oil_production_mmbbl' in production_data.columns:
                metrics['average_oil_rate'] = production_data['oil_production_mmbbl'].mean()
            if 'gas_production_bcf' in production_data.columns:
                metrics['average_gas_rate'] = production_data['gas_production_bcf'].mean()
        
        return metrics
    
    def calculate_portfolio_metrics(self, fields_data: pd.DataFrame) -> Dict[str, float]:
        """Calculate portfolio-level metrics"""
        metrics = {}
        ...
        metrics['total_boe_mmboe'] = (
            metrics['total_oil_reserves_mmbbl'] + 
            metrics['total_gas_reserves_bcf'] / 6
        )
        
        # Efficiency metrics
        metrics['average_recovery_factor'] = fields_data['recovery_factor'].mean()
        metrics['weighted_recovery_factor'] = np.average(
            fields_data['recovery_factor'],
            weights=fields_data['recoverable_oil_mmbbl']
        )
