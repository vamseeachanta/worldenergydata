...
class CoordinateTransformer:
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.warning("pyproj not available, using internal transformation methods")
    ...
    def utm_to_wgs84(
        ...
        try:
            if self.use_pyproj:
                return self._utm_to_wgs84_pyproj(easting, northing, zone, northern_hemisphere)
            else:
                return self._utm_to_wgs84_internal(easting, northing, zone, northern_hemisphere)
        except Exception as e:
            logger.error(f"Error converting UTM to WGS84: {str(e)}")
            self.error_count += 1
            raise
            
    def wgs84_to_utm(self, latitude: float, longitude: float) -> Tuple[float, float, int]:
        """
    ...
    def batch_utm_to_wgs84(
        ...
        for easting, northing, zone in coordinates:
            try:
                ...
                logger.error(f"Error in batch conversion for {easting}, {northing}, zone {zone}: {str(e)}")
    ...
    def _utm_to_wgs84_pyproj(
        ...
        utm_proj = self.pyproj.Proj(proj='utm', zone=zone, ellps='WGS84', 
    ...
    def _wgs84_to_utm_pyproj(self, latitude: float, longitude: float) -> Tuple[float, float, int]:
        """
        ...
        # Transform coordinates
        easting, northing = self.pyproj.transform(wgs84_proj, utm_proj, longitude, latitude)
    ...
    def _utm_to_wgs84_internal(
        ...
        M = y / self.UTM_K0
        mu = M / (self.WGS84_A * (1 - self.WGS84_E2/4 - 3*self.WGS84_E2**2/64 - 5*self.WGS84_E2**3/256))
        ...
        R1 = self.WGS84_A * (1 - self.WGS84_E2) / (1 - self.WGS84_E2 * math.sin(phi1)**2)**1.5
        D = x / (N1 * self.UTM_K0)
        
        # Calculate latitude
        lat = phi1 - (N1 * math.tan(phi1) / R1) * (
            D**2/2 - (5 + 3*T1 + 10*C1 - 4*C1**2 - 9*self.UTM_E_PRIME_2) * D**4/24
            + (61 + 90*T1 + 298*C1 + 45*T1**2 - 252*self.UTM_E_PRIME_2 - 3*C1**2) * D**6/720
        )
        
        # Calculate longitude
        lon = (D - (1 + 2*T1 + C1) * D**3/6
               + (5 - 2*C1 + 28*T1 - 3*C1**2 + 8*self.UTM_E_PRIME_2 + 24*T1**2) * D**5/120) / math.cos(phi1)
    ...
    def _wgs84_to_utm_internal(self, latitude: float, longitude: float) -> Tuple[float, float, int]:
        """
        ...
        M = self.WGS84_A * (
            (1 - self.WGS84_E2/4 - 3*self.WGS84_E2**2/64 - 5*self.WGS84_E2**3/256) * lat_rad
            - (3*self.WGS84_E2/8 + 3*self.WGS84_E2**2/32 + 45*self.WGS84_E2**3/1024) * math.sin(2*lat_rad)
            + (15*self.WGS84_E2**2/256 + 45*self.WGS84_E2**3/1024) * math.sin(4*lat_rad)
            - (35*self.WGS84_E2**3/3072) * math.sin(6*lat_rad)
        )
        
        # Calculate easting
        easting = self.UTM_K0 * N * (
            A + (1 - T + C) * A**3/6
