...
class TestStatisticsCSVParsing:
    ...
    def test_fetch_data_reads_csv_file(self, statistics_importer):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.csv', delete=False) as f:
            writer = csv.DictWriter(f, fieldnames=['incident_id', 'report_date', 'operator_name',
                                                     'operational_period', 'total_incidents', 'severity'])
            writer.writeheader()
            writer.writerow({
    ...
    def test_fetch_data_preserves_all_columns(self, statistics_importer):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.csv', delete=False) as f:
            writer = csv.DictWriter(f, fieldnames=['incident_id', 'report_date', 'operator_name',
                                                     'operational_period', 'total_incidents',
                                                     'fatality_count', 'facility_name'])
            writer.writeheader()
            writer.writerow({
                'incident_id': 'STATS-2024-Q1-001',
                'report_date': '2024-03-31',
    ...
    def test_fetch_data_handles_missing_optional_columns(self, statistics_importer):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.csv', delete=False) as f:
            writer = csv.DictWriter(f, fieldnames=['incident_id', 'report_date', 'operator_name', 'severity'])
            writer.writeheader()
            writer.writerow({
                'incident_id': 'STATS-2024-Q1-001',
                'report_date': '2024-03-31',
...
class TestStatisticsDataNormalization:
    ...
    def test_normalize_converts_report_date_with_time_component(self, statistics_importer):
    ...
    def test_normalize_sets_incident_type_to_equipment_failure(self, statistics_importer):
    ...
    def test_normalize_converts_operational_period_to_integer(self, statistics_importer):
