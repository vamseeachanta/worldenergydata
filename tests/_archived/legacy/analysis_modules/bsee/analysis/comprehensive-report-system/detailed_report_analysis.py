...
def detailed_excel_analysis(file_path, report_name):
    ...
    try:
        ...
        common_categories = [
            'Company', 'Water Depth (ft)', 'Well Purpose', 'Rig Name',
            'Spud Date', 'End Date', 'Total Depth (ft)', 'Days to Drill',
            'Completion Type', 'First Production', 'Peak Production (bopd)',
            'Cumulative Production (MMbbl)', 'Status'
        ]
        
        print("\nIdentified Data Categories:")
        for category in common_categories:
            matching = [label for label in row_labels if label and category.lower() in str(label).lower()]
            if matching:
        ...
        for well in sample_wells:
            ...
            for i, label in enumerate(row_labels):
                if pd.notna(label) and label:
                    value = df_clean.loc[label, well] if well in df_clean.columns else 'N/A'
                    if pd.notna(value):
                        print(f"  {label}: {value}")
...
def analyze_all_reports():
    ...
    for i, category in enumerate(sorted_categories, 1):
        ...
        count = sum(1 for a in all_analyses.values() 
                   if category in a['structure']['categories'])
...
def create_report_template(analyses):
    ...
    template = {
        "field_report": {
            ...
            "well_details": {
                "headers": [
                    "Well Name", "API Number", "Spud Date", "TD (ft)",
                    "Days to Drill", "Completion Type", "First Production",
                    "Peak Rate (bopd)", "Cumulative (MMbbl)", "Status"
                ],
    ...
    template_path = Path(r"tests\modules\bsee\analysis\comprehensive-report-system\results")
