...
def create_sample_comparison_data() -> pd.DataFrame:
    ...
    data = [
        ...
        {'API12': 608124010400, 'field_name': 'Stones', 'well_name': 'SN208', 
         'lease_drilling_days': 565, 'api12_drilling_days': 69, 'drilling_diff': 496,
         'lease_completion_days': 93, 'api12_completion_days': 80, 'completion_diff': 13, 'total_diff': 509},
        {'API12': 608124011200, 'field_name': 'Stones', 'well_name': 'SN206',
         'lease_drilling_days': 259, 'api12_drilling_days': 54, 'drilling_diff': 205,
         'lease_completion_days': 51, 'api12_completion_days': 61, 'completion_diff': -10, 'total_diff': 215},
        
        # St Malo field - lowest difference (001 well shows 2 days total diff)
        {'API12': 608124004400, 'field_name': 'St Malo', 'well_name': '001',
         'lease_drilling_days': 2, 'api12_drilling_days': 0, 'drilling_diff': 2,
         'lease_completion_days': 0, 'api12_completion_days': 0, 'completion_diff': 0, 'total_diff': 2},
        {'API12': 608124005300, 'field_name': 'St Malo', 'well_name': 'PN001',
         'lease_drilling_days': 331, 'api12_drilling_days': 58, 'drilling_diff': 273,
         'lease_completion_days': 94, 'api12_completion_days': 69, 'completion_diff': 25, 'total_diff': 298},
        
        # Jack field - moderate differences
        {'API12': 608124003100, 'field_name': 'Jack', 'well_name': '003',
         'lease_drilling_days': 230, 'api12_drilling_days': 109, 'drilling_diff': 121,
         'lease_completion_days': 18, 'api12_completion_days': 0, 'completion_diff': 18, 'total_diff': 139},
        {'API12': 608124000400, 'field_name': 'Jack', 'well_name': '001',
         'lease_drilling_days': 66, 'api12_drilling_days': 111, 'drilling_diff': -45,
         'lease_completion_days': 34, 'api12_completion_days': 0, 'completion_diff': 34, 'total_diff': 79},
        
        # Cascade field - high differences with completion focus
        {'API12': 608124003800, 'field_name': 'Cascade', 'well_name': 'CA003',
         'lease_drilling_days': 210, 'api12_drilling_days': 46, 'drilling_diff': 164,
         'lease_completion_days': 204, 'api12_completion_days': 136, 'completion_diff': 68, 'total_diff': 232},
        {'API12': 608124001600, 'field_name': 'Cascade', 'well_name': '002',
         'lease_drilling_days': 74, 'api12_drilling_days': 137, 'drilling_diff': -63,
         'lease_completion_days': 120, 'api12_completion_days': 0, 'completion_diff': 120, 'total_diff': 183},
        
        # Anchor field - milestone vs timeline differences
        {'API12': 608114075000, 'field_name': 'Anchor', 'well_name': 'AP001',
         'lease_drilling_days': 105, 'api12_drilling_days': 90, 'drilling_diff': 15,
         'lease_completion_days': 216, 'api12_completion_days': 133, 'completion_diff': 83, 'total_diff': 98},
        {'API12': 608114062101, 'field_name': 'Anchor', 'well_name': '001',
         'lease_drilling_days': 19, 'api12_drilling_days': 46, 'drilling_diff': -27,
         'lease_completion_days': 48, 'api12_completion_days': 0, 'completion_diff': 48, 'total_diff': 75},
         
        # Chinook field - completion methodology differences
        {'API12': 608124009700, 'field_name': 'Chinook', 'well_name': 'CH004',
         'lease_drilling_days': 94, 'api12_drilling_days': 91, 'drilling_diff': 3,
         'lease_completion_days': 261, 'api12_completion_days': 297, 'completion_diff': -36, 'total_diff': 39},
        {'API12': 608124004600, 'field_name': 'Chinook', 'well_name': '002',
         'lease_drilling_days': 29, 'api12_drilling_days': 41, 'drilling_diff': -12,
         'lease_completion_days': 12, 'api12_completion_days': 0, 'completion_diff': 12, 'total_diff': 24}
    ]
...
def analyze_completion_patterns(comparison_data: pd.DataFrame) -> Dict[str, Any]:
    ...
    api12_zero_completion = comparison_data[comparison_data['api12_completion_days'] == 0]
    lease_zero_completion = comparison_data[comparison_data['lease_completion_days'] == 0]
    
    # Categorize completion differences
    high_positive = comparison_data[completion_diffs > 50]
    moderate_positive = comparison_data[(completion_diffs > 0) & (completion_diffs <= 50)]
    ...
    for field in comparison_data['field_name'].unique():
        ...
        field_completion_patterns[field] = {
            ...
            'api12_zero_count': len(field_data[field_data['api12_completion_days'] == 0]),
            'lease_zero_count': len(field_data[field_data['lease_completion_days'] == 0]),
    ...
    return {
        'timeline_construction_differences': {
            'lease_method': {
                ...
                'weaknesses': ['Dependent on WAR data completeness', 'May include non-drilling activities', 'Fixed gap thresholds']
            },
            'api12_method': {
...
def correlate_differences_with_well_characteristics(comparison_data: pd.DataFrame) -> Dict[str, Any]:
    """
    ...
    for field in comparison_data['field_name'].unique():
        ...
        field_characteristics[field] = {
            ...
            'methodology_preference': 'lease' if field_data['drilling_diff'].mean() > 0 else 'api12'
        }
    ...
    return {
        ...
        'overall_patterns': {
            'drilling_complexity_correlation': comparison_data['lease_drilling_days'].corr(comparison_data['drilling_diff']),
            'completion_complexity_correlation': comparison_data['lease_completion_days'].corr(comparison_data['completion_diff']),
            'field_variability': comparison_data.groupby('field_name')['total_diff'].std().to_dict()
        }
    }


def validate_root_causes_with_extreme_cases(comparison_data: pd.DataFrame) -> Dict[str, Any]:
    """
    ...
    case_studies.append({
        ...
        'drilling_analysis': {
            ...
            'ratio': highest_case['lease_drilling_days'] / max(highest_case['api12_drilling_days'], 1)
        },
        'completion_analysis': {
...
def generate_comprehensive_root_cause_analysis() -> Dict[str, Any]:
    ...
    correlation_analysis = correlate_differences_with_well_characteristics(comparison_data)
    ...
    synthesis = {
        'primary_root_causes': [
            {
                ...
                'affected_wells': drilling_analysis['potential_causes'][0]['wells_affected'] if drilling_analysis['potential_causes'] else []
            },
            {
                ...
                'field_impact': list(correlation_analysis['field_characteristics'].keys())
            },
            {
                ...
                'complexity_correlation': correlation_analysis['overall_patterns']['drilling_complexity_correlation']
            },
            {
        ...
        'quantified_impact': {
            'extreme_drilling_differences': drilling_analysis['categorization']['extreme_high'],
            'negative_drilling_differences': drilling_analysis['categorization']['negative'],
            'api12_zero_completions': completion_analysis['zero_completion_analysis']['api12_zero_wells'],
            'high_completion_differences': completion_analysis['categorization']['high_positive'],
    ...
    return {
        ...
        'comparison_data_summary': {
            ...
            'drilling_diff_stats': {
                ...
                'range': [float(comparison_data['drilling_diff'].min()), float(comparison_data['drilling_diff'].max())]
            },
            'completion_diff_stats': {
                'mean': float(comparison_data['completion_diff'].mean()),
                'std': float(comparison_data['completion_diff'].std()),
                'range': [float(comparison_data['completion_diff'].min()), float(comparison_data['completion_diff'].max())]
            }
        },
