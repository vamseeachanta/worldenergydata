...
class AITestFixer:
    ...
    def _fix_direct_function_calls(self, content: str, file_path: Path) -> str:
        ...
        module_match = re.search(r'from worldenergydata\.(.+?) import (.+?)$', content, re.MULTILINE)
        if not module_match:
            return content
        ...
        for func_match in re.finditer(r'result = ([a-z_][a-zA-Z0-9_]*)\(', content):
            ...
            if func_name in ['len', 'str', 'int', 'float', 'list', 'dict', 'tuple', 
                           'print', 'open', 'range', 'enumerate']:
                continue
                
            # Check if this function should be a method call
            if f'def test_{func_name}' in content or f'def test__{func_name}' in content:
                ...
                if func_name not in imported_items:
                    ...
                    old_import = f'from worldenergydata.{module_path} import {imported_items}'
    ...
    def fix_all_tests(self, test_dir: Path) -> Dict[str, Any]:
        ...
        logger.info(f"No Changes Needed: {stats['total_files'] - stats['fixed_files'] - stats['failed_files']}")
...
def main():
    ...
    logger.info(f"Success rate: {(total_fixed/total_files*100):.1f}%" if total_files > 0 else "N/A")
