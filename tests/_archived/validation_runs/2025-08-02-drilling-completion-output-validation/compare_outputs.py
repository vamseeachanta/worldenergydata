...
class OutputComparison:
    ...
    def load_files(self):
        ...
        try:
            ...
            logger.info(f"Original file loaded: {len(self.original_df)} rows, {len(self.original_df.columns)} columns")
            
            logger.info(f"Loading test file: {self.test_path}")
            self.test_df = pd.read_excel(self.test_path)
            logger.info(f"Test file loaded: {len(self.test_df)} rows, {len(self.test_df.columns)} columns")
    ...
    def compare_structure(self):
        ...
        results = {
            ...
            'columns': {
                ...
                'missing_in_test': list(set(self.original_df.columns) - set(self.test_df.columns)),
                'extra_in_test': list(set(self.test_df.columns) - set(self.original_df.columns))
            }
        }
    ...
    def prepare_for_comparison(self):
        ...
        if 'API_WELL_NUMBER' in common_columns:
            self.original_df = self.original_df.sort_values('API_WELL_NUMBER').reset_index(drop=True)
            self.test_df = self.test_df.sort_values('API_WELL_NUMBER').reset_index(drop=True)
        ...
        for col in common_columns:
            ...
            for idx in range(min_rows):
                ...
                if match:
                    ...
                    if len(col_diff_details) < 10:  # Limit to first 10 differences per column
                        api_num = self.original_df_aligned.get('API_WELL_NUMBER', pd.Series()).iloc[idx] if 'API_WELL_NUMBER' in self.original_df_aligned else idx
    ...
    def _compare_values(self, val1, val2, column_name):
        ...
        if column_name in ['DRILLING_DAYS', 'COMPLETION_DAYS', 'MAX_BH_TOTAL_MD', 
                          'MAX_WELL_BORE_TVD', 'MAX_DRILL_FLUID_WGT', 'WATER_DEPTH']:
            try:
                num1 = float(val1)
                num2 = float(val2)
    ...
    def generate_summary_report(self):
        ...
        if 'structure' in self.comparison_results:
            ...
            report.append("\n## Structure Comparison")
            report.append(f"- **Row Count:** Original: {struct['row_count']['original']}, "
                         f"Test: {struct['row_count']['test']} "
                         f"({'✅ MATCH' if struct['row_count']['match'] else '❌ DIFFERENT'})")
            report.append(f"- **Column Count:** Original: {struct['column_count']['original']}, "
                         f"Test: {struct['column_count']['test']} "
                         f"({'✅ MATCH' if struct['column_count']['match'] else '❌ DIFFERENT'})")
            
            if struct['columns']['missing_in_test']:
                report.append(f"- **Missing Columns in Test:** {', '.join(struct['columns']['missing_in_test'])}")
            if struct['columns']['extra_in_test']:
                report.append(f"- **Extra Columns in Test:** {', '.join(struct['columns']['extra_in_test'])}")
        ...
        if 'data' in self.comparison_results:
            ...
            report.append(f"- **Overall Match Percentage:** {data['match_percentage']}%")
            ...
            for col, metrics in sorted(data['column_metrics'].items()):
                report.append(f"| {col} | {metrics['total_cells']} | "
            ...
            for col, metrics in data['column_metrics'].items():
                if metrics['sample_differences']:
                    ...
                    for diff in metrics['sample_differences']:
                        ...
                        report.append(f"| {diff['row']} | {api} | {diff['original']} | {diff['test']} |")
        ...
        if 'data' in self.comparison_results:
            ...
            if match_pct == 100:
                report.append("✅ **PERFECT MATCH** - The test output matches the original output exactly!")
            elif match_pct >= 95:
                report.append(f"✅ **EXCELLENT MATCH** - {match_pct}% of cells match. Minor differences detected.")
            elif match_pct >= 90:
                report.append(f"⚠️ **GOOD MATCH** - {match_pct}% of cells match. Some differences require review.")
            else:
                report.append(f"❌ **SIGNIFICANT DIFFERENCES** - Only {match_pct}% of cells match. Investigation needed.")
...
def main():
    ...
    print("="*50)
    print(f"Row Count Match: {'YES' if structure_results['row_count']['match'] else 'NO'}")
    print(f"Column Count Match: {'YES' if structure_results['column_count']['match'] else 'NO'}")
