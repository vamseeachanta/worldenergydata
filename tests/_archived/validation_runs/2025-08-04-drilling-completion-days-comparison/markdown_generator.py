...
class MarkdownReportGenerator:
    ...
    def _prepare_table_data(self, comparison_data: pd.DataFrame) -> pd.DataFrame:
        ...
        table_data['completion_lease'] = comparison_data.get('completion_days_lease', 'N/A')
        table_data['completion_api12'] = comparison_data.get('completion_days_api12', 'N/A')
        
        # Format numeric values
        for col in ['drilling_lease', 'drilling_api12', 'completion_lease', 'completion_api12']:
            table_data[col] = table_data[col].apply(self._format_numeric_value)
        
        # Format API numbers
        table_data['api_number'] = table_data['api_number'].apply(self._format_api_number)
    ...
    def _create_markdown_table(self, table_data: pd.DataFrame) -> str:
        ...
        separators = []
        for col_key in ['api_number', 'drilling_lease', 'drilling_api12', 'completion_lease', 'completion_api12']:
        ...
        for _, row in table_data.iterrows():
            formatted_row = []
            for col in ['api_number', 'drilling_lease', 'drilling_api12', 'completion_lease', 'completion_api12']:
                value = str(row[col])
    ...
    def _create_status_summary(self, comparison_data: pd.DataFrame) -> str:
        ...
        for status in ['OK', 'REVIEW', 'ERROR']:
            ...
            summary_lines.append(f"- **{emoji} {status}**: {count} wells ({percentage:.1f}%) - {description}")
        
        summary_lines.extend([
            "",
            f"**Total Wells Compared**: {total_wells}"
        ])
        
        return '\n'.join(summary_lines)
    
    def _generate_complete_report(self, comparison_data: pd.DataFrame, title: Optional[str] = None) -> str:
        """
