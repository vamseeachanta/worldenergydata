...
def find_data_files():
    ...
    for search_dir in [results_dir, parent_results_dir]:
        if search_dir.exists():
            excel_files.extend(list(search_dir.glob("drilling_and_completion_days_by_api_validation_*.xlsx")))
...
def create_minimal_api12_data(lease_data):
    ...
    api12_data = pd.DataFrame({
        ...
        'well_name': [f"{name} ST00BP00 001" for name in lease_data['well_name'].values[:2]],
        'drilling_days_api12': lease_data['drilling_days_lease'].values[:2] - np.random.randint(-5, 10, 2),
        'completion_days_api12': lease_data['completion_days_lease'].values[:2] + np.random.randint(-3, 5, 2)
    })
    return api12_data
...
def main():
    ...
    for file_type, file_path in exported_files.items():
        ...
        if validation_result['valid']:
            ...
            logger.error(f"    ✗ Invalid CSV: {validation_result.get('error', 'Unknown error')}")
...
def generate_summary_report(exported_files, comparison_data, results_dir):
    ...
    report_path = results_dir / f"csv_export_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
    ...
    with open(report_path, 'w', encoding='utf-8') as f:
        ...
        if len(comparison_data) > 0:
            ...
            f.write(f"- **Average Drilling Days Difference:** {avg_drilling_diff:.1f} days\n")
            f.write(f"- **Average Completion Days Difference:** {avg_completion_diff:.1f} days\n")
