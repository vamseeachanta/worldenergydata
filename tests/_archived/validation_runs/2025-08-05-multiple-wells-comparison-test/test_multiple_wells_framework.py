...
@pytest.mark.skipif(not PROCESSOR_AVAILABLE, reason="MultipleWellsDataProcessor not available")
class TestMultipleWellsFrameworkCore:
    ...
    def test_status_determination_logic(self, processor):
        ...
        status_review = processor._determine_status(6, 1, 12.0, 8.0)  # drilling abs > 5 AND drilling pct > 10%
        assert status_review == 'ERROR'  # This actually triggers ERROR due to both thresholds
        
        # Test actual REVIEW status (only one condition exceeded)
        status_review_actual = processor._determine_status(6, 1, 8.0, 3.0)  # only drilling abs > 5
        assert status_review_actual == 'REVIEW'
        
        # Test ERROR status (multiple thresholds exceeded)
        status_error = processor._determine_status(8, 7, 20.0, 15.0)  # multiple exceeded
    ...
    def test_results_directory_creation(self):
        ...
        try:
            ...
            sys.path.insert(0, os.path.dirname(__file__))
            from query_api_multiple_wells_rig_days_test import MultipleWellsComparisonFramework
