...
class DataCollectionConfig:
    ...
    log_level: str = "INFO"
    output_directory: str = "tests/modules/bsee/analysis/multiple_wells_comparison_test/results"
...
class ProgressTracker:
    ...
    def update(self, items_processed: int = 1, custom_message: str = None):
        ...
        should_log = (
            progress_pct % 10 < (items_processed / self.total_items) * 100 or
        ...
        if should_log and self.enable_logging:
            ...
            avg_time_per_item = elapsed_time / self.processed_items if self.processed_items > 0 else 0
            eta = avg_time_per_item * (self.total_items - self.processed_items)
            
            message = (f"{self.operation_name}: {self.processed_items}/{self.total_items} "
...
class LargeScaleDataCollector:
    ...
    def collect_lease_method_data(self, data_sources: List[str]) -> Generator[pd.DataFrame, None, None]:
        """
        ...
        """
        progress = ProgressTracker(len(data_sources), "Lease Method Collection", 
        ...
        for source in data_sources:
            try:
                ...
                if df is not None and not df.empty:
                    ...
                    if self.config.validation_enabled:
                        ...
                        if not validation_result['is_valid']:
                            ...
                            if self.config.enable_logging:
                                self.logger.warning(f"Validation issues in {source}: {validation_result['errors']}")
                ...
                self.collection_stats['failed_loads'] += 1
                if self.config.enable_logging:
                    self.logger.error(f"Error loading lease data from {source}: {str(e)}")
            ...
            if memory_status['approaching_limit']:
                ...
                if self.config.enable_logging:
                    self.logger.warning(f"Approaching memory limit: {memory_status['usage_percentage']:.1f}%")
        
        progress.finish()
    
    def collect_api12_method_data(self, data_sources: List[str]) -> Generator[pd.DataFrame, None, None]:
        """
        ...
        """
        progress = ProgressTracker(len(data_sources), "API12 Method Collection", 
                                 self.config.enable_progress_tracking)
        ...
        for source in data_sources:
            try:
                ...
                self.collection_stats['failed_loads'] += 1
                if self.config.enable_logging:
                    self.logger.error(f"Error loading API12 data from {source}: {str(e)}")
    ...
    def _validate_lease_data(self, df: pd.DataFrame, source: str) -> Dict[str, Union[bool, List[str]]]:
        ...
        if 'Drilling Days' in df.columns:
            invalid_drilling = df[(df['Drilling Days'] < 0) | (df['Drilling Days'] > 1000)]
        ...
        if 'Completion Days' in df.columns:
            invalid_completion = df[(df['Completion Days'] < 0) | (df['Completion Days'] > 500)]
            if not invalid_completion.empty:
                errors.append(f"Invalid completion days in {len(invalid_completion)} rows")
    ...
    def _validate_api12_data(self, df: pd.DataFrame, source: str) -> Dict[str, Union[bool, List[str]]]:
        ...
        if 'API12' in df.columns:
            ...
            if not duplicate_apis.empty:
                errors.append(f"Duplicate API12 values: {len(duplicate_apis)} duplicates")
    ...
    def aggregate_collected_data(self, data_generator: Generator[pd.DataFrame, None, None],
                               max_wells: Optional[int] = None) -> pd.DataFrame:
        """
        ...
        progress = ProgressTracker(max_wells or 1000, "Data Aggregation", 
                                 self.config.enable_progress_tracking)
    ...
    def export_collection_stats(self) -> str:
        ...
        # Export to JSON
        output_path = Path(self.config.output_directory) / f"data_collection_stats_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def create_mock_data_sources(num_files: int = 5, wells_per_file: int = 25) -> Tuple[List[str], List[str]]:
    ...
    for i in range(num_files):
        ...
        lease_data = pd.DataFrame({
            'API_WELL_NUMBER': [f'60812400{j:04d}' for j in range(i*wells_per_file, (i+1)*wells_per_file)],
            'WELL_NAME': [f'Lease Well {j}' for j in range(i*wells_per_file, (i+1)*wells_per_file)],
        ...
        api12_data = pd.DataFrame({
            'API12': [f'60812400{j:04d}' for j in range(i*wells_per_file, (i+1)*wells_per_file)],
            'Well_Name': [f'API12 Well {j}' for j in range(i*wells_per_file, (i+1)*wells_per_file)],
