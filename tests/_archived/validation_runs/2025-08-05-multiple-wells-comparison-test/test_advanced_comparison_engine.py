...
@pytest.mark.skipif(not ENGINE_AVAILABLE, reason="Advanced comparison engine not available")
...
class TestOutlierDetector:
    ...
    def test_modified_zscore_outlier_detection(self, detector, sample_data):
        ...
        results = detector.detect_outliers_statistical(sample_data, method='modified_zscore')
    ...
    def test_insufficient_data_clustering(self, detector):
        ...
        results = detector.detect_outliers_clustering(small_df, ['feature1', 'feature2'])
...
class TestStatisticalAnalyzer:
    ...
    def test_effect_size_calculation(self, analyzer, sample_data_sets):
        ...
        assert effect_size['effect_size_interpretation'] in ['negligible', 'small', 'medium', 'large']
...
class TestAdvancedComparisonEngine:
    ...
    def sample_well_data(self):
        ...
        api12_df = pd.DataFrame({
            ...
            'Drilling Days': (np.random.normal(47, 10, 122) + np.random.normal(0, 3, 122)).astype(int),
            'Completion Days': (np.random.normal(16, 4, 122) + np.random.normal(0, 2, 122)).astype(int)
        })
    ...
    def test_comprehensive_comparison(self, engine, sample_well_data):
        ...
        comparison_results, statistical_summary = engine.perform_comprehensive_comparison(lease_df, api12_df)
        
        # Check comparison results
        assert len(comparison_results) == 122
        assert all(isinstance(result, ComparisonResult) for result in comparison_results)
    ...
    def test_status_determination(self, engine):
        ...
        # Test REVIEW status
        status, flags = engine._determine_well_status(6, 1, 4.0, 8.0)  # One threshold exceeded
        assert status == 'REVIEW'
        assert len(flags) > 0
        
        # Test ERROR status
        status, flags = engine._determine_well_status(10, 8, 25.0, 30.0)  # Multiple thresholds exceeded
        assert status == 'ERROR'
        assert len(flags) >= 3
    ...
    def test_results_export(self, engine, sample_well_data, temp_dir):
        ...
        export_paths = engine.export_detailed_results(comparison_results, statistical_summary)
    ...
    def test_missing_data_handling (self, engine):
        ...
        for result in comparison_results:
            if pd.isna(result.lease_drilling_days) or pd.isna(result.api12_drilling_days):
...
class TestAdvancedComparisonEngineIntegration:
    ...
    def test_120_plus_wells_comparison(self):
        ...
        api12_df = pd.DataFrame({
            'API12': [f'60812400{i:04d}' for i in range(1, 123)],
            'Well Name': [f'API12 Well {i}' for i in range(1, 123)],
            'Drilling Days': (np.random.normal(52, 14, 122) + np.random.normal(0, 5, 122)).astype(int),
            'Completion Days': (np.random.normal(19, 5, 122) + np.random.normal(0, 3, 122)).astype(int)
        })
    ...
    def test_systematic_discrepancy_detection(self):
        ...
        api12_df = pd.DataFrame({
            ...
            'Drilling Days': (np.random.normal(40, 8, 100) + 8).astype(int),  # +8 days bias
            'Completion Days': (np.random.normal(12, 3, 100) + 3).astype(int)  # +3 days bias
        })
        ...
        if 'statistical_tests' in drilling_stats:
            ...
            if 'ttest' in tests:
                assert tests['ttest']['significant'] == True  # Should detect systematic difference
