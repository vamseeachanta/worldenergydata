...
@pytest.mark.skipif(not GENERATOR_AVAILABLE, reason="Strategic report generator not available")
...
class TestStrategicReportGenerator:
    ...
    def sample_comparison_results(self):
        ...
        for i in range(50):  # 50 wells for testing
            ...
            drilling_pct_diff = (drilling_diff / lease_drilling) * 100 if lease_drilling != 0 else 0
            completion_pct_diff = (completion_diff / lease_completion) * 100 if lease_completion != 0 else 0
            ...
            result = ComparisonResult(
                ...
                statistical_significance={'drilling_z_score': 0.0, 'completion_z_score': 0.0}
            )
    ...
    def sample_statistical_summary(self, sample_comparison_results):
        ...
        return StatisticalSummary(
            ...
            drilling_days_stats={
                ...
                'statistical_tests': {
                    'ttest': {'pvalue': 0.15, 'significant': False}
                }
            },
            completion_days_stats={
                ...
                'statistical_tests': {
                    'ttest': {'pvalue': 0.08, 'significant': False}
                }
            },
            outlier_wells=outlier_wells,
            cluster_analysis={'method': 'clustering', 'outlier_count': len(outlier_wells)},
            correlation_analysis={'drilling_days': 0.85, 'completion_days': 0.78},
            distribution_comparison={'drilling_outliers': {}, 'completion_outliers': {}}
        )
    ...
    def test_results_to_dataframe_conversion(self, generator, sample_comparison_results):
    ...
    def test_executive_summary_generation(self, generator, sample_comparison_results, 
                                         sample_statistical_summary, sample_processing_stats):
        """Test executive summary generation."""
        results_df = generator._results_to_dataframe(sample_comparison_results)
        
        generator._generate_executive_summary(results_df, sample_statistical_summary, sample_processing_stats)
    ...
    def test_key_findings_generation(self, generator, sample_comparison_results, sample_statistical_summary):
    ...
    def test_summary_tables_generation(self, generator, sample_comparison_results):
        ...
        assert "Top 5 Drilling Days Discrepancies" in section.content  # Using summary_top_n=5
    ...
    def test_conditional_detailed_analysis_with_issues(self, generator, sample_comparison_results):
    ...
    def test_appendix_generation_disabled(self, generator, sample_comparison_results, sample_statistical_summary):
    ...
    def test_appendix_generation_enabled(self, temp_dir, sample_comparison_results, sample_statistical_summary):
    ...
    def test_comprehensive_report_generation(self, generator, sample_comparison_results, 
                                           sample_statistical_summary, sample_processing_stats):
        """Test comprehensive report generation."""
        report_path = generator.generate_comprehensive_report(
            sample_comparison_results, sample_statistical_summary, sample_processing_stats
        )
        ...
        assert "# Multiple Wells Drilling and Completion Days Comparison Report" in report_content
    ...
    def test_large_dataset_report_optimization(self, temp_dir):
        ...
        for i in range(122):
            ...
            if i < 100:
                ...
                status = 'ERROR'
                outlier_flags = ['drilling_absolute_outlier', 'completion_percentage_outlier']
        ...
        assert ("Showing top" in report_content or 
                "Complete data available in exported CSV" in report_content or
                "wells requiring attention" in report_content)
...
class TestStrategicReportIntegration:
    ...
    def test_120_plus_wells_report_generation(self):
        ...
        detailed_wells_shown = report_content.count('60812400')  # Count API12 occurrences
        assert detailed_wells_shown <= 70  # Should not list all 124 wells individually (allows for summary tables)
