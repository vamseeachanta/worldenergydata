...
class ChartGenerator:
    ...
    def create_distribution_comparison_chart(self, lease_data: pd.Series, 
                                           api12_data: pd.Series, 
                                           metric_name: str) -> str:
        """
        ...
        if not clean_lease.empty and not clean_api12.empty:
            ax1.hist(clean_lease, bins=20, alpha=0.7, label='Lease Method', color='skyblue', density=True)
            ax1.hist(clean_api12, bins=20, alpha=0.7, label='API12 Method', color='lightcoral', density=True)
            ...
            return self._save_chart(fig, f'{metric_name.lower().replace(" ", "_")}_distribution')
        
        return ""
    
    def create_scatter_correlation_chart(self, lease_data: pd.Series, 
                                       api12_data: pd.Series, 
                                       metric_name: str) -> str:
        """
        ...
        # Scatter plot
        ax.scatter(aligned_data['lease'], aligned_data['api12'], 
        ...
        if len(aligned_data) > 1:
            ...
            p = np.poly1d(z)
            ax.plot(aligned_data['lease'], p(aligned_data['lease']), 
                   "r--", alpha=0.8, linewidth=2, label=f'Trend Line (R² = {np.corrcoef(aligned_data["lease"], aligned_data["api12"])[0,1]**2:.3f})')
        ...
        return self._save_chart(fig, f'{metric_name.lower().replace(" ", "_")}_correlation')
    
    def create_difference_analysis_chart(self, differences: pd.Series, 
                                       percentage_diffs: pd.Series, 
                                       metric_name: str) -> str:
        """
        ...
        if not clean_diffs.empty:
            ax1.hist(clean_diffs, bins=20, alpha=0.7, color='lightgreen', edgecolor='black')
            ax1.axvline(clean_diffs.mean(), color='red', linestyle='--', 
                       label=f'Mean: {clean_diffs.mean():.2f}')
            ax1.axvline(0, color='black', linestyle='-', alpha=0.5, label='Zero Difference')
        ...
        if not clean_pct_diffs.empty:
            ax2.hist(clean_pct_diffs, bins=20, alpha=0.7, color='orange', edgecolor='black')
            ax2.axvline(clean_pct_diffs.mean(), color='red', linestyle='--', 
                       label=f'Mean: {clean_pct_diffs.mean():.1f}%')
            ax2.axvline(0, color='black', linestyle='-', alpha=0.5, label='Zero Difference')
        ...
        return self._save_chart(fig, f'{metric_name.lower().replace(" ", "_")}_differences')
    ...
    def create_status_distribution_chart(self, status_counts: pd.Series) -> str:
        ...
        chart_colors = [colors.get(status, 'lightgray') for status in status_counts.index]
    ...
    def _save_chart(self, fig: plt.Figure, filename: str) -> str:
        ...
        if self.config.chart_format == 'inline':
            ...
            buffer = io.BytesIO()
            fig.savefig(buffer, format='png', dpi=self.config.chart_dpi, 
            ...
            # Save to file
            chart_path = Path(self.config.results_directory) / f"{filename}.{self.config.chart_format}"
            fig.savefig(chart_path, format=self.config.chart_format, 
                       dpi=self.config.chart_dpi, bbox_inches='tight', facecolor='white')
            plt.close(fig)
...
class StrategicReportGenerator:
    ...
    def generate_comprehensive_report(self, comparison_results: List[Any], 
        ...
        self._generate_executive_summary(results_df, statistical_summary, processing_stats)
        ...
        report_path = Path(self.config.results_directory) / f"strategic_comparison_report_{timestamp}.md"
    ...
    def _results_to_dataframe(self, comparison_results: List[Any]) -> pd.DataFrame:
        ...
        for result in comparison_results:
            data.append({
                ...
                'Outlier_Flags': ','.join(result.outlier_flags) if result.outlier_flags else ''
            })
        
        return pd.DataFrame(data)
    
    def _generate_executive_summary(self, results_df: pd.DataFrame, 
                                  statistical_summary: Any, 
...
"""
    ...
    def _generate_key_findings(self, results_df: pd.DataFrame, statistical_summary: Any):
        ...
        mean_completion_diff = completion_diffs.mean() if not completion_diffs.empty else 0
        
        outlier_count = len(statistical_summary.outlier_wells)
        correlation_drilling = statistical_summary.correlation_analysis.get('drilling_days', 0)
        correlation_completion = statistical_summary.correlation_analysis.get('completion_days', 0)
...
"""
        ...
        if hasattr(statistical_summary, 'drilling_days_stats') and 'statistical_tests' in statistical_summary.drilling_days_stats:
            drilling_tests = statistical_summary.drilling_days_stats['statistical_tests']
        ...
        if hasattr(statistical_summary, 'completion_days_stats') and 'statistical_tests' in statistical_summary.completion_days_stats:
            completion_tests = statistical_summary.completion_days_stats['statistical_tests']
    ...
    def _generate_statistical_analysis(self, results_df: pd.DataFrame, statistical_summary: Any):
...
"""
        ...
        if drilling_chart:
            if drilling_chart.startswith('data:image'):
                content += f"![Drilling Days Distribution Comparison]({drilling_chart})\n\n"
            else:
                content += f"![Drilling Days Distribution Comparison]({drilling_chart})\n\n"
...
"""
        ...
        if completion_diff_chart:
            if completion_diff_chart.startswith('data:image'):
                content += f"![Completion Days Differences]({completion_diff_chart})\n\n"
            else:
                content += f"![Completion Days Differences]({completion_diff_chart})\n\n"
        
        # Status distribution
        status_counts = results_df['Status'].value_counts()
        status_chart = self.chart_generator.create_status_distribution_chart(status_counts)
        if status_chart:
...
"""
        ...
        top_completion = results_df.nlargest(self.config.summary_top_n, 'Completion_Diff')[
...
"""
            ...
            review_wells = attention_wells[attention_wells['Status'] == 'REVIEW'].head(self.config.max_detailed_wells)
            ...
            if 'statistical_tests' in drilling_stats:
                ...
                for test_name, test_results in tests.items():
                    if isinstance(test_results, dict) and 'pvalue' in test_results:
                        ...
                        content += f"significant = {test_results.get('significant', False)}\n"
            content += "\n"
...
if __name__ == "__main__":
    ...
    print(f"Configuration: max_detailed_wells={config.max_detailed_wells}, include_charts={config.include_charts}")