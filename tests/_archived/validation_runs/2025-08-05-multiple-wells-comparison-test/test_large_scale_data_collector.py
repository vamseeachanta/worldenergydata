...
class TestLargeScaleDataCollector:
    ...
    def mock_data_sources(self, temp_dir):
        ...
        for i in range(3):  # 3 files with 10 wells each = 30 wells total
            ...
            lease_data = pd.DataFrame({
                'API_WELL_NUMBER': [f'60812400{j:04d}' for j in range(i*10, (i+1)*10)],
                'WELL_NAME': [f'Lease Well {j}' for j in range(i*10, (i+1)*10)],
            ...
            api12_data = pd.DataFrame({
                ...
                'Well_Name': [f'API12 Well {j}' for j in range(i*10, (i+1)*10)],
    ...
    def test_data_type_optimization(self, collector):
        ...
        assert optimized_df['Drilling Days'].dtype in ['uint8', 'uint16', 'int16', 'int32']
        assert optimized_df['Completion Days'].dtype in ['uint8', 'uint16', 'int16', 'int32']
...
class TestLargeScaleDataCollectionIntegration:
    ...
    def test_120_plus_wells_collection(self):
        ...
        try:
            ...
            assert lease_df['Drilling Days'].dtype in ['uint8', 'uint16', 'int16', 'int32']
            assert api12_df['Completion Days'].dtype in ['uint8', 'uint16', 'int16', 'int32']
