...
class TestDataTypeOptimizer:
    ...
    def sample_dataframe(self):
        ...
        return pd.DataFrame({
            ...
            'categories': np.random.choice(['A', 'B', 'C', 'D'], 1000),  # Can be category
            'high_cardinality': [f'item_{i}' for i in range(1000)],  # Should stay object
            'api12': np.random.choice([f'60812400{i:04d}' for i in range(100)], 1000)  # Repeated API12s - can be category
        })
    ...
    def test_datatype_optimizer_aggressive(self, sample_dataframe):
        ...
        optimized_df = DataTypeOptimizer.optimize_dataframe(sample_dataframe, aggressive=True)
...
class TestBatchProcessor:
    ...
    def test_dataframe_batch_processing(self, large_dataframe):
        ...
        expected_batches = len(large_dataframe) // 50 + (1 if len(large_dataframe) % 50 > 0 else 0)
        ...
        assert progress_calls[-1][0] == len(large_dataframe)  # Final progress should be total
    ...
    def test_batch_merge_operations(self, large_dataframe):
        ...
        right_df = pd.DataFrame({
            'api12': [f'60812400{i:04d}' for i in range(0, 250, 2)],  # Every other well
    ...
    def test_batch_merge_with_memory_limit(self, large_dataframe):
        ...
        merged = processor.merge_dataframes_batched(
            large_dataframe, right_df, on='api12', 
    ...
    def test_empty_batch_processing(self):
        ...
        results = list(processor.process_dataframe_batches(
            empty_df, lambda x: len(x)
        ))
...
class TestPerformanceOptimizer:
    ...
    def large_dataset(self):
        ...
        return pd.DataFrame({
            ...
            'field': np.random.choice(['Field1', 'Field2', 'Field3', 'Field4'], 150)
        })
...
class TestPerformanceBenchmarking:
    ...
    def test_benchmark_comparison_performance(self, benchmark_datasets):
        ...
        def simple_comparison(df1, df2):
            ...
            merged['drilling_diff'] = merged['Drilling_Days_api12'] - merged['Drilling_Days_lease']
            merged['completion_diff'] = merged['Completion_Days_api12'] - merged['Completion_Days_lease']
...
class TestPerformanceOptimizerIntegration:
    ...
    def test_120_plus_wells_optimization(self):
        ...
        def complex_analysis(df):
            ...
            grouped = df.groupby(['operator', 'field']).agg({
    ...
    def test_memory_efficiency_large_dataset(self):
        ...
        inefficient_dataset = pd.DataFrame({
            ...
            'status': np.random.choice(['Active', 'Inactive', 'Pending'], 200),  # Could be category
