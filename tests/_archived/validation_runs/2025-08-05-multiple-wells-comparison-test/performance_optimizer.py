...
class DataTypeOptimizer:
    ...
    def optimize_dataframe(df: pd.DataFrame, aggressive: bool = False) -> pd.DataFrame:
        ...
        for col in df_optimized.select_dtypes(include=['int64', 'int32', 'int16']).columns:
            ...
            if col_min >= -128 and col_max <= 127:
                ...
                if df_optimized[col].dtype != 'int32':  # Only convert if not already int32
        ...
        for col in df_optimized.select_dtypes(include=['float64']).columns:
            if aggressive:
                ...
                if len(original_values) > 0:
                    ...
                    relative_error = np.abs((original_values - float32_values) / original_values).max()
...
class BatchProcessor:
    ...
    def __init__(self, chunk_size: int = 50, memory_monitor: Optional[MemoryMonitor] = None):
        self.chunk_size = chunk_size
        self.memory_monitor = memory_monitor or MemoryMonitor()
        
    def process_dataframe_batches(self, df: pd.DataFrame, 
                                processor_func: Callable[[pd.DataFrame], Any],
                                progress_callback: Optional[Callable[[int, int], None]] = None) -> Generator[Any, None, None]:
        """
    ...
    def merge_dataframes_batched(self, left_df: pd.DataFrame, right_df: pd.DataFrame,
        ...
        estimated_memory_mb = (left_df.memory_usage(deep=True).sum() + 
                             right_df.memory_usage(deep=True).sum()) / 1024 / 1024
    ...
    def _merge_in_batches(self, left_df: pd.DataFrame, right_df: pd.DataFrame,
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            profile = MemoryProfile(
                ...
                max_memory_percent=(peak_memory / (psutil.virtual_memory().total / 1024 / 1024)) * 100,
...
class PerformanceOptimizer:
    ...
    def __init__(self, constraints: Optional[ResourceConstraints] = None):
        ...
        self.batch_processor = BatchProcessor(chunk_size=self.constraints.max_chunk_size)
    ...
    def optimize_for_large_dataset(self, df: pd.DataFrame, 
                                 ...
                                 progress_callback: Optional[Callable[[str], None]] = None) -> Tuple[Any, PerformanceMetrics]:
        """
        ...
        if (self.constraints.max_memory_mb and df_memory_mb > self.constraints.max_memory_mb) or \
           (df_memory_mb > available_memory_mb * self.constraints.memory_warning_threshold):
            # Use batch processing
            if progress_callback:
                progress_callback("Using batch processing for memory efficiency...")
            result = self._process_in_batches(optimized_df, processing_func, progress_callback)
        else:
            # Direct processing
            if progress_callback:
        ...
        metrics = PerformanceMetrics(
            ...
            memory_efficiency_score=self._calculate_memory_efficiency(start_memory, end_memory, df_memory_mb),
            processing_chunks=1 if df_memory_mb <= available_memory_mb * 0.8 else max(1, len(df) // self.constraints.max_chunk_size),
            average_chunk_time=execution_time / max(1, len(df) // self.constraints.max_chunk_size)
        )
        
        self._performance_history.append(metrics)
        
        if progress_callback:
            progress_callback(f"Processing complete: {rows_processed} rows in {execution_time:.2f}s")
        
        return result, metrics
    
    def _process_in_batches(self, df: pd.DataFrame, processing_func: Callable,
                          progress_callback: Optional[Callable[[str], None]] = None) -> Any:
        ...
        def batch_progress(processed_rows: int, total_rows: int):
            if progress_callback:
                ...
                progress_callback(f"Processing: {progress:.1f}% complete ({processed_rows}/{total_rows} rows)")
    ...
    def _calculate_memory_efficiency(self, start_memory: float, end_memory: float, 
                                   data_memory: float) -> float:
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'average_execution_time': np.mean([m.execution_time_seconds for m in metrics]),
            'average_rows_per_second': np.mean([m.rows_per_second for m in metrics]),
            'average_memory_efficiency': np.mean([m.memory_efficiency_score for m in metrics]),
            'total_rows_processed': sum(m.rows_processed for m in metrics),
            'performance_trend': 'improving' if len(metrics) > 1 and 
                               metrics[-1].memory_efficiency_score > metrics[0].memory_efficiency_score else 'stable'
        }
    ...
    def check_system_resources(self) -> Dict[str, Any]:
        ...
        return {
            'memory': {
                ...
                'available_for_processing_gb': (memory.available * self.constraints.memory_warning_threshold) / 1024 / 1024 / 1024
            },
            'cpu': {
    ...
    def _get_resource_recommendations(self, memory: Any, cpu_percent: float) -> List[str]:
        """Get resource optimization recommendations."""
        recommendations = []
        
        if memory.percent > 80:
            recommendations.append("High memory usage detected. Consider using batch processing.")
        
        if cpu_percent > 80:
            recommendations.append("High CPU usage detected. Consider reducing chunk size or processing concurrency.")
        
        if memory.available < 2 * 1024 * 1024 * 1024:  # Less than 2GB available
            recommendations.append("Low available memory. Enable aggressive optimization and reduce chunk sizes.")
...
def benchmark_comparison_performance(lease_df: pd.DataFrame, api12_df: pd.DataFrame,
    ...
    for i in range(iterations):
        ...
        results.append({
            ...
            'data_size_mb': (lease_df.memory_usage(deep=True).sum() + 
                           api12_df.memory_usage(deep=True).sum()) / 1024 / 1024
        })
