...
class TestMetricsTracker:
        ...
        try:
            ...
            if total_run > 0:
                metrics["pass_rate"] = round(
                    (metrics["tests_passed"] / total_run) * 100, 2
                )
            else:
            ...
            if coverage_file.exists():
                with open(coverage_file, "r") as f:
                    ...
                    if "totals" in coverage_data:
                        metrics["coverage_lines"] = coverage_data["totals"].get(
                            "num_statements", 0
                        )
                        metrics["coverage_covered"] = coverage_data["totals"].get(
                            "covered_lines", 0
                        )
                        if metrics["coverage_lines"] > 0:
                            metrics["coverage_percent"] = round(
                                (
                                    metrics["coverage_covered"]
                                    / metrics["coverage_lines"]
                                )
                                * 100,
                                2,
                            )
    ...
    def check_regression(self, current: Dict):
        ...
        if current.get("pass_rate", 0) < previous.get("pass_rate", 0) - 5:
            warnings.append(
                f"⚠️ Pass rate decreased: {previous.get('pass_rate', 0):.1f}% → {current.get('pass_rate', 0):.1f}%"
            )

        # Check coverage
        if current.get("coverage_percent", 0) < previous.get("coverage_percent", 0) - 2:
            warnings.append(
                f"⚠️ Coverage decreased: {previous.get('coverage_percent', 0):.1f}% → {current.get('coverage_percent', 0):.1f}%"
            )

        # Check test count
        if current.get("total_tests", 0) < previous.get("total_tests", 0) * 0.9:
            warnings.append(
                f"⚠️ Test count decreased: {previous.get('total_tests', 0)} → {current.get('total_tests', 0)}"
            )
