...
_ABS_PATH = re.compile(r"(?<![\w.])(?:file:/{0,3}|~/|/(?:home|mnt|Users|root|tmp|srv|opt)/)\S*")
...
def test_outputs_retain_native_schema_only_curated_publish(pilot_summary):
    ...
    for o in projected:
        ...
        assert not any(ref["content_digest"].endswith(".xlsx") for ref in o["artifact_refs"])
...
def test_metrics_have_definitions_units_derivations_quality(pilot_config):
    ...
    for definition in defs.values():
        ...
        assert definition["metric_id"].startswith(pilot_config["algorithm"]["algorithm_id"] + "/")
...
def test_dataset_projection_publishes_immutable_revision(pilot_summary):
    ...
    assert set(pilot_summary["eligible_run_ids"]) == set(pilot_summary["run_ids"].values())
...
def test_federal_claim_requires_real_extract_snapshot(pilot_config):
    ...
    forged = dict(d, source_authority="BSEE", redistribution_rights="public-domain",
                  license="cc-by-4.0")
...
def test_failed_run_excluded_from_metrics_and_decisions(pilot_config):
    ...
    obs = metrics.make_metric_observation(
        definition=definition, run_id="r", value=1.0, quality_state="valid",
...
def test_published_canonical_input_has_no_absolute_path(pilot_summary):
    ...
    assert rec["selection"]["fixture_ref"] == os.path.basename(rec["selection"]["fixture_ref"])
...
def test_fresh_checkout_replay_same_run_id(pilot_config, tmp_path, monkeypatch):
    ...
    rid1 = P.derive_identity(pilot_config, P.build_input_record(d1, pilot_config))["run_id"]
    ...
    rid2 = P.derive_identity(pilot_config, P.build_input_record(d2, pilot_config))["run_id"]
