...
from worldenergydata.common.data_resolver import get_module_data_safe  # noqa: E402
...
def _ogor_bin_dir() -> Path:
    return (
        get_module_data_safe("bsee") / "bin" / "historical_production_yearly"
    )
...
def _load_ogor_from_bin(start_year: int = 2000, end_year: int = 2026) -> pd.DataFrame:
    ...
    if current.exists():
        try:
            cur_raw = pd.read_pickle(current)
...
def _ensure_ogor_loader() -> str:
    ...
    v30_reproducer.load_ogor_production = _load_ogor_from_bin  # type: ignore[assignment]
    ...
    fin.load_ogor_production = _load_ogor_from_bin  # type: ignore[assignment]
...
def _read_war_bin(name: str) -> pd.DataFrame:
    ...
    return pd.read_pickle(war_dir / name)
...
def build_operations_timeline(
    ...
    ops = ops.drop_duplicates(
        subset=["API_WELL_NUMBER", "date", "operation", "detail"]
    )
