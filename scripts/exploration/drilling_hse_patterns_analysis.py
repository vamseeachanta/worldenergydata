...
# safety_analysis moved under packages/ in the monorepo reorg (post-2026-06 split).
TAXO = (
    REPO
    / "packages"
...
def _load_incinv():
    src = next(
        (p for p in INCINV_CANDIDATES if p.exists() and p.stat().st_size > 0), None
    )
    if src is None:
...
def main() -> None:
    ...
    payload = {
        ...
        "drill_distinct_leases": len(
            {r.get("LEASE_NUMBER", "") for r, _ in drill_rows if r.get("LEASE_NUMBER")}
        ),
    }
