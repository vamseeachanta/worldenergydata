...
def build_completion(available_viz: dict[str, bool]) -> list[tuple]:
    ...
    The pages are self-contained HTML artifacts (inline CSS, no external assets)
    generated by ``scripts/completion/build_completion_report.py`` from a frozen
    BSEE WAR-derived reference workbook. The whole ``reports/completion/`` set is
    copied verbatim &mdash; ``index.html`` plus the ``verification.html``
    reconciliation page it links to &mdash; so intra-set and ``../`` links resolve
    against the site root. Returns ``[]`` &mdash; the landing index keys off what
    exists in ``public/``."""
    src = REPORTS / "completion"
...
def build_field_atlas(available_viz: dict[str, bool]) -> list[tuple]:
    ...
    # Per-field client one-pager PDFs (+ committed HTML intermediates) for the 10
    # Explorer fields, generated by scripts/capabilities/build_field_onepagers.py
    # (#945). Copied verbatim so the Explorer's per-field one-pager links resolve
    # under /field-atlas/onepagers/.
    onepagers = REPORTS / "field-atlas" / "onepagers"
    if onepagers.exists():
        shutil.copytree(onepagers, dst / "onepagers", dirs_exist_ok=True)
    return []
