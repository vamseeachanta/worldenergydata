...
REGISTRY_FILE="$WORKSPACE_ROOT/config/ai_agents/ai-agents-registry.json"
BACKUP_DIR="$WORKSPACE_ROOT/.agent-backups"
UPDATE_LOG="$WORKSPACE_ROOT/config/ai_agents/agent-updates.log"
...
for dir in "$WORKSPACE_ROOT"/*/; do
    if [ -d "${dir}.git" ]; then
        ...
        TARGET_DIR="${dir}config/ai_agents"

        # Create config/ai_agents if it doesn't exist
        mkdir -p "$TARGET_DIR"
...
REPORT_FILE="$WORKSPACE_ROOT/config/ai_agents/agent-update-report-$(date +%Y%m%d).md"
