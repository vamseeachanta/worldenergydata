...
class DocumentCategorizer:
    ...
    def find_duplicates(
        self, files: List[Path], threshold: float = 0.8
...
class TestDocumentCategorization:
    ...
    def test_similarity_calculation(self):
        ...
        identical_similarity = difflib.SequenceMatcher(
            None, "test content", "test content"
        ).ratio()
        assert identical_similarity == 1.0

        # Test with similar content
        similar_similarity = difflib.SequenceMatcher(
            None, "test content", "test content updated"
        ).ratio()
        assert 0.5 < similar_similarity < 1.0

        # Test with different content
        different_similarity = difflib.SequenceMatcher(
            None, "test content", "completely different"
        ).ratio()
    ...
    def test_keyword_coverage(self):
        ...
        for category, rules in self.categorizer.category_keywords.items():
            assert (
                len(rules["keywords"]) >= 2
            ), f"Category {category} needs more keywords"
            assert (
                len(rules["path_patterns"]) >= 1
            ), f"Category {category} needs path patterns"
