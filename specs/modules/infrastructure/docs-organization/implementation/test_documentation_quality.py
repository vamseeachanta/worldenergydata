...
class DocumentationQualityChecker:
    ...
    def _check_content_quality(
        ...
        for i, section in enumerate(
            sections[1:], 1
        ):  # Skip first split (before first header)
    ...
    def _check_terminology(self, file_path: Path, content: str) -> List[QualityIssue]:
        ...
        for incorrect_term, correct_term in self.content_standards[
            "energy_terminology"
        ].items():
...
"""
        ...
        return {"good": good_doc, "poor": poor_doc, "terminology": terminology_doc}
    ...
    def test_filename_standards_checking(self, temp_docs_dir):
        ...
        long_filename = (
            temp_docs_dir
            / "this_is_an_extremely_long_filename_that_exceeds_reasonable_limits.md"
        )
        long_filename.write_text("# Test")

        issues = checker.check_all_documents()
        filename_issues = [
            i for i in issues if i.issue_type in ["filename_format", "filename_length"]
        ]

        assert len(filename_issues) >= 2

    def test_markdown_formatting_detection(
        self, temp_docs_dir, sample_quality_documents
    ):
    ...
    def test_terminology_checking(self, temp_docs_dir, sample_quality_documents):
        ...
        assert (
            len(terminology_issues) >= 2
        )  # sea floor -> seafloor, well head -> wellhead
