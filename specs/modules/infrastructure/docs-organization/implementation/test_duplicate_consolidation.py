...
class ContentConsolidator:
        ...
        for duplicate in duplicates:
            ...
            for other_dup in duplicates:
                if other_dup == duplicate:
                    continue
                if other_dup.file1 in cluster_files or other_dup.file2 in cluster_files:
    ...
    def _create_plan_for_cluster(
        self, cluster: List[DuplicateMatch]
    ) -> ConsolidationPlan:
        ...
        if exact_matches:
            ...
            files_to_remove = [
                m.file2 for m in exact_matches if m.file1 == primary_file
            ]
        else:
    ...
    def execute_consolidation_plan(
        self, plan: ConsolidationPlan, dry_run: bool = True
    ) -> bool:
        """Execute a consolidation plan"""
        if dry_run:
            print(
                f"DRY RUN: Would consolidate {len(plan.files_to_merge)} files into {plan.primary_file}"
            )
            return True

        try:
            # Read primary file content
            primary_content = plan.primary_file.read_text(
            ...
            for merge_file in plan.files_to_merge:
                ...
                primary_content = self._merge_content(
                    primary_content, merge_content, plan.merge_strategy
                )
...
if __name__ == "__main__":
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        for i, plan in enumerate(plans):
            print(
                f"Plan {i+1}: Merge {len(plan.files_to_merge)} files into {plan.primary_file.name}"
            )
