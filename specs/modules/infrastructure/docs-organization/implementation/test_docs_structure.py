...
class TestDocsStructure:
    ...
    def setup_method(self):
        ...
        self.expected_structure = {
            ...
            "data-sources": ["bsee", "sodir", "wind", "lng", "equipment", "onshore"],
            "analysis-guides": [
                "economic-evaluation",
                "production-analysis",
                "field-development",
            ...
            "development": [],
            "reference": ["literature", "equipment-specs", "industry-standards"],
            "examples": ["basic-usage", "field-analysis", "economic-modeling"],
        }
    ...
    def test_subdirectory_structure(self):
        ...
        for main_dir, subdirs in self.expected_structure.items():
            ...
            for subdir in subdirs:
                ...
                assert (
                    subdir_path.exists()
                ), f"Subdirectory {main_dir}/{subdir} should exist"
                assert (
                    subdir_path.is_dir()
                ), f"{main_dir}/{subdir} should be a directory"

    def test_naming_conventions(self):
        """Test that directory names follow kebab-case convention"""
        for main_dir in self.expected_structure.keys():
            # Check main directories use kebab-case
            assert self._is_kebab_case(
                main_dir
            ), f"Directory {main_dir} should use kebab-case"

            # Check subdirectories use kebab-case
            for subdir in self.expected_structure[main_dir]:
                assert self._is_kebab_case(
                    subdir
                ), f"Subdirectory {subdir} should use kebab-case"
        ...
        for main_dir in self.expected_structure.keys():
            ...
            if index_file.exists():
                ...
                assert content.startswith(
                    "#"
                ), f"index.md in {main_dir} should start with markdown header"
                assert (
                    main_dir.replace("-", " ").title() in content
                ), f"index.md should contain section title"
        ...
        if docs_root.exists():
            ...
            for section in new_structure_paths:
                ...
                if index_file.exists():
                    ...
                    ), f"Template file should be named index.md"

            # Test main README follows conventions
            main_readme = docs_root / "README.md"
            if main_readme.exists():
                assert (
                    main_readme.name == "README.md"
                ), "Main README should follow naming convention"
