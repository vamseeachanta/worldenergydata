...
class LinkValidator:
    ...
    def _validate_single_link(
        self, source_file: Path, link_text: str, link_target: str, line_number: int
    ) -> LinkValidationResult:
    ...
    def _determine_link_type(self, link_target: str) -> str:
        ...
        if link_target.startswith("mailto:"):
            ...
            self.relative_link_pattern.match(link_target)
            or not urlparse(link_target).scheme
        ):
            return "internal"
        else:
            return "external"
...
class NavigationAnalyzer:
    ...
    def __init__(self, docs_root: Path):
        ...
        self.user_types = ["energy-professional", "developer", "data-analyst"]
    ...
    def validate_navigation_completeness(
        self, nav_structure: NavigationStructure
    ) -> Dict[str, List[str]]:
        ...
        for section in self.expected_sections:
            if section not in nav_structure.section_indexes:
                section_dir = self.docs_root / section
                if section_dir.exists():
                    issues["missing_section_indexes"].append(
                        f"Index missing for {section}"
                    )
...
"""
    ...
    def test_link_type_detection(self, temp_docs_dir):
        ...
        for link_target, expected_type in test_cases:
            ...
            ), f"Link {link_target} should be {expected_type}, got {actual_type}"
    ...
    def test_internal_link_validation(self, temp_docs_dir, sample_markdown_files):
        ...
        assert (
            validator._validate_internal_link(main_file, "user-guide/index.md") == True
        )
    ...
    def test_email_link_validation(self, temp_docs_dir):
        ...
        assert (
            validator._validate_email_link("mailto:user.name+tag@domain.co.uk") == True
        )
    ...
    def test_broken_link_detection(self, temp_docs_dir, sample_markdown_files):
        ...
        missing_file_results = [
            r for r in broken_results if "missing-file.md" in r.link_target
        ]
