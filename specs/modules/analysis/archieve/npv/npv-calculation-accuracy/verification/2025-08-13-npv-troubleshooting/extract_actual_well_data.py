...
def extract_actual_well_data():
    ...
    test_data_path = (
        "/mnt/github/github/worldenergydata/tests/modules/bsee/analysis/results"
    )
    ...
    if os.path.exists(npv_summary_file):
        ...
        if not npv_data.empty:
            ...
            if row["OPEX_per_BBL_USD"] > 0:
                ...
                print(
                    f"  ${row['Total_OPEX_USD']:,.2f} / ${row['OPEX_per_BBL_USD']:.2f} = {implied_production:,.0f} BBL"
                )
                ...
                if implied_production > 0:
                    ...
                    print(
                        f"  ${row['Total_Revenue_USD']:,.2f} / {implied_production:,.0f} BBL = ${implied_oil_price:.2f}/BBL"
                    )

    # Look for actual production data files
    prod_data_path = (
        "/mnt/github/github/worldenergydata/data/modules/bsee/production/processed"
    )
                ...
                if "jack" in file.lower() or "stmalo" in file.lower():
                    ...
                    if file.endswith(".csv"):
                        ...
                        if "MON_O_PROD_VOL" in df.columns:
                            print(
                                f"  Sample production: {df['MON_O_PROD_VOL'].head().tolist()}"
                            )
