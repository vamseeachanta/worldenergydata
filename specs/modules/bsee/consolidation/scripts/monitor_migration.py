...
class MigrationMonitor:
    ...
    def check_critical_files(self) -> List[Dict]:
        ...
        for file_path in critical_files:
            ...
            status = {"path": file_path, "exists": full_path.exists()}
            if full_path.exists():
    ...
    def generate_dashboard(self):
        ...
        print(
            f"  Reduction: {round((1 - current_files/666) * 100, 1)}% files, "
        ...
        for file_status in self.check_critical_files():
            ...
            size = (
                f"({file_status.get('size_mb', 0)} MB)" if file_status["exists"] else ""
            )
