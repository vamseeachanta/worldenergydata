...
class BSEEInventoryGenerator:
    ...
    def generate_inventory(self):
        ...
        for root, dirs, files in os.walk(self.data_dir):
            ...
            for file in files:
                ...
                # Count by directory
                dir_name = (
                    file_info["directory"].split("/")[0]
                    if "/" in file_info["directory"]
                    else file_info["directory"]
                )
                self.stats["by_directory"][dir_name] = (
                    self.stats["by_directory"].get(dir_name, 0) + 1
                )
    ...
    def save_inventory(
        self, output_dir: str = "specs/modules/bsee/consolidation/sub-specs"
    ):
    ...
    def generate_summary_report(self, output_file: Path):
        ...
        report_lines.extend(["", "## Files by Directory", ""])
        ...
        # Add sample files
        report_lines.extend(
            ["", "## Sample Files", "", "### CSV Files with Most Rows", ""]
        )

        csv_files = [
            f for f in self.inventory if f["extension"] == ".csv" and "data_info" in f
        ]
        csv_files_with_rows = [
            f for f in csv_files if "row_count" in f.get("data_info", {})
        ]
