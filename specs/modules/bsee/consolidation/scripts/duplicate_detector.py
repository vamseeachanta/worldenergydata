...
class DuplicateDetector:
    def __init__(
        self,
        inventory_file: str = "specs/modules/bsee/consolidation/sub-specs/inventory.json",
    ):
        ...
        for file_info in self.inventory:
            ...
            if checksum and not checksum.startswith("ERROR"):
                checksum_map[checksum].append(
                    {"path": file_info["path"], "size_mb": file_info["size_mb"]}
                )
    ...
    def find_similar_csv_structures(self) -> Dict[str, List[str]]:
        ...
        self.similar_structure = {
            sig: files for sig, files in csv_structures.items() if len(files) > 1
        }
    ...
    def generate_report(
        self,
        output_file: str = "specs/modules/bsee/consolidation/sub-specs/duplicate-analysis.md",
    ):
        ...
        if len(self.exact_duplicates) > 10:
            report_lines.append(
                f"*... and {len(self.exact_duplicates) - 10} more duplicate sets*"
            )
            report_lines.append("")
        ...
        for sig, files in list(self.similar_structure.items())[:5]:
            ...
            for file_info in files[:5]:
                report_lines.append(
                    f"- `{file_info['path']}` ({file_info.get('row_count', 'Unknown')} rows)"
                )
            if len(files) > 5:
        ...
        for data_type, files in redundancy.items():
            if files:
                report_lines.append(
                    f"### {data_type.replace('_', ' ').title()}: {len(files)} files"
                )
...
if __name__ == "__main__":
    ...
    print(
        f"Space Savings: {savings['total_duplicate_size_mb']} MB ({savings['percentage_of_total']}%)"
    )
