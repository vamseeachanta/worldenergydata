...
class MigrationValidator:
    ...
    def create_backup(self) -> bool:
        ...
        try:
            ...
            if original_count == backup_count:
                ...
                self.error(
                    f"Backup mismatch: {original_count} original vs {backup_count} backup"
                )
                return False
    ...
    def validate_csv_integrity(self, filepath: Path) -> Dict:
        ...
        result = {"valid": True, "row_count": 0, "column_count": 0, "issues": []}
    ...
    def validate_data_consolidation(
        self, source_files: List[Path], consolidated_file: Path
    ) -> bool:
        ...
        try:
            ...
            if consolidated_rows < total_source_rows:
                self.warning(
                    f"Consolidated file has fewer rows: {consolidated_rows} vs {total_source_rows}"
                )
            ...
            if source_dfs:
                ...
                self.log(
                    f"Validated consolidation of {len(source_files)} files into {consolidated_file}"
                )
                return True
    ...
    def validate_no_data_loss(
        self, before_inventory: str, after_inventory: str
    ) -> bool:
            ...
            if lost_checksums:
                ...
                for item in before:
                    if item.get("checksum") in lost_checksums:
                        ...
                        count = sum(
                            1 for i in before if i.get("checksum") == item["checksum"]
                        )
                        if count == 1:
    ...
    def generate_validation_report(
        self, output_file: str = "specs/modules/bsee/consolidation/validation_report.md"
    ):
