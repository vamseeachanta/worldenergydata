...
class PostMigrationMonitor:
    def __init__(self, data_dir: str = "data/modules/bsee"):
        ...
        self.metrics_file = Path(
            "specs/modules/bsee/consolidation/post_migration_metrics.json"
        )
    ...
    def get_migration_date(self) -> datetime:
        ...
        validation_report = Path(
            "specs/modules/bsee/consolidation/validation_report.md"
        )
    ...
    def check_data_integrity(self) -> Dict:
        ...
        integrity = {"status": "healthy", "checks": [], "warnings": []}
        ...
        for dir_path in critical_dirs:
            ...
            if full_path.exists():
                ...
                integrity["checks"].append(
                    {"directory": dir_path, "status": "exists", "files": file_count}
                )
            else:
    ...
    def measure_performance(self) -> Dict:
        ...
        performance = {"file_access": {}, "directory_traversal": {}, "size_metrics": {}}
        ...
        if test_file.exists():
            ...
            with open(test_file) as f:
                _ = f.readline()
            performance["file_access"]["time_ms"] = round(
        ...
        performance["size_metrics"] = {
            ...
            "avg_file_size_kb": (
                round((total_size / file_count) / 1024, 2) if file_count > 0 else 0
            ),
        }
                    ...
                    for pattern in old_patterns:
                        if pattern in content:
                            outdated_references.append(
                                {"file": str(py_file), "pattern": pattern}
                            )
                            break
            except:
                pass
    ...
    def generate_dashboard(self):
        ...
        if compatibility["days_remaining"] > 0:
            print(
                f"  Status: Active ({compatibility['days_remaining']} days remaining)"
            )
        ...
        print(
            f"  Directory Traversal: {performance['directory_traversal']['time_ms']} ms"
        )
